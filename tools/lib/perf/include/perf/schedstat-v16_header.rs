...
// C conditional intent: the following CPU_FIELD entries are emitted only when
// CPU_FIELD is defined by the includer.
...
// C conditional intent: the following domain entries are emitted only when
// DOMAIN_FIELD is defined by the includer. DOMAIN_CATEGORY, DERIVED_CNT_FIELD,
// and DERIVED_AVG_FIELD calls are likewise conditional on their respective
// macro definitions in the original header.
