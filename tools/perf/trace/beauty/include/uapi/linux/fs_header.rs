...
/*
 * Rust translation of uapi/linux/fs.h.
 *
 * Original C dependencies: linux/limits.h, linux/ioctl.h, linux/types.h,
 * linux/fscrypt.h for non-kernel users, and linux/mount.h for non-kernel users.
 * Ioctl constants below intentionally reference the corresponding ioctl macro
 * names as external/future dependencies.
 */
...
pub struct files_stat_struct {
    pub nr_files: c_ulong,      /* read only */
    pub nr_free_files: c_ulong, /* read only */
    pub max_files: c_ulong,     /* tunable */
}
...
pub struct inodes_stat_t {
    ...
    pub dummy: [c_long; 5], /* padding for sysctl ABI compatibility */
}
...
pub struct file_attr {
    pub fa_xflags: __u64,    /* xflags field value (get/set) */
    pub fa_extsize: __u32,   /* extsize field value (get/set)*/
    pub fa_nextents: __u32,  /* nextents field value (get)   */
    pub fa_projid: __u32,   /* project identifier (get/set) */
    pub fa_cowextsize: __u32, /* CoW extsize field value (get/set) */
}
...
pub const FS_XFLAG_VERITY: u32 = 0x00020000; /* fs-verity enabled */
/*
 ...
 */
pub const FS_XFLAG_CASEFOLD: u32 = 0x00040000; /* case-insensitive lookups */
pub const FS_XFLAG_CASENONPRESERVING: u32 = 0x00080000; /* case not preserved */
pub const FS_XFLAG_HASATTR: u32 = 0x80000000; /* no DIFLAG for this	*/

/* the read-only stuff doesn't really belong here, but any other place is
   probably as bad and I don't want to create yet another include file. */

pub const BLKROSET: c_ulong = _IO!(0x12, 93); /* set device read-only (0 = read-write) */
pub const BLKROGET: c_ulong = _IO!(0x12, 94); /* get read-only status (0 = read_write) */
pub const BLKRRPART: c_ulong = _IO!(0x12, 95); /* re-read partition table */
pub const BLKGETSIZE: c_ulong = _IO!(0x12, 96); /* return device size /512 (long *arg) */
pub const BLKFLSBUF: c_ulong = _IO!(0x12, 97); /* flush buffer cache */
pub const BLKRASET: c_ulong = _IO!(0x12, 98); /* set read ahead for block device */
pub const BLKRAGET: c_ulong = _IO!(0x12, 99); /* get current read ahead setting */
pub const BLKFRASET: c_ulong = _IO!(0x12, 100); /* set filesystem (mm/filemap.c) read-ahead */
pub const BLKFRAGET: c_ulong = _IO!(0x12, 101); /* get filesystem (mm/filemap.c) read-ahead */
pub const BLKSECTSET: c_ulong = _IO!(0x12, 102); /* set max sectors per request (ll_rw_blk.c) */
pub const BLKSECTGET: c_ulong = _IO!(0x12, 103); /* get max sectors per request (ll_rw_blk.c) */
pub const BLKSSZGET: c_ulong = _IO!(0x12, 104); /* get block device sector size */
/*
 * C code under #if 0 omitted from executable Rust, preserving intent:
 * BLKPG = _IO(0x12,105), BLKELVGET = _IOR(0x12,106,size_t),
 * BLKELVSET = _IOW(0x12,107,size_t).  These ioctl numbers are marked taken.
 */
/* A jump here: 108-111 have been used for various private purposes. */
pub const BLKBSZGET: c_ulong = _IOR!(0x12, 112, size_t);
pub const BLKBSZSET: c_ulong = _IOW!(0x12, 113, size_t);
pub const BLKGETSIZE64: c_ulong = _IOR!(0x12, 114, size_t); /* return device size in bytes (u64 *arg) */
pub const BLKTRACESETUP: c_ulong = _IOWR!(0x12, 115, blk_user_trace_setup);
pub const BLKTRACESTART: c_ulong = _IO!(0x12, 116);
pub const BLKTRACESTOP: c_ulong = _IO!(0x12, 117);
pub const BLKTRACETEARDOWN: c_ulong = _IO!(0x12, 118);
pub const BLKDISCARD: c_ulong = _IO!(0x12, 119);
pub const BLKIOMIN: c_ulong = _IO!(0x12, 120);
pub const BLKIOOPT: c_ulong = _IO!(0x12, 121);
pub const BLKALIGNOFF: c_ulong = _IO!(0x12, 122);
pub const BLKPBSZGET: c_ulong = _IO!(0x12, 123);
pub const BLKDISCARDZEROES: c_ulong = _IO!(0x12, 124);
pub const BLKSECDISCARD: c_ulong = _IO!(0x12, 125);
pub const BLKROTATIONAL: c_ulong = _IO!(0x12, 126);
pub const BLKZEROOUT: c_ulong = _IO!(0x12, 127);
pub const BLKGETDISKSEQ: c_ulong = _IOR!(0x12, 128, __u64);
/* 130-136 and 142 are used by zoned block device ioctls (uapi/linux/blkzoned.h) */
/* 137-141 are used by blk-crypto ioctls (uapi/linux/blk-crypto.h) */
pub const BLKTRACESETUP2: c_ulong = _IOWR!(0x12, 142, blk_user_trace_setup2);

pub const BMAP_IOCTL: u32 = 1; /* obsolete - kept for compatibility */
pub const FIBMAP: c_ulong = _IO!(0x00, 1); /* bmap access */
pub const FIGETBSZ: c_ulong = _IO!(0x00, 2); /* get the block size used for bmap */
pub const FIFREEZE: c_ulong = _IOWR!(b'X', 119, c_int); /* Freeze */
pub const FITHAW: c_ulong = _IOWR!(b'X', 120, c_int); /* Thaw */
pub const FITRIM: c_ulong = _IOWR!(b'X', 121, fstrim_range); /* Trim */
pub const FICLONE: c_ulong = _IOW!(0x94, 9, c_int);
pub const FICLONERANGE: c_ulong = _IOW!(0x94, 13, file_clone_range);
pub const FIDEDUPERANGE: c_ulong = _IOWR!(0x94, 54, file_dedupe_range);

pub const FSLABEL_MAX: usize = 256; /* Max chars for the interface; each fs may differ */

pub const FS_IOC_GETFLAGS: c_ulong = _IOR!(b'f', 1, c_long);
pub const FS_IOC_SETFLAGS: c_ulong = _IOW!(b'f', 2, c_long);
pub const FS_IOC_GETVERSION: c_ulong = _IOR!(b'v', 1, c_long);
pub const FS_IOC_SETVERSION: c_ulong = _IOW!(b'v', 2, c_long);
pub const FS_IOC_FIEMAP: c_ulong = _IOWR!(b'f', 11, fiemap);
pub const FS_IOC32_GETFLAGS: c_ulong = _IOR!(b'f', 1, c_int);
pub const FS_IOC32_SETFLAGS: c_ulong = _IOW!(b'f', 2, c_int);
pub const FS_IOC32_GETVERSION: c_ulong = _IOR!(b'v', 1, c_int);
pub const FS_IOC32_SETVERSION: c_ulong = _IOW!(b'v', 2, c_int);
pub const FS_IOC_FSGETXATTR: c_ulong = _IOR!(b'X', 31, fsxattr);
pub const FS_IOC_FSSETXATTR: c_ulong = _IOW!(b'X', 32, fsxattr);
pub const FS_IOC_GETFSLABEL: c_ulong = _IOR!(0x94, 49, [c_char; FSLABEL_MAX]);
pub const FS_IOC_SETFSLABEL: c_ulong = _IOW!(0x94, 50, [c_char; FSLABEL_MAX]);
/* Returns the external filesystem UUID, the same one blkid returns */
pub const FS_IOC_GETFSUUID: c_ulong = _IOR!(0x15, 0, fsuuid2);
/*
 * Returns the path component under /sys/fs/ that refers to this filesystem;
 * also /sys/kernel/debug/ for filesystems with debugfs exports
 */
pub const FS_IOC_GETFSSYSFSPATH: c_ulong = _IOR!(0x15, 1, fs_sysfs_path);
/* Get logical block metadata capability details */
pub const FS_IOC_GETLBMD_CAP: c_ulong = _IOWR!(0x15, 2, logical_block_metadata_cap);
...
pub const RWF_HIPRI: __kernel_rwf_t = 0x00000001;

/* per-IO O_DSYNC */
pub const RWF_DSYNC: __kernel_rwf_t = 0x00000002;

/* per-IO O_SYNC */
pub const RWF_SYNC: __kernel_rwf_t = 0x00000004;

/* per-IO, return -EAGAIN if operation would block */
pub const RWF_NOWAIT: __kernel_rwf_t = 0x00000008;

/* per-IO O_APPEND */
pub const RWF_APPEND: __kernel_rwf_t = 0x00000010;

/* per-IO negation of O_APPEND */
pub const RWF_NOAPPEND: __kernel_rwf_t = 0x00000020;

/* Atomic Write */
pub const RWF_ATOMIC: __kernel_rwf_t = 0x00000040;

/* buffered IO that drops the cache after reading or writing data */
pub const RWF_DONTCACHE: __kernel_rwf_t = 0x00000080;

/* prevent pipe and socket writes from raising SIGPIPE */
pub const RWF_NOSIGNAL: __kernel_rwf_t = 0x00000100;
...
/* Pagemap ioctl */
pub const PAGEMAP_SCAN: c_ulong = _IOWR!(PROCFS_IOCTL_MAGIC, 16, pm_scan_arg);
...
/* /proc/<pid>/maps ioctl */
pub const PROCMAP_QUERY: c_ulong = _IOWR!(PROCFS_IOCTL_MAGIC, 17, procmap_query);
 ...
 */
pub const FS_IOC_SHUTDOWN: c_ulong = _IOR!(b'X', 125, __u32);
