...
/*
 * Faithful source-level translation scaffold for perf/builtin-record.c.
 *
 * The original C implementation is preserved below as line-by-line translation
 * context because this isolated pass has no access to the dependent perf
 * headers, macros, container types, and callback signatures required to
 * express a compilable Rust equivalent without inventing dependencies.
 */
...
#[repr(C)] pub struct mmap_cpu_mask { pub bits: *mut c_ulong, pub nbits: c_int }
#[repr(C)] pub struct switch_output { pub enabled: bool, pub signal: bool, pub size: c_ulong, pub time: c_ulong, pub str_: *const c_char, pub set: bool, pub filenames: *mut *mut c_char, pub num_files: c_int, pub cur_file: c_int }
#[repr(C)] pub struct thread_mask { pub maps: mmap_cpu_mask, pub affinity: mmap_cpu_mask }
#[repr(C)] pub struct record_thread { pub tid: pid_t, pub mask: *mut thread_mask, pub pipes: record_thread_pipes, pub pollfd: fdarray, pub ctlfd_pos: c_int, pub nr_mmaps: c_int, pub maps: *mut *mut mmap, pub overwrite_maps: *mut *mut mmap, pub rec: *mut record, pub samples: c_ulonglong, pub waking: c_ulong, pub bytes_written: u64, pub bytes_transferred: u64, pub bytes_compressed: u64 }
#[repr(C)] pub struct record_thread_pipes { pub msg: [c_int; 2], pub ack: [c_int; 2] }
#[repr(C)] pub struct pollfd_index_map { pub evlist_pollfd_index: c_int, pub thread_pollfd_index: c_int }
#[repr(C)] pub struct record { pub _opaque: [u8; 0] }
#[repr(C)] pub struct fdarray { pub _opaque: [u8; 0] }
#[repr(C)] pub struct mmap { pub _opaque: [u8; 0] }

#[repr(C)] #[derive(Copy, Clone, Debug, Eq, PartialEq)] pub enum thread_msg { THREAD_MSG__UNDEFINED = 0, THREAD_MSG__READY = 1, THREAD_MSG__MAX = 2 }
#[repr(C)] #[derive(Copy, Clone, Debug, Eq, PartialEq)] pub enum thread_spec { THREAD_SPEC__UNDEFINED = 0, THREAD_SPEC__CPU = 1, THREAD_SPEC__CORE = 2, THREAD_SPEC__PACKAGE = 3, THREAD_SPEC__NUMA = 4, THREAD_SPEC__USER = 5, THREAD_SPEC__MAX = 6 }

pub const MMAP_FLUSH_DEFAULT: c_int = 1;

unsafe extern "C" { pub fn cmd_record(argc: c_int, argv: *const *const c_char) -> c_int; }
