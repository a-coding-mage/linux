 ...
 * <stdint.h>, <stdlib.h>, <stdio.h>, <errno.h>, <linux/err.h>, "hashmap.h"
 */

/* make sure libbpf doesn't use kernel-only integer typedefs */
/* C source used: #pragma GCC poison u8 u16 u32 u64 s8 s16 s32 s64 */

/* prevent accidental re-addition of reallocarray() */
/* C source used: #pragma GCC poison reallocarray */
...
pub type hashmap_hash_fn = Option<unsafe extern "C" fn(key: libc_long, ctx: *mut c_void) -> usize>;
pub type hashmap_equal_fn =
    Option<unsafe extern "C" fn(key1: libc_long, key2: libc_long, ctx: *mut c_void) -> bool>;

type libc_long = core::ffi::c_long;
...
extern "C" {
    ...
    fn ERR_PTR(error: libc_long) -> *mut hashmap;
    fn IS_ERR_OR_NULL(ptr: *const hashmap) -> bool;
    fn hash_bits(hash: usize, bits: usize) -> usize;
}

#[inline]
unsafe fn call_hash_fn(map: *const hashmap, key: libc_long) -> usize {
    ((*map).hash_fn.expect("hashmap hash_fn is NULL"))(key, (*map).ctx)
}

#[inline]
unsafe fn call_equal_fn(map: *const hashmap, key1: libc_long, key2: libc_long) -> bool {
    ((*map).equal_fn.expect("hashmap equal_fn is NULL"))(key1, key2, (*map).ctx)
}
...
pub unsafe extern "C" fn hashmap__new(
    ...
    if map.is_null() {
        return ERR_PTR(-(ENOMEM as libc_long));
    }
...
unsafe fn hashmap_grow(map: *mut hashmap) -> i32 {
    ...
    let new_buckets = calloc(new_cap, mem::size_of::<*mut hashmap_entry>()) as *mut *mut hashmap_entry;
...
pub unsafe extern "C" fn hashmap_insert(
    ...
    old_key: *mut libc_long,
    old_value: *mut libc_long,
) -> i32 {
