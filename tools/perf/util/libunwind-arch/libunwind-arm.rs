...
//
// Translated from libunwind-arm.c.
// Original dependencies:
// "libunwind-arch.h", "../debug.h", "../maps.h", "../thread.h",
// "../../../arch/arm/include/uapi/asm/perf_regs.h",
// <linux/compiler.h>, <linux/kernel.h>, <linux/zalloc.h>, <elf.h>, <errno.h>
// When HAVE_LIBUNWIND_ARM_SUPPORT is enabled, <libunwind-arm.h> is also used.

use core::ffi::{c_char, c_int, c_void};

const EINVAL: c_int = 22;
const EM_ARM: c_int = 40;

extern "C" {
    static PERF_REG_ARM_MAX: c_int;
...
#[no_mangle]
pub unsafe extern "C" fn __get_perf_regnum_for_unw_regnum_arm(unw_regnum: c_int) -> c_int {
...
#[no_mangle]
pub unsafe extern "C" fn __libunwind_arch__flush_access_arm(maps: *mut maps) {
    #[cfg(HAVE_LIBUNWIND_ARM_SUPPORT)]
    {
        unw_flush_cache(maps__addr_space(maps), 0, 0);
    }

    #[cfg(not(HAVE_LIBUNWIND_ARM_SUPPORT))]
    {
        let _ = maps;
    }
}
...
#[no_mangle]
pub unsafe extern "C" fn __libunwind_arch__finish_access_arm(maps: *mut maps) {
    #[cfg(HAVE_LIBUNWIND_ARM_SUPPORT)]
    {
...
pub union unw_dyn_info_u {
    pub rti: unw_table_info_t,
    pub ti: unw_table_info_t,
}
...
extern "C" {
    ...
    fn unw_get_reg(cursor: *mut unw_cursor_t, regnum: c_int, valp: *mut unw_word_t) -> c_int;
...
#[cfg(all(HAVE_LIBUNWIND_ARM_SUPPORT, not(NO_LIBUNWIND_DEBUG_FRAME_ARM)))]
extern "C" {
...
#[no_mangle]
pub unsafe extern "C" fn __libunwind_arch__create_addr_space_arm() -> *mut c_void {
    #[cfg(HAVE_LIBUNWIND_ARM_SUPPORT)]
    {
...
#[no_mangle]
pub unsafe extern "C" fn __libunwind_arch__dwarf_search_unwind_table_arm(
    as_: *mut c_void,
    ip: u64,
    ...
    #[cfg(HAVE_LIBUNWIND_ARM_SUPPORT)]
    {
        ...
        let ret = dwarf_search_unwind_table(as_, ip, &mut di, pi, need_unwind_info, arg);
...
#[no_mangle]
pub unsafe extern "C" fn __libunwind_arch__dwarf_find_debug_frame_arm(
    found: c_int,
    _di: *mut libarch_unwind__dyn_info,
    ip: u64,
    segbase: u64,
    ...
    #[cfg(not(all(HAVE_LIBUNWIND_ARM_SUPPORT, not(NO_LIBUNWIND_DEBUG_FRAME_ARM))))]
    {
...
#[no_mangle]
pub unsafe extern "C" fn __libunwind_arch_unwind_info__new_arm(
    thread: *mut thread,
...
#[no_mangle]
pub unsafe extern "C" fn __libunwind_arch__unwind_step_arm(ui: *mut unwind_info) -> c_int {
    #[cfg(HAVE_LIBUNWIND_ARM_SUPPORT)]
    {
        let ret: c_int;
