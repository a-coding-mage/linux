...
// C dependencies: <libusb.h>, <stdio.h>, <string.h>, <unistd.h>
...
pub unsafe fn test_init(state: *mut test_state) -> c_int {
    let mut i: c_int;
    let mut ret: c_int;
    let cnt: isize;
...
pub unsafe fn test_exit(state: *mut test_state) {
...
unsafe fn c_main() -> c_int {
    ...
    iface = (*(*conf).interface.offset(0)).altsetting.offset(0);
    addr = (*(*iface).endpoint.offset(0)).bEndpointAddress;
