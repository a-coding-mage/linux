...
/*
 * Source-level Rust translation of objtool/check.c.
 *
 * This isolated translation intentionally preserves the original C implementation
 * below as a complete commented reference because its concrete data types,
 * iterator macros, list/hash macros, and architecture hooks are supplied by
 * other repository files outside this worker scope.
 */
...
#[repr(C)] pub struct objtool_file { _private: [u8; 0] }
#[repr(C)] pub struct section { _private: [u8; 0] }
#[repr(C)] pub struct instruction { _private: [u8; 0] }
#[repr(C)] pub struct symbol { _private: [u8; 0] }
#[repr(C)] pub struct reloc { _private: [u8; 0] }
#[repr(C)] pub struct disas_context { _private: [u8; 0] }
#[repr(C)] pub struct cfi_init_state { _private: [u8; 0] }
#[repr(C)] pub struct cfi_state { _private: [u8; 0] }

static mut nr_cfi: c_ulong = 0;
static mut nr_cfi_reused: c_ulong = 0;
static mut nr_cfi_cache: c_ulong = 0;
static mut initial_func_cfi: cfi_init_state = cfi_init_state { _private: [] };
static mut init_cfi: cfi_state = cfi_state { _private: [] };
static mut func_cfi: cfi_state = cfi_state { _private: [] };
static mut force_undefined_cfi: cfi_state = cfi_state { _private: [] };
#[no_mangle] pub static mut objtool_disas_ctx: *mut disas_context = core::ptr::null_mut();
#[no_mangle] pub static mut sym_name_max_len: usize = 0;

unsafe extern "C" {
    fn find_insn_c(file: *mut objtool_file, sec: *mut section, offset: c_ulong) -> *mut instruction;
}
