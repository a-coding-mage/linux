// SPDX-License-Identifier: GPL-2.0

// C dependency intent:
// check.h, stdio.h, stdlib.h, sched.h, limits.h, unistd.h, sys/sysinfo.h,
// linux/container_of.h, cli_params_assert.h, ../../src/cli.h

use core::ffi::{c_char, c_int};

const SCHED_FIFO: c_int = 1;

#[repr(C)]
pub struct sched_param {
    pub sched_priority: c_int,
    pub sched_policy: c_int,
}

#[repr(C)]
pub struct common_params {
    pub stop_us: c_int,
    pub stop_total_us: c_int,
    pub duration: c_int,
    pub cpus: *mut c_char,
    pub cgroup: c_int,
    pub cgroup_name: *mut c_char,
    pub kernel_workload: c_int,
    pub user_workload: c_int,
    pub user_data: c_int,
    pub sched_param: sched_param,
    pub output_divisor: c_int,
    pub quiet: c_int,
    pub buffer_size: c_int,
    pub warmup: c_int,
}

#[repr(C)]
pub struct timerlat_params {
    pub timerlat_period_us: c_int,
    pub print_stack: c_int,
    pub timerlat_align: c_int,
    pub timerlat_align_us: c_int,
    pub deepest_idle_state: c_int,
    pub dma_latency: c_int,
    pub bpf_action_program: *mut c_char,
    pub dump_tasks: c_int,
    pub no_aa: c_int,
    pub stack_format: c_int,
}

#[repr(C)]
pub struct Suite {
    _private: [u8; 0],
}

#[repr(C)]
pub struct TCase {
    _private: [u8; 0],
}

type TFun = Option<unsafe extern "C" fn(c_int)>;

unsafe extern "C" {
    static mut nr_cpus: c_int;
    static mut config_debug: c_int;

    fn timerlat_top_parse_args(argc: c_int, argv: *mut *mut c_char) -> *mut common_params;
    fn to_timerlat_params(params: *mut common_params) -> *mut timerlat_params;

    fn suite_create(name: *const c_char) -> *mut Suite;
    fn tcase_create(name: *const c_char) -> *mut TCase;
    fn tcase_add_test(tc: *mut TCase, test: TFun);
    fn suite_add_tcase(s: *mut Suite, tc: *mut TCase);
}

macro_rules! c_str {
    ($s:literal) => {
        concat!($s, "\0").as_ptr() as *mut c_char
    };
}

macro_rules! PARSE_ARGS {
    ($($arg:literal),+ $(,)?) => {
        let mut argv: [*mut c_char; PARSE_ARGS!(@count $($arg),+) + 1] =
            [$(c_str!($arg)),+, core::ptr::null_mut()];
        let argc = (argv.len() - 1) as c_int;
        let params = timerlat_top_parse_args(argc, argv.as_mut_ptr());
        let tlat_params = to_timerlat_params(params);
    };
    (@count $head:literal $(, $tail:literal)*) => {
        1usize $(+ { let _ = $tail; 1usize })*
    };
}

// Assertion helper macros supplied by cli_params_assert.h in the original C source.
// Their Rust equivalents are expected from the surrounding translated test support.

/* Tracing Options */

unsafe extern "C" fn test_irq_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-i", "20");

    assert_eq!((*params).stop_us, 20);
}

unsafe extern "C" fn test_irq_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--irq", "20");

    assert_eq!((*params).stop_us, 20);
}

unsafe extern "C" fn test_period_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-p", "200");

    assert_eq!((*tlat_params).timerlat_period_us, 200);
}

unsafe extern "C" fn test_period_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--period", "200");

    assert_eq!((*tlat_params).timerlat_period_us, 200);
}

unsafe extern "C" fn test_period_unset_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-p", "200", "--no-period");

    assert_eq!((*tlat_params).timerlat_period_us, 0);
}

unsafe extern "C" fn test_period_unset_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--period", "200", "--no-period");

    assert_eq!((*tlat_params).timerlat_period_us, 0);
}

unsafe extern "C" fn test_stack_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-s", "20");

    assert_eq!((*tlat_params).print_stack, 20);
}

unsafe extern "C" fn test_stack_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--stack", "20");

    assert_eq!((*tlat_params).print_stack, 20);
}

unsafe extern "C" fn test_thread_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-T", "20");

    assert_eq!((*params).stop_total_us, 20);
}

unsafe extern "C" fn test_thread_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--thread", "20");

    assert_eq!((*params).stop_total_us, 20);
}

/* Event Configuration */

unsafe extern "C" fn test_event_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-e", "system:event");

    CLI_ASSERT_SINGLE_EVENT!("system", "event");
}

unsafe extern "C" fn test_event_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--event", "system:event");

    CLI_ASSERT_SINGLE_EVENT!("system", "event");
}

unsafe extern "C" fn test_filter(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-e", "system:event", "--filter", "filter");

    CLI_ASSERT_SINGLE_FILTER!("filter");
}

unsafe extern "C" fn test_trigger(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-e", "system:event", "--trigger", "trigger");

    CLI_ASSERT_SINGLE_TRIGGER!("trigger");
}

unsafe extern "C" fn test_trace_short_noarg(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-t");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "timerlat_trace.txt");
}

unsafe extern "C" fn test_trace_short_followarg(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-t", "-d", "20");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "timerlat_trace.txt");
    assert_eq!((*params).duration, 20); /* check if next argument is read correctly */
}

unsafe extern "C" fn test_trace_short_space(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-t", "tracefile");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "tracefile");
}

unsafe extern "C" fn test_trace_short_equals(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-t=tracefile");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "tracefile");
}

unsafe extern "C" fn test_trace_long_noarg(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--trace");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "timerlat_trace.txt");
}

unsafe extern "C" fn test_trace_long_followarg(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--trace", "-d", "20");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "timerlat_trace.txt");
    assert_eq!((*params).duration, 20); /* check if next argument is read correctly */
}

unsafe extern "C" fn test_trace_long_space(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--trace", "tracefile");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "tracefile");
}

unsafe extern "C" fn test_trace_long_equals(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--trace=tracefile");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "tracefile");
}

/* CPU Configuration */

unsafe extern "C" fn test_cpus_short(_: c_int) {
    nr_cpus = 4;

    PARSE_ARGS!("timerlat", "top", "-c", "0-1,3");

    ck_assert_str_eq!((*params).cpus, "0-1,3");
    CLI_ASSERT_CPUSET!(monitored_cpus, 0, 1, 3);
}

unsafe extern "C" fn test_cpus_long(_: c_int) {
    nr_cpus = 4;

    PARSE_ARGS!("timerlat", "top", "--cpus", "0-1,3");

    ck_assert_str_eq!((*params).cpus, "0-1,3");
    CLI_ASSERT_CPUSET!(monitored_cpus, 0, 1, 3);
}

unsafe extern "C" fn test_housekeeping_short(_: c_int) {
    nr_cpus = 4;

    PARSE_ARGS!("timerlat", "top", "-H", "0-1,3");

    CLI_ASSERT_CPUSET!(hk_cpu_set, 0, 1, 3);
}

unsafe extern "C" fn test_housekeeping_long(_: c_int) {
    nr_cpus = 4;

    PARSE_ARGS!("timerlat", "top", "--house-keeping", "0-1,3");

    CLI_ASSERT_CPUSET!(hk_cpu_set, 0, 1, 3);
}

/* Thread Configuration */

unsafe extern "C" fn test_cgroup_short_noarg(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-C");

    assert!((*params).cgroup != 0);
    assert!((*params).cgroup_name.is_null());
}

unsafe extern "C" fn test_cgroup_short_space(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-C", "cgroup");

    assert!((*params).cgroup != 0);
    ck_assert_str_eq!((*params).cgroup_name, "cgroup");
}

unsafe extern "C" fn test_cgroup_short_equals(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-C=cgroup");

    assert!((*params).cgroup != 0);
    ck_assert_str_eq!((*params).cgroup_name, "cgroup");
}

unsafe extern "C" fn test_cgroup_long_noarg(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--cgroup");

    assert!((*params).cgroup != 0);
    assert!((*params).cgroup_name.is_null());
}

unsafe extern "C" fn test_cgroup_long_space(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--cgroup", "cgroup");

    assert!((*params).cgroup != 0);
    ck_assert_str_eq!((*params).cgroup_name, "cgroup");
}

unsafe extern "C" fn test_cgroup_long_equals(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--cgroup=cgroup");

    assert!((*params).cgroup != 0);
    ck_assert_str_eq!((*params).cgroup_name, "cgroup");
}

unsafe extern "C" fn test_kernel_threads_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-k");

    assert!((*params).kernel_workload != 0);
    assert!((*params).user_workload == 0);
    assert!((*params).user_data == 0);
}

unsafe extern "C" fn test_kernel_threads_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--kernel-threads");

    assert!((*params).kernel_workload != 0);
    assert!((*params).user_workload == 0);
    assert!((*params).user_data == 0);
}

unsafe extern "C" fn test_priority_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-P", "f:95");

    assert_eq!((*params).sched_param.sched_policy, SCHED_FIFO);
    assert_eq!((*params).sched_param.sched_priority, 95);
}

unsafe extern "C" fn test_priority_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--priority", "f:95");

    assert_eq!((*params).sched_param.sched_policy, SCHED_FIFO);
    assert_eq!((*params).sched_param.sched_priority, 95);
}

unsafe extern "C" fn test_user_load_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-U");

    assert!((*params).kernel_workload == 0);
    assert!((*params).user_workload == 0);
    assert!((*params).user_data != 0);
}

unsafe extern "C" fn test_user_load_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--user-load");

    assert!((*params).kernel_workload == 0);
    assert!((*params).user_workload == 0);
    assert!((*params).user_data != 0);
}

unsafe extern "C" fn test_user_threads_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-u");

    assert!((*params).kernel_workload == 0);
    assert!((*params).user_workload != 0);
    assert!((*params).user_data != 0);
}

unsafe extern "C" fn test_user_threads_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--user-threads");

    assert!((*params).kernel_workload == 0);
    assert!((*params).user_workload != 0);
    assert!((*params).user_data != 0);
}

unsafe extern "C" fn test_aligned_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-A", "500");

    assert!((*tlat_params).timerlat_align != 0);
    assert_eq!((*tlat_params).timerlat_align_us, 500);
}

unsafe extern "C" fn test_aligned_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--aligned", "500");

    assert!((*tlat_params).timerlat_align != 0);
    assert_eq!((*tlat_params).timerlat_align_us, 500);
}

/* Output */

unsafe extern "C" fn test_nano_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-n");

    assert_eq!((*params).output_divisor, 1);
}

unsafe extern "C" fn test_nano_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--nano");

    assert_eq!((*params).output_divisor, 1);
}

unsafe extern "C" fn test_quiet_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-q");

    assert!((*params).quiet != 0);
}

unsafe extern "C" fn test_quiet_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--quiet");

    assert!((*params).quiet != 0);
}

/* System Tuning */

unsafe extern "C" fn test_deepest_idle_state(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--deepest-idle-state", "1");

    assert_eq!((*tlat_params).deepest_idle_state, 1);
}

unsafe extern "C" fn test_dma_latency(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--dma-latency", "10");

    assert_eq!((*tlat_params).dma_latency, 10);
}

unsafe extern "C" fn test_trace_buffer_size(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--trace-buffer-size", "200");

    assert_eq!((*params).buffer_size, 200);
}

unsafe extern "C" fn test_warm_up(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--warm-up", "5");

    assert_eq!((*params).warmup, 5);
}

/* Auto Analysis and Actions */

unsafe extern "C" fn test_auto(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-a", "20");

    CLI_TIMERLAT_ASSERT_AUTO!(20);
}

unsafe extern "C" fn test_aa_only(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--aa-only", "20");

    CLI_TIMERLAT_ASSERT_AA_ONLY!(20);
}

unsafe extern "C" fn test_bpf_action(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--bpf-action", "program");

    ck_assert_str_eq!((*tlat_params).bpf_action_program, "program");
}

unsafe extern "C" fn test_dump_tasks(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--dump-tasks");

    assert!((*tlat_params).dump_tasks != 0);
}

unsafe extern "C" fn test_no_aa(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--no-aa");

    assert!((*tlat_params).no_aa != 0);
}

unsafe extern "C" fn test_on_end(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--on-end", "trace");

    CLI_ASSERT_SINGLE_ACTION!(end_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "timerlat_trace.txt");
}

unsafe extern "C" fn test_on_threshold(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--on-threshold", "trace");

    CLI_ASSERT_SINGLE_ACTION!(threshold_actions, ACTION_TRACE_OUTPUT, trace_output, str,
                              "timerlat_trace.txt");
}

unsafe extern "C" fn test_stack_format(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--stack-format", "truncate");

    assert_eq!((*tlat_params).stack_format, STACK_FORMAT_TRUNCATE);
}

/* General */

unsafe extern "C" fn test_debug_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-D");

    assert!(config_debug != 0);
}

unsafe extern "C" fn test_debug_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--debug");

    assert!(config_debug != 0);
}

unsafe extern "C" fn test_duration_short(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "-d", "1m");

    assert_eq!((*params).duration, 60);
}

unsafe extern "C" fn test_duration_long(_: c_int) {
    PARSE_ARGS!("timerlat", "top", "--duration", "1m");

    assert_eq!((*params).duration, 60);
}

#[no_mangle]
pub unsafe extern "C" fn timerlat_top_cli_suite() -> *mut Suite {
    let s = suite_create(c_str!("timerlat_top_cli"));
    let mut tc: *mut TCase;

    tc = tcase_create(c_str!("tracing_options"));
    tcase_add_test(tc, Some(test_irq_short));
    tcase_add_test(tc, Some(test_irq_long));
    tcase_add_test(tc, Some(test_period_short));
    tcase_add_test(tc, Some(test_period_long));
    tcase_add_test(tc, Some(test_period_unset_short));
    tcase_add_test(tc, Some(test_period_unset_long));
    tcase_add_test(tc, Some(test_stack_short));
    tcase_add_test(tc, Some(test_stack_long));
    tcase_add_test(tc, Some(test_thread_short));
    tcase_add_test(tc, Some(test_thread_long));
    tcase_add_test(tc, Some(test_trace_short_noarg));
    tcase_add_test(tc, Some(test_trace_short_followarg));
    tcase_add_test(tc, Some(test_trace_short_space));
    tcase_add_test(tc, Some(test_trace_short_equals));
    tcase_add_test(tc, Some(test_trace_long_noarg));
    tcase_add_test(tc, Some(test_trace_long_followarg));
    tcase_add_test(tc, Some(test_trace_long_space));
    tcase_add_test(tc, Some(test_trace_long_equals));
    suite_add_tcase(s, tc);

    tc = tcase_create(c_str!("event_configuration"));
    tcase_add_test(tc, Some(test_event_short));
    tcase_add_test(tc, Some(test_event_long));
    tcase_add_test(tc, Some(test_filter));
    tcase_add_test(tc, Some(test_trigger));
    suite_add_tcase(s, tc);

    tc = tcase_create(c_str!("cpu_configuration"));
    tcase_add_test(tc, Some(test_cpus_short));
    tcase_add_test(tc, Some(test_cpus_long));
    tcase_add_test(tc, Some(test_housekeeping_short));
    tcase_add_test(tc, Some(test_housekeeping_long));
    suite_add_tcase(s, tc);

    tc = tcase_create(c_str!("thread_configuration"));
    tcase_add_test(tc, Some(test_cgroup_short_noarg));
    tcase_add_test(tc, Some(test_cgroup_short_space));
    tcase_add_test(tc, Some(test_cgroup_short_equals));
    tcase_add_test(tc, Some(test_cgroup_long_noarg));
    tcase_add_test(tc, Some(test_cgroup_long_space));
    tcase_add_test(tc, Some(test_cgroup_long_equals));
    tcase_add_test(tc, Some(test_kernel_threads_short));
    tcase_add_test(tc, Some(test_kernel_threads_long));
    tcase_add_test(tc, Some(test_priority_short));
    tcase_add_test(tc, Some(test_priority_long));
    tcase_add_test(tc, Some(test_user_load_short));
    tcase_add_test(tc, Some(test_user_load_long));
    tcase_add_test(tc, Some(test_user_threads_short));
    tcase_add_test(tc, Some(test_user_threads_long));
    tcase_add_test(tc, Some(test_aligned_short));
    tcase_add_test(tc, Some(test_aligned_long));
    suite_add_tcase(s, tc);

    tc = tcase_create(c_str!("output"));
    tcase_add_test(tc, Some(test_nano_short));
    tcase_add_test(tc, Some(test_nano_long));
    tcase_add_test(tc, Some(test_quiet_short));
    tcase_add_test(tc, Some(test_quiet_long));
    suite_add_tcase(s, tc);

    tc = tcase_create(c_str!("system_tuning"));
    tcase_add_test(tc, Some(test_deepest_idle_state));
    tcase_add_test(tc, Some(test_dma_latency));
    tcase_add_test(tc, Some(test_trace_buffer_size));
    tcase_add_test(tc, Some(test_warm_up));
    suite_add_tcase(s, tc);

    tc = tcase_create(c_str!("aa_actions"));
    tcase_add_test(tc, Some(test_auto));
    tcase_add_test(tc, Some(test_aa_only));
    tcase_add_test(tc, Some(test_bpf_action));
    tcase_add_test(tc, Some(test_dump_tasks));
    tcase_add_test(tc, Some(test_no_aa));
    tcase_add_test(tc, Some(test_on_end));
    tcase_add_test(tc, Some(test_on_threshold));
    tcase_add_test(tc, Some(test_stack_format));
    suite_add_tcase(s, tc);

    tc = tcase_create(c_str!("general"));
    tcase_add_test(tc, Some(test_debug_short));
    tcase_add_test(tc, Some(test_debug_long));
    tcase_add_test(tc, Some(test_duration_short));
    tcase_add_test(tc, Some(test_duration_long));
    suite_add_tcase(s, tc);

    s
}
