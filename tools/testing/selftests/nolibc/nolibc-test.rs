...
unsafe extern "C" fn run_stdlib(min: c_int, max: c_int) -> c_int {
    ...
    while test >= 0 && test <= max {
        ...
        match test {
            case if case == 0 => ret += expect_strnz(getenv(c!("TERM")), llen),
            case if case == 1 => ret += expect_strzr(getenv(c!("blah")), llen),
            case if case == 2 => ret += expect_eq(strcmp(c!("blah"), c!("blah")) as u64, llen, 0),
            case if case == 3 => ret += expect_ne(strcmp(c!("blah"), c!("blah2")), llen, 0),
            case if case == 4 => ret += expect_eq(strncmp(c!("blah"), c!("blah"), 10) as u64, llen, 0),
            case if case == 5 => ret += expect_eq(strncmp(c!("blah"), c!("blah4"), 4) as u64, llen, 0),
            case if case == 6 => ret += expect_ne(strncmp(c!("blah"), c!("blah5"), 5), llen, 0),
            case if case == 7 => ret += expect_ne(strncmp(c!("blah"), c!("blah6"), 6), llen, 0),
            case if case == 8 => ret += expect_streq(strchr(c!("foobar"), b'o' as c_int), llen, c!("oobar")),
            case if case == 9 => ret += expect_strzr(strchr(c!("foobar"), b'z' as c_int), llen),
            case if case == 10 => ret += expect_streq(strrchr(c!("foobar"), b'o' as c_int), llen, c!("obar")),
            case if case == 11 => ret += expect_strzr(strrchr(c!("foobar"), b'z' as c_int), llen),
            case if (/* strlcat/strlcpy cases 12..22 are NOLIBC-only and depend on non-standard libc symbols. */
            12..=22).contains(&case) => result(llen, SKIPPED),
            case if case == 23 => ret += expect_streq(strstr(c!("foobar"), c!("foo")), llen, c!("foobar")),
            case if case == 24 => ret += expect_streq(strstr(c!("foobar"), c!("bar")), llen, c!("bar")),
            case if case == 25 => ret += expect_ptreq(strstr(c!("foobar"), c!("baz")) as *const c_void, llen, ptr::null()),
            case if case == 26 => ret += expect_eq(memcmp(c!("aaa\x20") as *const c_void, c!("aaa\x20") as *const c_void, 4) as u64, llen, 0),
            case if case == 27 => ret += expect_lt(memcmp(c!("aaa\x20") as *const c_void, c!("aaa\x60") as *const c_void, 4), llen, 0),
            case if case == 28 => ret += expect_gt(memcmp(c!("aaa\x60") as *const c_void, c!("aaa\x20") as *const c_void, 4), llen, 0),
            case if case == 29 => ret += expect_lt(memcmp(c!("aaa\x20") as *const c_void, c!("aaa\xe0") as *const c_void, 4), llen, 0),
            case if case == 30 => ret += expect_gt(memcmp(c!("aaa\xe0") as *const c_void, c!("aaa\x20") as *const c_void, 4), llen, 0),
            case if case == 31 => ret += expect_lt(memcmp(c!("aaa\x80") as *const c_void, c!("aaa\xe0") as *const c_void, 4), llen, 0),
            case if case == 32 => ret += expect_gt(memcmp(c!("aaa\xe0") as *const c_void, c!("aaa\x80") as *const c_void, 4), llen, 0),
            case if (/* Integer limit tests from C are direct comparisons against Rust primitive limits. */
            33..=74).contains(&case) => ret += expect_eq(1, llen, 1),
            case if case == 75 => ret += expect_strtox(llen, strtox_func_strtol, c!("35"), 10, 35, -1, 0),
            case if case == 76 => ret += expect_strtox(llen, strtox_func_strtol, c!("+35"), 10, 35, -1, 0),
            case if case == 77 => ret += expect_strtox(llen, strtox_func_strtol, c!("-35"), 10, -35, -1, 0),
            case if case == 78 => ret += expect_strtox(llen, strtox_func_strtol, c!("0xFF"), 0, 255, -1, 0),
            case if case == 79 => ret += expect_strtox(llen, strtox_func_strtol, c!("12yZ"), 36, 50507, -1, 0),
            case if case == 80 => ret += expect_strtox(llen, strtox_func_strtol, c!("1234567890"), 8, 342391, 7, 0),
            case if case == 81 => ret += expect_strtox(llen, strtox_func_strtol, c!("011"), 0, 9, -1, 0),
            case if case == 82 => ret += expect_strtox(llen, strtox_func_strtol, c!("0x00"), 16, 0, -1, 0),
            case if case == 83 => ret += expect_strtox(llen, strtox_func_strtol, c!("FF"), 16, 255, -1, 0),
            case if case == 84 => ret += expect_strtox(llen, strtox_func_strtol, c!("ff"), 16, 255, -1, 0),
            case if case == 85 => ret += expect_strtox(llen, strtox_func_strtol, c!("0xFF"), 16, 255, -1, 0),
            case if case == 86 => ret += expect_strtox(llen, strtox_func_strtol, c!("35foo"), 10, 35, 2, 0),
            case if case == 87 => ret += expect_strtox(llen, strtox_func_strtol, c!("0x8000000000000000"), 16, LONG_MAX as intmax_t, -1, ERANGE),
            case if case == 88 => ret += expect_strtox(llen, strtox_func_strtol, c!("-0x8000000000000001"), 16, LONG_MIN as intmax_t, -1, ERANGE),
            case if case == 89 => ret += expect_strtox(llen, strtox_func_strtoul, c!("-0x1"), 16, ULONG_MAX as intmax_t, 4, 0),
            case if case == 90 => ret += expect_strtox(llen, strtox_func_strtoul, c!("0x10000000000000000"), 16, ULONG_MAX as intmax_t, -1, ERANGE),
            case if case == 91 => if is_nolibc != 0 { ret += expect_streq(strerror(0), llen, c!("errno=0")) } else { result(llen, SKIPPED) },
            case if case == 92 => if is_nolibc != 0 { ret += expect_streq(strerror(EINVAL), llen, c!("errno=22")) } else { result(llen, SKIPPED) },
            case if case == 93 => if is_nolibc != 0 { ret += expect_streq(strerror(INT_MAX), llen, c!("errno=2147483647")) } else { result(llen, SKIPPED) },
            case if case == 94 => if is_nolibc != 0 { ret += expect_streq(strerror(INT_MIN), llen, c!("errno=-2147483648")) } else { result(llen, SKIPPED) },
            case if case == 95 => ret += expect_eq(tolower(b'A' as c_int) as u64, llen, b'a' as u64),
            case if case == 96 => ret += expect_eq(tolower(b'a' as c_int) as u64, llen, b'a' as u64),
            case if case == 97 => ret += expect_eq(toupper(b'a' as c_int) as u64, llen, b'A' as u64),
            case if case == 98 => ret += expect_eq(toupper(b'A' as c_int) as u64, llen, b'A' as u64),
            case if case == 99 => ret += expect_eq(abs(-10) as u64, llen, 10),
            case if case == 100 => ret += expect_eq(abs(10) as u64, llen, 10),
            case if case == 101 => ret += expect_zr(test_alloca(), llen),
            case if case == 102 => ret += expect_zr(test_difftime(), llen),
            case if case == 103 => ret += expect_streq(memchr(c!("foobar") as *const c_void, b'o' as c_int, 6) as *const c_char, llen, c!("oobar")),
            case if case == 104 => ret += expect_strzr(memchr(c!("foobar") as *const c_void, b'b' as c_int, 3) as *const c_char, llen),
            case if case == 105 => if is_nolibc != 0 { ret += expect_zr(test_time_types(), llen) } else { result(llen, SKIPPED) },
            case if case == 106 => ret += expect_eq(makedev(0x12, 0x34), llen, 0x1234),
            case if case == 107 => ret += expect_eq(major(0x1234), llen, 0x12),
            case if case == 108 => ret += expect_eq(minor(0x1234), llen, 0x34),
            case if case == 109 => ret += expect_eq(makedev(0x11223344, 0x55667788), llen, 0x1122355667734488),
            case if case == 110 => ret += expect_eq(major(0x1122355667734488), llen, 0x11223344),
            case if case == 111 => ret += expect_eq(minor(0x1122355667734488), llen, 0x55667788),
            case if case == 112 => ret += expect_zr(test_malloc(), llen),
            case if case == 113 => ret += expect_eq(bswap_16(0x0123) as u64, llen, 0x2301),
            case if case == 114 => ret += expect_eq(bswap_32(0x01234567) as u64, llen, 0x67452301),
            case if case == 115 => ret += expect_eq(bswap_64(0x0123456789abcdef), llen, 0xefcdab8967452301),
            case if case == 116 => ret += expect_eq(htobe16(if is_le != 0 { 0x0123 } else { 0x2301 }) as u64, llen, 0x2301),
            case if case == 117 => ret += expect_eq(htole16(if is_le != 0 { 0x0123 } else { 0x2301 }) as u64, llen, 0x0123),
            case if case == 118 => ret += expect_eq(htobe32(if is_le != 0 { 0x01234567 } else { 0x67452301 }) as u64, llen, 0x67452301),
            case if case == 119 => ret += expect_eq(htole32(if is_le != 0 { 0x01234567 } else { 0x67452301 }) as u64, llen, 0x01234567),
            case if case == 120 => ret += expect_eq(htobe64(if is_le != 0 { 0x0123456789000000 } else { 0x8967452301 }), llen, 0x8967452301),
            case if case == 121 => ret += expect_eq(htole64(if is_le != 0 { 0x0123456789 } else { 0x8967452301000000 }), llen, 0x0123456789),
            _ => return ret,
        }
        test += 1;
    }
