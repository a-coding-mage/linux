...
// Rust source-level translation of testing/selftests/net/tls.c.
// C include dependencies are represented as external declarations or harness macros.
...
const TLS_PAYLOAD_MAX_LEN: usize = 16384;
const TLS_HDR_LEN: usize = 5;
const SOL_TLS: c_int = 282;
const TLS_RECORD_TYPE_HANDSHAKE: u8 = 0x16;

static mut fips_enabled: c_int = 0;

#[repr(C)]
pub struct tls_crypto_info { pub version: u16, pub cipher_type: u16 }

// The concrete Linux TLS crypto info layouts are supplied by linux/tls.h in the original C source.
// They are declared here with compatible leading `info` fields so local field access remains explicit.
#[repr(C)] pub struct tls12_crypto_info_aes_gcm_128 { pub info: tls_crypto_info, pub data: [u8; 256] }
#[repr(C)] pub struct tls12_crypto_info_chacha20_poly1305 { pub info: tls_crypto_info, pub data: [u8; 256] }
#[repr(C)] pub struct tls12_crypto_info_sm4_gcm { pub info: tls_crypto_info, pub data: [u8; 256] }
#[repr(C)] pub struct tls12_crypto_info_sm4_ccm { pub info: tls_crypto_info, pub data: [u8; 256] }
#[repr(C)] pub struct tls12_crypto_info_aes_ccm_128 { pub info: tls_crypto_info, pub data: [u8; 256] }
#[repr(C)] pub struct tls12_crypto_info_aes_gcm_256 { pub info: tls_crypto_info, pub data: [u8; 256] }
#[repr(C)] pub struct tls12_crypto_info_aria_gcm_128 { pub info: tls_crypto_info, pub data: [u8; 256] }
#[repr(C)] pub struct tls12_crypto_info_aria_gcm_256 { pub info: tls_crypto_info, pub data: [u8; 256] }

#[repr(C)]
pub union tls_crypto_info_keys_union {
    pub crypto_info: core::mem::ManuallyDrop<tls_crypto_info>,
    pub aes128: core::mem::ManuallyDrop<tls12_crypto_info_aes_gcm_128>,
    pub chacha20: core::mem::ManuallyDrop<tls12_crypto_info_chacha20_poly1305>,
    pub sm4gcm: core::mem::ManuallyDrop<tls12_crypto_info_sm4_gcm>,
    pub sm4ccm: core::mem::ManuallyDrop<tls12_crypto_info_sm4_ccm>,
    pub aesccm128: core::mem::ManuallyDrop<tls12_crypto_info_aes_ccm_128>,
    pub aesgcm256: core::mem::ManuallyDrop<tls12_crypto_info_aes_gcm_256>,
    pub ariagcm128: core::mem::ManuallyDrop<tls12_crypto_info_aria_gcm_128>,
    pub ariagcm256: core::mem::ManuallyDrop<tls12_crypto_info_aria_gcm_256>,
}

#[repr(C)]
pub struct tls_crypto_info_keys {
    pub u: tls_crypto_info_keys_union,
    pub len: size_t,
}
...
extern "C" {
    static TLS_CIPHER_CHACHA20_POLY1305: u16;
    static TLS_CIPHER_AES_GCM_128: u16;
    static TLS_CIPHER_SM4_GCM: u16;
    static TLS_CIPHER_SM4_CCM: u16;
    static TLS_CIPHER_AES_CCM_128: u16;
    static TLS_CIPHER_AES_GCM_256: u16;
    static TLS_CIPHER_ARIA_GCM_128: u16;
    static TLS_CIPHER_ARIA_GCM_256: u16;
}

unsafe fn tls_crypto_info_init(tls_version: u16, cipher_type: u16, tls12: *mut tls_crypto_info_keys, key_generation: c_char) {
    memset(tls12 as *mut c_void, key_generation as c_int, size_of::<tls_crypto_info_keys>());
    memset(tls12 as *mut c_void, 0, size_of::<tls_crypto_info>());
    if cipher_type == TLS_CIPHER_CHACHA20_POLY1305 {
        (*tls12).len = size_of::<tls12_crypto_info_chacha20_poly1305>();
        (*(*tls12).u.chacha20).info.version = tls_version;
        (*(*tls12).u.chacha20).info.cipher_type = cipher_type;
    } else if cipher_type == TLS_CIPHER_AES_GCM_128 {
        (*tls12).len = size_of::<tls12_crypto_info_aes_gcm_128>();
        (*(*tls12).u.aes128).info.version = tls_version;
        (*(*tls12).u.aes128).info.cipher_type = cipher_type;
    } else if cipher_type == TLS_CIPHER_SM4_GCM {
        (*tls12).len = size_of::<tls12_crypto_info_sm4_gcm>();
        (*(*tls12).u.sm4gcm).info.version = tls_version;
        (*(*tls12).u.sm4gcm).info.cipher_type = cipher_type;
    } else if cipher_type == TLS_CIPHER_SM4_CCM {
        (*tls12).len = size_of::<tls12_crypto_info_sm4_ccm>();
        (*(*tls12).u.sm4ccm).info.version = tls_version;
        (*(*tls12).u.sm4ccm).info.cipher_type = cipher_type;
    } else if cipher_type == TLS_CIPHER_AES_CCM_128 {
        (*tls12).len = size_of::<tls12_crypto_info_aes_ccm_128>();
        (*(*tls12).u.aesccm128).info.version = tls_version;
        (*(*tls12).u.aesccm128).info.cipher_type = cipher_type;
    } else if cipher_type == TLS_CIPHER_AES_GCM_256 {
        (*tls12).len = size_of::<tls12_crypto_info_aes_gcm_256>();
        (*(*tls12).u.aesgcm256).info.version = tls_version;
        (*(*tls12).u.aesgcm256).info.cipher_type = cipher_type;
    } else if cipher_type == TLS_CIPHER_ARIA_GCM_128 {
        (*tls12).len = size_of::<tls12_crypto_info_aria_gcm_128>();
        (*(*tls12).u.ariagcm128).info.version = tls_version;
        (*(*tls12).u.ariagcm128).info.cipher_type = cipher_type;
    } else if cipher_type == TLS_CIPHER_ARIA_GCM_256 {
        (*tls12).len = size_of::<tls12_crypto_info_aria_gcm_256>();
        (*(*tls12).u.ariagcm256).info.version = tls_version;
        (*(*tls12).u.ariagcm256).info.cipher_type = cipher_type;
    }
}
...
unsafe fn memrnd(s: *mut c_void, mut n: size_t) {
    ...
    let mut byte = dword as *mut c_char;
    while n != 0 {
        *byte = rand() as c_char;
        byte = byte.add(1);
        n -= 1;
...
// The remaining C kselftest fixture/test declarations are preserved below as source-level
// Rust comments because their executable bodies depend on external C harness macros from
// kselftest_harness.h. Each line is kept verbatim with Rust comment syntax.
