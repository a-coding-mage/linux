pub const ATOMIC_FETCH_ADD_TESTS: &[BpfVerifierTest] = &[
    BpfVerifierTest {
        descr: "BPF_ATOMIC_FETCH_ADD smoketest - 64bit",
        insns: &[
    ...
    BpfVerifierTest {
        descr: "BPF_ATOMIC_FETCH_ADD smoketest - 32bit",
        insns: &[
            BPF_MOV64_IMM(BPF_REG_0, 0),
            /* Write 3 to stack */
    ...
    BpfVerifierTest {
        descr: "Can't use ATM_FETCH_ADD on frame pointer",
        insns: &[
            BPF_MOV64_IMM(BPF_REG_0, 0),
            BPF_ST_MEM(BPF_DW, BPF_REG_10, -8, 3),
            BPF_ATOMIC_OP(
    ...
    BpfVerifierTest {
        descr: "Can't use ATM_FETCH_ADD on uninit src reg",
        insns: &[
            BPF_MOV64_IMM(BPF_REG_0, 0),
            BPF_ST_MEM(BPF_DW, BPF_REG_10, -8, 3),
            BPF_ATOMIC_OP(
    ...
    BpfVerifierTest {
        descr: "Can't use ATM_FETCH_ADD on uninit dst reg",
        insns: &[
            BPF_MOV64_IMM(BPF_REG_0, 0),
            BPF_ATOMIC_OP(
                BPF_DW,
    ...
    BpfVerifierTest {
        descr: "Can't use ATM_FETCH_ADD on kernel memory",
        insns: &[
