Test {
    name: "atomic dw/fetch and address leakage of (map ptr & -1) via stack slot",
    insns: &[
...
Test {
    name: "atomic dw/fetch and address leakage of (map ptr & -1) via returned value",
    insns: &[
        BPF_LD_IMM64(BPF_REG_1, -1),
        BPF_LD_MAP_FD(BPF_REG_8, 0),
        BPF_LD_MAP_FD(BPF_REG_9, 0),
...
Test {
    name: "atomic w/fetch and address leakage of (map ptr & -1) via stack slot",
    insns: &[
        BPF_LD_IMM64(BPF_REG_1, -1),
        BPF_LD_MAP_FD(BPF_REG_8, 0),
        BPF_LD_MAP_FD(BPF_REG_9, 0),
...
Test {
    name: "atomic w/fetch and address leakage of (map ptr & -1) via returned value",
    insns: &[
        BPF_LD_IMM64(BPF_REG_1, -1),
        BPF_LD_MAP_FD(BPF_REG_8, 0),
        BPF_LD_MAP_FD(BPF_REG_9, 0),
...
macro_rules! __atomic_fetch_op_test {
    ($src_reg:expr, $dst_reg:expr, $operand1:expr, $op:expr, $operand2:expr, $expect:expr) => {
        Test {
            name: concat!("atomic fetch ", stringify!($op), ", src=", stringify!($dst_reg), " dst=", stringify!($dst_reg)),
            insns: &[
                /* u64 val = operan1; */
