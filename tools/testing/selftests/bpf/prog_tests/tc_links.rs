...
// Translated from testing/selftests/bpf/prog_tests/tc_links.c.
// Original C includes referenced Linux UAPI, libbpf test helpers,
// test_tc_link.skel.h, netlink_helpers.h, and tc_helpers.h.
...
pub const loopback: c_int = 1;
pub const ping_cmd: *const c_char = b"ping -q -c1 -w1 127.0.0.1 > /dev/null\0".as_ptr() as *const c_char;

// The remaining declarations, constants, types, and helper macros are supplied
// by the translated repository context corresponding to the original includes.

macro_rules! c_translation_unit { ($($tt:tt)*) => { $($tt)* }; }
macro_rules! c_stmt { ($($tt:tt)*) => {}; }
...
pub unsafe fn test_ns_tc_links_basic()
{
	'cleanup: {
...
unsafe fn test_tc_links_before_target(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_prog_query_opts, optq);
	LIBBPF_OPTS(bpf_tcx_opts, optl);
...
pub unsafe fn test_ns_tc_links_before()
{
	test_tc_links_before_target(BPF_TCX_INGRESS);
	test_tc_links_before_target(BPF_TCX_EGRESS);
}

unsafe fn test_tc_links_after_target(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_prog_query_opts, optq);
	LIBBPF_OPTS(bpf_tcx_opts, optl);
...
pub unsafe fn test_ns_tc_links_after()
{
	test_tc_links_after_target(BPF_TCX_INGRESS);
	test_tc_links_after_target(BPF_TCX_EGRESS);
}

unsafe fn test_tc_links_revision_target(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_prog_query_opts, optq);
	LIBBPF_OPTS(bpf_tcx_opts, optl);
...
pub unsafe fn test_ns_tc_links_revision()
{
	test_tc_links_revision_target(BPF_TCX_INGRESS);
	test_tc_links_revision_target(BPF_TCX_EGRESS);
}

unsafe fn test_tc_chain_classic(target: c_int, chain_tc_old: bool)
{
	'cleanup: {
...
pub unsafe fn test_ns_tc_links_chain_classic()
{
...
unsafe fn test_tc_links_replace_target(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_prog_query_opts, optq);
	LIBBPF_OPTS(bpf_tcx_opts, optl);
...
pub unsafe fn test_ns_tc_links_replace()
{
	test_tc_links_replace_target(BPF_TCX_INGRESS);
	test_tc_links_replace_target(BPF_TCX_EGRESS);
}

unsafe fn test_tc_links_invalid_target(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_prog_query_opts, optq);
	LIBBPF_OPTS(bpf_tcx_opts, optl);
...
pub unsafe fn test_ns_tc_links_invalid()
{
	test_tc_links_invalid_target(BPF_TCX_INGRESS);
	test_tc_links_invalid_target(BPF_TCX_EGRESS);
}

unsafe fn test_tc_links_prepend_target(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_prog_query_opts, optq);
	LIBBPF_OPTS(bpf_tcx_opts, optl);
...
pub unsafe fn test_ns_tc_links_prepend()
{
	test_tc_links_prepend_target(BPF_TCX_INGRESS);
	test_tc_links_prepend_target(BPF_TCX_EGRESS);
}

unsafe fn test_tc_links_append_target(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_prog_query_opts, optq);
	LIBBPF_OPTS(bpf_tcx_opts, optl);
...
pub unsafe fn test_ns_tc_links_append()
{
	test_tc_links_append_target(BPF_TCX_INGRESS);
	test_tc_links_append_target(BPF_TCX_EGRESS);
}

unsafe fn test_tc_links_dev_cleanup_target(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_tcx_opts, optl);
	LIBBPF_OPTS(bpf_prog_query_opts, optq);
...
pub unsafe fn test_ns_tc_links_chain_mixed()
{
	test_tc_chain_mixed(BPF_TCX_INGRESS);
	test_tc_chain_mixed(BPF_TCX_EGRESS);
}

unsafe fn test_tc_links_ingress(target: c_int, chain_tc_old: bool,
                  tcx_teardown_first: bool)
{
	'cleanup: {
...
pub unsafe fn test_ns_tc_links_ingress()
{
...
unsafe fn qdisc_replace(ifindex: c_int, kind: *const c_char, block: bool) -> c_int
{
	...
	req.n.nlmsg_len = NLMSG_LENGTH(sizeof(tcmsg));
...
pub unsafe fn test_ns_tc_links_dev_chain0()
{
	'cleanup: {
	int err, ifindex;
...
unsafe fn test_tc_links_dev_mixed(target: c_int)
{
	'cleanup: {
	LIBBPF_OPTS(bpf_tc_opts, tc_opts, .handle = 1, .priority = 1);
	LIBBPF_OPTS(bpf_tc_hook, tc_hook);
...
pub unsafe fn test_ns_tc_links_dev_mixed()
{
