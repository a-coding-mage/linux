...
//
// Translated from include/uapi/asm-generic/socket.h.
...
// C conditional: #ifndef SO_PASSCRED
...
pub const SCM_DEVMEM_LINEAR: i32 = SO_DEVMEM_LINEAR;
pub const SO_DEVMEM_DMABUF: i32 = 79;
pub const SCM_DEVMEM_DMABUF: i32 = SO_DEVMEM_DMABUF;
...
// C conditional: #if !defined(__KERNEL__)
// These aliases are userspace-only in the original header.
//
// C conditional:
// #if __BITS_PER_LONG == 64 || (defined(__x86_64__) && defined(__ILP32__))
// on 64-bit and x32, avoid the ?: operator.
#[cfg(any(target_pointer_width = "64", all(target_arch = "x86_64", target_env = "gnu")))]
...
#[cfg(not(any(target_pointer_width = "64", all(target_arch = "x86_64", target_env = "gnu"))))]
pub const SO_TIMESTAMP: i32 = {
    if core::mem::size_of::<time_t>() == core::mem::size_of::<__kernel_long_t>() {
        SO_TIMESTAMP_OLD
    } else {
        SO_TIMESTAMP_NEW
    }
