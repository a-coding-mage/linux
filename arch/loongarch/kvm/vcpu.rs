...
// Low-level Rust translation of loongarch/kvm/vcpu.c.
// Kernel-provided types, constants, functions, and macros remain external dependencies.
...
const kvm_stats_desc kvm_vcpu_stats_desc[] = {
...
const kvm_stats_header kvm_vcpu_stats_header = {
	name_size: KVM_STATS_NAME_SIZE,
	num_desc: ARRAY_SIZE(kvm_vcpu_stats_desc),
	id_offset: sizeof(kvm_stats_header),
	desc_offset: sizeof(kvm_stats_header) + KVM_STATS_NAME_SIZE,
	data_offset: sizeof(kvm_stats_header) + KVM_STATS_NAME_SIZE +
		       sizeof(kvm_vcpu_stats_desc),
};

void kvm_save_host_pmu(*mut kvm_vcpuvcpu)
{
	*mut kvm_contextcontext;
...
void kvm_restore_host_pmu(*mut kvm_vcpuvcpu)
{
	*mut kvm_contextcontext;
...
void kvm_save_guest_pmu(*mut kvm_vcpuvcpu)
{
	*mut loongarch_csrscsr = (*vcpu).arch.csr;
...
void kvm_restore_guest_pmu(*mut kvm_vcpuvcpu)
{
	*mut loongarch_csrscsr = (*vcpu).arch.csr;
...
static int kvm_own_pmu(*mut kvm_vcpuvcpu)
{
	u64 val;
...
static void kvm_lose_pmu(*mut kvm_vcpuvcpu)
{
	u64 val;
	*mut loongarch_csrscsr = (*vcpu).arch.csr;
...
static void kvm_update_stolen_time(*mut kvm_vcpuvcpu)
{
	'out: {
	u32 version;
	u64 steal;
	gpa_t gpa;
	*mut kvm_memslotsslots;
	kvm_steal_time __user *st;
	*mut gfn_to_hva_cacheghc;
	...
	st = (*(kvm_steal_time __user *)ghc).hva;
 ...
 */
static int kvm_check_requests(*mut kvm_vcpuvcpu)
{
...
static void kvm_late_check_requests(*mut kvm_vcpuvcpu)
{
 ...
 *         indicates an error
 */
static int kvm_enter_guest_check(*mut kvm_vcpuvcpu)
{
	int idx, ret;
 ...
 */
static int kvm_pre_enter_guest(*mut kvm_vcpuvcpu)
{
	int ret;
 ...
 */
static int kvm_handle_exit(*mut kvm_runrun, *mut kvm_vcpuvcpu)
{
...
int kvm_arch_vcpu_runnable(*mut kvm_vcpuvcpu)
{
	return !!((*vcpu).arch.irq_pending) &&
		(*vcpu).arch.mp_state.mp_state == KVM_MP_STATE_RUNNABLE;
}

int kvm_arch_vcpu_should_kick(*mut kvm_vcpuvcpu)
{
	return kvm_vcpu_exiting_guest_mode(vcpu) == IN_GUEST_MODE;
}

bool kvm_arch_vcpu_in_kernel(*mut kvm_vcpuvcpu)
{
	u64 val;
...
u64 kvm_arch_vcpu_get_ip(*mut kvm_vcpuvcpu)
{
	return (*vcpu).arch.pc;
}
 ...
 */
bool kvm_arch_pmi_in_guest(*mut kvm_vcpuvcpu)
{
	return (vcpu && !((*vcpu).arch.aux_inuse & KVM_LARCH_PMU));
}
// #endif

bool kvm_arch_vcpu_preempted_in_kernel(*mut kvm_vcpuvcpu)
{
	return false;
}

vm_fault_t kvm_arch_vcpu_fault(*mut kvm_vcpuvcpu, *mut vm_faultvmf)
{
	return VM_FAULT_SIGBUS;
}

int kvm_arch_vcpu_ioctl_translate(*mut kvm_vcpuvcpu,
				  *mut kvm_translationtr)
{
	return -EINVAL;
}

int kvm_cpu_has_pending_timer(*mut kvm_vcpuvcpu)
{
	int ret;
...
int kvm_arch_vcpu_dump_regs(*mut kvm_vcpuvcpu)
{
	int i;
...
int kvm_arch_vcpu_ioctl_get_mpstate(*mut kvm_vcpuvcpu,
				*mut kvm_mp_statemp_state)
{
...
int kvm_arch_vcpu_ioctl_set_mpstate(*mut kvm_vcpuvcpu,
				*mut kvm_mp_statemp_state)
{
	int ret = 0;
...
int kvm_arch_vcpu_ioctl_set_guest_debug(*mut kvm_vcpuvcpu,
					*mut kvm_guest_debugdbg)
{
...
int kvm_set_cpuid(*mut kvm_vcpuvcpu, val: u64)
{
	int cpuid;
	*mut kvm_phyid_mapmap;
	*mut loongarch_csrscsr = (*vcpu).arch.csr;
...
void kvm_drop_cpuid(*mut kvm_vcpuvcpu)
{
	int cpuid;
	*mut kvm_phyid_mapmap;
	*mut loongarch_csrscsr = (*vcpu).arch.csr;
	...
	if ((*map).phys_map[cpuid].enabled) {
		(*map).phys_map[cpuid].vcpu = core::ptr::null_mut();
...
*mut kvm_vcpukvm_get_vcpu_by_cpuid(*mut kvmkvm, int cpuid)
{
	*mut kvm_phyid_mapmap;
...
static int _kvm_getcsr(*mut kvm_vcpuvcpu, id: u32, u64 *val)
{
	estat: u64, gintc;
	*mut loongarch_csrscsr = (*vcpu).arch.csr;
...
static int _kvm_setcsr(*mut kvm_vcpuvcpu, id: u32, val: u64)
{
	int ret = 0;
	estat: u64, gintc;
	*mut loongarch_csrscsr = (*vcpu).arch.csr;
...
static int kvm_check_cpucfg(int id, val: u64)
{
	int ret;
	u32 host;
	u64 mask = 0;
...
static int kvm_get_one_reg(*mut kvm_vcpuvcpu,
		const *mut kvm_one_regreg, u64 *v)
{
	int id, ret = 0;
...
static int kvm_get_reg(*mut kvm_vcpuvcpu, const *mut kvm_one_regreg)
{
	int ret = 0;
	v: u64, size = (*reg).id & KVM_REG_SIZE_MASK;
...
static int kvm_set_one_reg(*mut kvm_vcpuvcpu,
			const *mut kvm_one_regreg, v: u64)
{
	int id, ret = 0;
	u64 type = (*reg).id & KVM_REG_LOONGARCH_MASK;
...
static int kvm_set_reg(*mut kvm_vcpuvcpu, const *mut kvm_one_regreg)
{
	int ret = 0;
	v: u64, size = (*reg).id & KVM_REG_SIZE_MASK;
...
int kvm_arch_vcpu_ioctl_get_sregs(*mut kvm_vcpuvcpu, *mut kvm_sregssregs)
{
	return -ENOIOCTLCMD;
}

int kvm_arch_vcpu_ioctl_set_sregs(*mut kvm_vcpuvcpu, *mut kvm_sregssregs)
{
	return -ENOIOCTLCMD;
}

int kvm_arch_vcpu_ioctl_get_regs(*mut kvm_vcpuvcpu, *mut kvm_regsregs)
{
	int i;
...
int kvm_arch_vcpu_ioctl_set_regs(*mut kvm_vcpuvcpu, *mut kvm_regsregs)
{
	int i;
...
static int kvm_vcpu_ioctl_enable_cap(*mut kvm_vcpuvcpu,
				     *mut kvm_enable_capcap)
{
	/* FPU is enabled by default, will support LSX/LASX later. */
	return -EINVAL;
}

static int kvm_loongarch_cpucfg_has_attr(*mut kvm_vcpuvcpu,
					 *mut kvm_device_attrattr)
{
	switch ((*attr).attr) {
	case LOONGARCH_CPUCFG2:
	case LOONGARCH_CPUCFG6:
...
static int kvm_loongarch_pvtime_has_attr(*mut kvm_vcpuvcpu,
					 *mut kvm_device_attrattr)
{
...
static int kvm_loongarch_vcpu_has_attr(*mut kvm_vcpuvcpu,
				       *mut kvm_device_attrattr)
{
	int ret = -ENXIO;
...
static int kvm_loongarch_cpucfg_get_attr(*mut kvm_vcpuvcpu,
					 *mut kvm_device_attrattr)
{
	int ret = 0;
	u64 val;
...
static int kvm_loongarch_pvtime_get_attr(*mut kvm_vcpuvcpu,
					 *mut kvm_device_attrattr)
{
	u64 gpa;
...
static int kvm_loongarch_vcpu_get_attr(*mut kvm_vcpuvcpu,
				       *mut kvm_device_attrattr)
{
	int ret = -ENXIO;
...
static int kvm_loongarch_cpucfg_set_attr(*mut kvm_vcpuvcpu,
					 *mut kvm_device_attrattr)
{
	val: u64, valid;
	u64 __user *user = (*(u64 __user *)attr).addr;
...
static int kvm_loongarch_pvtime_set_attr(*mut kvm_vcpuvcpu,
					 *mut kvm_device_attrattr)
{
	int idx, ret = 0;
	gpa: u64, __user *user = (*(u64 __user *)attr).addr;
	*mut kvmkvm = (*vcpu).kvm;
...
static int kvm_loongarch_vcpu_set_attr(*mut kvm_vcpuvcpu,
				       *mut kvm_device_attrattr)
{
	int ret = -ENXIO;
...
long kvm_arch_vcpu_ioctl(*mut filefilp,
			 ioctl: u32, arg: u64)
{
	long r;
	kvm_device_attr attr;
	void __user *argp = (void __user *)arg;
	*mut kvm_vcpuvcpu = filp->private_data;
...
int kvm_arch_vcpu_ioctl_get_fpu(*mut kvm_vcpuvcpu, *mut kvm_fpufpu)
{
	int i = 0;
	...
	for (i = 0; i < NUM_FPU_REGS; i++)
		memcpy(&fpu->fpr[i], &vcpu->arch.fpu.fpr[i], sizeof(fpureg));

	return 0;
}

int kvm_arch_vcpu_ioctl_set_fpu(*mut kvm_vcpuvcpu, *mut kvm_fpufpu)
{
	int i = 0;
	...
	for (i = 0; i < NUM_FPU_REGS; i++)
		memcpy(&vcpu->arch.fpu.fpr[i], &fpu->fpr[i], sizeof(fpureg));

	return 0;
}
// #ifdef CONFIG_CPU_HAS_LBT
int kvm_own_lbt(*mut kvm_vcpuvcpu)
{
...
static void kvm_lose_lbt(*mut kvm_vcpuvcpu)
{
	preempt_disable();
	if (vcpu->arch.aux_inuse & KVM_LARCH_LBT) {
...
static void kvm_check_fcsr(*mut kvm_vcpuvcpu, fcsr: u64)
{
	/*
...
static void kvm_check_fcsr_alive(*mut kvm_vcpuvcpu)
{
...
// #else
void kvm_lose_lbt(*mut kvm_vcpuvcpu) { }
void kvm_check_fcsr(*mut kvm_vcpuvcpu, fcsr: u64) { }
void kvm_check_fcsr_alive(*mut kvm_vcpuvcpu) { }
// #endif

/* Enable FPU and restore context */
void kvm_own_fpu(*mut kvm_vcpuvcpu)
{
	/*
...
int kvm_own_lsx(*mut kvm_vcpuvcpu)
{
...
int kvm_own_lasx(*mut kvm_vcpuvcpu)
{
	kvm_check_fcsr(vcpu, vcpu->arch.fpu.fcsr);
...
void kvm_lose_fpu(*mut kvm_vcpuvcpu)
{
	'done: {
	preempt_disable();
...
int kvm_vcpu_ioctl_interrupt(*mut kvm_vcpuvcpu, *mut kvm_interruptirq)
{
...
long kvm_arch_vcpu_unlocked_ioctl(*mut filefilp, ioctl: u32,
				  arg: u64)
{
	void __user *argp = (void __user *)arg;
	*mut kvm_vcpuvcpu = filp->private_data;
...
int kvm_arch_vcpu_precreate(*mut kvmkvm, id: u32)
{
	return 0;
}

int kvm_arch_vcpu_create(*mut kvm_vcpuvcpu)
{
	u64 timer_hz;
	*mut loongarch_csrscsr;
	 ...
	 */
	vcpu->arch.host_pgd = (u64)vcpu->kvm->mm->pgd;

	vcpu->arch.handle_exit = kvm_handle_exit;
	vcpu->arch.guest_eentry = (u64)kvm_loongarch_ops->exc_entry;
	vcpu->arch.csr = kzalloc_obj(loongarch_csrs);
	if (!vcpu->arch.csr)
		return -ENOMEM;
...
void kvm_arch_vcpu_postcreate(*mut kvm_vcpuvcpu)
{
}

void kvm_arch_vcpu_destroy(*mut kvm_vcpuvcpu)
{
	int cpu;
	*mut kvm_contextcontext;
...
static int _kvm_vcpu_load(*mut kvm_vcpuvcpu, int cpu)
{
	bool migrated;
	*mut kvm_contextcontext;
	*mut loongarch_csrscsr = vcpu->arch.csr;
...
void kvm_arch_vcpu_load(*mut kvm_vcpuvcpu, int cpu)
{
	u64 flags;
...
static int _kvm_vcpu_put(*mut kvm_vcpuvcpu, int cpu)
{
	'out: {
	*mut loongarch_csrscsr = vcpu->arch.csr;
...
static void kvm_vcpu_set_pv_preempted(*mut kvm_vcpuvcpu)
{
	'out: {
	gpa_t gpa;
	*mut gfn_to_hva_cacheghc;
...
void kvm_arch_vcpu_put(*mut kvm_vcpuvcpu)
{
	int cpu, idx;
	u64 flags;
...
int kvm_arch_vcpu_ioctl_run(*mut kvm_vcpuvcpu)
{
	'out: {
	int r = -EINTR;
	*mut kvm_runrun = vcpu->run;
