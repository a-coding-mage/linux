...
// Dependencies supplied by the Linux PXA platform and local headers.

static mut pxa320_mfp_addr_map: [mfp_addr_map; 47] = [
...
unsafe fn pxa320_init() -> i32 {
    if cpu_is_pxa320() {
        ...
        mfp_init_addr(pxa320_mfp_addr_map.as_mut_ptr());
    }
