...
#[cfg(any(CONFIG_CURRENT_POINTER_IN_TPIDRURO, CONFIG_SMP))]
extern "C" {
...
#[cfg(not(CONFIG_CURRENT_POINTER_IN_TPIDRURO))]
...
#[no_mangle] pub unsafe extern "C" fn arch_cpu_idle_enter() {
    ...
    #[cfg(CONFIG_PL310_ERRATA_769419)] wmb();
}
...
pub unsafe extern "C" fn __show_regs(regs: *mut pt_regs) {
    ...
    #[cfg(CONFIG_CPU_V7M)] printk(b"xPSR: %08lx\0".as_ptr(), (*regs).ARM_cpsr);
}
...
pub unsafe extern "C" fn copy_thread(p: *mut task_struct, args: *const kernel_clone_args) -> c_int {
    ...
    #[cfg(CONFIG_CPU_USE_DOMAINS)] { (*thread).cpu_domain = get_domain(); }
...
#[cfg(all(CONFIG_MMU, CONFIG_KUSER_HELPERS))]
...
#[cfg(CONFIG_MMU)]
#[no_mangle] pub unsafe extern "C" fn arch_vma_name(vma: *mut vm_area_struct) -> *const u8 { #[cfg(CONFIG_KUSER_HELPERS)] if vma == &mut gate_vma { return b"[vectors]\0".as_ptr(); } core::ptr::null() }
