// Direct source-level Rust translation of softfloat.c.
// External types, helpers, constants, and target-specific items are supplied by dependencies.
/*
===============================================================================
...
// External dependency from the original source: #include <asm/div64.h>

// External dependency from the original source: #include "fpa11.h"
//#include "milieu.h"
//#include "softfloat.h"
...
-------------------------------------------------------------------------------
*/
// External dependency from the original source: #include "softfloat-macros"
...
-------------------------------------------------------------------------------
*/
// External dependency from the original source: #include "softfloat-specialize"
...
// Conditional compilation intent: #endif
...
// Conditional compilation intent: #if 0
   ...
   __asm__("@ packFloat32				\n\
   	    ...
   	    : "cc");
   return f;
// Conditional compilation intent: #else
    return ( ( (u32) zSign )<<31 ) + ( ( (u32) zExp )<<23 ) + zSig;
// Conditional compilation intent: #endif 
}
...
// Conditional compilation intent: #ifdef FLOATX80
...
*/
#[inline]
fn u64 extractFloatx80Frac( floatx80 a )
{
...
*/
#[inline]
fn i32 extractFloatx80Exp( floatx80 a )
{
...
*/
#[inline]
fn bool extractFloatx80Sign( floatx80 a )
{
...
u64 float32_to_float64( u32 a )
{
    ...
    return packFloat64( aSign, aExp + 0x380, ( (u64) aSig )<<29 );
...
floatx80 float32_to_floatx80( u32 a )
{
    ...
    aSig |= 0x00800000;
    return packFloatx80( aSign, aExp + 0x3F80, ( (u64) aSig )<<40 );
...
u32 float32_mul( RoundingData *roundData, u32 a, u32 b )
{
    ...
    shift64RightJamming( ( (u64) aSig ) * bSig, 32, &zSig64 );
    zSig = zSig64;
    if ( 0 <= (i32) ( zSig<<1 ) ) {
        zSig <<= 1;
        --zExp;
...
bool float32_le( u32 a, u32 b )
{
    ...
    aSign = extractFloat32Sign( a );
    bSign = extractFloat32Sign( b );
    if ( aSign != bSign ) return aSign || ( (u32) ( ( a | b )<<1 ) == 0 );
...
bool float64_le( u64 a, u64 b )
{
    ...
    aSign = extractFloat64Sign( a );
    bSign = extractFloat64Sign( b );
    if ( aSign != bSign ) return aSign || ( (u64) ( ( a | b )<<1 ) == 0 );
    return ( a == b ) || ( aSign ^ ( a < b ) );
...
i32 floatx80_to_int32( RoundingData *roundData, floatx80 a )
{
    ...
    if ( ( aExp == 0x7FFF ) && (u64) ( aSig<<1 ) ) aSign = 0;
...
-------------------------------------------------------------------------------
*/
u32 floatx80_to_float32( RoundingData *roundData, floatx80 a )
{
    bool aSign;
    i32 aExp;
    u64 aSig;
...
-------------------------------------------------------------------------------
*/
u64 floatx80_to_float64( RoundingData *roundData, floatx80 a )
{
    bool aSign;
    i32 aExp;
    u64 aSig, zSig;
...
-------------------------------------------------------------------------------
*/
floatx80 floatx80_round_to_int( RoundingData *roundData, floatx80 a )
{
    bool aSign;
    i32 aExp;
    u64 lastBitMask, roundBitsMask;
    ...
    aExp = extractFloatx80Exp( a );
    if ( 0x403E <= aExp ) {
        if ( ( aExp == 0x7FFF ) && (u64) ( extractFloatx80Frac( a )<<1 ) ) {
    ...
    if ( aExp <= 0x3FFE ) {
        ...
        switch ( roundData.mode ) {
         case float_round_nearest_even:
            if ( ( aExp == 0x3FFE ) && (u64) ( extractFloatx80Frac( a )<<1 )
               ) {
                return
...
floatx80 floatx80_sqrt( RoundingData *roundData, floatx80 a )
{
    ...
    aSign = extractFloatx80Sign( a );
    if ( aExp == 0x7FFF ) {
        if ( (u64) ( aSig0<<1 ) ) return propagateFloatx80NaN( a, a );
        if ( ! aSign ) return a;
        goto invalid;
    }
    if ( aSign ) {
 ...
 invalid:
    ...
    if ( 0 <= (i64) zSig0 ) zSig0 = LIT64( 0xFFFFFFFFFFFFFFFF );
...
-------------------------------------------------------------------------------
*/
bool floatx80_eq( floatx80 a, floatx80 b )
{

    if (    (    ( extractFloatx80Exp( a ) == 0x7FFF )
              && (u64) ( extractFloatx80Frac( a )<<1 ) )
         || (    ( extractFloatx80Exp( b ) == 0x7FFF )
              && (u64) ( extractFloatx80Frac( b )<<1 ) )
       ) {
...
bool floatx80_le( floatx80 a, floatx80 b )
{
    ...
    if ( aSign != bSign ) {
        return
               aSign
            || (    ( ( (u16) ( ( a.high | b.high )<<1 ) ) | a.low | b.low )
                 == 0 );
    }
    return
...
-------------------------------------------------------------------------------
*/
bool floatx80_eq_signaling( floatx80 a, floatx80 b )
{
