...
// Dependency declarations supplied by the kernel's I/O interface.
extern "C" {
    fn inb(port: u16) -> u8;
    fn outb(value: u8, port: u16);
}

pub unsafe fn es1888_init() {
    ...
    while (inb(0x022e) & 0x80 == 0) { // wait for bit 7 to assert
        continue;
    }
    inb(0x022a); // pause
    outb(0xc6, 0x022c); // enable extended mode
    inb(0x022a); // pause, also forces the write
    while (inb(0x022c) & 0x80 != 0) { // wait for bit 7 to deassert
        continue;
    }
