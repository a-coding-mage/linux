...
#[cfg(all(CONFIG_NVRAM, CONFIG_ADB_CUDA))]
...
#[cfg(all(CONFIG_NVRAM, CONFIG_ADB_PMU))]
...
pub unsafe fn mac_pram_read_byte(addr: i32) -> u8 { match (*macintosh_config).adb_type {
    ...
    #[cfg(CONFIG_ADB_CUDA)] MAC_ADB_EGRET | MAC_ADB_CUDA => cuda_pram_read_byte(addr),
    #[cfg(CONFIG_ADB_PMU)] MAC_ADB_PB2 => pmu_pram_read_byte(addr), _ => 0xff } }
...
pub unsafe fn mac_pram_write_byte(val: u8, addr: i32) { match (*macintosh_config).adb_type {
    ...
    #[cfg(CONFIG_ADB_CUDA)] MAC_ADB_EGRET | MAC_ADB_CUDA => cuda_pram_write_byte(val, addr),
    #[cfg(CONFIG_ADB_PMU)] MAC_ADB_PB2 => pmu_pram_write_byte(val, addr), _ => {} } }

pub unsafe fn mac_poweroff() -> ! { if oss_present { oss_shutdown(); } else if (*macintosh_config).adb_type == MAC_ADB_II { via_shutdown(); }
    #[cfg(CONFIG_ADB_CUDA)] if (*macintosh_config).adb_type == MAC_ADB_EGRET || (*macintosh_config).adb_type == MAC_ADB_CUDA { cuda_shutdown(); }
    pr_crit("It is now safe to turn off your Macintosh.\n"); local_irq_disable(); loop {} }

pub unsafe fn mac_reset() -> ! {
    #[cfg(CONFIG_ADB_CUDA)] if (*macintosh_config).adb_type == MAC_ADB_EGRET || (*macintosh_config).adb_type == MAC_ADB_CUDA { cuda_restart(); }
