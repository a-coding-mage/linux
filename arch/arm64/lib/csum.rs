...
unsafe fn accumulate(sum: u64, data: u64) -> u64 {
    let tmp = (sum as u128).wrapping_add(data as u128);
    (tmp.wrapping_add(tmp >> 64)) as u64
}
...
extern "C" {
    fn kasan_check_read(addr: *const u8, len: usize);
...
pub unsafe fn do_csum(buff: *const u8, mut len: i32) -> u32 {
    ...
    offset = (buff as usize) & 7;
    kasan_check_read(buff, len as usize);
    ptr = buff.sub(offset) as *const u64;
    len = len + offset as i32 - 8;

    shift = (offset * 8) as u32;
    data = ptr.read();
    ptr = ptr.add(1);
    ...
    while len > 64 {
        let mut tmp1 = (ptr as *const u128).read();
        let mut tmp2 = (ptr.add(2) as *const u128).read();
        let mut tmp3 = (ptr.add(4) as *const u128).read();
        let mut tmp4 = (ptr.add(6) as *const u128).read();
    ...
    while len > 8 {
        ...
        let tmp = (ptr as *const u128).read();
        len -= 16;
        ptr = ptr.add(2);
        #[cfg(target_endian = "little")]
        {
        ...
        {
            data = tmp as u64;
            sum64 = accumulate(sum64, (tmp >> 64) as u64);
        }
    }
    if len > 0 {
        sum64 = accumulate(sum64, data);
    ...
    shift = (len.wrapping_mul(-8) as u32);
    #[cfg(target_endian = "little")]
    {
        data = (data << shift) >> shift;
    }
    ...
    if offset & 1 != 0 {
        return swab32(sum) as u16 as u32;
    }
    sum >> 16
}
...
pub unsafe fn csum_ipv6_magic(
    ...
    let mut src = (saddr.read().s6_addr.as_ptr() as *const u128).read();
    let mut dst = (daddr.read().s6_addr.as_ptr() as *const u128).read();
    let mut sum = csum as u64;
    sum = sum.wrapping_add(htonl(len) as u64);
    #[cfg(target_endian = "little")]
    {
        sum = sum.wrapping_add((proto as u64) << 24);
    }
    #[cfg(target_endian = "big")]
    {
