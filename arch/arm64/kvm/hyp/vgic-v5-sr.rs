...
pub unsafe fn __vgic_v5_save_apr(cpu_if: *mut vgic_v5_cpu_if) {
	(*cpu_if).vgic_apr = read_sysreg_s(SYS_ICH_APR_EL2);
}

unsafe fn __vgic_v5_compat_mode_disable() {
	sysreg_clear_set_s(SYS_ICH_VCTLR_EL2, ICH_VCTLR_EL2_V3, 0);
	isb();
}

pub unsafe fn __vgic_v5_restore_vmcr_apr(cpu_if: *mut vgic_v5_cpu_if) {
...
pub unsafe fn __vgic_v5_save_ppi_state(cpu_if: *mut vgic_v5_cpu_if) {
	...
	bitmap_write(
		host_data_ptr(vgic_v5_ppi_state).activer_exit,
		read_sysreg_s(SYS_ICH_PPI_ACTIVER0_EL2),
		0,
		64,
	);
	bitmap_write(
		host_data_ptr(vgic_v5_ppi_state).pendr,
...
pub unsafe fn __vgic_v5_restore_ppi_state(cpu_if: *mut vgic_v5_cpu_if) {
	let mut pendr: [u64; 1] = [0; 1];

	/* Enable DVI so that the guest's interrupt config takes over */
	write_sysreg_s(
		bitmap_read((*cpu_if).vgic_ppi_dvir, 0, 64),
		SYS_ICH_PPI_DVIR0_EL2,
	);

	write_sysreg_s(
		bitmap_read((*cpu_if).vgic_ppi_activer, 0, 64),
		SYS_ICH_PPI_ACTIVER0_EL2,
	);
	write_sysreg_s(
		bitmap_read((*cpu_if).vgic_ppi_enabler, 0, 64),
	...
	bitmap_andnot(
		&mut pendr,
		host_data_ptr(vgic_v5_ppi_state).pendr,
		(*cpu_if).vgic_ppi_dvir,
		VGIC_V5_NR_PRIVATE_IRQS,
	);
	write_sysreg_s(bitmap_read(&pendr, 0, 64), SYS_ICH_PPI_PENDR0_EL2);

	write_sysreg_s((*cpu_if).vgic_ppi_priorityr[0], SYS_ICH_PPI_PRIORITYR0_EL2);
	write_sysreg_s((*cpu_if).vgic_ppi_priorityr[1], SYS_ICH_PPI_PRIORITYR1_EL2);
	write_sysreg_s((*cpu_if).vgic_ppi_priorityr[2], SYS_ICH_PPI_PRIORITYR2_EL2);
	write_sysreg_s((*cpu_if).vgic_ppi_priorityr[3], SYS_ICH_PPI_PRIORITYR3_EL2);
	write_sysreg_s((*cpu_if).vgic_ppi_priorityr[4], SYS_ICH_PPI_PRIORITYR4_EL2);
	write_sysreg_s((*cpu_if).vgic_ppi_priorityr[5], SYS_ICH_PPI_PRIORITYR5_EL2);
	write_sysreg_s((*cpu_if).vgic_ppi_priorityr[6], SYS_ICH_PPI_PRIORITYR6_EL2);
	write_sysreg_s((*cpu_if).vgic_ppi_priorityr[7], SYS_ICH_PPI_PRIORITYR7_EL2);
...
pub unsafe fn __vgic_v5_save_state(cpu_if: *mut vgic_v5_cpu_if) {
	(*cpu_if).vgic_vmcr = read_sysreg_s(SYS_ICH_VMCR_EL2);
	(*cpu_if).vgic_icsr = read_sysreg_s(SYS_ICC_ICSR_EL1);
}

pub unsafe fn __vgic_v5_restore_state(cpu_if: *mut vgic_v5_cpu_if) {
