...
// external kernel dependency: #include <linux/bitfield.h>
// external kernel dependency: #include <linux/kvm.h>
// external kernel dependency: #include <linux/kvm_host.h>

// external kernel dependency: #include <asm/fixmap.h>
// external kernel dependency: #include <asm/kvm_arm.h>
// external kernel dependency: #include <asm/kvm_emulate.h>
// external kernel dependency: #include <asm/kvm_mmu.h>
// external kernel dependency: #include <asm/kvm_nested.h>
// external kernel dependency: #include <asm/sysreg.h>

// external kernel dependency: #include "sys_regs.h"
// external kernel dependency: #include "vgic/vgic.h"
...
{
	kvm.arch.nested_mmus = core::ptr::null_mut();
	kvm.arch.nested_mmus_size = 0;
	atomic_set(&kvm.arch.vncr_tlb_count, 0);
}

fn i32 init_nested_s2_mmu(kvm *kvm, kvm_s2_mmu *mmu)
{
	/*
...
i32 kvm_vcpu_init_nested(kvm_vcpu *vcpu)
{
	kvm *kvm = vcpu.kvm;
	kvm_s2_mmu *tmp;
	i32 num_mmus, ret = 0;
	...
	if (num_mmus > kvm.arch.nested_mmus_size) {
		...
		if (kvm.arch.nested_mmus_size) {
			...
			for (i32 i = 0; i < kvm.arch.nested_mmus_size; i++)
				tmp[i].pgt.mmu = &tmp[i];
		}
	...
	for (i32 i = kvm.arch.nested_mmus_size; !ret && i < num_mmus; i++)
		ret = init_nested_s2_mmu(kvm, &kvm.arch.nested_mmus[i]);

	if (ret) {
		for (i32 i = kvm.arch.nested_mmus_size; i < num_mmus; i++)
			kvm_free_stage2_pgd(&kvm.arch.nested_mmus[i]);

		free_page((usize)vcpu.arch.ctxt.vncr_array);
		vcpu.arch.ctxt.vncr_array = core::ptr::null_mut();
...
s2_walk_info {
	u64		baddr;
	unsigned i32	max_oa_bits;
	unsigned i32	pgshift;
	unsigned i32	sl;
	unsigned i32	t0sz;
	bool		be;
	bool		ha;
...
fn i32 get_ia_size(s2_walk_info *wi)
{
	return 64 - wi.t0sz;
}
...
again:
	...
	tmp &= ~(sz - 1);
	if (kvm_pgtable_get_leaf(mmu.pgt, tmp, &pte, core::ptr::null_mut()))
		goto again;
	if (!(pte & PTE_VALID))
		goto again;
						      ...
						      const union tlbi_info *))
{
	...
	for (i32 i = 0; i < kvm.arch.nested_mmus_size; i++) {
		kvm_s2_mmu *mmu = &kvm.arch.nested_mmus[i];
...
kvm_s2_mmu *lookup_s2_mmu(kvm_vcpu *vcpu)
{
	kvm *kvm = vcpu.kvm;
...
fn kvm_s2_mmu *get_s2_mmu_nested(kvm_vcpu *vcpu)
{
	...
	BUG_ON(atomic_read(&s2_mmu.refcnt)); /* We have MMUs to spare */
...
fn kvm_init_nested_s2_mmu(kvm_s2_mmu *mmu)
{
...
fn kvm_vcpu_load_hw_mmu(kvm_vcpu *vcpu)
{
	/*
 ...
 */
fn i32 unmap_l1_vncr(vncr_tlb *vt)
{
	i32 cpu = atomic_xchg_relaxed(&vt.cpu, -1);
...
fn fn this_cpu_reset_vncr_fixmap(kvm_vcpu *vcpu)
{
...
fn kvm_vcpu_put_hw_mmu(kvm_vcpu *vcpu)
{
	...
	vcpu.arch.hw_mmu = core::ptr::null_mut();
}
 ...
 * level hypervisor.
 */
i32 kvm_s2_handle_perm_fault(kvm_vcpu *vcpu, kvm_s2_trans *trans)
{
...
fn fn invalidate_vncr(kvm *kvm, vncr_tlb *vt)
{
	BUG_ON(!vt.valid);
	vt.valid = false;
	unmap_l1_vncr(vt);
...
fn fn invalidate_vncr_va(kvm *kvm,
...
fn kvm_nested_s2_flush(kvm *kvm)
{
	i32 i;
...
fn kvm_arch_flush_shadow_all(kvm *kvm)
{
	i32 i;
...
i32 kvm_vcpu_allocate_vncr_tlb(kvm_vcpu *vcpu)
{
	if (!kvm_has_feat(vcpu.kvm, ID_AA64MMFR4_EL1, NV_frac, NV2_ONLY))
		return 0;

	if (!vcpu.arch.vncr_tlb) {
		vncr_tlb *vt = kzalloc_obj(*vcpu.arch.vncr_tlb,
						  GFP_KERNEL_ACCOUNT);
...
fn fn read_vncr_el2(kvm_vcpu *vcpu)
{
...
fn i32 kvm_translate_vncr(kvm_vcpu *vcpu, bool *is_gmem)
{
	...
	*is_gmem = kvm_slot_has_gmem(memslot);
	if (!*is_gmem) {
		...
		ret = kvm_gmem_get_pfn(vcpu.kvm, memslot, gfn, &pfn, &page, core::ptr::null_mut());
		if (ret) {
...
fn fn handle_vncr_perm(kvm_vcpu *vcpu)
{
	vncr_tlb *vt = vcpu.arch.vncr_tlb;
...
i32 kvm_handle_vncr_abort(kvm_vcpu *vcpu)
{
	vncr_tlb *vt = vcpu.arch.vncr_tlb;
	u64 esr = kvm_vcpu_get_esr(vcpu);
	bool is_gmem = false;
...
fn fn kvm_map_l1_vncr(kvm_vcpu *vcpu)
{
	vncr_tlb *vt = vcpu.arch.vncr_tlb;
	pgprot_t prot;
...
fn check_nested_vcpu_requests(kvm_vcpu *vcpu)
{
	if (kvm_check_request(KVM_REQ_NESTED_S2_UNMAP, vcpu)) {
		kvm_s2_mmu *mmu = vcpu.arch.hw_mmu;
 ...
 */
fn kvm_nested_flush_hwstate(kvm_vcpu *vcpu)
{
	if (!vcpu_has_nv(vcpu))
		return;
...
fn kvm_nested_sync_hwstate(kvm_vcpu *vcpu)
{
	usize *hcr = vcpu_hcr(vcpu);
...
fn kvm_nested_setup_mdcr_el2(kvm_vcpu *vcpu)
{
