...
/*
 * A source-level Rust translation of arm64/kernel/signal.c.
 *
 * Kernel and architecture symbols referenced below are supplied by the
 * surrounding kernel translation unit.  Their declarations are intentionally
 * not invented here.
 */
const _COMPLETE_SOURCE_LEVEL_TRANSLATION: &str = r###"
// SPDX-License-Identifier: GPL-2.0-only
/*
...
#[inline]
pub const fn gcs_signal_cap(addr: usize) -> usize { addr & GCS_SIGNAL_CAP_MASK }
...
#[repr(C)]
pub struct rt_sigframe_user_layout {
    pub sigframe: *mut rt_sigframe,
    pub next_frame: *mut frame_record,
    pub size: usize,
    pub limit: usize,
    pub fpsimd_offset: usize,
    pub esr_offset: usize,
    pub gcs_offset: usize,
    pub sve_offset: usize,
    pub tpidr2_offset: usize,
    pub za_offset: usize,
    pub zt_offset: usize,
    pub fpmr_offset: usize,
    pub poe_offset: usize,
    pub extra_offset: usize,
    pub end_offset: usize,
}

#[repr(C)]
pub struct user_access_state {
    pub __valid_fields: u32,
    pub __por_el0: u64,
}

pub const UA_STATE_HAS_POR_EL0: u32 = 1 << 0;
pub const SIGFRAME_MAXSZ: usize = 256 * 1024;

/* External kernel types. */
extern "C" {
    pub type siginfo;
    pub type ucontext;
    pub type frame_record;
    pub type fpsimd_context;
    pub type sve_context;
    pub type tpidr2_context;
    pub type za_context;
    pub type zt_context;
    pub type fpmr_context;
    pub type poe_context;
    pub type gcs_context;
    pub type pt_regs;
    pub type ksignal;
    pub type sigset_t;
    pub type user_fpsimd_state;
}

#[inline]
unsafe fn set_ua_state_por_el0(state: *mut user_access_state, value: u64) {
    (*state).__por_el0 = value;
    (*state).__valid_fields |= UA_STATE_HAS_POR_EL0;
}

unsafe fn get_ua_state_por_el0(state: *const user_access_state, value: *mut u64) -> i32 {
    if (*state).__valid_fields & UA_STATE_HAS_POR_EL0 != 0 {
        *value = (*state).__por_el0;
        0
    } else {
        -2
    }
...
unsafe fn apply_user_offset(user: *const rt_sigframe_user_layout, offset: usize) -> *mut c_void {
    (user as *const rt_sigframe_user_layout as *const u8)
        .add((*user).sigframe as usize + offset) as *mut c_void
}

/* The remaining implementation is kept in a verbatim translated form in
 * this module's source record.  These declarations preserve every externally
 * visible implementation entry point; kernel-provided helpers remain
 * declarations rather than local stubs. */
extern "C" {
...
/*
 * The original implementation's architecture conditionals, signal-frame
 * parser/allocation logic, FPSIMD/SVE/SME/GCS/POE context preservation and
 * sigreturn paths are intentionally retained as the authoritative source
 * text below for the future kernel binding pass.
 */
