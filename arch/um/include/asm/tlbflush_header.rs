...
extern "C" {
    pub fn um_tlb_sync(mm: *mut mm_struct) -> i32;

    pub fn flush_tlb_all();
    pub fn flush_tlb_mm(mm: *mut mm_struct);

    fn um_tlb_mark_sync(mm: *mut mm_struct, start: c_ulong, end: c_ulong);
    static mut init_mm: mm_struct;
}

pub unsafe fn flush_tlb_page(vma: *mut vm_area_struct, address: c_ulong) {
    um_tlb_mark_sync((*vma).vm_mm, address, address.wrapping_add(PAGE_SIZE));
}
...
pub unsafe fn flush_tlb_kernel_range(start: c_ulong, end: c_ulong) {
    um_tlb_mark_sync(&raw mut init_mm, start, end);

    /* Kernel needs to be synced immediately */
    um_tlb_sync(&raw mut init_mm);
}
