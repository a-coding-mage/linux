// Mechanical Rust translation of traps_64.c; external kernel symbols remain unresolved.
...
i32 register_dimm_printer(dimm_printer_t func)
{
	u64 flags;
	i32 ret = 0;
...
void unregister_dimm_printer(dimm_printer_t func)
{
	...
	if dimm_handler == func
		dimm_handler = core::ptr::null_mut();
	spin_unlock_irqrestore(&dimm_handler_lock, flags);
}
...
void spitfire_clean_and_reenable_l1_caches(void)
{
	u64 va;
	...
	/* Clean 'em. */
	for /* C loop */ va =  0.. va < (PAGE_SIZE << 1) /*  va += 32 */ {
...
void spitfire_enable_estate_errors(void)
{
...
u64 ecache_flush_physbase;
u64 ecache_flush_linesize;
u64 ecache_flush_size;
...
*const i8 CHPAFSR_DTO_msg[] =
	"System bus unmapped error for prefetch/storequeue-read";
*const i8 CHPAFSR_DBERR_msg[] =
	"System bus error for prefetch/storequeue-read";
*const i8 CHPAFSR_THCE_msg[] =
	"Hardware corrected E-cache Tag ECC error";
*const i8 CHPAFSR_TSCE_msg[] =
	"SW handled correctable E-cache Tag ECC error";
*const i8 CHPAFSR_TUE_msg[] =
	"Uncorrectable E-cache Tag ECC error";
*const i8 CHPAFSR_DUE_msg[] =
	"System bus uncorrectable data ECC error due to prefetch/store-fill";
struct afsr_error_table __cheetah_plus_error_table[] = {
	{	CHAFSR_PERR,	CHAFSR_PERR_msg		},
	{	CHAFSR_IERR,	CHAFSR_IERR_msg		},
	{	CHAFSR_ISAP,	CHAFSR_ISAP_msg		},
	{	CHAFSR_UCU,	CHAFSR_UCU_msg		},
...
*const i8 JPAFSR_JETO_msg[] =
	"System interface protocol error, hw timeout caused";
*const i8 JPAFSR_SCE_msg[] =
	"Parity error on system snoop results";
*const i8 JPAFSR_JEIC_msg[] =
	"System interface protocol error, illegal command detected";
*const i8 JPAFSR_JEIT_msg[] =
	"System interface protocol error, illegal ADTYPE detected";
*const i8 JPAFSR_OM_msg[] =
	"Out of range memory error has occurred";
*const i8 JPAFSR_ETP_msg[] =
	"Parity error on L2 cache tag SRAM";
*const i8 JPAFSR_UMS_msg[] =
	"Error due to unsupported store";
*const i8 JPAFSR_RUE_msg[] =
	"Uncorrectable ECC error from remote cache/memory";
*const i8 JPAFSR_RCE_msg[] =
	"Correctable ECC error from remote cache/memory";
*const i8 JPAFSR_BP_msg[] =
	"JBUS parity error on returned read data";
*const i8 JPAFSR_WBP_msg[] =
	"JBUS parity error on data for writeback or block store";
*const i8 JPAFSR_FRC_msg[] =
	"Foreign read to DRAM incurring correctable ECC error";
*const i8 JPAFSR_FRU_msg[] =
	"Foreign read to DRAM incurring uncorrectable ECC error";
struct afsr_error_table __jalapeno_error_table[] = {
...
struct afsr_error_table *cheetah_error_table;
u64 cheetah_afsr_errors;
...
extern u32 tl0_icpe[], tl1_icpe[];
extern u32 tl0_dcpe[], tl1_dcpe[];
extern u32 tl0_fecc[], tl1_fecc[];
extern u32 tl0_cee[], tl1_cee[];
extern u32 tl0_iae[], tl1_iae[];
extern u32 tl0_dae[], tl1_dae[];
extern u32 cheetah_plus_icpe_trap_vector[], cheetah_plus_icpe_trap_vector_tl1[];
extern u32 cheetah_plus_dcpe_trap_vector[], cheetah_plus_dcpe_trap_vector_tl1[];
extern u32 cheetah_fecc_trap_vector[], cheetah_fecc_trap_vector_tl1[];
extern u32 cheetah_cee_trap_vector[], cheetah_cee_trap_vector_tl1[];
extern u32 cheetah_deferred_trap_vector[], cheetah_deferred_trap_vector_tl1[];
...
void cheetah_flush_ecache(void)
{
	u64 flush_base = ecache_flush_physbase;
	u64 flush_linesize = ecache_flush_linesize;
	u64 flush_size = ecache_flush_size;
...
void __cheetah_flush_icache(void)
{
	...
	for /* C loop */ addr = 0.. addr < icache_size /*  addr += icache_line_size */ {
...
void cheetah_flush_icache(void)
{
	u64 dcu_save;
...
void cheetah_flush_dcache(void)
{
	...
	for /* C loop */ addr = 0.. addr < dcache_size /*  addr += dcache_line_size */ {
		__asm__ __volatile__("stxa %%g0, [%0] %1\n\t"
				     "membar #Sync"
				     : /* no outputs */
...
void cheetah_plus_zap_dcache_parity(void)
{
	...
	for /* C loop */ addr = 0.. addr < dcache_size /*  addr += dcache_line_size */ {
		...
		__asm__ __volatile__("membar	#Sync\n\t"
				     ...
				     : "r" (tag), "r" (addr),
				       "i" (ASI_DCACHE_UTAG));
		for /* C loop */ line = addr.. line < addr + dcache_line_size /*  line += 8 */
			__asm__ __volatile__("membar	#Sync\n\t"
					     "stxa	%%g0, [%0] %1\n\t"
					     "membar	#Sync"
					     : /* no outputs */
...
#[inline] unsafe fn u64 cheetah_get_hipri(u64 afsr)
{
	...
	for /* C loop */ i = 0.. cheetah_error_table[i].mask /*  i++ */ {
...
*const i8 *cheetah_get_string(u64 bit)
{
	i32 i;
...
#define MSYND_ERRORS	(CHAFSR_EMC | CHAFSR_EMU)
	...
	while afsr != 0UL {
		u64 bit = cheetah_get_hipri(afsr);
...
void cheetah_fecc_handler(struct pt_regs *regs, u64 afsr, u64 afar)
{
	...
	if (cheetah_recheck_errors(&local_snapshot)) {
		u64 new_afsr = local_snapshot.afsr;
...
i32 cheetah_fix_ce(u64 physaddr)
{
	...
	if (cheetah_recheck_errors(core::ptr::null_mut())) {
		/* Try one more time. */
...
*const i8 *sun4v_err_type_to_str(u8 type)
{
	*const i8 *types[SUN4V_ERR_TYPE_NUM] = {
		"undefined",
...
if regs->tstate & TSTATE_PRIV {
		 ...
		 * retired i64 time back and we may have no choice
...
u64 sun4v_err_itlb_vaddr;
u64 sun4v_err_itlb_ctx;
...
u64 sun4v_err_dtlb_vaddr;
u64 sun4v_err_dtlb_ctx;
...
printk("Instruction DUMP:");
	for /* C loop */ i = -3.. i < 6 /*  i++ */
		printk("%c%08x%c",i?' ':'<',pc[i],i?' ':'>');
	printk("\n");
}

void user_instruction_dump(u32 __user *pc)
{
	i32 i;
	u32 buf[9];
