...
/*
 * Rust translation of parisc/include/asm/assembly.h.
 *
 * This header is consumed by the PA-RISC assembler.  The assembler macros
 * below are retained verbatim as documentation because Rust has no direct
 * equivalent for defining PA-RISC assembler preprocessor macros here.
 */

#[cfg(target_pointer_width = "64")]
pub const RP_OFFSET: usize = 16;
#[cfg(target_pointer_width = "64")]
pub const FRAME_SIZE: usize = 128;
#[cfg(target_pointer_width = "64")]
pub const CALLEE_REG_FRAME_SIZE: usize = 144;
#[cfg(target_pointer_width = "64")]
pub const REG_SZ: usize = 8;
#[cfg(target_pointer_width = "32")]
pub const RP_OFFSET: usize = 20;
#[cfg(target_pointer_width = "32")]
pub const FRAME_SIZE: usize = 64;
#[cfg(target_pointer_width = "32")]
pub const CALLEE_REG_FRAME_SIZE: usize = 128;
#[cfg(target_pointer_width = "32")]
pub const REG_SZ: usize = 4;

pub const FRAME_ALIGN: usize = 64;
pub const CALLEE_FLOAT_FRAME_SIZE: usize = 80;
pub const CALLEE_SAVE_FRAME_SIZE: usize = CALLEE_REG_FRAME_SIZE + CALLEE_FLOAT_FRAME_SIZE;

pub const PRIV_USER: usize = 3;
pub const PRIV_KERNEL: usize = 0;
pub const SR_KERNEL: usize = 0;
pub const SR_TEMP1: usize = 1;
pub const SR_TEMP2: usize = 2;
pub const SR_USER: usize = 3;

// CONFIG_PA20 selects: LDCW = `ldcw,co`, BL = `b,l`, PA_ASM_LEVEL = `2.0w`
// (64-bit) or `2.0` (32-bit); otherwise they are `ldcw`, `bl`, and `1.1`.

/*
 * The following definitions are the source assembler macros.  They are kept
 * as comments so their register operations, ordering, and conditional forms
 * remain available to the assembler-side translation.
 *
 * #ifdef __ASSEMBLER__
 * #ifdef CONFIG_64BIT
 * #define LDREG ldd
 ...
 *   // CONFIG_64BIT: extrd,u \r, 32+(\p), \len, \t; otherwise extru.
 * .endm
 * .macro depi_safe i, p, len, t
 *   // CONFIG_64BIT: depdi \i, 32+(\p), \len, \t; otherwise depi.
 * .endm
 * .macro dep_safe i, p, len, t
 *   // CONFIG_64BIT: depd \i, 32+(\p), \len, \t; otherwise depw.
 * .endm
 ...
 * .endm
 * .macro loadgp
 *   // CONFIG_64BIT loads __gp; otherwise loads $global$ into %r27.
 * .endm
 *
 * SAVE_SP, REST_SP, SAVE_CR, REST_CR, save_general, rest_general, save_fp,
 * rest_fp, callee_save_float, callee_rest_float, callee_save, callee_rest,
 * save_specials, rest_specials, pcxt_ssm_bug, and virt_map are assembler
 * macros whose complete instruction bodies are preserved in the source
 * header and require PA-RISC assembler syntax and external asm-offsets.
 * ASM_EXCEPTIONTABLE_ENTRY emits the __ex_table section and relative words.
 */
// /* SPDX-License-Identifier: GPL-2.0-or-later */
// /*
