...
pub unsafe extern "C" fn btlb_init_per_cpu() { if cfg!(CONFIG_PA20) { return; } if pdc_btlb_info(&mut btlb_info) < 0 { core::ptr::write_bytes(&mut btlb_info, 0, 1); } let s = dereference_function_descriptor(&_stext) as usize; let e = dereference_function_descriptor(&_etext) as usize; let t = dereference_function_descriptor(&_sdata) as usize; BUG_ON(t != e); let mut slot = 0; alloc_btlb(s, e, &mut slot, 0x13800000); let t = dereference_function_descriptor(&_edata) as usize; let e = dereference_function_descriptor(&__bss_start) as usize; BUG_ON(t != e); let s = dereference_function_descriptor(&_sdata) as usize; let e = dereference_function_descriptor(&__bss_stop) as usize; alloc_btlb(s, e, &mut slot, 0x11800000); }

#[cfg(CONFIG_PA20)] const NR_SPACE_IDS: usize = 262144;
#[cfg(not(CONFIG_PA20))] const NR_SPACE_IDS: usize = 32768;
