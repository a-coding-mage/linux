...
// external kernel dependency: #include <linux/bpf.h>
// external kernel dependency: #include <linux/filter.h>
// external kernel dependency: #include <linux/libgcc.h>
// external kernel dependency: #include "bpf_jit.h"
...
// C macro dependency: #define STACK_OFFSET(k)	(- (NR_SAVED_REGISTERS + k + 1))
// C macro dependency: #define STACK_ALIGN	FRAME_SIZE

// C macro dependency: #define EXIT_PTR_LOAD(reg)	hppa_ldw(-0x08, HPPA_REG_SP, reg)
// C macro dependency: #define EXIT_PTR_STORE(reg)	hppa_stw(reg, -0x08, HPPA_REG_SP)
// C macro dependency: #define EXIT_PTR_JUMP(reg, nop)	hppa_bv(HPPA_REG_ZERO, reg, nop)

// C macro dependency: #define TMP_REG_1	(MAX_BPF_JIT_REG + 0)
// C macro dependency: #define TMP_REG_2	(MAX_BPF_JIT_REG + 1)
// C macro dependency: #define TMP_REG_R0	(MAX_BPF_JIT_REG + 2)
...
fn void emit_hppa_copy(const i8 rs, const i8 rd, *mut hppa_jit_contextctx)
{
	REG_SET_SEEN(ctx, rd);
...
fn void emit_hppa_xor(const i8 r1, const i8 r2, const i8 r3, *mut hppa_jit_contextctx)
{
	REG_SET_SEEN(ctx, r1);
	REG_SET_SEEN(ctx, r2);
	REG_SET_SEEN(ctx, r3);
...
fn void bpf_save_R0(*mut hppa_jit_contextctx)
{
	bpf_put_reg64(regmap[TMP_REG_R0], regmap[BPF_REG_0], ctx);
}

fn void bpf_restore_R0(*mut hppa_jit_contextctx)
{
...
{
	*const i8tmp1 = regmap[TMP_REG_1];
	*const i8rd = bpf_get_reg32(dst, tmp1, ctx);
			 ...
			 *mut hppa_jit_contextctx, const u8 op)
{
	...
	*const i8tmp2 = regmap[TMP_REG_2];
	*const i8rd;
	*const i8rs = bpf_get_reg32(src, tmp2, ctx);
			   ...
			   *mut hppa_jit_contextctx, const u8 op)
{
	...
	*const i8rs1 = bpf_get_reg64(src1, tmp1, ctx);
	*const i8rs2 = bpf_get_reg64(src2, tmp2, ctx);
	 ...
	 */
// C macro dependency: #define NO_JUMP(idx)	(2 + (idx) - 1)
// C macro dependency: #define JUMP(idx)	(0 + (idx) - 1)
...
fn int emit_bpf_tail_call(int insn, *mut hppa_jit_contextctx)
{
	/*
	 * R1 -> &ctx
	 * R2 -> &array
	 ...
	 */
	int off;
	*const i8arr_reg = regmap[BPF_REG_2];
	*const i8idx_reg = regmap[BPF_REG_3];
			  ...
			  const u8 mode)
{
	*const i8tmp1 = regmap[TMP_REG_1];
	*const i8tmp2 = regmap[TMP_REG_2];
	*const i8rd = bpf_get_reg64(dst, tmp1, ctx);
	*const i8rs = bpf_get_reg64(src, tmp2, ctx);
	i8 dstreg;
...
fn void emit_rev16(const i8 rd, *mut hppa_jit_contextctx)
{
...
fn void emit_rev32(const i8 rs, const i8 rd, *mut hppa_jit_contextctx)
{
		      ...
		      bool extra_pass)
{
	...
	*const i8tmp2 = regmap[TMP_REG_2];
	...
	case BPF_LD | BPF_IMM | BPF_DW:
	{
		...
		u32 upper = insn1.imm;
		u32 lower = imm;
		*const i8rd = bpf_get_reg64_ref(dst, tmp1, false, ctx);
...
void bpf_jit_build_prologue(*mut hppa_jit_contextctx)
{
	*const i8tmp = regmap[TMP_REG_1];
	*const i8dst, *reg;
	int stack_adjust = 0;
...
void bpf_jit_build_epilogue(*mut hppa_jit_contextctx)
{
