...
pub unsafe fn dbl_fsqrt(
    ...
    let mut newbitp1: u32;
    let mut newbitp2: u32;
    let mut sump1: u32;
    let mut sump2: u32;
    let mut src_exponent: i32;
    let mut guardbit: bool = false;
    let mut even_exponent: bool;
    ...
    if src_exponent == DBL_INFINITY_EXPONENT {
        ...
        if Dbl_iszero_sign(srcp1) || Dbl_isnotzero_mantissa(srcp1, srcp2) {
    ...
    if src_exponent > 0 {
        ...
        even_exponent = (src_exponent & 1) != 0;
    }
    if even_exponent {
        /* exponent is even */
    ...
    while Dbl_isnotzero(newbitp1, newbitp2) && Dbl_isnotzero(srcp1, srcp2) {
        ...
        if Dbl_isnotgreaterthan(sump1, sump2, srcp1, srcp2) {
            ...
            /* update result */
            Dbl_addition(resultp1, resultp2, newbitp1, newbitp2, resultp1, resultp2);
    ...
    if even_exponent { Dbl_rightshiftby1(resultp1, resultp2); }
    ...
    if Dbl_isnotzero(srcp1, srcp2) {
        ...
        match Rounding_mode() {
            ...
            ROUNDNEAREST => {
                /* stickybit is always true, so guardbit is enough to determine rounding */
                if guardbit { Dbl_increment(resultp1, resultp2); }
            },
            _ => {}
        }
