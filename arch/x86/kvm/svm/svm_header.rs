...
extern "C" {
    fn nested_svm_init_msrpm_merge_offsets()->c_int; fn svm_copy_vmrun_state(_: *mut vmcb_save_area,_: *mut vmcb_save_area); fn svm_copy_vmloadsave_state(_: *mut vmcb,_: *mut vmcb); fn nested_svm_exit_handled(_: *mut vcpu_svm)->c_int; fn nested_svm_check_permissions(_: *mut kvm_vcpu)->c_int; fn nested_svm_check_cached_vmcb12(_: *mut kvm_vcpu)->c_int; fn nested_svm_check_exception(_: *mut vcpu_svm,_: c_uint,_: bool,_: u32)->c_int; fn nested_svm_exit_special(_: *mut vcpu_svm)->c_int; fn nested_svm_update_tsc_ratio_msr(_: *mut kvm_vcpu); fn svm_write_tsc_multiplier(_: *mut kvm_vcpu); fn svm_switch_vmcb(_: *mut vcpu_svm,_: *mut kvm_vmcb_info);
    fn avic_incomplete_ipi_interception(_: *mut kvm_vcpu)->c_int; fn avic_unaccelerated_access_interception(_: *mut kvm_vcpu)->c_int; fn avic_apicv_post_state_restore(_: *mut kvm_vcpu); fn avic_refresh_apicv_exec_ctrl(_: *mut kvm_vcpu); fn avic_vcpu_blocking(_: *mut kvm_vcpu); fn avic_vcpu_unblocking(_: *mut kvm_vcpu); fn avic_refresh_virtual_apic_mode(_: *mut kvm_vcpu);
    fn sev_vcpu_create(_: *mut kvm_vcpu)->c_int; fn sev_free_vcpu(_: *mut kvm_vcpu); fn sev_vm_init(_: *mut kvm); fn sev_vm_destroy(_: *mut kvm); fn sev_set_cpu_caps(); fn sev_hardware_setup(); fn sev_hardware_unsetup(); fn sev_cpu_init(_: *mut svm_cpu_data)->c_int; fn sev_dev_get_attr(_: u32,_: u64,_: *mut u64)->c_int; fn sev_handle_rmp_fault(_: *mut kvm_vcpu,_: gpa_t,_: u64); fn sev_snp_reload_vmsa(_: *mut kvm_vcpu);
}
...
extern "C" {
    fn svm_set_efer(_: *mut kvm_vcpu,_: u64)->c_int; fn svm_set_cr0(_: *mut kvm_vcpu,_: c_ulong); fn svm_set_cr4(_: *mut kvm_vcpu,_: c_ulong); fn disable_nmi_singlestep(_: *mut vcpu_svm); fn svm_smi_blocked(_: *mut kvm_vcpu)->bool; fn svm_nmi_blocked(_: *mut kvm_vcpu)->bool; fn svm_interrupt_blocked(_: *mut kvm_vcpu)->bool; fn svm_set_gif(_: *mut vcpu_svm,_: bool); fn svm_invoke_exit_handler(_: *mut kvm_vcpu,_: u64)->c_int; fn svm_complete_interrupt_delivery(_: *mut kvm_vcpu,_: c_int,_: c_int,_: c_int); fn svm_skip_emulated_instruction(_: *mut kvm_vcpu)->c_int;
    fn avic_hardware_setup()->bool; fn avic_hardware_unsetup(); fn avic_vcpu_precreate(_: *mut kvm)->c_int; fn avic_vm_pre_destroy(_: *mut kvm); fn avic_vm_destroy(_: *mut kvm); fn avic_init_vmcb(_: *mut vcpu_svm,_: *mut vmcb); fn avic_init_vcpu(_: *mut vcpu_svm)->c_int; fn avic_vcpu_load(_: *mut kvm_vcpu,_: c_int); fn avic_vcpu_put(_: *mut kvm_vcpu); fn avic_ring_doorbell(_: *mut kvm_vcpu);
    fn pre_sev_run(_: *mut vcpu_svm,_: c_int)->c_int; fn sev_init_vmcb(_: *mut vcpu_svm,_: bool); fn sev_vcpu_after_set_cpuid(_: *mut vcpu_svm); fn sev_es_string_io(_: *mut vcpu_svm,_: c_int,_: c_uint,_: c_int)->c_int; fn sev_es_recalc_msr_intercepts(_: *mut kvm_vcpu); fn sev_vcpu_deliver_sipi_vector(_: *mut kvm_vcpu,_: u8); fn sev_es_unmap_ghcb(_: *mut vcpu_svm);
    fn enter_svm_guest_mode(_: *mut kvm_vcpu,_: u64,_: bool)->c_int; fn svm_leave_nested(_: *mut kvm_vcpu); fn svm_free_nested(_: *mut vcpu_svm); fn svm_allocate_nested(_: *mut vcpu_svm)->c_int; fn nested_svm_vmrun(_: *mut kvm_vcpu)->c_int;
}
