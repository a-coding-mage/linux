...
// pr_fmt preserved from C build configuration




const CC: u32 = KVM_NESTED_VMENTER_CONSISTENCY_CHECK;
...
unsafe static usize nested_svm_get_tdp_cr3(*mut kvm_vcpu *vcpu)
{
	*mut vcpu_svm *svm = to_svm(vcpu);
...
unsafe static c_void nested_svm_init_mmu_context(*mut kvm_vcpu *vcpu)
{
	*mut vcpu_svm *svm = to_svm(vcpu);
...
unsafe static c_void nested_svm_uninit_mmu_context(*mut kvm_vcpu *vcpu)
{
	vcpu.arch.mmu = &vcpu.arch.root_mmu;
}

unsafe fn nested_vmcb_needs_vls_intercept(*mut vcpu_svm *svm)
{
...
c_void nested_vmcb02_recalc_intercepts(*mut vcpu_svm *svm)
{
	*mut vmcb_ctrl_area_cached *vmcb12_ctrl = &svm.nested.ctl;
	*mut vmcb *vmcb02 = svm.nested.vmcb02.ptr;
	*mut vmcb *vmcb01 = svm.vmcb01.ptr;
	u32 i;
 ...
 */
unsafe static i32 nested_svm_msrpm_merge_offsets[10] ;
unsafe static i32 nested_svm_nr_msrpm_merge_offsets ;
type NsvmMsrpmMergeT = usize;

i32  nested_svm_init_msrpm_merge_offsets(c_void)
{
	unsafe static const u32 merge_msrs[]  = {
		MSR_STAR,
...
// conditional compilation from C source
	if (!svm.nested.force_msr_bitmap_recalc) {
		*mut hv_vmcb_enlightenments *hve = &svm.nested.ctl.hv_enlightenments;
...
// end conditional compilation
	...
	for (i = 0; i < nested_svm_nr_msrpm_merge_offsets; i++) {
		const i32 p = nested_svm_msrpm_merge_offsets[i];
...
unsafe fn nested_vmcb_check_controls(*mut kvm_vcpu *vcpu,
				       *mut vmcb_ctrl_area_cached *control)
{
...
i32 nested_svm_check_cached_vmcb12(*mut kvm_vcpu *vcpu)
{
	*mut vcpu_svm *svm = to_svm(vcpu);
...
unsafe static
c_void __nested_copy_vmcb_control_to_cache(*mut kvm_vcpu *vcpu,
					 *mut vmcb_ctrl_area_cached *to,
					 *mut vmcb_control_area *from)
{
	u32 i;
...
c_void nested_copy_vmcb_control_to_cache(*mut vcpu_svm *svm,
				       *mut vmcb_control_area *control)
{
	__nested_copy_vmcb_control_to_cache(&svm.vcpu, &svm.nested.ctl, control);
}

unsafe static c_void __nested_copy_vmcb_save_to_cache(*mut vmcb_save_area_cached *to,
					     *mut vmcb_save_area *from)
{
	to.es = from.es;
	to.cs = from.cs;
	to.ss = from.ss;
...
c_void nested_copy_vmcb_save_to_cache(*mut vcpu_svm *svm,
				    *mut vmcb_save_area *save)
{
 ...
 */
c_void nested_sync_control_from_vmcb02(*mut vcpu_svm *svm)
{
	u32 mask;
 ...
 * EXIT_INT_INFO.
 */
unsafe static c_void nested_save_pending_event_to_vmcb12(*mut vcpu_svm *svm,
						*mut vmcb *vmcb12)
{
	*mut kvm_vcpu *vcpu = &svm.vcpu;
	u32 exit_int_info = 0;
	u32 nr;
...
unsafe static c_void nested_svm_transition_tlb_flush(*mut kvm_vcpu *vcpu)
{
...
unsafe fn nested_vmcb12_has_lbrv(*mut kvm_vcpu *vcpu)
{
	return guest_cpu_cap_has(vcpu, X86_FEATURE_LBRV) &&
		(to_svm(vcpu).nested.ctl.misc_ctl2 & SVM_MISC2_ENABLE_V_LBR);
}

unsafe static c_void nested_vmcb02_prepare_save(*mut vcpu_svm *svm)
{
	*mut vmcb_ctrl_area_cached *control = &svm.nested.ctl;
	*mut vmcb_save_area_cached *save = &svm.nested.save;
...
unsafe static c_void nested_vmcb02_prepare_control(*mut vcpu_svm *svm)
{
...
unsafe static c_void nested_svm_copy_common_state(*mut vmcb *from_vmcb, *mut vmcb *to_vmcb)
{
	/*
...
i32 nested_svm_vmrun(*mut kvm_vcpu *vcpu)
{
	...
	*mut vmcb *vmcb01 = svm.vmcb01.ptr;
	...
	if (ret == -EFAULT)
		return kvm_handle_memory_failure(vcpu, X86EMUL_IO_NEEDED, core::ptr::null_mut());
...
c_void svm_copy_vmrun_state(*mut vmcb_save_area *to_save,
			  *mut vmcb_save_area *from_save)
{
...
c_void svm_copy_vmloadsave_state(*mut vmcb *to_vmcb, *mut vmcb *from_vmcb)
{
...
unsafe static i32 nested_svm_vmexit_update_vmcb12(*mut kvm_vcpu *vcpu)
{
	*mut vcpu_svm *svm = to_svm(vcpu);
	*mut vmcb *vmcb02 = svm.nested.vmcb02.ptr;
	*mut vmcb *vmcb12;
...
c_void nested_svm_vmexit(*mut vcpu_svm *svm)
{
	*mut kvm_vcpu *vcpu = &svm.vcpu;
	*mut vmcb *vmcb01 = svm.vmcb01.ptr;
	*mut vmcb *vmcb02 = svm.nested.vmcb02.ptr;
...
unsafe static c_void nested_svm_triple_fault(*mut kvm_vcpu *vcpu)
{
	*mut vcpu_svm *svm = to_svm(vcpu);
...
c_void svm_free_nested(*mut vcpu_svm *svm)
{
	if (!svm.nested.initialized)
		return;
	...
	svm.nested.msrpm = core::ptr::null_mut();

	__free_page(virt_to_page(svm.nested.vmcb02.ptr));
	svm.nested.vmcb02.ptr = core::ptr::null_mut();
...
c_void svm_leave_nested(*mut kvm_vcpu *vcpu)
{
	*mut vcpu_svm *svm = to_svm(vcpu);
...
unsafe static i32 nested_svm_intercept(*mut vcpu_svm *svm)
{
	u64 exit_code = svm.vmcb.control.exit_code;
	i32 vmexit = NESTED_EXIT_HOST;
...
i32 nested_svm_exit_handled(*mut vcpu_svm *svm)
{
	i32 vmexit;
...
i32 nested_svm_check_permissions(*mut kvm_vcpu *vcpu)
{
...
unsafe static c_void nested_svm_inject_exception_vmexit(*mut kvm_vcpu *vcpu)
{
	*mut kvm_queued_exception *ex = &vcpu.arch.exception_vmexit;
	*mut vcpu_svm *svm = to_svm(vcpu);
	*mut vmcb *vmcb = svm.vmcb;
...
#[inline]
unsafe fn fn nested_exit_on_init(*mut vcpu_svm *svm)
{
	return vmcb12_is_intercept(&svm.nested.ctl, INTERCEPT_INIT);
}

unsafe static i32 svm_check_nested_events(*mut kvm_vcpu *vcpu)
{
	*mut kvm_lapic *apic = vcpu.arch.apic;
	*mut vcpu_svm *svm = to_svm(vcpu);
	/*
...
i32 nested_svm_exit_special(*mut vcpu_svm *svm)
{
...
c_void nested_svm_update_tsc_ratio_msr(*mut kvm_vcpu *vcpu)
{
	*mut vcpu_svm *svm = to_svm(vcpu);
...
unsafe static c_void nested_copy_vmcb_cache_to_control(*mut vmcb_control_area *dst,
					      *mut vmcb_ctrl_area_cached *from)
{
	u32 i;

	memset(dst, 0, sizeof(*mut vmcb_control_area));
...
unsafe static i32 svm_set_nested_state(*mut kvm_vcpu *vcpu,
				*mut kvm_nested_state  *user_kvm_nested_state,
				*mut kvm_nested_state *kvm_state)
{
	*mut vcpu_svm *svm = to_svm(vcpu);
	...
	*mut vmcb_control_area *ctl;
	*mut vmcb_save_area *save;
	*mut vmcb_save_area_cached save_cached;
	*mut vmcb_ctrl_area_cached ctl_cached;
	bool use_separate_l2_pat;
	usize cr0;
	i32 ret;

	BUILD_BUG_ON(sizeof(*mut vmcb_control_area) + sizeof(*mut vmcb_save_area) >
...
unsafe fn svm_get_nested_state_pages(*mut kvm_vcpu *vcpu)
{
