// The C header requires at least one of SHADOW_FIELD_RO or SHADOW_FIELD_RW
// to be defined by the including translation unit. Rust macro expansion is
// represented here with local no-op defaults; callers may adapt these entries
// to their required generated items.
macro_rules! SHADOW_FIELD_RO {
    ($x:ident, $y:ident) => {};
}
macro_rules! SHADOW_FIELD_RW {
    ($x:ident, $y:ident) => {};
}
 ...
 * Note the code assumes this logic. If for some reason we
 * start shadowing these fields then we need to
