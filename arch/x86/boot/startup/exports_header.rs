 ...
 *
 * These are linker-script PROVIDE aliases. Rust has no file-local equivalent
 * for defining linker aliases without the corresponding symbol declarations
 * and signatures, so the mappings are preserved here as source-level intent.
 */
