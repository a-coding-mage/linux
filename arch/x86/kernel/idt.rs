...
static early_idts: &[idt_data] = &[
    ...
    #[cfg(all(not(target_arch = "x86_64"), CONFIG_X86_32))]
...
static def_idts: &[idt_data] = &[
    ...
    #[cfg(CONFIG_X86_32)] TSKG!(X86_TRAP_DF, GDT_ENTRY_DOUBLEFAULT_TSS),
    #[cfg(not(CONFIG_X86_32))] ISTG!(X86_TRAP_DF, asm_exc_double_fault, IST_INDEX_DF),
    ISTG!(X86_TRAP_DB, asm_exc_debug, IST_INDEX_DB),
    #[cfg(CONFIG_X86_MCE)] ISTG!(X86_TRAP_MC, asm_exc_machine_check, IST_INDEX_MCE),
    #[cfg(CONFIG_X86_CET)] INTG!(X86_TRAP_CP, asm_exc_control_protection),
    #[cfg(CONFIG_AMD_MEM_ENCRYPT)] ISTG!(X86_TRAP_VC, asm_exc_vmm_communication, IST_INDEX_VC),
    SYSG!(X86_TRAP_OF, asm_exc_overflow),
];

static ia32_idt: &[idt_data] = &[
    #[cfg(CONFIG_IA32_EMULATION)] SYSG!(IA32_SYSCALL_VECTOR, asm_int80_emulation),
    #[cfg(all(not(CONFIG_IA32_EMULATION), CONFIG_X86_32))] SYSG!(IA32_SYSCALL_VECTOR, entry_INT80_32),
];

static apic_idts: &[idt_data] = &[
    #[cfg(CONFIG_SMP)] INTG!(RESCHEDULE_VECTOR, asm_sysvec_reschedule_ipi),
    #[cfg(CONFIG_SMP)] INTG!(CALL_FUNCTION_VECTOR, asm_sysvec_call_function),
    #[cfg(CONFIG_SMP)] INTG!(CALL_FUNCTION_SINGLE_VECTOR, asm_sysvec_call_function_single),
    #[cfg(CONFIG_SMP)] INTG!(REBOOT_VECTOR, asm_sysvec_reboot),
    #[cfg(CONFIG_X86_THERMAL_VECTOR)] INTG!(THERMAL_APIC_VECTOR, asm_sysvec_thermal),
    #[cfg(CONFIG_X86_MCE_THRESHOLD)] INTG!(THRESHOLD_APIC_VECTOR, asm_sysvec_threshold),
    #[cfg(CONFIG_X86_MCE_AMD)] INTG!(DEFERRED_ERROR_VECTOR, asm_sysvec_deferred_error),
    #[cfg(CONFIG_X86_LOCAL_APIC)] INTG!(LOCAL_TIMER_VECTOR, asm_sysvec_apic_timer_interrupt),
    #[cfg(CONFIG_X86_LOCAL_APIC)] INTG!(X86_PLATFORM_IPI_VECTOR, asm_sysvec_x86_platform_ipi),
    #[cfg(all(CONFIG_X86_LOCAL_APIC, CONFIG_KVM))] INTG!(POSTED_INTR_VECTOR, asm_sysvec_kvm_posted_intr_ipi),
    #[cfg(all(CONFIG_X86_LOCAL_APIC, CONFIG_KVM))] INTG!(POSTED_INTR_WAKEUP_VECTOR, asm_sysvec_kvm_posted_intr_wakeup_ipi),
    #[cfg(all(CONFIG_X86_LOCAL_APIC, CONFIG_KVM))] INTG!(POSTED_INTR_NESTED_VECTOR, asm_sysvec_kvm_posted_intr_nested_ipi),
    #[cfg(all(CONFIG_X86_LOCAL_APIC, CONFIG_GUEST_PERF_EVENTS))] INTG!(PERF_GUEST_MEDIATED_PMI_VECTOR, asm_sysvec_perf_guest_mediated_pmi_handler),
    #[cfg(all(CONFIG_X86_LOCAL_APIC, CONFIG_IRQ_WORK))] INTG!(IRQ_WORK_VECTOR, asm_sysvec_irq_work),
    #[cfg(CONFIG_X86_LOCAL_APIC)] INTG!(SPURIOUS_APIC_VECTOR, asm_sysvec_spurious_apic_interrupt),
    #[cfg(CONFIG_X86_LOCAL_APIC)] INTG!(ERROR_APIC_VECTOR, asm_sysvec_error_interrupt),
    #[cfg(all(CONFIG_X86_LOCAL_APIC, CONFIG_X86_POSTED_MSI))] INTG!(POSTED_MSI_NOTIFICATION_VECTOR, asm_sysvec_posted_msi_notification),
];
...
#[cfg(all(CONFIG_KVM_INTEL, not(target_arch = "x86_64")))]
...
pub unsafe fn idt_setup_early_handler() { let mut i = 0; while i < NUM_EXCEPTION_VECTORS { set_intr_gate(i, early_idt_handler_array[i] as _); i += 1; } #[cfg(CONFIG_X86_32)] while i < NR_VECTORS { set_intr_gate(i, early_ignore_irq as _); i += 1; } load_idt(&idt_descr); }
