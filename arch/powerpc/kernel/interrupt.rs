...
#[cfg(all(CONFIG_PPC_ADV_DEBUG_REGS, CONFIG_PPC32))]
...
pub unsafe fn syscall_exit_prepare(mut r3: c_ulong, regs: *mut pt_regs, scv: c_long) -> c_ulong {
    ...
    let is_not_scv = !cfg!(CONFIG_PPC_BOOK3S_64) || scv == 0;
...
pub unsafe fn interrupt_exit_kernel_prepare(regs: *mut pt_regs) -> c_ulong {
    ...
    if !cfg!(CONFIG_PPC_BOOK3E_64) && TRAP(regs) != INTERRUPT_PROGRAM && TRAP(regs) != INTERRUPT_PERFMON {
