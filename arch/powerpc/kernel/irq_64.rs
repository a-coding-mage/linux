...
unsafe fn next_interrupt(regs: *mut pt_regs) {
    if cfg!(CONFIG_PPC_IRQ_SOFT_MASK_DEBUG) {
...
pub unsafe fn __replay_soft_interrupts() {
    ...
    if cfg!(CONFIG_PPC_BOOK3S) && irq_happened_test_and_clear(PACA_IRQ_HMI) {
    ...
    if cfg!(CONFIG_PPC_DOORBELL) && irq_happened_test_and_clear(PACA_IRQ_DBELL) {
        regs.trap = INTERRUPT_DOORBELL; doorbell_exception(&mut regs); next_interrupt(&mut regs);
    }
    if cfg!(CONFIG_PPC_BOOK3S) && irq_happened_test_and_clear(PACA_IRQ_PMI) {
...
#[cfg(all(CONFIG_PPC_BOOK3S_64, CONFIG_PPC_KUAP))]
...
#[cfg(not(all(CONFIG_PPC_BOOK3S_64, CONFIG_PPC_KUAP)))]
...
pub unsafe fn arch_local_irq_restore(mask: u64) {
    ...
    'again: loop {
        ...
        if cfg!(CONFIG_PPC_IRQ_SOFT_MASK_DEBUG) { WARN_ON_ONCE(mfmsr() & MSR_EE == 0); }
    ...
    } else if cfg!(CONFIG_PPC_IRQ_SOFT_MASK_DEBUG) && mfmsr() & MSR_EE != 0 { __hard_irq_disable(); }
