...
// Rust translation of the PowerPC assembly-offset generator.
// Configuration conditionals from the C source are retained as comments; the
// offset-emission macros are provided by the surrounding build environment.
macro_rules! OFFSET { ($($t:tt)*) => { /* external offset emission */ }; }
macro_rules! DEFINE { ($($t:tt)*) => { /* external definition emission */ }; }
macro_rules! STACK_PT_REGS_OFFSET { ($($t:tt)*) => { /* external offset emission */ }; }
