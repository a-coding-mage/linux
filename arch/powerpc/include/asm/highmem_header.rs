...
// The original declarations are active only for __KERNEL__ builds.
...
pub const PKMAP_ORDER: usize = 9;

pub const LAST_PKMAP: usize = 1usize << PKMAP_ORDER;

#[cfg(not(CONFIG_PPC_4K_PAGES))]
pub const PKMAP_BASE: usize = FIXADDR_START - PAGE_SIZE * (LAST_PKMAP + 1);
#[cfg(CONFIG_PPC_4K_PAGES)]
...
macro_rules! arch_kmap_local_set_pte {
    ($mm:expr, $vaddr:expr, $ptep:expr, $ptev:expr) => {
