...
/* PTE level */
#[cfg(all(CONFIG_PPC_8xx, CONFIG_PPC_16K_PAGES))]
#[repr(C)]
pub struct pte_t {
    pub pte: pte_basic_t,
...
#[cfg(all(not(all(CONFIG_PPC_8xx, CONFIG_PPC_16K_PAGES)), feature = "STRICT_MM_TYPECHECKS"))]
#[repr(C)]
pub struct pte_t {
    pub pte: pte_basic_t,
}

#[cfg(all(not(all(CONFIG_PPC_8xx, CONFIG_PPC_16K_PAGES)), not(feature = "STRICT_MM_TYPECHECKS")))]
pub type pte_t = pte_basic_t;

#[cfg(any(feature = "STRICT_MM_TYPECHECKS", all(CONFIG_PPC_8xx, CONFIG_PPC_16K_PAGES)))]
#[inline]
...
#[cfg(all(not(feature = "STRICT_MM_TYPECHECKS"), not(all(CONFIG_PPC_8xx, CONFIG_PPC_16K_PAGES))))]
#[inline]
pub fn __pte(x: pte_basic_t) -> pte_t {
    x
}
