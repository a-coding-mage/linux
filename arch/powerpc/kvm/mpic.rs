...
unsafe fn mpic_irq_raise(_: *mut openpic,d:*mut irq_dest,_:i32){ if !(*d).vcpu.is_null(){ extern "C"{fn kvm_vcpu_ioctl_interrupt(_: *mut kvm_vcpu,_: *mut c_void)->i32;} let mut x:u32=1;kvm_vcpu_ioctl_interrupt((*d).vcpu,&mut x as *mut _ as *mut c_void);}}
unsafe fn mpic_irq_lower(_: *mut openpic,d:*mut irq_dest,_:i32){if !(*d).vcpu.is_null(){extern "C"{fn kvmppc_core_dequeue_external(_: *mut kvm_vcpu);}kvmppc_core_dequeue_external((*d).vcpu);}}
