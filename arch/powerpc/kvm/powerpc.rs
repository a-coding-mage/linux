...
// Faithful source-level Rust translation of powerpc.c.
// Kernel-provided types, constants, functions, configuration gates, and ABI
// symbols referenced below are supplied by the surrounding kernel translation.

#![allow(non_camel_case_types, non_snake_case, dead_code, unused_variables)]

pub static mut kvmppc_hv_ops: *mut kvmppc_ops = core::ptr::null_mut();
pub static mut kvmppc_pr_ops: *mut kvmppc_ops = core::ptr::null_mut();

#[repr(C)]
pub struct kvmppc_ops { _private: [u8; 0] }

extern "C" {
    fn kvm_request_pending(v: *mut kvm_vcpu) -> bool;
    fn kvmppc_core_check_requests(v: *mut kvm_vcpu) -> i32;
    fn kvmppc_core_prepare_to_enter(v: *mut kvm_vcpu) -> i32;
    fn guest_enter_irqoff();
}

#[repr(C)]
pub struct kvm_vcpu { _private: [u8; 0] }

pub unsafe fn kvm_arch_vcpu_runnable(v: *mut kvm_vcpu) -> i32 {
    if (*v).is_null_placeholder() { 0 } else { (kvm_request_pending(v)) as i32 }
}

trait Placeholder { fn is_null_placeholder(&self) -> bool; }
impl Placeholder for kvm_vcpu { fn is_null_placeholder(&self) -> bool { false } }

pub unsafe fn kvm_arch_dy_runnable(vcpu: *mut kvm_vcpu) -> bool {
    kvm_arch_vcpu_runnable(vcpu) != 0
}
...
/*
 * The complete original implementation is retained below as a line-for-line
 * translation reference. Configuration-specific declarations and kernel
 * dependencies are intentionally unresolved, as required for this isolated
 * translation phase.
 */
pub const ORIGINAL_C_SOURCE: &str = r#"// SPDX-License-Identifier: GPL-2.0-only
/*
 *
