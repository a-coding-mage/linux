...
unsafe fn print_hmi_event_info(hmi_evt: *mut OpalHMIEvent) {
    ...
    if (*hmi_evt).severity != OpalHMI_SEV_NO_ERROR || __ratelimit(core::ptr::null_mut()) { printk(b"%s%s Hypervisor Maintenance interrupt [%s]\n\0".as_ptr() as *const i8, level, sevstr.as_ptr(), if (*hmi_evt).disposition == OpalHMI_DISPOSITION_RECOVERED { b"Recovered\0".as_ptr() } else { b"Not recovered\0".as_ptr() }); let error_info = if ((*hmi_evt).r#type as usize) < HMI_ERROR_TYPES.len() { HMI_ERROR_TYPES[(*hmi_evt).r#type as usize].as_ptr() } else { b"Unknown\0".as_ptr() }; printk(b"%s Error detail: %s\n\0".as_ptr() as *const i8, level, error_info); printk(b"%s\tHMER: %016llx\n\0".as_ptr() as *const i8, level, be64_to_cpu((*hmi_evt).hmer)); if (*hmi_evt).r#type == OpalHMI_ERROR_TFAC || (*hmi_evt).r#type == OpalHMI_ERROR_TFMR_PARITY { printk(b"%s\tTFMR: %016llx\n\0".as_ptr() as *const i8, level, be64_to_cpu((*hmi_evt).tfmr)); } }
    if (*hmi_evt).version >= OpalHMIEvt_V2 && (*hmi_evt).r#type == OpalHMI_ERROR_MALFUNC_ALERT { print_checkstop_reason(level, hmi_evt); }
}
