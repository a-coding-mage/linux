...
static mut OPAL_TRACE_DEPTH: u32 = 0;

#[cfg(CONFIG_TRACEPOINTS)]
unsafe fn __trace_opal_entry(
    a0: i64, a1: i64, a2: i64, a3: i64, a4: i64, a5: i64, a6: i64, a7: i64,
    opcode: usize,
) {
    let depth = &mut OPAL_TRACE_DEPTH;
    if *depth != 0 { return; }
    let args = [a0, a1, a2, a3, a4, a5, a6, a7];
    *depth += 1;
    trace_opal_entry(opcode, args.as_ptr());
    *depth -= 1;
}

#[cfg(CONFIG_TRACEPOINTS)]
unsafe fn __trace_opal_exit(opcode: usize, retval: usize) {
    let depth = &mut OPAL_TRACE_DEPTH;
    if *depth != 0 { return; }
    *depth += 1;
...
#[cfg(CONFIG_TRACEPOINTS)]
static mut OPAL_TRACEPOINT_KEY: bool = false;

#[cfg(CONFIG_TRACEPOINTS)]
pub unsafe fn opal_tracepoint_regfunc() -> i32 {
    static_branch_inc(&mut OPAL_TRACEPOINT_KEY);
    0
}

#[cfg(CONFIG_TRACEPOINTS)]
pub unsafe fn opal_tracepoint_unregfunc() {
    static_branch_dec(&mut OPAL_TRACEPOINT_KEY);
}

#[cfg(CONFIG_TRACEPOINTS)]
unsafe fn __opal_call_trace(
    a0: i64, a1: i64, a2: i64, a3: i64, a4: i64, a5: i64, a6: i64, a7: i64,
    opcode: usize, msr: usize,
) -> i64 {
    __trace_opal_entry(a0, a1, a2, a3, a4, a5, a6, a7, opcode);
    let ret = __opal_call(a0, a1, a2, a3, a4, a5, a6, a7, opcode, msr);
    __trace_opal_exit(opcode, ret as usize);
    ret
}

#[cfg(not(CONFIG_TRACEPOINTS))]
unsafe fn __opal_call_trace(
    _a0: i64, _a1: i64, _a2: i64, _a3: i64, _a4: i64, _a5: i64, _a6: i64, _a7: i64,
    _opcode: usize, _msr: usize,
) -> i64 { 0 }

#[cfg(CONFIG_TRACEPOINTS)]
const DO_TRACE: bool = true; // static_branch_unlikely(&opal_tracepoint_key)
#[cfg(not(CONFIG_TRACEPOINTS))]
...
unsafe fn opal_call(
    ...
    if !mmu {
        return __opal_call(a0, a1, a2, a3, a4, a5, a6, a7, opcode as usize, msr);
    }
    local_save_flags(&mut flags);
    hard_irq_disable();
    let ret = if DO_TRACE {
        __opal_call_trace(a0, a1, a2, a3, a4, a5, a6, a7, opcode as usize, msr)
    } else {
        __opal_call(a0, a1, a2, a3, a4, a5, a6, a7, opcode as usize, msr)
    };
    local_irq_restore(flags);
    ret
}

macro_rules! OPAL_CALL {
    ($name:ident, $opcode:ident) => {
        pub unsafe fn $name(a0: i64, a1: i64, a2: i64, a3: i64,
                            a4: i64, a5: i64, a6: i64, a7: i64) -> i64 {
            opal_call(a0, a1, a2, a3, a4, a5, a6, a7, $opcode as i64)
        }
    };
}
