...
static struct patch_params patch_params  = &[
	1, 0x802a, 0x8028, 0x802e, 0x802c,
];

static u32 patch_2000[]  = &[
...
static void  cpm_write_patch(cpm8xx_t *cp, int offset, u32 *patch, int len)
{
	if (!len)
		return;
...
/// External kernel/CPM types and helpers are supplied by the surrounding translation unit.
use crate::*;

unsafe fn cpm_write_patch(cp: *mut cpm8xx_t, offset: i32, patch: &[u32], len: i32) {
    if len == 0 { return; }
    memcpy_toio(unsafe { (*cp).cp_dpmem.add(offset as usize) } as *mut core::ffi::c_void,
        patch.as_ptr() as *const core::ffi::c_void, len as usize);
}

pub unsafe fn cpm_load_patch(cp: *mut cpm8xx_t) {
    out_be16(&mut (*cp).cp_rccr, 0);
    cpm_write_patch(cp, 0, patch_2000, core::mem::size_of_val(patch_2000) as i32);
    cpm_write_patch(cp, 0xf00, patch_2f00, core::mem::size_of_val(patch_2f00) as i32);
    ...
    #[cfg(feature = "CONFIG_SMC_UCODE_PATCH")]
    {
        #[cfg(feature = "CONFIG_PPC_EARLY_DEBUG_CPM")]
        {
            ...
            let _ = smp;
            let mut i = 0usize;
            while i < core::mem::size_of::<smc_uart_t>() {
                let src = (&mut (*cp).cp_dparam[PROFF_SMC1 + i]) as *mut _ as *mut u32;
                let dst = (&mut (*cp).cp_dparam[PROFF_DSP1 + i]) as *mut _ as *mut u32;
                out_be32(dst, in_be32(src));
                i += 4;
            }
        }
        let smp = (&mut (*cp).cp_dparam[PROFF_SMC1]) as *mut _ as *mut smc_uart_t;
        out_be16(&mut (*smp).smc_rpbase, 0x1ec0);
        let smp = (&mut (*cp).cp_dparam[PROFF_SMC2]) as *mut _ as *mut smc_uart_t;
        out_be16(&mut (*smp).smc_rpbase, 0x1fc0);
    }
    out_be16(&mut (*cp).cp_cpmcr1, patch_params.cpmcr1);
    out_be16(&mut (*cp).cp_cpmcr2, patch_params.cpmcr2);
    out_be16(&mut (*cp).cp_cpmcr3, patch_params.cpmcr3);
    out_be16(&mut (*cp).cp_cpmcr4, patch_params.cpmcr4);
    out_be16(&mut (*cp).cp_rccr, patch_params.rccr);
    pr_info!("{} microcode patch installed\n", patch_name);
}
