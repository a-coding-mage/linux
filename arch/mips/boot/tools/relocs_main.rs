...
fn run() -> Result<(), Vec<u8>> {
    ...
    for argument in std::env::args_os().skip(1) {
        match argument.as_encoded_bytes() {
            ...
            bytes if !bytes.starts_with(b"-") && path.is_none() => path = Some(PathBuf::from(argument)),
    ...
    file.read_exact(&mut ident).map_err(|error| {
        let reason = if error.kind() == io::ErrorKind::UnexpectedEof { "Success".into() } else { errno_text(&error) };
        path_error("read", &path, &reason, false)
    })?;
    file.rewind().map_err(|error| path_error("seek", &path, &errno_text(&error), true))?;
    let mut data = Vec::new();
    file.read_to_end(&mut data).map_err(|error| path_error("read", &path, &errno_text(&error), true))?;
    ...
    file.flush().map_err(|error| path_error("write", &path, &errno_text(&error), true))
}
