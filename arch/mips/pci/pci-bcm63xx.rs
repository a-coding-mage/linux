 ...
 * configuration
 */
pub static mut bcm63xx_pci_enabled: i32 = 0;

static mut bcm_pci_mem_resource: resource = resource {
...
pub static mut bcm63xx_controller: pci_controller = pci_controller {
...
static mut bcm_pcie_mem_resource: resource = resource {
...
static mut bcm_pcie_io_resource: resource = resource {
...
pub static mut bcm63xx_pcie_controller: pci_controller = pci_controller {
    pci_ops: &bcm63xx_pcie_ops,
    io_resource: &bcm_pcie_io_resource,
    mem_resource: &bcm_pcie_mem_resource,
};

unsafe fn bcm63xx_int_cfg_readl(reg: u32) -> u32 {
    let mut tmp: u32 = reg & MPI_PCICFGCTL_CFGADDR_MASK;
    tmp |= MPI_PCICFGCTL_WRITEEN_MASK;
    bcm_mpi_writel(tmp, MPI_PCICFGCTL_REG);
    iob();
    bcm_mpi_readl(MPI_PCICFGDATA_REG)
}

unsafe fn bcm63xx_int_cfg_writel(val: u32, reg: u32) {
    let mut tmp: u32 = reg & MPI_PCICFGCTL_CFGADDR_MASK;
    tmp |= MPI_PCICFGCTL_WRITEEN_MASK;
    bcm_mpi_writel(tmp, MPI_PCICFGCTL_REG);
...
pub static mut pci_iospace_start: *mut core::ffi::c_void = core::ptr::null_mut();

unsafe fn bcm63xx_reset_pcie() {
    let reg: u32 = if BCMCPU_IS_6328() {
        MISC_SERDES_CTRL_6328_REG
    } else {
        MISC_SERDES_CTRL_6362_REG
    };

    let mut val = bcm_misc_readl(reg);
...
static mut pcie_clk: *mut clk = core::ptr::null_mut();

unsafe fn bcm63xx_register_pcie() -> i32 {
    pcie_clk = clk_get(core::ptr::null_mut(), "pcie");
    ...
    let mut val = bcm_pcie_readl(PCIE_BRIDGE_OPT1_REG);
    val |= OPT1_RD_BE_OPT_EN | OPT1_RD_REPLY_BE_FIX_EN | OPT1_PCIE_BRIDGE_HOLE_DET_EN | OPT1_L1_INT_STATUS_MASK_POL;
    ...
    val |= OPT2_TX_CREDIT_CHK_EN | OPT2_UBUS_UR_DECODE_DIS;
    val |= (PCIE_BUS_DEVICE << OPT2_CFG_TYPE1_BUS_NO_SHIFT) | OPT2_CFG_TYPE1_BD_SEL;
...
unsafe fn bcm63xx_register_pci() -> i32 {
    let mut val: u32;
    pci_iospace_start = ioremap(BCM_PCI_IO_BASE_PA, 4);
    if pci_iospace_start.is_null() { return -ENOMEM; }
    ...
    val = if BCMCPU_IS_3368() || BCMCPU_IS_6358() || BCMCPU_IS_6368() { MPI_SP0_REMAP_ENABLE_MASK } else { 0 };
    bcm_mpi_writel(val, MPI_SP0_REMAP_REG);
    bcm63xx_int_cfg_writel(0, PCI_BASE_ADDRESS_4);
    bcm_mpi_writel(0, MPI_SP1_REMAP_REG);

    let mem_size = bcm63xx_get_memory_size();
    if BCMCPU_IS_6348() && (bcm63xx_get_cpu_rev() & 0xf0) == 0xa0 {
        if mem_size > 16 * 1024 * 1024 {
            printk(KERN_WARNING, "bcm63xx: this CPU revision cannot handle more than 16MB of RAM for PCI bus mastering\n");
        }
    } else {
...
unsafe fn bcm63xx_pci_init() -> i32 {
    if bcm63xx_pci_enabled == 0 { return -ENODEV; }
    match bcm63xx_get_cpu_id() {
        BCM6328_CPU_ID | BCM6362_CPU_ID => bcm63xx_register_pcie(),
        BCM3368_CPU_ID | BCM6348_CPU_ID | BCM6358_CPU_ID | BCM6368_CPU_ID => bcm63xx_register_pci(),
        _ => -ENODEV,
    }
}
