...
bad_area_nosemaphore:
    if user_mode(regs) {
        ...
        if show_unhandled_signals != 0 && unhandled_signal(tsk, SIGSEGV)
            ...
            print_vma_addr!(c"\x01c ".as_ptr(), (*regs).cp0_epc);
            pr_cont!("\n");
            pr_info!("ra  = %0*lx in", field, (*regs).regs[31]);
            print_vma_addr!(c"\x01c ".as_ptr(), (*regs).regs[31]);
            pr_cont!("\n");
        }
...
no_context:
    ...
    bust_spinlocks(1);
    printk!(c"\x011CPU %d Unable to handle kernel paging request at virtual address %0*lx, epc == %0*lx, ra == %0*lx\n".as_ptr(),
