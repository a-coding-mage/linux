...
// Faithful low-level Rust translation of the s390 vSIE implementation.
// External kernel types, constants, intrinsics, and functions are supplied by
// the surrounding kernel translation units.
//
// The original implementation is retained below as a source-level reference
// while its declarations and control flow are represented in the unsafe Rust
// interface below.
// SPDX-License-Identifier: GPL-2.0
/*
...
static_assert(sizeof(vsie_page) == PAGE_SIZE);
static_assert(offsetof(vsie_page, mcck_info) == offsetof(sie_page, mcck_info));
static_assert(IS_ALIGNED(offsetof(vsie_page, crycb), 8));

/* trigger a validity icpt for the given scb */
static int set_validity_icpt(kvm_s390_sie_block *scb,
			     __u16 reason_code)
{
	(*scb).ipa = 0x1000;
...
static void prefix_mapped(vsie_page *vsie_page)
{
	atomic_andnot(PROG_REQUEST, (*&vsie_page).scb_s.prog20);
}

/* test if the prefix is mapped into the gmap shadow */
static int prefix_is_mapped(vsie_page *vsie_page)
{
	return !(atomic_read((*&vsie_page).scb_s.prog20) & PROG_REQUEST);
}

/* copy the updated intervention request bits into the shadow scb */
static void update_intervention_requests(vsie_page *vsie_page)
{
...
static int prepare_cpuflags(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
			...
			crycb_gpa: core::ffi::c_ulong, core::ffi::c_ulong *apcb_h)
{
	...
	bitmap_and(apcb_s, apcb_s, apcb_h,
		   BITS_PER_BYTE * sizeof(kvm_s390_apcb0));
			...
			core::ffi::c_ulong *apcb_h)
{
	...
	if (read_guest_real(vcpu, apcb_gpa, apcb_s,
			    sizeof(kvm_s390_apcb1)))
		return -EFAULT;

	bitmap_and(apcb_s, apcb_s, apcb_h,
		   BITS_PER_BYTE * sizeof(kvm_s390_apcb1));
 ...
 */
static int setup_apcb(kvm_vcpu *vcpu, kvm_s390_crypto_cb *crycb_s,
	       const u32 crycb_gpa,
	       kvm_s390_crypto_cb *crycb_h,
	       int fmt_o, int fmt_h)
{
	switch (fmt_o) {
	case CRYCB_FORMAT2:
 ...
 */
static int shadow_crycb(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	'end: {
	struct kvm_s390_sie_block *scb_s = (*&vsie_page).scb_s;
	struct kvm_s390_sie_block *scb_o = (*vsie_page).scb_o;
...
static void shadow_esa(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	struct kvm_s390_sie_block *scb_s = (*&vsie_page).scb_s;
...
static void prepare_ibc(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	struct kvm_s390_sie_block *scb_s = (*&vsie_page).scb_s;
	struct kvm_s390_sie_block *scb_o = (*vsie_page).scb_o;
...
static void unshadow_scb(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	struct kvm_s390_sie_block *scb_s = (*&vsie_page).scb_s;
	struct kvm_s390_sie_block *scb_o = (*vsie_page).scb_o;
 ...
 *          - > 0 if control has to be given to guest 2
 */
static int shadow_scb(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	'out: {
	struct kvm_s390_sie_block *scb_o = (*vsie_page).scb_o;
	struct kvm_s390_sie_block *scb_s = (*&vsie_page).scb_s;
 ...
 */
static int map_prefix(kvm_vcpu *vcpu, vsie_page *vsie_page, gmap *sg)
{
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
 ...
 */
static int pin_guest_page(kvm *kvm, gpa_t gpa, hpa_t *hpa)
{
	struct page *page;
...
static void unpin_guest_page(kvm *kvm, gpa_t gpa, hpa_t hpa)
{
...
static void unpin_blocks(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
	hpa_t hpa;
 ...
 *          - -ENOMEM if out of memory
 */
static int pin_blocks(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	'unpin: {
	struct kvm_s390_sie_block *scb_o = vsie_page->scb_o;
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
	...
	if (gpa) {
		if (gpa < 2 * PAGE_SIZE)
			...
			rc = set_validity_icpt(scb_s, 0x0011U);
			 ...
			 ((gpa + offsetof(bsca_block, cpu[0]) - 1) & PAGE_MASK))
...
static void unpin_scb(kvm_vcpu *vcpu, vsie_page *vsie_page,
		      gpa_t gpa)
{
 ...
 *          - > 0 if control has to be given to guest 2
 */
static int pin_scb(kvm_vcpu *vcpu, vsie_page *vsie_page,
		   gpa_t gpa)
{
	hpa_t hpa;
	int rc;
 ...
 */
static int inject_fault(kvm_vcpu *vcpu, __u16 code, __u64 vaddr,
			write_flag: bool)
{
 ...
 */
static int handle_fault(kvm_vcpu *vcpu, vsie_page *vsie_page, gmap *sg)
{
 ...
 */
static void handle_last_fault(kvm_vcpu *vcpu, vsie_page *vsie_page, gmap *sg)
{
...
void clear_vsie_icpt(vsie_page *vsie_page)
{
	vsie_page->scb_s.icptcode = 0;
}

/* rewind the psw and clear the vsie icpt, so we can retry execution */
static void retry_vsie_icpt(vsie_page *vsie_page)
{
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
...
static int handle_stfle_0(kvm_vcpu *vcpu, vsie_page *vsie_page,
			  fac_list_origin: u32)
{
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
...
static int handle_stfle_2(kvm_vcpu *vcpu, vsie_page *vsie_page, fac_list_origin: u32)
{
	struct kvm_s390_flcb2 *flcb_s = (kvm_s390_flcb2 *)vsie_page->fac;
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
	u64 len;
	...
	if (read_guest_real(vcpu, fac_list_origin + offsetof(kvm_s390_flcb2, facilities),
 ...
 *          - > 0 if control has to be given to guest 2
 */
static int handle_stfle(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	...
	BUILD_BUG_ON(!IS_ALIGNED(offsetof(vsie_page, fac), 8));
 ...
 */
static u64 vsie_get_register(kvm_vcpu *vcpu, vsie_page *vsie_page, reg: u8)
{
 ...
 *          - < 0 if an error occurred
 */
static int do_vsie_run(kvm_vcpu *vcpu, vsie_page *vsie_page, gmap *sg)
	...
	__acquires(vcpu->kvm->srcu)
{
    ...
    local_irq_disable();
		...
		if (xfer_to_guest_mode_work_pending()) {
			...
			if (rc)
				break 'skip_sie;
			continue 'xfer_to_guest_mode_check;
		}
...
static void release_gmap_shadow(vsie_page *vsie_page)
{
...
static struct gmap *acquire_gmap_shadow(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	union ctlreg0 cr0;
	struct gmap *gmap;
	union asce asce;
 ...
 */
static void register_shadow_scb(kvm_vcpu *vcpu,
				vsie_page *vsie_page)
{
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
 ...
 */
static void unregister_shadow_scb(kvm_vcpu *vcpu)
{
 ...
 *          - -ENOMEM if out of memory
 */
static int vsie_run(kvm_vcpu *vcpu, vsie_page *vsie_page)
{
	struct kvm_s390_sie_block *scb_s = &vsie_page->scb_s;
	struct gmap *sg = NULL;
	int rc = 0;
...
bool try_get_vsie_page(vsie_page *vsie_page)
{
...
static void put_vsie_page(vsie_page *vsie_page)
{
...
static struct vsie_page *get_vsie_page(kvm *kvm, addr: core::ffi::c_ulong)
{
	...
	memset(&vsie_page->scb_s, 0, sizeof(kvm_s390_sie_block));
	if (vsie_page->gmap_cache.gmap) {
...
int kvm_s390_handle_vsie(kvm_vcpu *vcpu)
{
	'out_put: {
	'out_unpin_scb: {
	'out_unshadow: {
...
void kvm_s390_vsie_init(kvm *kvm)
{
...
void kvm_s390_vsie_destroy(kvm *kvm)
{
	struct vsie_page *vsie_page;
	int i;
...
void kvm_s390_vsie_kick(kvm_vcpu *vcpu)
{
