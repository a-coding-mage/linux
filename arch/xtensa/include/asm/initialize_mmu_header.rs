//! Rust translation of `arch/xtensa/include/asm/initialize_mmu.h`.
//!
//! The source header is an assembler header.  Its macro bodies are retained
//! below as source-level assembly comments because they depend on the Xtensa
//! assembler, kernel preprocessor symbols, and architecture-specific
//! registers supplied by the surrounding build.

// C preprocessor condition: XCHAL_HAVE_PTP_MMU.
#[cfg(xchal_have_ptp_mmu)]
pub const CA_BYPASS: u32 = _PAGE_CA_BYPASS | _PAGE_HW_WRITE | _PAGE_HW_EXEC;
#[cfg(xchal_have_ptp_mmu)]
pub const CA_WRITEBACK: u32 = _PAGE_CA_WB | _PAGE_HW_WRITE | _PAGE_HW_EXEC;
#[cfg(not(xchal_have_ptp_mmu))]
pub const CA_WRITEBACK: u32 = 0x4;

pub const XTENSA_HWVERSION_RC_2009_0: u32 = 230000;

/*
 * Original assembler macro: initialize_mmu
 *
 * The following is the complete assembler body, including its original
 * conditional build-time branches.  It must be emitted/assembled by the
 * Xtensa build that provides these symbols and instructions.
 *
 ...
 * add a5, a2, a4
 * 3: idtlb a5; iitlb a5; add a5, a5, a4; bne a5, a2, 3b
 * movi a6, 0x01000000
 * wsr a6, ITLBCFG
 * wsr a6, DTLBCFG
 * isync
...
/* Original assembler macro: initialize_cacheattr.  Its complete body is
 * conditional on CONFIG_MMU, XCHAL_HAVE_TLBS/XCHAL_HAVE_MPU, and the kernel
 * cache-attribute symbols; it is intentionally retained as assembler source
 * in the source header rather than represented by a Rust runtime function. */
/*
