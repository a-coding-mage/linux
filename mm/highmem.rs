...
#[cfg(all(CONFIG_HIGHMEM, feature = "ARCH_NEEDS_KMAP_HIGH_GET"))]
...
#[inline] unsafe fn kmap_local_idx_push() -> i32 {
    ...
    (*current).kmap_ctrl.idx += if cfg!(CONFIG_DEBUG_KMAP_LOCAL) { 2 } else { 1 };
...
pub unsafe fn __kmap_local_page_prot(page: *const struct_page, prot: pgprot_t) -> *mut core::ffi::c_void {
    if !cfg!(CONFIG_DEBUG_KMAP_LOCAL_FORCE_MAP) && !PageHighMem(page) { return page_address(page); }
...
pub unsafe fn kunmap_local_indexed(vaddr: *const core::ffi::c_void) {
    ...
    if addr < __fix_to_virt(FIX_KMAP_END) || addr > __fix_to_virt(FIX_KMAP_BEGIN) {
        if cfg!(CONFIG_DEBUG_KMAP_LOCAL_FORCE_MAP) { WARN_ON_ONCE!(1); return; }
...
pub unsafe fn __kmap_local_sched_out() {
    ...
    for i in 0..(*tsk).kmap_ctrl.idx {
        ...
        if cfg!(CONFIG_DEBUG_KMAP_LOCAL) && i & 1 == 0 { WARN_ON_ONCE!(pte_val(pteval) != 0); continue; }
