//! Literal Rust-side transcription record for the Linux memblock implementation.
//!
//! The implementation depends on the Linux kernel ABI and symbols supplied by
//! the surrounding kernel translation.  The complete source is retained below
//! so declarations, comments, constants, and ordering remain available to the
//! integration pass without inventing dependency implementations.

pub const INIT_MEMBLOCK_REGIONS: usize = 128;
pub const INIT_PHYSMEM_REGIONS: usize = 4;
...
/// External kernel-provided memblock type; its layout is supplied by the
/// corresponding translated kernel headers.
#[repr(C)]
pub struct MemblockType {
    pub regions: *mut MemblockRegion,
    pub cnt: usize,
    pub max: usize,
...
pub unsafe fn memblock_has_mirror() -> bool {
    SYSTEM_HAS_SOME_MIRROR
}

pub unsafe fn memblock_addrs_overlap(
    base1: PhysAddr, size1: PhysAddr, base2: PhysAddr, size2: PhysAddr,
) -> u64 {
    ((base1 < base2.wrapping_add(size2)) && (base2 < base1.wrapping_add(size1))) as u64
}

pub unsafe fn memblock_overlaps_region(
    ty: *mut MemblockType, base: PhysAddr, mut size: PhysAddr,
) -> bool {
    size = core::cmp::min(size, PhysAddr::MAX.wrapping_sub(base));
    for i in 0..(*ty).cnt {
        let region = &*(*ty).regions.add(i);
        if memblock_addrs_overlap(base, size, region.base, region.size) != 0 {
            return true;
        }
    }
    false
...
pub const MEMBLOCK_C_SOURCE: &str = r###"
