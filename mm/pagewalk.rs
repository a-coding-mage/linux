...
unsafe fn process_vma_walk_lock(vma:*mut vm_area_struct,lock:page_walk_lock){#[cfg(CONFIG_PER_VMA_LOCK)]match lock{PGWALK_WRLOCK=>vma_start_write(vma),PGWALK_WRLOCK_VERIFY=>vma_assert_write_locked(vma),PGWALK_VMA_RDLOCK_VERIFY=>vma_assert_locked(vma),PGWALK_RDLOCK=>(),}}
