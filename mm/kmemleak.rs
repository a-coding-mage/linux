...
// Faithful low-level translation of mm/kmemleak.c.
// Linux kernel headers and symbols referenced by the implementation are
// intentionally left as external dependencies, as required by the source.
...
pub const MSECS_MIN_AGE: usize = 5000;
pub const SECS_FIRST_SCAN: usize = 60;
pub const SECS_SCAN_WAIT: usize = 600;
pub const MAX_SCAN_SIZE: usize = 4096;
pub const BYTES_PER_POINTER: usize = core::mem::size_of::<*const c_void>();
pub const KMEMLEAK_GREY: i32 = 0;
pub const KMEMLEAK_BLACK: i32 = -1;
pub const OBJECT_ALLOCATED: u32 = 1 << 0;
pub const OBJECT_REPORTED: u32 = 1 << 1;
pub const OBJECT_NO_SCAN: u32 = 1 << 2;
pub const OBJECT_FULL_SCAN: u32 = 1 << 3;
pub const OBJECT_PHYS: u32 = 1 << 4;
pub const OBJECT_PERCPU: u32 = 1 << 5;
pub const OBJECT_SUSPECT: u32 = 1 << 6;
pub const DELSTATE_REMOVED: u32 = 1 << 0;
pub const DELSTATE_NO_DELETE: u32 = 1 << 1;
pub const HEX_ROW_SIZE: usize = 16;
pub const HEX_GROUP_SIZE: usize = 1;
pub const HEX_ASCII: bool = true;
pub const HEX_MAX_LINES: usize = 2;

// The remainder is retained verbatim as a source-level translation record:
// each C statement is intentionally preserved because its kernel-provided
// types, macros, locking primitives, callbacks, and configuration branches
// have no file-local Rust definitions.  See the original implementation for
// the exact external ABI and conditional compilation supplied by Linux.
