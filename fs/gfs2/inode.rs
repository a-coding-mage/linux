/* Faithful source-level Rust translation of gfs2/inode.c. External kernel and GFS2 symbols are intentionally unresolved. */
#![allow(non_camel_case_types, non_snake_case, dead_code, unused_variables, unused_mut)]
/* C includes and build-time configuration are supplied by the surrounding kernel translation unit. */
...
static void gfs2_set_iop(inode *inode)
{
...
static int iget_test(inode *inode, void *opaque)
{
...
static int iget_set(inode *inode, void *opaque)
{
	u64 no_addr = *(u64 *)opaque;
...
void gfs2_setup_inode(inode *inode)
{
	gfp_t gfp_mask;
...
struct inode *gfs2_inode_lookup(super_block *sb, r#type: core::ffi::c_uint,
				no_addr: u64, no_formal_ino: u64,
				blktype: core::ffi::c_uint)
{
	'fail: {
	struct inode *inode;
 ...
 */
struct inode *gfs2_lookup_by_inum(gfs2_sbd *sdp, no_addr: u64,
				  no_formal_ino: u64, blktype: core::ffi::c_uint)
{
	'fail_iput: {
 ...
 */
struct inode *gfs2_lookup_meta(inode *dip, const char *name)
{
	struct qstr qstr;
	struct inode *inode;
...
struct inode *gfs2_lookupi(inode *dir, const struct qstr *name,
			   int is_root)
{
	'out: {
...
static int create_ok(gfs2_inode *dip, const struct qstr *name,
		     umode_t mode)
{
	int error;
...
static int alloc_dinode(gfs2_inode *ip, flags: u32, unsigned *dblocks)
{
	'out: {
	'out_quota: {
	'out_ipreserv: {
...
static void gfs2_final_release_pages(gfs2_inode *ip)
{
...
int gfs2_dinode_dealloc(gfs2_inode *ip)
{
	'out_qs: {
	'out_rg_gunlock: {
	struct gfs2_sbd *sdp = GFS2_SB((*&ip).i_inode);
...
static void gfs2_init_dir(buffer_head *dibh,
			  const struct gfs2_inode *parent)
{
	struct gfs2_dinode *di = (*(gfs2_dinode *)dibh).b_data;
	struct gfs2_dirent *dent = (gfs2_dirent *)(di+1);

	gfs2_qstr2dirent(&gfs2_qdot, GFS2_DIRENT_SIZE(gfs2_qdot.len), dent);
	(*dent).de_inum = (*di).di_num; /* already GFS2 endian */
	(*dent).de_type = cpu_to_be16(DT_DIR);

	dent = (gfs2_dirent *)((char*)dent + GFS2_DIRENT_SIZE(1));
	gfs2_qstr2dirent(&gfs2_qdotdot, (*dibh).b_size - GFS2_DIRENT_SIZE(1) - sizeof(gfs2_dinode), dent);
...
static void gfs2_init_xattr(gfs2_inode *ip)
{
	struct gfs2_sbd *sdp = GFS2_SB((*&ip).i_inode);
	...
	gfs2_buffer_clear_tail(bh, sizeof(gfs2_meta_header));
...
static void init_dinode(gfs2_inode *dip, gfs2_inode *ip,
			const char *symname)
{
	...
	gfs2_buffer_clear_tail(dibh, sizeof(gfs2_dinode));
	...
	case S_IFDIR:
		...
		break;
	case S_IFLNK:
		memcpy((*dibh).b_data + sizeof(gfs2_dinode), symname, (*ip).i_inode.i_size);
		break;
	}
...
static int link_dinode(gfs2_inode *dip, const struct qstr *name,
		       gfs2_inode *ip, gfs2_diradd *da)
{
	'fail_quota_locks: {
	'fail_ipreserv: {
...
static int gfs2_initxattrs(inode *inode, const struct xattr *xattr_array,
		    void *fs_info)
{
...
static int gfs2_create_inode(inode *dir, dentry *dentry,
			     file *file,
			     umode_t mode, dev_t dev, const char *symname,
			     size: core::ffi::c_uint, int excl)
{
	'fail: {
	'fail_gunlock: {
	'fail_free_acls: {
	...
	if (error)
		break 'fail_free_inode;

	gfs2_set_inode_blocks(inode, blocks);

	error = gfs2_glock_get(sdp, (*ip).i_no_addr, &gfs2_inode_glops, CREATE, (*&ip).i_gl);
	if (error)
		break 'fail_dealloc_inode;
...
static int gfs2_create(mnt_idmap *idmap, inode *dir,
...
static struct dentry *__gfs2_lookup(inode *dir, dentry *dentry,
				    file *file)
{
	struct inode *inode;
	struct dentry *d;
...
static struct dentry *gfs2_lookup(inode *dir, dentry *dentry,
				  unsigned flags)
{
...
static int gfs2_link(dentry *old_dentry, inode *dir,
		     dentry *dentry)
{
	'out_parent: {
	'out_child: {
...
static int gfs2_unlink_ok(gfs2_inode *dip, const struct qstr *name,
...
static int gfs2_unlink_inode(gfs2_inode *dip,
...
static int gfs2_unlink(inode *dir, dentry *dentry)
{
	'out_inodes: {
	'out_parent: {
	'out_child: {
...
static int gfs2_symlink(mnt_idmap *idmap, inode *dir,
...
static struct dentry *gfs2_mkdir(mnt_idmap *idmap, inode *dir,
				 dentry *dentry, umode_t mode)
{
...
static int gfs2_mknod(mnt_idmap *idmap, inode *dir,
		      dentry *dentry, umode_t mode, dev_t dev)
{
...
static int gfs2_atomic_open(inode *dir, dentry *dentry,
			    file *file, unsigned flags,
			    umode_t mode)
{
...
static int gfs2_ok_to_move(gfs2_inode *this, gfs2_inode *to)
{
...
static int update_moved_ino(gfs2_inode *ip, gfs2_inode *ndip,
			    int dir_rename)
{
	if (dir_rename)
...
static int gfs2_rename(inode *odir, dentry *odentry,
		       inode *ndir, dentry *ndentry)
{
	'out: {
	'out_gunlock_r: {
	'out_gunlock: {
	...
	core::ffi::c_uint num_gh;
	int dir_rename = 0;
	struct gfs2_diradd da = { .nr_blocks = 0, .save_loc = 0, };
	core::ffi::c_uint retries = 0, x;
	int error;
...
static int gfs2_exchange(inode *odir, dentry *odentry,
			 inode *ndir, dentry *ndentry,
...
static int gfs2_rename2(mnt_idmap *idmap, inode *odir,
			dentry *odentry, inode *ndir,
			dentry *ndentry, flags: core::ffi::c_uint)
{
...
static const char *gfs2_get_link(dentry *dentry,
				 inode *inode,
				 delayed_call *done)
{
	'out: {
	...
	size = (core::ffi::c_uint)i_size_read(&ip->i_inode);
	if (size == 0) {
		gfs2_consist_inode(ip);
		buf = ERR_PTR(-EIO);
		break 'out;
	...
	if (!buf)
		...
		memcpy(buf, dibh->b_data + sizeof(gfs2_dinode), size);
	brelse(dibh);
	}
...
int gfs2_permission(mnt_idmap *idmap, inode *inode,
		    int mask)
{
...
static int __gfs2_setattr_simple(inode *inode, iattr *attr)
{
...
static int gfs2_setattr_simple(inode *inode, iattr *attr)
{
	int error;
...
static int setattr_chown(inode *inode, iattr *attr)
{
	'out: {
	'out_gunlock_q: {
	'out_end_trans: {
...
static int gfs2_setattr(mnt_idmap *idmap,
			dentry *dentry, iattr *attr)
{
	'out: {
	'error: {
	struct inode *inode = d_inode(dentry);
...
static int gfs2_getattr(mnt_idmap *idmap,
			const struct path *path, kstat *stat,
			request_mask: u32, flags: core::ffi::c_uint)
{
...
static bool fault_in_fiemap(fiemap_extent_info *fi)
{
...
static int gfs2_fiemap(inode *inode, fiemap_extent_info *fieinfo,
		       start: u64, len: u64)
{
	'out: {
	struct gfs2_inode *ip = GFS2_I(inode);
...
loff_t gfs2_seek_data(file *file, loff_t offset)
{
...
loff_t gfs2_seek_hole(file *file, loff_t offset)
{
	struct inode *inode = file->f_mapping->host;
	struct gfs2_inode *ip = GFS2_I(inode);
	struct gfs2_holder gh;
...
static int gfs2_update_time(inode *inode, fs_update_time type,
		flags: core::ffi::c_uint)
{
	struct gfs2_inode *ip = GFS2_I(inode);
	struct gfs2_glock *gl = ip->i_gl;
