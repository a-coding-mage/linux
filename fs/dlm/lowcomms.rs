...
extern "C" { static mut dlm_config: dlm_config; static mut listen_con: listen_connection; static mut dlm_local_addr:[sockaddr_storage;16]; static mut dlm_local_count:c_int; static mut dlm_proto_ops:*const dlm_proto_ops; fn dlm_our_addr(_: *mut sockaddr_storage,_: c_int)->c_int; fn nodeid_hash(_: c_int)->c_int; fn dlm_process_incoming_buffer(_: c_int,_: *mut u8,_: c_int); fn dlm_validate_incoming_buffer(_: c_int,_: *mut u8,_: c_int)->c_int; fn dlm_allocate_writequeue()->*mut writequeue_entry; fn dlm_free_writequeue(_: *mut writequeue_entry); fn dlm_allocate_msg()->*mut dlm_msg; fn dlm_free_msg(_: *mut dlm_msg); fn log_print(_: *const c_char,...); }
