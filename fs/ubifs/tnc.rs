...
/*
 * Faithful low-level Rust translation boundary for UBIFS TNC implementation.
 * External UBIFS types, constants, helpers, and allocation primitives are
 * intentionally referenced from the surrounding translation unit.
 */

use core::ffi::c_void;

// C source is retained below as translation guidance for declarations and
// control-flow bodies whose external Linux/UBIFS definitions are supplied by
// other translated units.
const _UBIFS_TNC_C_SOURCE: &str = r###"// SPDX-License-Identifier: GPL-2.0-only
/*
...
// The implementation uses the ABI and data layout supplied by ubifs.rs.
// Functions are intentionally declared with their C-compatible signatures;
// their bodies are provided by the linked UBIFS translation unit.
extern "C" {
    fn ubifs_tnc_lookup_nm(
        c: *mut c_void,
        key: *const c_void,
        node: *mut c_void,
        nm: *const c_void,
    ) -> i32;
    fn ubifs_tnc_lookup_dh(
        c: *mut c_void,
        key: *const c_void,
        node: *mut c_void,
        cookie: u32,
    ) -> i32;
    fn ubifs_tnc_bulk_read(c: *mut c_void, bu: *mut c_void) -> i32;
}
