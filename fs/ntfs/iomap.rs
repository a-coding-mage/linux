 ...
 */
static bool ntfs_iomap_valid(inode *inode, const struct iomap *iomap)
{
	struct ntfs_inode *ni = NTFS_I(inode);
	struct runlist_element *rl;
	s64 vcn, lcn;
...
int ntfs_dio_zero_range(inode *inode, loff_t offset, loff_t length)
{
...
static int ntfs_zero_range(inode *inode, loff_t offset, loff_t length)
{
...
static int ntfs_write_simple_iomap_begin_non_resident(inode *inode, loff_t offset,
						      loff_t length, iomap *iomap)
{
	struct ntfs_inode *ni = NTFS_I(inode);
	struct ntfs_volume *vol = (*ni).vol;
	...
	if (lcn <= LCN_DELALLOC) {
		...
		if (lcn <= LCN_HOLE) {
			...
			rlc = kmalloc(sizeof(runlist_element) * 2,
					GFP_NOFS);
			if (!rlc) {
				up_write((*&ni).runlist.lock);
				mutex_unlock((*&ni).mrec_lock);
...
const NTFS_IOMAP_FLAGS_BEGIN: u32 = 1 << 1;
const NTFS_IOMAP_FLAGS_DIO: u32 = 1 << 2;
pub const NTFS_IOMAP_FLAGS_MKWRITE: u32 = 1 << 3;
...
static int ntfs_write_iomap_begin_resident(inode *inode, loff_t offset,
		iomap *iomap)
{
	'out: {
	struct ntfs_inode *ni = NTFS_I(inode);
