...
// Dependency declarations supplied by the surrounding kernel/Ceph code are
// intentionally omitted here.

/* Call with exclusively locked inode->i_rwsem */
unsafe fn ceph_block_o_direct(ci: *mut ceph_inode_info, inode: *mut inode) {
    let mut is_odirect: bool;
    ...
    is_odirect = READ_ONCE((*ci).i_ceph_flags) & CEPH_I_ODIRECT != 0;
    if is_odirect {
        clear_bit(CEPH_I_ODIRECT_BIT, &mut (*ci).i_ceph_flags);
 ...
 */
unsafe fn ceph_start_io_read(inode: *mut inode) -> i32 {
 ...
 */
unsafe fn ceph_end_io_read(inode: *mut inode) {
    up_read(&mut (*inode).i_rwsem);
}
 ...
 * that we block all direct I/O.
 */
unsafe fn ceph_start_io_write(inode: *mut inode) -> i32 {
 ...
 * lock on inode->i_rwsem.
 */
unsafe fn ceph_end_io_write(inode: *mut inode) {
    up_write(&mut (*inode).i_rwsem);
}

/* Call with exclusively locked inode->i_rwsem */
unsafe fn ceph_block_buffered(ci: *mut ceph_inode_info, inode: *mut inode) {
    let mut is_odirect: bool;
    ...
    is_odirect = READ_ONCE((*ci).i_ceph_flags) & CEPH_I_ODIRECT != 0;
    if !is_odirect {
        set_bit(CEPH_I_ODIRECT_BIT, &mut (*ci).i_ceph_flags);
        /* ensure modified bit is visible */
        smp_mb__after_atomic();
    }
    spin_unlock(&mut (*ci).i_ceph_lock);
 ...
 */
unsafe fn ceph_start_io_direct(inode: *mut inode) -> i32 {
    let ci: *mut ceph_inode_info = ceph_inode(inode);
    let mut is_odirect: bool;
    let err: i32;
 ...
 * lock on inode->i_rwsem.
 */
unsafe fn ceph_end_io_direct(inode: *mut inode) {
    up_read(&mut (*inode).i_rwsem);
}
