...
unsafe fn journal_end_buffer_io_sync(struct biobio)
{
	...
	struct buffer_headorig_bh = bh.b_private;
					...
					u32 crc32_sum)
{
	...
	u32 write_flags = REQ_OP_WRITE | JBD2_JOURNAL_REQ_FLAGS;

	*cbh = core::ptr::null_mut();
 ...
 */
unsafe fn journal_submit_data_buffers(journal_t *journal,
		transaction_t *commit_transaction)
{
	struct jbd2_inodejinode;
	int err, ret = 0;
 ...
 *
 */
unsafe fn journal_finish_inode_data_buffers(journal_t *journal,
		transaction_t *commit_transaction)
{
	struct jbd2_inodejinode, *next_i;
	int err, ret = 0;
				    ...
				    struct buffer_headbh, u32 sequence)
{
	...
	addr = kmap_local_folio(bh.b_folio, bh_offset(bh));
	csum32 = jbd2_chksum(j.j_csum_seed, (u8)&seq, core::mem::size_of::<seq>());
 ...
 */
pub unsafe fn jbd2_journal_commit_transaction(journal_t *journal)
{
	struct transaction_stats_s stats;
	transaction_t *commit_transaction;
	struct journal_headjh;
	struct buffer_headdescriptor;
	...
	journal_block_tag_t *tag = core::ptr::null_mut();
	int space_left = 0;
	int first_tag = 0;
	int tag_flag;
	int i;
	int tag_bytes = journal_tag_bytes(journal);
	struct buffer_headcbh = core::ptr::null_mut(); /* For transactional checksums */
	u32 crc32_sum = ~0;
	...
	int update_tail;
	int csum_size = 0;
	let mut io_bufs = unsafe { core::mem::zeroed() };
	...
	while (journal.j_flags & JBD2_FAST_COMMIT_ONGOING) {
		let mut wait = unsafe { core::mem::zeroed() };
	...
	while (commit_transaction.t_reserved_list) {
		...
		if (jh.b_committed_data) {
			...
			jh.b_committed_data = core::ptr::null_mut();
	...
	journal.j_running_transaction = core::ptr::null_mut();
	...
	while (commit_transaction.t_buffers) {
		...
		if (!descriptor) {
			...
			space_left = descriptor.b_size -
						core::mem::size_of::<journal_header_t>();
			first_tag = 1;
		...
		if (bufs == journal.j_wbufsize ||
		    commit_transaction.t_buffers == core::ptr::null_mut() ||
...
start_journal_io:
	...
	*/
	...
	J_ASSERT(commit_transaction.t_buffers == core::ptr::null_mut());
...
restart_loop:
	...
	while (commit_transaction.t_forget) {
		...
		if (jh.b_committed_data) {
			...
			if (jh.b_frozen_data) {
				...
				jh.b_frozen_data = core::ptr::null_mut();
				jh.b_frozen_triggers = core::ptr::null_mut();
			}
		...
		if (buffer_freed(bh) && !jh.b_next_transaction) {
			 ...
			 * our hands. Note that if mapping == core::ptr::null_mut(), we don't
	...
	if (journal.j_checkpoint_transactions == core::ptr::null_mut()) {
	...
	journal.j_committing_transaction = core::ptr::null_mut();
	...
	if (commit_transaction.t_checkpoint_list == core::ptr::null_mut()) {
