...
unsafe fn zonefs_inode_update_mode(i:*mut inode){let z=zonefs_inode_zone(i);if (*z).z_flags&ZONEFS_ZONE_OFFLINE!=0{(*i).i_flags|=S_IMMUTABLE;(*i).i_mode&=!0o777}else if (*z).z_flags&ZONEFS_ZONE_READONLY!=0{(*i).i_flags|=S_IMMUTABLE;if (*z).z_flags&ZONEFS_ZONE_INIT_MODE!=0{(*i).i_mode&=!0o777}else{(*i).i_mode&=!0o222}}(*z).z_flags&=!ZONEFS_ZONE_INIT_MODE;(*z).z_mode=(*i).i_mode;}
