...
unsafe fn nfsd_set_fh_dentry(rqstp: *mut svc_rqst, net: *mut net, cred: *mut svc_cred, client: *mut auth_domain, gssclient: *mut auth_domain, fhp: *mut svc_fh) -> __be32 {
    ...
    if (*exp).ex_flags & NFSEXP_NOSUBTREECHECK != 0 {
        let new = prepare_creds(); if new.is_null() { error = nfserrno(-ENOMEM); break 'out; }
        (*new).cap_effective = cap_raise_nfsd_set((*new).cap_effective, (*new).cap_permitted);
        put_cred(override_creds(new));
    } else { error = nfsd_setuser_and_check_port(rqstp, cred, exp); if error != 0 { break 'out; } }
    fileid_type = fh.fh_fileid_type; error = nfserr_stale;
    if fileid_type == FILEID_ROOT { dentry = dget((*exp).ex_path.dentry); }
    else {
        if (*exp).ex_flags & NFSEXP_SIGN_FH != 0 { if !fh_verify_mac(fhp, net) { trace_nfsd_set_fh_dentry_badmac(rqstp, fhp, -ESTALE); break 'out; } data_left -= FH_MAC_WORDS; }
    ...
    error = nfserr_badhandle; if dentry.is_null() { break 'out; } if IS_ERR(dentry) { if PTR_ERR(dentry) != -EINVAL { error = nfserrno(PTR_ERR(dentry)); } break 'out; }
    match (*fhp).fh_maxsize { NFS4_FHSIZE => { if (*(*dentry).d_sb).s_export_op.flags & EXPORT_OP_NOATOMIC_ATTR != 0 { (*fhp).fh_no_atomic_attr = true; } (*fhp).fh_64bit_cookies = true; }, NFS3_FHSIZE => { if (*(*dentry).d_sb).s_export_op.flags & EXPORT_OP_NOWCC != 0 { (*fhp).fh_no_wcc = true; } (*fhp).fh_64bit_cookies = true; if (*exp).ex_flags & NFSEXP_V4ROOT != 0 { dput(dentry); break 'out; } }, NFS_FHSIZE => { (*fhp).fh_no_wcc = true; if EX_WGATHER(exp) { (*fhp).fh_use_wgather = true; } if (*exp).ex_flags & NFSEXP_V4ROOT != 0 { dput(dentry); break 'out; } }, _ => {} }
...
unsafe fn __fh_verify(rqstp: *mut svc_rqst, net: *mut net, cred: *mut svc_cred, client: *mut auth_domain, gssclient: *mut auth_domain, fhp: *mut svc_fh, typ: umode_t, access: i32) -> __be32 {
    ...
    'out: {
    if (*fhp).fh_dentry.is_null() { error = nfsd_set_fh_dentry(rqstp, net, cred, client, gssclient, fhp); if error != 0 { break 'out; } }
    let dentry = (*fhp).fh_dentry; exp = (*fhp).fh_export;
    trace_nfsd_fh_verify(rqstp, fhp, typ, access); error = check_pseudo_root(dentry, exp); if error != 0 { break 'out; }
    error = nfsd_setuser_and_check_port(rqstp, cred, exp); if error != 0 { break 'out; }
    error = nfsd_mode_check(dentry, typ); if error != 0 { break 'out; }
    if rqstp.is_null() { goto check_permissions; }
    if access & NFSD_MAY_NLM != 0 && (*exp).ex_flags & NFSEXP_NOAUTHNLM != 0 { break 'out; }
    if access & NFSD_MAY_NLM == 0 { error = check_xprtsec_policy(exp, rqstp); if error != 0 { break 'out; } }
    ...
    error = check_security_flavor(exp, rqstp, may_bypass_gss); if error != 0 { break 'out; }
    svc_xprt_set_valid((*rqstp).rq_xprt);
check_permissions: error = nfsd_permission(cred, exp, dentry, access);
    }
    trace_nfsd_fh_verify_err(rqstp, fhp, typ, access, error); if error == nfserr_stale { nfsd_stats_fh_stale_inc(nn, exp); } error
}
...
pub unsafe fn fh_update(fhp: *mut svc_fh) -> __be32 { if (*fhp).fh_dentry.is_null() { printk!(c"\x013fh_update: fh not verified!\n".as_ptr()); return nfserr_serverfault; } if d_really_is_negative((*fhp).fh_dentry) { printk!(c"\x013fh_update: %pd2 still negative!\n".as_ptr(), (*fhp).fh_dentry); return nfserr_serverfault; } if (*fhp).fh_handle.fh_fileid_type == FILEID_ROOT { _fh_update(fhp, (*fhp).fh_export, (*fhp).fh_dentry); if (*fhp).fh_handle.fh_fileid_type == FILEID_INVALID { return nfserr_stale; } } 0 }
