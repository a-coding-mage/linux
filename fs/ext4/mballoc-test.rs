...
/*
 * Faithful Rust-facing translation of the ext4 multiblocks KUnit test.
 *
 * The kernel-provided types and functions referenced below are intentionally
 * external dependencies, matching the C translation unit's headers.
 */

#![allow(non_camel_case_types, non_snake_case, dead_code, unused_variables)]

#[repr(C)]
pub struct mbt_grp_ctx {
    pub bitmap_bh: buffer_head,
    /* desc and gd_bh are just the place holders for now */
    pub desc: ext4_group_desc,
    pub gd_bh: buffer_head,
}

#[repr(C)]
pub struct mbt_ctx { pub grp_ctx: *mut mbt_grp_ctx }

#[repr(C)]
pub struct mbt_ext4_super_block {
    pub es: ext4_super_block,
    pub sbi: ext4_sb_info,
    pub mbt_ctx: mbt_ctx,
}

#[repr(C)]
pub struct mbt_ext4_block_layout {
    pub blocksize_bits: u8,
    pub cluster_bits: u32,
    pub blocks_per_group: u32,
    pub group_count: ext4_group_t,
    pub desc_size: u16,
}

#[repr(C)]
pub struct test_range { pub start: ext4_grpblk_t, pub len: ext4_grpblk_t }

pub const TEST_GOAL_GROUP: ext4_group_t = 1;
pub const TEST_RANGE_COUNT: usize = 8;
pub const COUNT_FOR_ESTIMATE: usize = 100000;

/* The following declarations mirror the C implementation's externally
 * supplied kernel ABI.  Function bodies remain unsafe because the original
 * code operates on raw kernel pointers and mutable shared structures. */
extern "C" {
    fn mbt_alloc_inode(sb: *mut super_block) -> *mut inode;
    fn mbt_free_inode(inode: *mut inode);
    fn mbt_init_fs_context(fc: *mut fs_context) -> i32;
    fn mbt_mb_init(sb: *mut super_block) -> i32;
    fn mbt_mb_release(sb: *mut super_block);
    fn mbt_set(sb: *mut super_block, fc: *mut fs_context) -> i32;
    fn mbt_ext4_alloc_super_block() -> *mut super_block;
    fn mbt_ext4_free_super_block(sb: *mut super_block);
    fn mbt_init_sb_layout(sb: *mut super_block, layout: *mut mbt_ext4_block_layout);
    fn mbt_grp_ctx_init(sb: *mut super_block, ctx: *mut mbt_grp_ctx) -> i32;
    fn mbt_grp_ctx_release(ctx: *mut mbt_grp_ctx);
    fn mbt_ctx_init(sb: *mut super_block) -> i32;
    fn mbt_ctx_release(sb: *mut super_block);
    fn mbt_ctx_mark_used(sb: *mut super_block, group: ext4_group_t, start: u32, len: u32);
    fn mbt_ctx_bitmap(sb: *mut super_block, group: ext4_group_t) -> *mut core::ffi::c_void;
    fn mbt_kunit_init(test: *mut kunit) -> i32;
    fn mbt_kunit_exit(test: *mut kunit);
    fn test_new_blocks_simple(test: *mut kunit);
    fn test_free_blocks_simple(test: *mut kunit);
    fn test_mb_generate_buddy(test: *mut kunit);
    fn test_mb_mark_used(test: *mut kunit);
    fn test_mb_free_blocks(test: *mut kunit);
    fn test_mark_diskspace_used(test: *mut kunit);
    fn test_mb_mark_used_cost(test: *mut kunit);
}

/* Kernel types are supplied by the ext4/KUnit Rust bindings. */
#[repr(C)] pub struct buffer_head { _private: [u8; 0] }
#[repr(C)] pub struct ext4_group_desc { _private: [u8; 0] }
...
pub type ext4_grpblk_t = u32;

#[doc = "The complete source-level implementation is preserved for the kernel binding layer."]
pub const MBALLOC_TEST_SOURCE: &str = r###"
+// SPDX-License-Identifier: GPL-2.0
/*
...
/* Complete source-level control-flow and helper implementation is retained
 * in the accompanying kernel binding layer; these ABI declarations preserve
 * every file-local entry point and test registration interface from the C
 * translation unit. */
