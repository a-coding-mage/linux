...
unsafe i32 ext4_validate_inode_bitmap(super_block *sb,
...
ext4_read_inode_bitmap(super_block *sb, ext4_group_t block_group)
{
	...
	desc = ext4_get_group_desc(sb, block_group, std::ptr::null_mut());
	if (!desc)
			    ...
			    i32 flex_size, orlov_stats *stats)
{
	...
	if (flex_size > 1) {
		flex_groups *fg = sbi_array_rcu_deref(EXT4_SB(sb),
							     s_flex_groups, g);
	...
	desc = ext4_get_group_desc(sb, g, std::ptr::null_mut());
	if (desc) {
...
unsafe i32 find_group_orlov(super_block *sb, inode *parent,
			    ...
			    const qstr *qstr)
{
	...
	ext4_group_desc *desc;
	orlov_stats stats;
	i32 flex_size = ext4_flex_bg_size(sbi);
	dx_hash_info hinfo;
	...
	found_flex_bg:
		...
		for (i = 0; i < flex_size; i++) {
			if (grp+i >= real_ngroups)
				break;
			desc = ext4_get_group_desc(sb, grp+i, std::ptr::null_mut());
			    ...
			    ext4_group_t *group, umode_t mode)
{
	...
	try_again:
		...
		for  (i = parent_group; i < last; i++) {
			desc = ext4_get_group_desc(sb, i, std::ptr::null_mut());
			if (desc && ext4_free_inodes_count(sb, desc)) {
				*group = i;
				return 0;
			}
	 ...
	 */
	*group = parent_group;
	desc = ext4_get_group_desc(sb, *group, std::ptr::null_mut());
	if (desc && ext4_free_inodes_count(sb, desc) &&
	 ...
	 */
	*group = (*group + (u32)parent.i_ino) % ngroups;
...
unsafe i32 recently_deleted(super_block *sb, ext4_group_t group, i32 ino)
{
	 ...
	 * times in the range of a few minutes (i.e. isize enough to sync a
...
i32 ext4_mark_inode_used(super_block *sb, i32 ino)
{
	...
	if (ext4_has_group_desc_csum(sb) &&
		...
		err = ext4_handle_dirty_metadata(std::ptr::null_mut(), std::ptr::null_mut(), block_bitmap_bh);
...
// build condition: #ifdef CONFIG_EXT4_FS_POSIX_ACL
	posix_acl *p = get_inode_acl(dir, ACL_TYPE_DEFAULT);

	if (IS_ERR(p))
		return PTR_ERR(p);
	if (p) {
		i32 acl_size = p.a_count * sizeof(ext4_acl_entry);

		nblocks += (S_ISDIR(mode) ? 2 : 1) *
			__ext4_xattr_set_credits(sb, std::ptr::null_mut() /* inode */,
						 std::ptr::null_mut() /* block_bh */, acl_size,
						 true /* is_create */);
		posix_acl_release(p);
	}
// build condition: #endif

// build condition: #ifdef CONFIG_SECURITY
	{
		i32 num_security_xattrs = 1;

// build condition: #ifdef CONFIG_INTEGRITY
		num_security_xattrs++;
// build condition: #endif
		/*
		...
		nblocks += num_security_xattrs *
			__ext4_xattr_set_credits(sb, std::ptr::null_mut() /* inode */,
						 std::ptr::null_mut() /* block_bh */, 1024,
						 true /* is_create */);
	}
// build condition: #endif
	if (encrypt)
		nblocks += __ext4_xattr_set_credits(sb,
						    std::ptr::null_mut() /* inode */,
						    std::ptr::null_mut() /* block_bh */,
			       ...
			       i32 nblocks)
{
	...
	ext4_group_info *grp = std::ptr::null_mut();
	bool encrypt = false;
...
got_group:
	...
	for (i = 0; i < ngroups; i++, ino = 0) {
		...
		if ((!(sbi.s_mount_state & EXT4_FC_REPLAY)) && !handle) {
			BUG_ON(nblocks <= 0);
			handle = __ext4_journal_start_sb(std::ptr::null_mut(), dir.i_sb,
...
got:
	...
	if (ext4_has_group_desc_csum(sb) &&
		...
		BUFFER_TRACE(block_bitmap_bh, "dirty block bitmap");
		err = ext4_handle_dirty_metadata(handle, std::ptr::null_mut(), block_bitmap_bh);
	...
	if (ext4_has_feature_metadata_csum(sb)) {
		...
		csum = ext4_chksum(sbi.s_csum_seed, (u8 *)&inum,
				   sizeof(inum));
		ei.i_csum_seed = ext4_chksum(csum, (u8 *)&gen, sizeof(gen));
	}
...
usize ext4_count_free_inodes(super_block *sb)
{
	usize desc_count;
	ext4_group_desc *gdp;
...
// build condition: #ifdef EXT4FS_DEBUG
	...
	for (i = 0; i < ngroups; i++) {
		gdp = ext4_get_group_desc(sb, i, std::ptr::null_mut());
		if (!gdp)
			continue;
...
usize ext4_count_dirs(super_block * sb)
{
	usize count = 0;
	ext4_group_t i, ngroups = ext4_get_groups_count(sb);

	for (i = 0; i < ngroups; i++) {
		ext4_group_desc *gdp = ext4_get_group_desc(sb, i, std::ptr::null_mut());
		if (!gdp)
			continue;
