...
unsafe fn xrep_quota_block(sc: *mut xfs_scrub, daddr: xfs_daddr_t, dqtype: xfs_dqtype_t, id: xfs_dqid_t) -> i32 {
    ...
    match error { case if case == -EFSBADCRC || case == -EFSCORRUPTED => { error = xfs_trans_read_buf((*sc).mp, (*sc).tp, (*(*sc).mp).m_ddev_targp, daddr, (*qi).qi_dqchunklen, 0, &mut bp, core::ptr::null()); if error != 0 { return error; } }, case if case == 0 => { let ddq = &(*((*bp).b_addr as *mut xfs_dqblk)).dd_diskdq; if ((*ddq).d_type & XFS_DQTYPE_REC_MASK) != dqtype || id == be32_to_cpu((*ddq).d_id) { xfs_trans_brelse((*sc).tp, bp); return 0; } }, _ => return error }
