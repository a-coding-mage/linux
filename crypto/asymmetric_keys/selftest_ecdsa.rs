 ...
 */
static CERTS_SELFTEST_ECDSA_KEYS: &[u8] = concat!(
    ...
    "\x14\x9d\x79\x69\x1c\x7a\xd7\x5d\0"
).as_bytes();

/* Signed data and detached signature blobs that form the verification tests. */
static CERTS_SELFTEST_ECDSA_DATA: &[u8] = concat!(
    "This is some test data used for self-testing certificate verification.\n\0"
).as_bytes();

static CERTS_SELFTEST_ECDSA_SIG: &[u8] = concat!(
    ...
    "\x43\x0a\x5f\xf5\xfd\xa2\xf1\0"
).as_bytes();

extern "C" {
    fn fips_signature_selftest(
        name: *const c_char,
        certs: *const u8,
        certs_len: usize,
        data: *const u8,
...
pub unsafe fn fips_signature_selftest_ecdsa() {
    fips_signature_selftest(
        b"ECDSA\0".as_ptr() as *const c_char,
        CERTS_SELFTEST_ECDSA_KEYS.as_ptr(),
        CERTS_SELFTEST_ECDSA_KEYS.len() - 1,
        CERTS_SELFTEST_ECDSA_DATA.as_ptr(),
        CERTS_SELFTEST_ECDSA_DATA.len() - 1,
        CERTS_SELFTEST_ECDSA_SIG.as_ptr(),
        CERTS_SELFTEST_ECDSA_SIG.len() - 1,
    );
}
