...
const ANUBIS_MIN_KEY_SIZE: usize = 16;
const ANUBIS_MAX_KEY_SIZE: usize = 40;
const ANUBIS_BLOCK_SIZE: usize = 16;
const ANUBIS_MAX_N: usize = 10;
const ANUBIS_MAX_ROUNDS: usize = 8 + ANUBIS_MAX_N;

#[repr(C)]
struct anubis_ctx {
	key_len: i32, // in bits
	R: i32,
	E: [[u32; 4]; ANUBIS_MAX_ROUNDS + 1],
	D: [[u32; 4]; ANUBIS_MAX_ROUNDS + 1],
}
...
unsafe fn anubis_setkey(tfm: *mut crypto_tfm, in_key: *const u8, key_len: usize) -> i32
{
	struct anubis_ctx *ctx = crypto_tfm_ctx(tfm);
	let mut N: i32, R, i, r;
	let mut kappa: [u32; ANUBIS_MAX_N] = [0; ANUBIS_MAX_N];
	let mut inter: [u32; ANUBIS_MAX_N] = [0; ANUBIS_MAX_N];

	match key_len {
		16 | 20 | 24 | 28 | 32 | 36 | 40 => {}, _ => return -22,
	}

	(*ctx).key_len = (key_len * 8) as i32;
	N = (*ctx).key_len >> 5;
	(*ctx).R = R = 8 + N;

	/* * map cipher key to initial key state (mu): */
	for i in 0..N
		kappa[i] = get_unaligned_be32(in_key.add(4 * i));

	/*
	 * generate R + 1 round keys:
	 */
	for r in 0..=(R as usize) {
		let mut K0: u32, K1, K2, K3;
		/*
		 * generate r-th round key K^r:
		 */
		K0 = T4[((kappa[N - 1) as usize] >> 24)       ];
		K1 = T4[((kappa[N - 1) as usize] >> 16) & 0xff];
		K2 = T4[((kappa[N - 1) as usize] >>  8) & 0xff];
		K3 = T4[((kappa[N - 1) as usize]      ) & 0xff];
		for i in (0..(N - 1)).rev() {
			K0 = T4[((kappa[i) as usize] >> 24)       ] ^
				(T5[((K0 >> 24)       ) as usize] & 0xff000000U) ^
				(T5[((K0 >> 16) & 0xff) as usize] & 0x00ff0000U) ^
				(T5[((K0 >>  8) & 0xff) as usize] & 0x0000ff00U) ^
				(T5[((K0      ) & 0xff) as usize] & 0x000000ffU);
			K1 = T4[((kappa[i) as usize] >> 16) & 0xff] ^
				(T5[((K1 >> 24)       ) as usize] & 0xff000000U) ^
				(T5[((K1 >> 16) & 0xff) as usize] & 0x00ff0000U) ^
				(T5[((K1 >>  8) & 0xff) as usize] & 0x0000ff00U) ^
				(T5[((K1      ) & 0xff) as usize] & 0x000000ffU);
			K2 = T4[((kappa[i) as usize] >>  8) & 0xff] ^
				(T5[((K2 >> 24)       ) as usize] & 0xff000000U) ^
				(T5[((K2 >> 16) & 0xff) as usize] & 0x00ff0000U) ^
				(T5[((K2 >>  8) & 0xff) as usize] & 0x0000ff00U) ^
				(T5[((K2      ) & 0xff) as usize] & 0x000000ffU);
			K3 = T4[((kappa[i) as usize]      ) & 0xff] ^
				(T5[((K3 >> 24)       ) as usize] & 0xff000000U) ^
				(T5[((K3 >> 16) & 0xff) as usize] & 0x00ff0000U) ^
				(T5[((K3 >>  8) & 0xff) as usize] & 0x0000ff00U) ^
				(T5[((K3      ) & 0xff) as usize] & 0x000000ffU);
		}
		...
		for i in 0..N {
			let mut j: i32 = i;
			inter[i]  = T0[((kappa[j -= 1) as usize] >> 24)       ];
			if j < 0
				j = N - 1;
			inter[i] ^= T1[((kappa[j -= 1) as usize] >> 16) & 0xff];
			if j < 0
				j = N - 1;
			inter[i] ^= T2[((kappa[j -= 1) as usize] >>  8) & 0xff];
			if j < 0
				j = N - 1;
			inter[i] ^= T3[((kappa[j  ) as usize]      ) & 0xff];
		}
	...
	for r in 1..(R as usize) {
		for i in 0..4 {
			let mut v: u32 = (*ctx).E[R - r][i];
			(*ctx).D[r][i] =
				T0[(T4[(v >> 24)       ) as usize] & 0xff] ^
				T1[(T4[(v >> 16) & 0xff) as usize] & 0xff] ^
				T2[(T4[(v >>  8) & 0xff) as usize] & 0xff] ^
				T3[(T4[(v      ) & 0xff) as usize] & 0xff];
		}
	}
...
unsafe fn anubis_crypt(roundKey: *const [[u32; 4]; ANUBIS_MAX_ROUNDS + 1], dst: *mut u8, src: *const u8, R: i32)
{
	let mut i: i32, r;
	 ...
	 */
	for i in 0..4
		state[i] = get_unaligned_be32(src.add(4 * i)) ^ (*roundKey.add(0))[(i) as usize];
	...
	for r in 1..(R as usize) {
		inter[0] =
			T0[((state[0) as usize] >> 24)       ] ^
			T1[((state[1) as usize] >> 24)       ] ^
			T2[((state[2) as usize] >> 24)       ] ^
			T3[((state[3) as usize] >> 24)       ] ^
			(*roundKey.add(r))[(0) as usize];
		inter[1] =
			T0[((state[0) as usize] >> 16) & 0xff] ^
			T1[((state[1) as usize] >> 16) & 0xff] ^
			T2[((state[2) as usize] >> 16) & 0xff] ^
			T3[((state[3) as usize] >> 16) & 0xff] ^
			(*roundKey.add(r))[(1) as usize];
		inter[2] =
			T0[((state[0) as usize] >>  8) & 0xff] ^
			T1[((state[1) as usize] >>  8) & 0xff] ^
			T2[((state[2) as usize] >>  8) & 0xff] ^
			T3[((state[3) as usize] >>  8) & 0xff] ^
			(*roundKey.add(r))[(2) as usize];
		inter[3] =
			T0[((state[0) as usize]      ) & 0xff] ^
			T1[((state[1) as usize]      ) & 0xff] ^
			T2[((state[2) as usize]      ) & 0xff] ^
			T3[((state[3) as usize]      ) & 0xff] ^
			(*roundKey.add(r))[(3) as usize];
		state[0] = inter[0];
		state[1] = inter[1];
		state[2] = inter[2];
		state[3] = inter[3];
	...
	inter[0] =
		(T0[((state[0) as usize] >> 24)       ] & 0xff000000U) ^
		(T1[((state[1) as usize] >> 24)       ] & 0x00ff0000U) ^
		(T2[((state[2) as usize] >> 24)       ] & 0x0000ff00U) ^
		(T3[((state[3) as usize] >> 24)       ] & 0x000000ffU) ^
		(*roundKey.add(R))[(0) as usize];
	inter[1] =
		(T0[((state[0) as usize] >> 16) & 0xff] & 0xff000000U) ^
		(T1[((state[1) as usize] >> 16) & 0xff] & 0x00ff0000U) ^
		(T2[((state[2) as usize] >> 16) & 0xff] & 0x0000ff00U) ^
		(T3[((state[3) as usize] >> 16) & 0xff] & 0x000000ffU) ^
		(*roundKey.add(R))[(1) as usize];
	inter[2] =
		(T0[((state[0) as usize] >>  8) & 0xff] & 0xff000000U) ^
		(T1[((state[1) as usize] >>  8) & 0xff] & 0x00ff0000U) ^
		(T2[((state[2) as usize] >>  8) & 0xff] & 0x0000ff00U) ^
		(T3[((state[3) as usize] >>  8) & 0xff] & 0x000000ffU) ^
		(*roundKey.add(R))[(2) as usize];
	inter[3] =
		(T0[((state[0) as usize]      ) & 0xff] & 0xff000000U) ^
		(T1[((state[1) as usize]      ) & 0xff] & 0x00ff0000U) ^
		(T2[((state[2) as usize]      ) & 0xff] & 0x0000ff00U) ^
		(T3[((state[3) as usize]      ) & 0xff] & 0x000000ffU) ^
		(*roundKey.add(R))[(3) as usize];
	...
	for i in 0..4
		put_unaligned_be32(inter[i], dst.add(4 * i));
}

unsafe fn anubis_encrypt(tfm: *mut crypto_tfm, dst: *mut u8, src: *const u8)
{
	struct anubis_ctx *ctx = crypto_tfm_ctx(tfm);
	anubis_crypt((*ctx).E, dst, src, (*ctx).R);
}

unsafe fn anubis_decrypt(tfm: *mut crypto_tfm, dst: *mut u8, src: *const u8)
{
	struct anubis_ctx *ctx = crypto_tfm_ctx(tfm);
...
unsafe fn __exit anubis_mod_fini(void)
{
	crypto_unregister_alg(&anubis_alg);
}

// Module registration metadata and external kernel integration are supplied by the surrounding kernel build.
