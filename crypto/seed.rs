 ...
 */
#[inline]
fn byte(x: u32, n: u32) . u8 {
    (x >> (n << 3)) as u8
}

#[repr(C)]
struct seed_ctx {
    keysched: [u32; SEED_KEYSCHED_LEN],
};
...
static KC: [u32; SEED_NUM_KCONSTANTS] = {
...
macro_rules! op {
    ($x1:ident, $x2:ident, $x3:ident, $x4:ident, $rbase:expr) => {{
        t0 = $x3 ^ *ks.add($rbase);
        t1 = $x4 ^ *ks.add($rbase + 1);
        t1 ^= t0;
        t1 = SS0[byte(t1, 0) as usize] ^ SS1[byte(t1, 1) as usize] ^
            SS2[byte(t1, 2) as usize] ^ SS3[byte(t1, 3) as usize];
        t0 = t0.wrapping_add(t1);
        t0 = SS0[byte(t0, 0) as usize] ^ SS1[byte(t0, 1) as usize] ^
            SS2[byte(t0, 2) as usize] ^ SS3[byte(t0, 3) as usize];
...
unsafe fn seed_set_key(tfm: *mut crypto_tfm, in_key: *const u8, key_len: u32) . i32
{
	...
	for i in 0..SEED_NUM_KCONSTANTS {
		t0 = x1.wrapping_add(x3).wrapping_sub(KC[i as usize]);
		t1 = x2.wrapping_add(KC[i as usize]).wrapping_sub(x4);
...
unsafe fn seed_encrypt(tfm: *mut crypto_tfm, out: *mut u8, input: *const u8)
{
	...
	x2 = get_unaligned_be32(input.add(4));
	x3 = get_unaligned_be32(input.add(8));
	x4 = get_unaligned_be32(input.add(12));
	...
	put_unaligned_be32(x3, out.add(0));
	put_unaligned_be32(x4, out.add(4));
	put_unaligned_be32(x1, out.add(8));
	put_unaligned_be32(x2, out.add(12));
}
...
unsafe fn seed_init() -> i32
{
	return crypto_register_alg(&raw mut seed_alg);
}

unsafe fn seed_fini()
{
	crypto_unregister_alg(&raw mut seed_alg);
}
