...
#[cfg(all(CONFIG_MMU, not(CONFIG_HAVE_ARCH_TLB_REMOVE_TABLE)))]
#[inline]
...
#[cfg(all(CONFIG_MMU, CONFIG_MMU_GATHER_TABLE_FREE))]
extern "C" { pub fn tlb_remove_table(tlb: *mut mmu_gather, table: *mut c_void); }

#[cfg(all(CONFIG_MMU, not(CONFIG_MMU_GATHER_TABLE_FREE)))]
...
#[cfg(not(CONFIG_MMU_GATHER_RCU_TABLE_FREE))]
#[inline] pub fn tlb_remove_table_sync_one() {}
#[cfg(not(CONFIG_MMU_GATHER_RCU_TABLE_FREE))]
#[inline] pub fn tlb_remove_table_sync_rcu() {}

#[cfg(all(CONFIG_MMU, not(CONFIG_MMU_GATHER_NO_GATHER)))]
...
pub struct mmu_gather {
    ...
    #[cfg(all(not(CONFIG_MMU_GATHER_NO_GATHER), CONFIG_MMU_GATHER_PAGE_SIZE))]
...
#[cfg(CONFIG_MMU_GATHER_NO_RANGE)]
#[inline] pub unsafe fn tlb_flush(tlb: *mut mmu_gather) { if (*tlb).end != 0 { flush_tlb_mm((*tlb).mm); } }

#[cfg(all(not(CONFIG_MMU_GATHER_NO_RANGE), not(feature = "tlb_flush")))]
#[inline]
...
pub unsafe fn tlb_start_vma(tlb: *mut mmu_gather, vma: *mut vm_area_struct) {
    ...
    #[cfg(not(CONFIG_MMU_GATHER_NO_FLUSH_CACHE))]
    flush_cache_range(vma, (*vma).vm_start, (*vma).vm_end);
}
#[inline] pub unsafe fn tlb_end_vma(tlb: *mut mmu_gather, _vma: *mut vm_area_struct) { if (*tlb).fullmm == 0 && !cfg!(CONFIG_MMU_GATHER_MERGE_VMAS) { tlb_flush_mmu_tlbonly(tlb); } }
...
#[cfg(CONFIG_HUGETLB_PMD_PAGE_TABLE_SHARING)]
