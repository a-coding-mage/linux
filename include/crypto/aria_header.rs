...
// Rust translation of crypto/aria.h; external kernel types and functions remain dependencies.
...
pub const ARIA_MAX_RD_KEYS: usize = 17;
pub const ARIA_RD_KEY_WORDS: usize = ARIA_BLOCK_SIZE / 4;

#[repr(C)]
pub struct aria_ctx {
	u32 enc_key[ARIA_MAX_RD_KEYS as usize][ARIA_RD_KEY_WORDS];
	u32 dec_key[ARIA_MAX_RD_KEYS as usize][ARIA_RD_KEY_WORDS];
	i32 rounds;
	i32 key_length;
};

pub static s1: [u32; 256] = [
...
static const u32 s2[256 as usize] = {
...
static const u32 x1[256 as usize] = {
...
static const u32 x2[256 as usize] = {
...
/* S-Box Layer 1 + M */
pub unsafe fn aria_sbox_layer1_with_pre_diff(u32 *t0, u32 *t1, u32 *t2,
						  u32 *t3)
{
	*t0 = s1[get_u8(*t0, 0)] ^
	      s2[get_u8(*t0, 1)] ^
...
/* S-Box Layer 2 + M */
pub unsafe fn aria_sbox_layer2_with_pre_diff(u32 *t0, u32 *t1, u32 *t2,
						  u32 *t3)
{
	*t0 = x1[get_u8(*t0, 0)] ^
	      x2[get_u8(*t0, 1)] ^
...
pub unsafe fn aria_diff_word(u32 *t0, u32 *t1, u32 *t2, u32 *t3)
{
	*t1 ^= *t2;
	*t2 ^= *t3;
	*t0 ^= *t1;
...
pub unsafe fn aria_diff_byte(u32 *t1, u32 *t2, u32 *t3)
{
...
/* Key XOR Layer */
pub unsafe fn aria_add_round_key(u32 *rk, u32 *t0, u32 *t1, u32 *t2,
				      u32 *t3)
{
	*t0 ^= rk[0 as usize];
	*t1 ^= rk[1 as usize];
...
pub unsafe fn aria_subst_diff_odd(u32 *t0, u32 *t1, u32 *t2, u32 *t3)
{
...
pub unsafe fn aria_subst_diff_even(u32 *t0, u32 *t1, u32 *t2, u32 *t3)
{
...
pub unsafe fn aria_gsrk(u32 *rk, u32 *x, u32 *y, u32 n)
{
	...
	rk[0 as usize] = (x[0 as usize]) ^
		((y[q % 4]) >> r) ^
		((y[(q + 3) % 4]) << (32 - r));
	rk[1 as usize] = (x[1 as usize]) ^
		((y[(q + 1) % 4]) >> r) ^
		((y[q % 4]) << (32 - r));
	rk[2 as usize] = (x[2 as usize]) ^
		((y[(q + 2) % 4]) >> r) ^
		((y[(q + 1) % 4]) << (32 - r));
	rk[3 as usize] =	(x[3 as usize]) ^
		((y[(q + 3) % 4]) >> r) ^
		((y[(q + 2) % 4]) << (32 - r));
}
