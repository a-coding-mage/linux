...
pub unsafe fn rseq_v2(t: *mut task_struct) -> bool {
    cfg!(CONFIG_GENERIC_IRQ_ENTRY) && likely((*t).rseq.event.has_rseq > 1)
}

#[cfg(CONFIG_RSEQ)]
#[inline]
pub unsafe fn rseq_handle_slowpath(regs: *mut pt_regs) {
    if cfg!(CONFIG_GENERIC_ENTRY) {
...
pub unsafe fn rseq_virt_userspace_exit() {
    if !cfg!(CONFIG_HAVE_GENERIC_TIF_BITS) && (*current).rseq.event.sched_switch {
        rseq_raise_notify_resume(current);
    }
}
...
#[cfg(not(CONFIG_RSEQ_SLICE_EXTENSION))]
#[inline]
