...
#[cfg(all(CONFIG_TRACE_IRQFLAGS, not(CONFIG_PREEMPT_RT)))]
...
#[cfg(any(not(CONFIG_TRACE_IRQFLAGS), CONFIG_PREEMPT_RT))]
...
#[cfg(not(any(CONFIG_IRQSOFF_TRACER, CONFIG_PREEMPT_TRACER)))]
...
#[macro_export] macro_rules! raw_check_bogus_irq_restore { () => { #[cfg(CONFIG_DEBUG_IRQFLAGS)] unsafe { if !$crate::arch_irqs_disabled() { $crate::warn_bogus_irq_restore(); } } }; }
