...
pub const SPRD_DMA_REQ_SHIFT: u32 = 8;
pub const SPRD_DMA_TRG_MODE_SHIFT: u32 = 16;
pub const SPRD_DMA_CHN_MODE_SHIFT: u32 = 24;

macro_rules! SPRD_DMA_FLAGS {
    ($chn_mode:expr, $trg_mode:expr, $req_mode:expr, $int_type:expr) => {
        (($chn_mode) << SPRD_DMA_CHN_MODE_SHIFT
            | ($trg_mode) << SPRD_DMA_TRG_MODE_SHIFT
            | ($req_mode) << SPRD_DMA_REQ_SHIFT
            | ($int_type))
    };
}
...
/* DMA channel trigger mode for 2-stage transfer. */
#[repr(i32)]
...
/* DMA request mode: fragment, block, transaction, or link-list. */
#[repr(i32)]
 ...
 * configurations by writing the physical address of each configuration into
 * the link-list register.
 *
