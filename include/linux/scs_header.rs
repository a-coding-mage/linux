...
pub unsafe fn __scs_magic(s: *mut c_void) -> *mut usize {
    ((s as *mut u8).add(SCS_SIZE) as *mut usize).sub(1)
}
...
#[cfg(all(CONFIG_SHADOW_CALL_STACK, not(CONFIG_DYNAMIC_SCS)))]
#[inline]
pub fn scs_is_dynamic() -> bool { false }

#[cfg(all(CONFIG_SHADOW_CALL_STACK, CONFIG_DYNAMIC_SCS))]
#[inline]
...
pub unsafe fn scs_is_enabled() -> bool {
    if cfg!(CONFIG_DYNAMIC_SCS) { scs_is_dynamic() } else { true }
}
