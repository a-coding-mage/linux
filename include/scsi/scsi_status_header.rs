...
// Dependencies supplied by the corresponding Linux and SCSI headers.
...
pub enum scsi_msg_byte {
    ...
    BUS_DEVICE_RESET = TARGET_RESET as i32,
    ABORT = ABORT_TASK_SET as i32,
}
...
pub enum scsi_host_status {
    ...
    DID_REQUEUE = 0x0d, // Requeue command (no immediate retry) also without decrementing the retry count
    DID_TRANSPORT_DISRUPTED = 0x0e, // Transport error disrupted execution and the driver blocked the port to recover the link. Transport class will retry or fail IO
