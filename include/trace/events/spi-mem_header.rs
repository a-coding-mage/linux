...
// C header dependencies translated as external dependencies:
// linux/tracepoint.h, linux/spi/spi-mem.h, and trace/define_trace.h.
// The TRACE_EVENT machinery is supplied by the surrounding tracepoint
// implementation.

pub const TRACE_SYSTEM: &str = "spi-mem";
pub const TRACE_SYSTEM_VAR: &str = "spi_mem";

/// Equivalent of the C decode_dtr(dtr) symbolic trace formatter.
#[inline]
pub const fn decode_dtr(dtr: bool) -> &'static str {
    match dtr {
        false => "S",
        true => "D",
    }
...
//         __get_str(name), __entry->cmd_buswidth,
//         decode_dtr(__entry->cmd_dtr), __entry->addr_buswidth,
//         decode_dtr(__entry->addr_dtr), __entry->data_buswidth,
//         decode_dtr(__entry->data_dtr), __entry->max_freq,
//         __get_dynamic_array_len(op), __get_dynamic_array(op),
//         __entry->data_len, __get_dynamic_array_len(data),
...
//                    __get_dynamic_array_len(data));
//     ),
//     TP_printk("%s len=%u rx=[%*phD]", __get_str(name),
//         __entry->data_len, __get_dynamic_array_len(data),
//         __get_dynamic_array(data))
// );
