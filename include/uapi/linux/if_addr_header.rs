...
/* Dependencies supplied by the corresponding Linux type and netlink headers. */

#[repr(C)]
pub struct ifaddrmsg {
    pub ifa_family: __u8,
    pub ifa_prefixlen: __u8, /* The prefix length */
    pub ifa_flags: __u8,    /* Flags */
    pub ifa_scope: __u8,    /* Address scope */
    pub ifa_index: __u32,   /* Link index */
}
...
pub const IFA_MC_USERS: u32 = 12; /* u32, multicast group users */
pub const __IFA_MAX: u32 = 13;
...
pub struct ifa_cacheinfo {
    ...
    pub ifa_valid: __u32,
    pub cstamp: __u32, /* created timestamp, hundredths of seconds */
    pub tstamp: __u32, /* updated timestamp, hundredths of seconds */
}

/* Backwards compatibility for userspace.  The original definitions are
 * available when the kernel-only condition is not selected. */
#[cfg(not(feature = "kernel"))]
macro_rules! IFA_RTA {
    ($r:expr) => {
        (($r as *mut u8).wrapping_add(NLMSG_ALIGN(core::mem::size_of::<ifaddrmsg>())))
            as *mut rtattr
    };
}
