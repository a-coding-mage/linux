...
const MAX98927_DEV0_NAME: *const c_char = b"i2c-MX98927:00\0".as_ptr() as *const c_char;
const MAX98927_DEV1_NAME: *const c_char = b"i2c-MX98927:01\0".as_ptr() as *const c_char;
const MAX98927_CODEC_NAME: *const c_char = b"max98927-aif1\0".as_ptr() as *const c_char;
...
// COMP_CODEC_CONF(MAX98927_DEVx_NAME)
...
// SOC_DAPM_PIN_SWITCH("Left Spk"), SOC_DAPM_PIN_SWITCH("Right Spk")
static card_controls: [snd_kcontrol_new; 2] = [
    snd_kcontrol_new { _private: [] },
    snd_kcontrol_new { _private: [] },
];

// SND_SOC_DAPM_SPK("Left Spk", NULL), SND_SOC_DAPM_SPK("Right Spk", NULL)
...
unsafe extern "C" fn avs_max98927_be_fixup(
...
unsafe extern "C" fn avs_max98927_hw_params(
    ...
    while {
        ...
        if strcmp((*(*codec_dai).component).name, MAX98927_DEV0_NAME) == 0 {
            ret = snd_soc_dai_set_tdm_slot(codec_dai, 0x30, 3, 8, 16);
        } else if strcmp((*(*codec_dai).component).name, MAX98927_DEV1_NAME) == 0 {
...
static avs_max98927_ops: snd_soc_ops = snd_soc_ops {
    hw_params: Some(avs_max98927_hw_params),
};
...
unsafe extern "C" fn avs_create_dai_link(
    ...
    (*(*dl).codecs.add(0)).name = devm_kasprintf(dev, GFP_KERNEL, MAX98927_DEV0_NAME);
    (*(*dl).codecs.add(0)).dai_name = devm_kasprintf(dev, GFP_KERNEL, MAX98927_CODEC_NAME);
    (*(*dl).codecs.add(1)).name = devm_kasprintf(dev, GFP_KERNEL, MAX98927_DEV1_NAME);
    (*(*dl).codecs.add(1)).dai_name = devm_kasprintf(dev, GFP_KERNEL, MAX98927_CODEC_NAME);
    ...
    (*dl).be_hw_params_fixup = Some(avs_max98927_be_fixup);
    (*dl).nonatomic = 1;
    (*dl).no_pcm = 1;
...
unsafe extern "C" fn avs_max98927_probe(pdev: *mut platform_device) -> c_int {
    ...
    if (*pdata).obsolete_card_names {
        (*card).name = b"avs_max98927\0".as_ptr() as *const c_char;
    } else {
        (*card).driver_name = b"avs_max98927\0".as_ptr() as *const c_char;
        (*card).name = b"AVS I2S MAX98927\0".as_ptr() as *const c_char;
...
static avs_max98927_driver_ids: [platform_device_id; 2] = [
    platform_device_id {
        name: [
            b'a' as c_char,
            b'v' as c_char,
...
static mut avs_max98927_driver: platform_driver = platform_driver {
    probe: Some(avs_max98927_probe),
    driver: device_driver {
        name: b"avs_max98927\0".as_ptr() as *const c_char,
        pm: unsafe { &snd_soc_pm_ops },
    },
    id_table: avs_max98927_driver_ids.as_ptr(),
};
