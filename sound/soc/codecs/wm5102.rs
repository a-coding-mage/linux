...
/*
 * wm5102.rs -- Rust source-level translation of wm5102.c
 *
 * Original C dependency includes are intentionally not emitted as Rust items.
 * Kernel/ALSA/Arizona macros and external symbols are referenced in their Rust forms
 * where directly translatable; macro-only tables are preserved below as source
 * translation blocks because their definitions are external to this isolated file.
 */
...
pub const DRV_NAME: &[u8] = b"wm5102-codec\0";

#[repr(C)]
pub struct wm5102_priv {
    pub core: arizona_priv,
    pub fll: [arizona_fll; 2],
}

unsafe extern "C" {
    static mut wm5102_sysclk_reva_patch: [reg_default; 258];
    static mut wm5102_sysclk_revb_patch: [reg_default; 4];
    static mut wm5102_dsp1_regions: [cs_dsp_region; 4];
}

/* External C types and constants expected from translated dependencies. */
...
unsafe fn wm5102_sysclk_ev(w: *mut snd_soc_dapm_widget, kcontrol: *mut snd_kcontrol, event: c_int) -> c_int {
    let component = snd_soc_dapm_to_component((*w).dapm);
    let arizona = dev_get_drvdata((*(*component).dev).parent) as *mut arizona;
    let regmap = (*arizona).regmap;
    let mut patch: *const reg_default = core::ptr::null();
    let mut patch_size: c_int;
    match (*arizona).rev {
        0 => {
            patch = wm5102_sysclk_reva_patch.as_ptr();
            patch_size = wm5102_sysclk_reva_patch.len() as c_int;
        }
        _ => {
            patch = wm5102_sysclk_revb_patch.as_ptr();
            patch_size = wm5102_sysclk_revb_patch.len() as c_int;
        }
    }
    match event {
        SND_SOC_DAPM_POST_PMU => {
            if !patch.is_null() {
                let mut i: c_int = 0;
                while i < patch_size {
                    regmap_write_async(regmap, (*patch.add(i as usize)).reg, (*patch.add(i as usize)).def);
                    i += 1;
                }
            }
        }
        SND_SOC_DAPM_PRE_PMD => {}
        SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD => return arizona_clk_ev(w, kcontrol, event),
        _ => return 0,
    }
    arizona_dvfs_sysclk_ev(w, kcontrol, event)
}

unsafe fn wm5102_adsp_power_ev(w: *mut snd_soc_dapm_widget, kcontrol: *mut snd_kcontrol, event: c_int) -> c_int {
    let component = snd_soc_dapm_to_component((*w).dapm);
    let arizona = dev_get_drvdata((*(*component).dev).parent) as *mut arizona;
    let mut v: c_uint = 0;
    let mut ret: c_int;
    match event {
        SND_SOC_DAPM_PRE_PMU => {
            ret = regmap_read((*arizona).regmap, ARIZONA_SYSTEM_CLOCK_1, &mut v);
            if ret != 0 {
            ...
            if v >= 3 {
                ...
                if ret != 0 {
                    dev_err((*component).dev, b"Failed to raise DVFS: %d\n\0".as_ptr() as *const c_char, ret);
                    return ret;
                }
            }
        ...
        SND_SOC_DAPM_POST_PMD => {
            ...
            if ret != 0 {
                dev_warn((*component).dev, b"Failed to lower DVFS: %d\n\0".as_ptr() as *const c_char, ret);
            }
        }
        _ => {}
    }
...
unsafe fn wm5102_out_comp_coeff_get(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
    let component = snd_kcontrol_chip(kcontrol) as *mut snd_soc_component;
    let arizona = dev_get_drvdata((*(*component).dev).parent) as *mut arizona;
    mutex_lock(&mut (*arizona).dac_comp_lock);
    put_unaligned_be16((*arizona).dac_comp_coeff, (*ucontrol).value.bytes.data.as_mut_ptr());
    mutex_unlock(&mut (*arizona).dac_comp_lock);
    0
}

unsafe fn wm5102_out_comp_coeff_put(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
    let component = snd_kcontrol_chip(kcontrol) as *mut snd_soc_component;
    let arizona = dev_get_drvdata((*(*component).dev).parent) as *mut arizona;
    let dac_comp_coeff: uint16_t = get_unaligned_be16((*ucontrol).value.bytes.data.as_ptr());
    mutex_lock(&mut (*arizona).dac_comp_lock);
...
unsafe fn wm5102_out_comp_switch_get(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
    let component = snd_kcontrol_chip(kcontrol) as *mut snd_soc_component;
    let arizona = dev_get_drvdata((*(*component).dev).parent) as *mut arizona;
    mutex_lock(&mut (*arizona).dac_comp_lock);
    (*ucontrol).value.integer.value[0] = (*arizona).dac_comp_enabled as _;
    mutex_unlock(&mut (*arizona).dac_comp_lock);
    0
}

unsafe fn wm5102_out_comp_switch_put(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
    let component = snd_kcontrol_chip(kcontrol) as *mut snd_soc_component;
    let arizona = dev_get_drvdata((*(*component).dev).parent) as *mut arizona;
    let mc = (*kcontrol).private_value as *mut soc_mixer_control;
    if (*ucontrol).value.integer.value[0] > (*mc).max as _ {
        return -EINVAL;
    }
    mutex_lock(&mut (*arizona).dac_comp_lock);
    if (*arizona).dac_comp_enabled as _ != (*ucontrol).value.integer.value[0] {
        (*arizona).dac_comp_enabled = (*ucontrol).value.integer.value[0] as _;
        mutex_unlock(&mut (*arizona).dac_comp_lock);
        return 1;
    }
    mutex_unlock(&mut (*arizona).dac_comp_lock);
...
/*
Original macro-heavy declarations, constant tables, DAPM widgets/routes, DAI tables, platform driver registration, and module metadata preserved from wm5102.c for dependency macro translation:
