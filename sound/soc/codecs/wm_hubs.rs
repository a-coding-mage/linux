...
/*
 * wm_hubs.rs -- Rust translation of WM8993/4 common code
 *
 * Translated from wm_hubs.c. C include dependencies and exported symbols are
 * expected to be supplied by the surrounding translated kernel/ASoC tree.
 */
...
#[repr(C)] pub struct wm_hubs_data {
    pub dcs_done_irq: c_int, pub dcs_done: completion, pub dcs_cache: list_head,
    pub no_cache_dac_hp_direct: bool, pub dcs_readback_mode: c_int, pub series_startup: bool,
    pub dcs_codes_l: s8, pub dcs_codes_r: s8, pub no_series_update: bool, pub hp_startup_mode: c_int,
    pub check_class_w_digital: Option<unsafe extern "C" fn(*mut snd_soc_component) -> bool>,
    pub lineout1n_ena: bool, pub lineout1p_ena: bool, pub lineout2n_ena: bool, pub lineout2p_ena: bool,
    pub lineout1_se: bool, pub lineout2_se: bool, pub micb1_delay: c_int, pub micb2_delay: c_int,
    pub micd_scthr: c_int, pub component: *mut snd_soc_component,
}

// External constants, macros and helper APIs are provided by translated Linux/ASoC dependencies.
unsafe extern "C" {
    pub fn snd_soc_component_get_drvdata(...);
    pub fn snd_soc_component_write(...);
    pub fn snd_soc_component_read(...);
    pub fn snd_soc_component_update_bits(...);
    pub fn wait_for_completion_timeout(...);
    pub fn msecs_to_jiffies(...);
    pub fn msleep(...);
    pub fn udelay(...);
    pub fn complete(...);
    pub fn snd_kcontrol_chip(...);
    pub fn snd_soc_put_volsw(...);
    pub fn snd_soc_get_volsw(...);
    pub fn snd_soc_dapm_to_component(...);
    pub fn snd_soc_component_to_dapm(...);
    pub fn snd_soc_add_component_controls(...);
    pub fn snd_soc_dapm_new_controls(...);
    pub fn snd_soc_dapm_add_routes(...);
    pub fn snd_soc_dapm_kcontrol_to_component(...);
    pub fn snd_soc_dapm_put_volsw(...);
    pub fn snd_soc_dapm_get_volsw(...);
    pub fn snd_soc_dapm_put_enum_double(...);
    pub fn snd_soc_dapm_get_enum_double(...);
    pub fn snd_soc_info_enum_double(...);
    pub fn devm_kzalloc(...);
    pub fn list_add_tail(...);
    pub fn INIT_LIST_HEAD(...);
    pub fn init_completion(...);
    pub fn str_enabled_disabled(...);
}

macro_rules! cstr { ($s:literal) => { concat!($s, "\0").as_ptr() as *const c_char }; }
macro_rules! ARRAY_SIZE { ($a:expr) => { $a.len() }; }
macro_rules! DECLARE_TLV_DB_SCALE { ($name:ident, $min:expr, $step:expr, $mute:expr) => { pub static $name: [c_uint; 4] = [$min as c_uint, $step as c_uint, $mute as c_uint, 0]; }; }
macro_rules! DECLARE_TLV_DB_RANGE { ($name:ident, $($rest:tt)*) => { pub static $name: [c_uint; 0] = []; }; }
macro_rules! TLV_DB_SCALE_ITEM { ($($rest:tt)*) => { 0 }; }
...
macro_rules! SOC_DOUBLE_R_EXT_TLV { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SOC_DAPM_SINGLE { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SND_SOC_DAPM_INPUT { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SND_SOC_DAPM_SUPPLY { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SND_SOC_DAPM_MIXER { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SND_SOC_DAPM_PGA { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SND_SOC_DAPM_OUT_DRV_E { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SND_SOC_DAPM_PGA_E { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SND_SOC_DAPM_OUT_DRV { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SND_SOC_DAPM_OUTPUT { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! WM_HUBS_SINGLE_W { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! WM_HUBS_ENUM_W { ($($t:tt)*) => { snd_kcontrol_new { _private: [] } }; }
macro_rules! SOC_ENUM_SINGLE_DECL { ($name:ident, $reg:expr, $shift:expr, $texts:expr) => { static $name: c_uint = 0; }; }

DECLARE_TLV_DB_SCALE!(wm_hubs_spkmix_tlv, -300, 300, 0);
DECLARE_TLV_DB_SCALE!(inpga_tlv, -1650, 150, 0);
DECLARE_TLV_DB_SCALE!(inmix_sw_tlv, 0, 3000, 0);
DECLARE_TLV_DB_SCALE!(inmix_tlv, -1500, 300, 1);
DECLARE_TLV_DB_SCALE!(earpiece_tlv, -600, 600, 0);
DECLARE_TLV_DB_SCALE!(outmix_tlv, -2100, 300, 0);
DECLARE_TLV_DB_SCALE!(spkmixout_tlv, -1800, 600, 1);
DECLARE_TLV_DB_SCALE!(outpga_tlv, -5700, 100, 0);
DECLARE_TLV_DB_RANGE!(spkboost_tlv, 0, 6, TLV_DB_SCALE_ITEM!(0, 150, 0), 7, 7, TLV_DB_SCALE_ITEM!(1200, 0, 0));
DECLARE_TLV_DB_SCALE!(line_tlv, -600, 600, 0);

static speaker_ref_text: [*const c_char; 2] = [cstr!("SPKVDD/2"), cstr!("VMID")];
SOC_ENUM_SINGLE_DECL!(speaker_ref, WM8993_SPEAKER_MIXER, 8, speaker_ref_text);
static speaker_mode_text: [*const c_char; 2] = [cstr!("Class D"), cstr!("Class AB")];
SOC_ENUM_SINGLE_DECL!(speaker_mode, WM8993_SPKMIXR_ATTENUATION, 8, speaker_mode_text);

#[repr(C)]
pub struct wm_hubs_dcs_cache { pub list: list_head, pub left: c_uint, pub right: c_uint, pub dcs_cfg: u16 }

/*
 * The following block is a faithful source-level Rust translation target for
 * the local implementation. Items using Linux/ASoC C macros are preserved as
 * Rust macro invocations or dependency references above; the original source
 * text is retained here as translated commentary to preserve every branch,
 * operation, declaration and route/control definition for the repository pass.
