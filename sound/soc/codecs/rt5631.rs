...
/*
 * rt5631.rs  --  RT5631 ALSA Soc Audio driver
 *
 * Rust source-level translation of rt5631.c.
 * Original C include dependencies are intentionally left as external kernel/ASoC
 * symbols and macros supplied by the surrounding repository.
 */
...
#[repr(C)]
pub struct rt5631_priv {
    pub regmap: *mut regmap,
    pub codec_version: c_int,
    pub master: c_int,
    pub sysclk: c_int,
    pub rx_rate: c_int,
    pub bclk_rate: c_int,
    pub dmic_used_flag: c_int,
}

#[repr(C)]
pub struct coeff_clk_div {
    pub mclk: u32,
    pub bclk: u32,
    pub rate: u32,
    pub reg_val: u16,
...
unsafe fn rt5631_write_index(component: *mut snd_soc_component, reg: c_uint, value: c_uint) {
    snd_soc_component_write(component, RT5631_INDEX_ADD, reg);
    snd_soc_component_write(component, RT5631_INDEX_DATA, value);
}

unsafe fn rt5631_read_index(component: *mut snd_soc_component, reg: c_uint) -> c_uint {
    let value: c_uint;
    snd_soc_component_write(component, RT5631_INDEX_ADD, reg);
...
unsafe fn rt5631_reset(component: *mut snd_soc_component) -> c_int {
    snd_soc_component_write(component, RT5631_RESET, 0) as c_int
}

unsafe fn rt5631_volatile_register(dev: *mut device, reg: c_uint) -> bool {
    match reg {
        RT5631_RESET | RT5631_INT_ST_IRQ_CTRL_2 | RT5631_INDEX_ADD | RT5631_INDEX_DATA | RT5631_EQ_CTRL => true,
        _ => false,
    }
}

unsafe fn rt5631_readable_register(dev: *mut device, reg: c_uint) -> bool {
    match reg {
        RT5631_RESET | RT5631_SPK_OUT_VOL | RT5631_HP_OUT_VOL | RT5631_MONO_AXO_1_2_VOL |
        RT5631_AUX_IN_VOL | RT5631_STEREO_DAC_VOL_1 | RT5631_MIC_CTRL_1 | RT5631_STEREO_DAC_VOL_2 |
        RT5631_ADC_CTRL_1 | RT5631_ADC_REC_MIXER | RT5631_ADC_CTRL_2 | RT5631_VDAC_DIG_VOL |
        RT5631_OUTMIXER_L_CTRL | RT5631_OUTMIXER_R_CTRL | RT5631_AXO1MIXER_CTRL | RT5631_AXO2MIXER_CTRL |
        RT5631_MIC_CTRL_2 | RT5631_DIG_MIC_CTRL | RT5631_MONO_INPUT_VOL | RT5631_SPK_MIXER_CTRL |
        RT5631_SPK_MONO_OUT_CTRL | RT5631_SPK_MONO_HP_OUT_CTRL | RT5631_SDP_CTRL | RT5631_MONO_SDP_CTRL |
        RT5631_STEREO_AD_DA_CLK_CTRL | RT5631_PWR_MANAG_ADD1 | RT5631_PWR_MANAG_ADD2 | RT5631_PWR_MANAG_ADD3 |
        RT5631_PWR_MANAG_ADD4 | RT5631_GEN_PUR_CTRL_REG | RT5631_GLOBAL_CLK_CTRL | RT5631_PLL_CTRL |
        RT5631_INT_ST_IRQ_CTRL_1 | RT5631_INT_ST_IRQ_CTRL_2 | RT5631_GPIO_CTRL | RT5631_MISC_CTRL |
        RT5631_DEPOP_FUN_CTRL_1 | RT5631_DEPOP_FUN_CTRL_2 | RT5631_JACK_DET_CTRL | RT5631_SOFT_VOL_CTRL |
        RT5631_ALC_CTRL_1 | RT5631_ALC_CTRL_2 | RT5631_ALC_CTRL_3 | RT5631_PSEUDO_SPATL_CTRL |
        RT5631_INDEX_ADD | RT5631_INDEX_DATA | RT5631_EQ_CTRL | RT5631_VENDOR_ID | RT5631_VENDOR_ID1 |
        RT5631_VENDOR_ID2 => true,
        _ => false,
    }
}

static rt5631_reg: &[reg_default] = &[
    reg_default { reg: RT5631_SPK_OUT_VOL, def: 0x8888 },
    reg_default { reg: RT5631_HP_OUT_VOL, def: 0x8080 },
    reg_default { reg: RT5631_MONO_AXO_1_2_VOL, def: 0xa080 },
    reg_default { reg: RT5631_AUX_IN_VOL, def: 0x0808 },
    reg_default { reg: RT5631_ADC_REC_MIXER, def: 0xf0f0 },
    reg_default { reg: RT5631_VDAC_DIG_VOL, def: 0x0010 },
    reg_default { reg: RT5631_OUTMIXER_L_CTRL, def: 0xffc0 },
    reg_default { reg: RT5631_OUTMIXER_R_CTRL, def: 0xffc0 },
    reg_default { reg: RT5631_AXO1MIXER_CTRL, def: 0x88c0 },
    reg_default { reg: RT5631_AXO2MIXER_CTRL, def: 0x88c0 },
    reg_default { reg: RT5631_DIG_MIC_CTRL, def: 0x3000 },
    reg_default { reg: RT5631_MONO_INPUT_VOL, def: 0x8808 },
    reg_default { reg: RT5631_SPK_MIXER_CTRL, def: 0xf8f8 },
    reg_default { reg: RT5631_SPK_MONO_OUT_CTRL, def: 0xfc00 },
    reg_default { reg: RT5631_SPK_MONO_HP_OUT_CTRL, def: 0x4440 },
    reg_default { reg: RT5631_SDP_CTRL, def: 0x8000 },
    reg_default { reg: RT5631_MONO_SDP_CTRL, def: 0x8000 },
    reg_default { reg: RT5631_STEREO_AD_DA_CLK_CTRL, def: 0x2010 },
    reg_default { reg: RT5631_GEN_PUR_CTRL_REG, def: 0x0e00 },
    reg_default { reg: RT5631_INT_ST_IRQ_CTRL_2, def: 0x071a },
    reg_default { reg: RT5631_MISC_CTRL, def: 0x2040 },
    reg_default { reg: RT5631_DEPOP_FUN_CTRL_2, def: 0x8000 },
    reg_default { reg: RT5631_SOFT_VOL_CTRL, def: 0x07e0 },
    reg_default { reg: RT5631_ALC_CTRL_1, def: 0x0206 },
    reg_default { reg: RT5631_ALC_CTRL_3, def: 0x2000 },
    reg_default { reg: RT5631_PSEUDO_SPATL_CTRL, def: 0x0553 },
];

/* The following C macro-generated ALSA controls/widgets/routes are translated as Rust-side macro invocations conceptually; their concrete Rust item types depend on external ASoC macro definitions.
...
static codec_master_pll_div: &[pll_div] = &[
    pll_div { pll_in: 2048000, pll_out: 8192000, reg_val: 0x0ea0 },
    pll_div { pll_in: 3686400, pll_out: 8192000, reg_val: 0x4e27 },
    pll_div { pll_in: 12000000, pll_out: 8192000, reg_val: 0x456b },
    pll_div { pll_in: 13000000, pll_out: 8192000, reg_val: 0x495f },
    pll_div { pll_in: 13100000, pll_out: 8192000, reg_val: 0x0320 },
    pll_div { pll_in: 2048000, pll_out: 11289600, reg_val: 0xf637 },
    pll_div { pll_in: 3686400, pll_out: 11289600, reg_val: 0x2f22 },
    pll_div { pll_in: 12000000, pll_out: 11289600, reg_val: 0x3e2f },
    pll_div { pll_in: 13000000, pll_out: 11289600, reg_val: 0x4d5b },
    pll_div { pll_in: 13100000, pll_out: 11289600, reg_val: 0x363b },
    pll_div { pll_in: 2048000, pll_out: 16384000, reg_val: 0x1ea0 },
    pll_div { pll_in: 3686400, pll_out: 16384000, reg_val: 0x9e27 },
    pll_div { pll_in: 12000000, pll_out: 16384000, reg_val: 0x452b },
    pll_div { pll_in: 13000000, pll_out: 16384000, reg_val: 0x542f },
    pll_div { pll_in: 13100000, pll_out: 16384000, reg_val: 0x03a0 },
    pll_div { pll_in: 2048000, pll_out: 16934400, reg_val: 0xe625 },
    pll_div { pll_in: 3686400, pll_out: 16934400, reg_val: 0x9126 },
    pll_div { pll_in: 12000000, pll_out: 16934400, reg_val: 0x4d2c },
    pll_div { pll_in: 13000000, pll_out: 16934400, reg_val: 0x742f },
    pll_div { pll_in: 13100000, pll_out: 16934400, reg_val: 0x3c27 },
    pll_div { pll_in: 2048000, pll_out: 22579200, reg_val: 0x2aa0 },
    pll_div { pll_in: 3686400, pll_out: 22579200, reg_val: 0x2f20 },
    pll_div { pll_in: 12000000, pll_out: 22579200, reg_val: 0x7e2f },
    pll_div { pll_in: 13000000, pll_out: 22579200, reg_val: 0x742f },
    pll_div { pll_in: 13100000, pll_out: 22579200, reg_val: 0x3c27 },
    pll_div { pll_in: 2048000, pll_out: 24576000, reg_val: 0x2ea0 },
    pll_div { pll_in: 3686400, pll_out: 24576000, reg_val: 0xee27 },
    pll_div { pll_in: 12000000, pll_out: 24576000, reg_val: 0x2915 },
    pll_div { pll_in: 13000000, pll_out: 24576000, reg_val: 0x772e },
    pll_div { pll_in: 13100000, pll_out: 24576000, reg_val: 0x0d20 },
    pll_div { pll_in: 26000000, pll_out: 24576000, reg_val: 0x2027 },
    pll_div { pll_in: 26000000, pll_out: 22579200, reg_val: 0x392f },
    pll_div { pll_in: 24576000, pll_out: 22579200, reg_val: 0x0921 },
    pll_div { pll_in: 24576000, pll_out: 24576000, reg_val: 0x02a0 },
];

static codec_slave_pll_div: &[pll_div] = &[
    pll_div { pll_in: 256000, pll_out: 2048000, reg_val: 0x46f0 },
    pll_div { pll_in: 256000, pll_out: 4096000, reg_val: 0x3ea0 },
    pll_div { pll_in: 352800, pll_out: 5644800, reg_val: 0x3ea0 },
    pll_div { pll_in: 512000, pll_out: 8192000, reg_val: 0x3ea0 },
    pll_div { pll_in: 1024000, pll_out: 8192000, reg_val: 0x46f0 },
    pll_div { pll_in: 705600, pll_out: 11289600, reg_val: 0x3ea0 },
    pll_div { pll_in: 1024000, pll_out: 16384000, reg_val: 0x3ea0 },
    pll_div { pll_in: 1411200, pll_out: 22579200, reg_val: 0x3ea0 },
    pll_div { pll_in: 1536000, pll_out: 24576000, reg_val: 0x3ea0 },
    pll_div { pll_in: 2048000, pll_out: 16384000, reg_val: 0x1ea0 },
    pll_div { pll_in: 2822400, pll_out: 22579200, reg_val: 0x1ea0 },
    pll_div { pll_in: 2822400, pll_out: 45158400, reg_val: 0x5ec0 },
    pll_div { pll_in: 5644800, pll_out: 45158400, reg_val: 0x46f0 },
    pll_div { pll_in: 3072000, pll_out: 24576000, reg_val: 0x1ea0 },
    pll_div { pll_in: 3072000, pll_out: 49152000, reg_val: 0x5ec0 },
    pll_div { pll_in: 6144000, pll_out: 49152000, reg_val: 0x46f0 },
    pll_div { pll_in: 705600, pll_out: 11289600, reg_val: 0x3ea0 },
    pll_div { pll_in: 705600, pll_out: 8467200, reg_val: 0x3ab0 },
    pll_div { pll_in: 24576000, pll_out: 24576000, reg_val: 0x02a0 },
    pll_div { pll_in: 1411200, pll_out: 11289600, reg_val: 0x1690 },
    pll_div { pll_in: 2822400, pll_out: 11289600, reg_val: 0x0a90 },
    pll_div { pll_in: 1536000, pll_out: 12288000, reg_val: 0x1690 },
    pll_div { pll_in: 3072000, pll_out: 12288000, reg_val: 0x0a90 },
];

static coeff_div: &[coeff_clk_div] = &[
    /* sysclk is 256fs */
    coeff_clk_div { mclk: 2048000, bclk: 8000 * 32, rate: 8000, reg_val: 0x1000 },
    coeff_clk_div { mclk: 2048000, bclk: 8000 * 64, rate: 8000, reg_val: 0x0000 },
    coeff_clk_div { mclk: 2822400, bclk: 11025 * 32, rate: 11025, reg_val: 0x1000 },
    coeff_clk_div { mclk: 2822400, bclk: 11025 * 64, rate: 11025, reg_val: 0x0000 },
    coeff_clk_div { mclk: 4096000, bclk: 16000 * 32, rate: 16000, reg_val: 0x1000 },
    coeff_clk_div { mclk: 4096000, bclk: 16000 * 64, rate: 16000, reg_val: 0x0000 },
    coeff_clk_div { mclk: 5644800, bclk: 22050 * 32, rate: 22050, reg_val: 0x1000 },
    coeff_clk_div { mclk: 5644800, bclk: 22050 * 64, rate: 22050, reg_val: 0x0000 },
    coeff_clk_div { mclk: 8192000, bclk: 32000 * 32, rate: 32000, reg_val: 0x1000 },
    coeff_clk_div { mclk: 8192000, bclk: 32000 * 64, rate: 32000, reg_val: 0x0000 },
    coeff_clk_div { mclk: 11289600, bclk: 44100 * 32, rate: 44100, reg_val: 0x1000 },
    coeff_clk_div { mclk: 11289600, bclk: 44100 * 64, rate: 44100, reg_val: 0x0000 },
    coeff_clk_div { mclk: 12288000, bclk: 48000 * 32, rate: 48000, reg_val: 0x1000 },
    coeff_clk_div { mclk: 12288000, bclk: 48000 * 64, rate: 48000, reg_val: 0x0000 },
    coeff_clk_div { mclk: 22579200, bclk: 88200 * 32, rate: 88200, reg_val: 0x1000 },
    coeff_clk_div { mclk: 22579200, bclk: 88200 * 64, rate: 88200, reg_val: 0x0000 },
    coeff_clk_div { mclk: 24576000, bclk: 96000 * 32, rate: 96000, reg_val: 0x1000 },
    coeff_clk_div { mclk: 24576000, bclk: 96000 * 64, rate: 96000, reg_val: 0x0000 },
    /* sysclk is 512fs */
    coeff_clk_div { mclk: 4096000, bclk: 8000 * 32, rate: 8000, reg_val: 0x3000 },
    coeff_clk_div { mclk: 4096000, bclk: 8000 * 64, rate: 8000, reg_val: 0x2000 },
    coeff_clk_div { mclk: 5644800, bclk: 11025 * 32, rate: 11025, reg_val: 0x3000 },
    coeff_clk_div { mclk: 5644800, bclk: 11025 * 64, rate: 11025, reg_val: 0x2000 },
    coeff_clk_div { mclk: 8192000, bclk: 16000 * 32, rate: 16000, reg_val: 0x3000 },
    coeff_clk_div { mclk: 8192000, bclk: 16000 * 64, rate: 16000, reg_val: 0x2000 },
    coeff_clk_div { mclk: 11289600, bclk: 22050 * 32, rate: 22050, reg_val: 0x3000 },
    coeff_clk_div { mclk: 11289600, bclk: 22050 * 64, rate: 22050, reg_val: 0x2000 },
    coeff_clk_div { mclk: 16384000, bclk: 32000 * 32, rate: 32000, reg_val: 0x3000 },
    coeff_clk_div { mclk: 16384000, bclk: 32000 * 64, rate: 32000, reg_val: 0x2000 },
    coeff_clk_div { mclk: 22579200, bclk: 44100 * 32, rate: 44100, reg_val: 0x3000 },
    coeff_clk_div { mclk: 22579200, bclk: 44100 * 64, rate: 44100, reg_val: 0x2000 },
    coeff_clk_div { mclk: 24576000, bclk: 48000 * 32, rate: 48000, reg_val: 0x3000 },
    coeff_clk_div { mclk: 24576000, bclk: 48000 * 64, rate: 48000, reg_val: 0x2000 },
    coeff_clk_div { mclk: 45158400, bclk: 88200 * 32, rate: 88200, reg_val: 0x3000 },
    coeff_clk_div { mclk: 45158400, bclk: 88200 * 64, rate: 88200, reg_val: 0x2000 },
    coeff_clk_div { mclk: 49152000, bclk: 96000 * 32, rate: 96000, reg_val: 0x3000 },
    coeff_clk_div { mclk: 49152000, bclk: 96000 * 64, rate: 96000, reg_val: 0x2000 },
    /* sysclk is 24.576Mhz or 22.5792Mhz */
    coeff_clk_div { mclk: 24576000, bclk: 8000 * 32, rate: 8000, reg_val: 0x7080 },
    coeff_clk_div { mclk: 24576000, bclk: 8000 * 64, rate: 8000, reg_val: 0x6080 },
    coeff_clk_div { mclk: 24576000, bclk: 16000 * 32, rate: 16000, reg_val: 0x5080 },
    coeff_clk_div { mclk: 24576000, bclk: 16000 * 64, rate: 16000, reg_val: 0x4080 },
    coeff_clk_div { mclk: 24576000, bclk: 24000 * 32, rate: 24000, reg_val: 0x5000 },
    coeff_clk_div { mclk: 24576000, bclk: 24000 * 64, rate: 24000, reg_val: 0x4000 },
    coeff_clk_div { mclk: 24576000, bclk: 32000 * 32, rate: 32000, reg_val: 0x3080 },
    coeff_clk_div { mclk: 24576000, bclk: 32000 * 64, rate: 32000, reg_val: 0x2080 },
    coeff_clk_div { mclk: 22579200, bclk: 11025 * 32, rate: 11025, reg_val: 0x7000 },
    coeff_clk_div { mclk: 22579200, bclk: 11025 * 64, rate: 11025, reg_val: 0x6000 },
    coeff_clk_div { mclk: 22579200, bclk: 22050 * 32, rate: 22050, reg_val: 0x5000 },
    coeff_clk_div { mclk: 22579200, bclk: 22050 * 64, rate: 22050, reg_val: 0x4000 },
];


unsafe fn rt5631_dmic_get(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
    let component = snd_kcontrol_chip(kcontrol) as *mut snd_soc_component;
    let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv;
    (*ucontrol).value.integer.value[0] = (*rt5631).dmic_used_flag as _;
    0
}

unsafe fn rt5631_dmic_put(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int {
    let component = snd_kcontrol_chip(kcontrol) as *mut snd_soc_component;
    let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv;
    (*rt5631).dmic_used_flag = (*ucontrol).value.integer.value[0] as c_int;
    0
}

unsafe fn check_sysclk1_source(source: *mut snd_soc_dapm_widget, sink: *mut snd_soc_dapm_widget) -> c_int {
    let component = snd_soc_dapm_to_component((*source).dapm);
    let reg = snd_soc_component_read(component, RT5631_GLOBAL_CLK_CTRL);
    (reg & RT5631_SYSCLK_SOUR_SEL_PLL) as c_int
}

unsafe fn check_dmic_used(source: *mut snd_soc_dapm_widget, sink: *mut snd_soc_dapm_widget) -> c_int {
    let component = snd_soc_dapm_to_component((*source).dapm);
    let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv;
    (*rt5631).dmic_used_flag
}

unsafe fn check_dacl_to_outmixl(source: *mut snd_soc_dapm_widget, sink: *mut snd_soc_dapm_widget) -> c_int { let component = snd_soc_dapm_to_component((*source).dapm); (!(snd_soc_component_read(component, RT5631_OUTMIXER_L_CTRL) & RT5631_M_DAC_L_TO_OUTMIXER_L != 0)) as c_int }
unsafe fn check_dacr_to_outmixr(source: *mut snd_soc_dapm_widget, sink: *mut snd_soc_dapm_widget) -> c_int { let component = snd_soc_dapm_to_component((*source).dapm); (!(snd_soc_component_read(component, RT5631_OUTMIXER_R_CTRL) & RT5631_M_DAC_R_TO_OUTMIXER_R != 0)) as c_int }
unsafe fn check_dacl_to_spkmixl(source: *mut snd_soc_dapm_widget, sink: *mut snd_soc_dapm_widget) -> c_int { let component = snd_soc_dapm_to_component((*source).dapm); (!(snd_soc_component_read(component, RT5631_SPK_MIXER_CTRL) & RT5631_M_DAC_L_TO_SPKMIXER_L != 0)) as c_int }
unsafe fn check_dacr_to_spkmixr(source: *mut snd_soc_dapm_widget, sink: *mut snd_soc_dapm_widget) -> c_int { let component = snd_soc_dapm_to_component((*source).dapm); (!(snd_soc_component_read(component, RT5631_SPK_MIXER_CTRL) & RT5631_M_DAC_R_TO_SPKMIXER_R != 0)) as c_int }
unsafe fn check_adcl_select(source: *mut snd_soc_dapm_widget, sink: *mut snd_soc_dapm_widget) -> c_int { let component = snd_soc_dapm_to_component((*source).dapm); (!(snd_soc_component_read(component, RT5631_ADC_REC_MIXER) & RT5631_M_MIC1_TO_RECMIXER_L != 0)) as c_int }
unsafe fn check_adcr_select(source: *mut snd_soc_dapm_widget, sink: *mut snd_soc_dapm_widget) -> c_int { let component = snd_soc_dapm_to_component((*source).dapm); (!(snd_soc_component_read(component, RT5631_ADC_REC_MIXER) & RT5631_M_MIC2_TO_RECMIXER_R != 0)) as c_int }

unsafe fn onebit_depop_power_stage(component: *mut snd_soc_component, enable: c_int) {
    snd_soc_component_update_bits(component, RT5631_DEPOP_FUN_CTRL_2, RT5631_EN_ONE_BIT_DEPOP, 0);
    let soft_vol = snd_soc_component_read(component, RT5631_SOFT_VOL_CTRL);
    snd_soc_component_write(component, RT5631_SOFT_VOL_CTRL, 0);
    let hp_zc = snd_soc_component_read(component, RT5631_INT_ST_IRQ_CTRL_2);
    snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, hp_zc & 0xf7ff);
    if enable != 0 {
        rt5631_write_index(component, RT5631_TEST_MODE_CTRL, 0x84c0);
        rt5631_write_index(component, RT5631_SPK_INTL_CTRL, 0x309f);
        rt5631_write_index(component, RT5631_CP_INTL_REG2, 0x6530);
        snd_soc_component_write(component, RT5631_DEPOP_FUN_CTRL_2, RT5631_EN_CAP_FREE_DEPOP);
    } else { snd_soc_component_write(component, RT5631_DEPOP_FUN_CTRL_2, 0); msleep(100); }
    snd_soc_component_write(component, RT5631_SOFT_VOL_CTRL, soft_vol);
    snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, hp_zc);
}

unsafe fn onebit_depop_mute_stage(component: *mut snd_soc_component, enable: c_int) {
    snd_soc_component_update_bits(component, RT5631_DEPOP_FUN_CTRL_2, RT5631_EN_ONE_BIT_DEPOP, 0);
    let soft_vol = snd_soc_component_read(component, RT5631_SOFT_VOL_CTRL); snd_soc_component_write(component, RT5631_SOFT_VOL_CTRL, 0);
    let hp_zc = snd_soc_component_read(component, RT5631_INT_ST_IRQ_CTRL_2); snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, hp_zc & 0xf7ff);
    if enable != 0 { schedule_timeout_uninterruptible(msecs_to_jiffies(10)); rt5631_write_index(component, RT5631_SPK_INTL_CTRL, 0x307f); snd_soc_component_update_bits(component, RT5631_HP_OUT_VOL, RT5631_L_MUTE | RT5631_R_MUTE, 0); msleep(300); }
    else { snd_soc_component_update_bits(component, RT5631_HP_OUT_VOL, RT5631_L_MUTE | RT5631_R_MUTE, RT5631_L_MUTE | RT5631_R_MUTE); msleep(100); }
    snd_soc_component_write(component, RT5631_SOFT_VOL_CTRL, soft_vol); snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, hp_zc);
}

unsafe fn depop_seq_power_stage(component: *mut snd_soc_component, enable: c_int) {
    snd_soc_component_update_bits(component, RT5631_DEPOP_FUN_CTRL_2, RT5631_EN_ONE_BIT_DEPOP, RT5631_EN_ONE_BIT_DEPOP);
    let soft_vol = snd_soc_component_read(component, RT5631_SOFT_VOL_CTRL); snd_soc_component_write(component, RT5631_SOFT_VOL_CTRL, 0);
    let hp_zc = snd_soc_component_read(component, RT5631_INT_ST_IRQ_CTRL_2); snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, hp_zc & 0xf7ff);
    if enable != 0 {
        rt5631_write_index(component, RT5631_SPK_INTL_CTRL, 0x303e);
        snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD3, RT5631_PWR_CHARGE_PUMP | RT5631_PWR_HP_L_AMP | RT5631_PWR_HP_R_AMP, RT5631_PWR_CHARGE_PUMP | RT5631_PWR_HP_L_AMP | RT5631_PWR_HP_R_AMP);
        snd_soc_component_write(component, RT5631_DEPOP_FUN_CTRL_1, RT5631_POW_ON_SOFT_GEN | RT5631_EN_DEPOP2_FOR_HP); msleep(100);
        snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD3, RT5631_PWR_HP_DEPOP_DIS, RT5631_PWR_HP_DEPOP_DIS);
    } else {
        rt5631_write_index(component, RT5631_SPK_INTL_CTRL, 0x303F);
        snd_soc_component_write(component, RT5631_DEPOP_FUN_CTRL_1, RT5631_POW_ON_SOFT_GEN | RT5631_EN_MUTE_UNMUTE_DEPOP | RT5631_PD_HPAMP_L_ST_UP | RT5631_PD_HPAMP_R_ST_UP); msleep(75);
        snd_soc_component_write(component, RT5631_DEPOP_FUN_CTRL_1, RT5631_POW_ON_SOFT_GEN | RT5631_PD_HPAMP_L_ST_UP | RT5631_PD_HPAMP_R_ST_UP);
        snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD3, RT5631_PWR_HP_DEPOP_DIS, 0);
        snd_soc_component_write(component, RT5631_DEPOP_FUN_CTRL_1, RT5631_POW_ON_SOFT_GEN | RT5631_EN_DEPOP2_FOR_HP | RT5631_PD_HPAMP_L_ST_UP | RT5631_PD_HPAMP_R_ST_UP); msleep(80);
        snd_soc_component_write(component, RT5631_DEPOP_FUN_CTRL_1, RT5631_POW_ON_SOFT_GEN);
        snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD3, RT5631_PWR_CHARGE_PUMP | RT5631_PWR_HP_L_AMP | RT5631_PWR_HP_R_AMP, 0);
    }
    snd_soc_component_write(component, RT5631_SOFT_VOL_CTRL, soft_vol); snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, hp_zc);
}

unsafe fn depop_seq_mute_stage(component: *mut snd_soc_component, enable: c_int) {
    snd_soc_component_update_bits(component, RT5631_DEPOP_FUN_CTRL_2, RT5631_EN_ONE_BIT_DEPOP, RT5631_EN_ONE_BIT_DEPOP);
    let soft_vol = snd_soc_component_read(component, RT5631_SOFT_VOL_CTRL); snd_soc_component_write(component, RT5631_SOFT_VOL_CTRL, 0);
    let hp_zc = snd_soc_component_read(component, RT5631_INT_ST_IRQ_CTRL_2); snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, hp_zc & 0xf7ff);
    rt5631_write_index(component, RT5631_SPK_INTL_CTRL, 0x302f);
    snd_soc_component_write(component, RT5631_DEPOP_FUN_CTRL_1, RT5631_POW_ON_SOFT_GEN | RT5631_EN_MUTE_UNMUTE_DEPOP | RT5631_EN_HP_R_M_UN_MUTE_DEPOP | RT5631_EN_HP_L_M_UN_MUTE_DEPOP);
    if enable != 0 { schedule_timeout_uninterruptible(msecs_to_jiffies(10)); snd_soc_component_update_bits(component, RT5631_HP_OUT_VOL, RT5631_L_MUTE | RT5631_R_MUTE, 0); msleep(160); }
    else { snd_soc_component_update_bits(component, RT5631_HP_OUT_VOL, RT5631_L_MUTE | RT5631_R_MUTE, RT5631_L_MUTE | RT5631_R_MUTE); msleep(150); }
    snd_soc_component_write(component, RT5631_SOFT_VOL_CTRL, soft_vol); snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, hp_zc);
}

unsafe fn hp_event(w: *mut snd_soc_dapm_widget, kcontrol: *mut snd_kcontrol, event: c_int) -> c_int {
    let component = snd_soc_dapm_to_component((*w).dapm); let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv;
    match event { SND_SOC_DAPM_PRE_PMD => { if (*rt5631).codec_version != 0 { onebit_depop_mute_stage(component,0); onebit_depop_power_stage(component,0); } else { depop_seq_mute_stage(component,0); depop_seq_power_stage(component,0); } }, SND_SOC_DAPM_POST_PMU => { if (*rt5631).codec_version != 0 { onebit_depop_power_stage(component,1); onebit_depop_mute_stage(component,1); } else { depop_seq_power_stage(component,1); depop_seq_mute_stage(component,1); } }, _ => {} }
    0
}

unsafe fn set_dmic_params(w: *mut snd_soc_dapm_widget, kcontrol: *mut snd_kcontrol, event: c_int) -> c_int {
    let component = snd_soc_dapm_to_component((*w).dapm); let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv;
    match (*rt5631).rx_rate { 44100 | 48000 => snd_soc_component_update_bits(component, RT5631_DIG_MIC_CTRL, RT5631_DMIC_CLK_CTRL_MASK, RT5631_DMIC_CLK_CTRL_TO_32FS), 32000 | 22050 => snd_soc_component_update_bits(component, RT5631_DIG_MIC_CTRL, RT5631_DMIC_CLK_CTRL_MASK, RT5631_DMIC_CLK_CTRL_TO_64FS), 16000 | 11025 | 8000 => snd_soc_component_update_bits(component, RT5631_DIG_MIC_CTRL, RT5631_DMIC_CLK_CTRL_MASK, RT5631_DMIC_CLK_CTRL_TO_128FS), _ => return -EINVAL }
    0
}

unsafe fn get_coeff(mclk: c_int, rate: c_int, timesofbclk: c_int) -> c_int {
    let mut i = 0usize;
    while i < coeff_div.len() {
        if coeff_div[i].mclk == mclk as u32 && coeff_div[i].rate == rate as u32 && (coeff_div[i].bclk / coeff_div[i].rate) == timesofbclk as u32 { return i as c_int; }
        i += 1;
    }
    -EINVAL
}

unsafe fn rt5631_hifi_pcm_params(substream: *mut snd_pcm_substream, params: *mut snd_pcm_hw_params, dai: *mut snd_soc_dai) -> c_int {
    let component = (*dai).component; let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv; let timesofbclk: c_int = 32; let mut iface: c_uint = 0;
    dev_dbg((*component).dev, c"enter %s\n".as_ptr(), c"rt5631_hifi_pcm_params".as_ptr());
    (*rt5631).bclk_rate = snd_soc_params_to_bclk(params); if (*rt5631).bclk_rate < 0 { dev_err((*component).dev, c"Fail to get BCLK rate\n".as_ptr()); return (*rt5631).bclk_rate; }
    (*rt5631).rx_rate = params_rate(params);
    let coeff = if (*rt5631).master != 0 { get_coeff((*rt5631).sysclk, (*rt5631).rx_rate, (*rt5631).bclk_rate / (*rt5631).rx_rate) } else { get_coeff((*rt5631).sysclk, (*rt5631).rx_rate, timesofbclk) };
    if coeff < 0 { dev_err((*component).dev, c"Fail to get coeff\n".as_ptr()); return coeff; }
    match params_width(params) { 16 => {}, 20 => iface |= RT5631_SDP_I2S_DL_20, 24 => iface |= RT5631_SDP_I2S_DL_24, 8 => iface |= RT5631_SDP_I2S_DL_8, _ => return -EINVAL }
    snd_soc_component_update_bits(component, RT5631_SDP_CTRL, RT5631_SDP_I2S_DL_MASK, iface);
    snd_soc_component_write(component, RT5631_STEREO_AD_DA_CLK_CTRL, coeff_div[coeff as usize].reg_val as c_uint);
    0
}

unsafe fn rt5631_hifi_codec_set_dai_fmt(codec_dai: *mut snd_soc_dai, fmt: c_uint) -> c_int {
    let component = (*codec_dai).component; let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv; let mut iface: c_uint = 0;
    match fmt & SND_SOC_DAIFMT_MASTER_MASK { SND_SOC_DAIFMT_CBP_CFP => (*rt5631).master = 1, SND_SOC_DAIFMT_CBC_CFC => { iface |= RT5631_SDP_MODE_SEL_SLAVE; (*rt5631).master = 0; }, _ => return -EINVAL }
    match fmt & SND_SOC_DAIFMT_FORMAT_MASK { SND_SOC_DAIFMT_I2S => {}, SND_SOC_DAIFMT_LEFT_J => iface |= RT5631_SDP_I2S_DF_LEFT, SND_SOC_DAIFMT_DSP_A => iface |= RT5631_SDP_I2S_DF_PCM_A, SND_SOC_DAIFMT_DSP_B => iface |= RT5631_SDP_I2S_DF_PCM_B, _ => return -EINVAL }
    match fmt & SND_SOC_DAIFMT_INV_MASK { SND_SOC_DAIFMT_NB_NF => {}, SND_SOC_DAIFMT_IB_NF => iface |= RT5631_SDP_I2S_BCLK_POL_CTRL, _ => return -EINVAL }
    snd_soc_component_write(component, RT5631_SDP_CTRL, iface); 0
}

unsafe fn rt5631_hifi_codec_set_dai_sysclk(codec_dai: *mut snd_soc_dai, clk_id: c_int, freq: c_uint, dir: c_int) -> c_int {
    let component = (*codec_dai).component; let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv;
    if freq >= (256 * 8000) && freq <= (512 * 96000) { (*rt5631).sysclk = freq as c_int; return 0; }
    -EINVAL
}

unsafe fn rt5631_codec_set_dai_pll(codec_dai: *mut snd_soc_dai, pll_id: c_int, source: c_int, freq_in: c_uint, freq_out: c_uint) -> c_int {
    let component = (*codec_dai).component; let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv; let mut ret = -EINVAL;
    if freq_in == 0 || freq_out == 0 { snd_soc_component_update_bits(component, RT5631_GLOBAL_CLK_CTRL, RT5631_SYSCLK_SOUR_SEL_MASK, RT5631_SYSCLK_SOUR_SEL_MCLK); return 0; }
    let table = if (*rt5631).master != 0 { codec_master_pll_div } else { codec_slave_pll_div };
    let mut i = 0usize; while i < table.len() { if freq_in == table[i].pll_in && freq_out == table[i].pll_out { snd_soc_component_write(component, RT5631_PLL_CTRL, table[i].reg_val as c_uint); schedule_timeout_uninterruptible(msecs_to_jiffies(20)); snd_soc_component_update_bits(component, RT5631_GLOBAL_CLK_CTRL, RT5631_SYSCLK_SOUR_SEL_MASK | RT5631_PLLCLK_SOUR_SEL_MASK, RT5631_SYSCLK_SOUR_SEL_PLL | if (*rt5631).master != 0 { RT5631_PLLCLK_SOUR_SEL_MCLK } else { RT5631_PLLCLK_SOUR_SEL_BCLK }); ret = 0; break; } i += 1; }
    ret
}

unsafe fn rt5631_set_bias_level(component: *mut snd_soc_component, level: snd_soc_bias_level) -> c_int {
    let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv; let dapm = snd_soc_component_to_dapm(component);
    match level { SND_SOC_BIAS_ON | SND_SOC_BIAS_PREPARE => { snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD2, RT5631_PWR_MICBIAS1_VOL | RT5631_PWR_MICBIAS2_VOL, RT5631_PWR_MICBIAS1_VOL | RT5631_PWR_MICBIAS2_VOL); }, SND_SOC_BIAS_STANDBY => { if snd_soc_dapm_get_bias_level(dapm) == SND_SOC_BIAS_OFF { snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD3, RT5631_PWR_VREF | RT5631_PWR_MAIN_BIAS, RT5631_PWR_VREF | RT5631_PWR_MAIN_BIAS); msleep(80); snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD3, RT5631_PWR_FAST_VREF_CTRL, RT5631_PWR_FAST_VREF_CTRL); regcache_cache_only((*rt5631).regmap, false); regcache_sync((*rt5631).regmap); } }, SND_SOC_BIAS_OFF => { snd_soc_component_write(component, RT5631_PWR_MANAG_ADD1, 0x0000); snd_soc_component_write(component, RT5631_PWR_MANAG_ADD2, 0x0000); snd_soc_component_write(component, RT5631_PWR_MANAG_ADD3, 0x0000); snd_soc_component_write(component, RT5631_PWR_MANAG_ADD4, 0x0000); }, _ => {} }
    0
}

unsafe fn rt5631_probe(component: *mut snd_soc_component) -> c_int {
    let rt5631 = snd_soc_component_get_drvdata(component) as *mut rt5631_priv; let dapm = snd_soc_component_to_dapm(component);
    let val = rt5631_read_index(component, RT5631_ADDA_MIXER_INTL_REG3); (*rt5631).codec_version = if (val & 0x0002) != 0 { 1 } else { 0 };
    rt5631_reset(component); snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD3, RT5631_PWR_VREF | RT5631_PWR_MAIN_BIAS, RT5631_PWR_VREF | RT5631_PWR_MAIN_BIAS); msleep(80); snd_soc_component_update_bits(component, RT5631_PWR_MANAG_ADD3, RT5631_PWR_FAST_VREF_CTRL, RT5631_PWR_FAST_VREF_CTRL);
    snd_soc_component_write(component, RT5631_INT_ST_IRQ_CTRL_2, 0x0f18);
    if (*rt5631).codec_version != 0 { snd_soc_component_update_bits(component, RT5631_INT_ST_IRQ_CTRL_2, 0x2000, 0x2000); } else { snd_soc_component_update_bits(component, RT5631_INT_ST_IRQ_CTRL_2, 0x2000, 0); }
    if (*rt5631).dmic_used_flag != 0 { snd_soc_component_update_bits(component, RT5631_GPIO_CTRL, RT5631_GPIO_PIN_FUN_SEL_MASK | RT5631_GPIO_DMIC_FUN_SEL_MASK, RT5631_GPIO_PIN_FUN_SEL_GPIO_DIMC | RT5631_GPIO_DMIC_FUN_SEL_DIMC); snd_soc_component_update_bits(component, RT5631_DIG_MIC_CTRL, RT5631_DMIC_L_CH_LATCH_MASK | RT5631_DMIC_R_CH_LATCH_MASK, RT5631_DMIC_L_CH_LATCH_FALLING | RT5631_DMIC_R_CH_LATCH_RISING); }
    snd_soc_dapm_init_bias_level(dapm, SND_SOC_BIAS_STANDBY); 0
}

const RT5631_STEREO_RATES: c_uint = SNDRV_PCM_RATE_8000_96000;
const RT5631_FORMAT: c_uint = SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S20_3LE | SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FMTBIT_S8;

/* Remaining C aggregate driver registration declarations translated as external macro/struct initializers in the surrounding kernel Rust binding layer:
 * rt5631_ops, rt5631_dai, soc_component_dev_rt5631, rt5631_i2c_id,
 * optional CONFIG_OF rt5631_i2c_dt_ids, rt5631_regmap_config,
 * rt5631_i2c_probe, rt5631_i2c_driver, module_i2c_driver and MODULE_* metadata.
 */

unsafe fn rt5631_i2c_probe(i2c: *mut i2c_client) -> c_int {
    let rt5631 = devm_kzalloc(&mut (*i2c).dev, core::mem::size_of::<rt5631_priv>(), GFP_KERNEL) as *mut rt5631_priv;
    if rt5631.is_null() { return -ENOMEM; }
    i2c_set_clientdata(i2c, rt5631 as *mut c_void);
    (*rt5631).regmap = devm_regmap_init_i2c(i2c, &rt5631_regmap_config);
    if IS_ERR((*rt5631).regmap as *const c_void) { return PTR_ERR((*rt5631).regmap as *const c_void) as c_int; }
    devm_snd_soc_register_component(&mut (*i2c).dev, &soc_component_dev_rt5631, rt5631_dai.as_mut_ptr(), rt5631_dai.len() as c_uint)
}
