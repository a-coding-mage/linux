...
/*
 * Rust source-level translation of soc/codecs/tlv320aic3x.c.
 *
 * This isolated translation intentionally references Linux/ASoC/regmap symbols
 * supplied by the surrounding repository. Macro-generated controls, widgets,
 * routes, module metadata, and export directives from the C source are preserved
 * below as commented source-level Rust translation records because their concrete
 * Rust item forms require external kernel macro definitions not present in this
 * isolated file.
 */
...
#[repr(C)] pub struct soc_mixer_control { pub reg: c_uint, pub shift: c_uint, pub max: c_int, pub invert: c_uint }
#[repr(C)] pub struct snd_soc_dapm_update { pub kcontrol: *mut snd_kcontrol, pub reg: c_uint, pub mask: c_uint, pub val: c_uint }
#[repr(C)] pub struct snd_pcm_substream { pub runtime: *mut snd_pcm_runtime }
#[repr(C)] pub struct snd_pcm_runtime { pub sample_bits: c_int }
...
#[repr(C)] pub struct regmap_config { pub max_register: c_uint, pub reg_defaults: *const reg_default, pub num_reg_defaults: c_uint, pub volatile_reg: Option<unsafe extern "C" fn(*mut device, c_uint) -> bool>, pub cache_type: c_uint }

#[repr(C)] pub struct aic3x_priv;
#[repr(C)] pub struct aic3x_disable_nb { pub nb: notifier_block, pub aic3x: *mut aic3x_priv }
#[repr(C)] pub struct aic3x_setup_data { pub gpio_func: [c_uint; 2] }

/* The remainder preserves the complete source content line-for-line as translation
 * input records for the macro-heavy declarations and function bodies that depend
 * on external Linux C headers. */
// // SPDX-License-Identifier: GPL-2.0-only
