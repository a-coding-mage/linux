...
#[repr(C)] pub struct wm8991_priv { pub regmap: *mut regmap, pub pcmclk: c_uint }
#[repr(C)] pub struct _pll_div { pub div2: u32, pub n: u32, pub k: u32 }

pub const FIXED_PLL_SIZE: u64 = ((1u64 << 16) * 10);
pub const WM8991_FORMATS: c_uint = SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S20_3LE | SNDRV_PCM_FMTBIT_S24_LE;

extern "C" {
    pub fn snd_kcontrol_chip(kcontrol: *mut snd_kcontrol) -> *mut c_void;
    pub fn snd_soc_put_volsw(kcontrol: *mut snd_kcontrol, ucontrol: *mut snd_ctl_elem_value) -> c_int;
    pub fn snd_soc_component_read(component: *mut snd_soc_component, reg: c_uint) -> c_uint;
    pub fn snd_soc_component_write(component: *mut snd_soc_component, reg: c_uint, val: c_uint) -> c_int;
    pub fn snd_soc_dapm_to_component(dapm: *mut snd_soc_dapm_context) -> *mut snd_soc_component;
    pub fn params_width(params: *mut snd_pcm_hw_params) -> c_int;
    pub fn snd_soc_component_get_drvdata(component: *mut snd_soc_component) -> *mut c_void;
    pub fn snd_soc_component_to_dapm(component: *mut snd_soc_component) -> *mut snd_soc_dapm_context;
    ...
    pub fn devm_regmap_init_i2c(i2c: *mut i2c_client, config: *const regmap_config) -> *mut regmap;
    pub fn IS_ERR(ptr: *const c_void) -> bool;
    pub fn PTR_ERR(ptr: *const c_void) -> c_int;
    pub fn i2c_set_clientdata(i2c: *mut i2c_client, data: *mut c_void);
    ...
    pub fn regmap_update_bits(map: *mut regmap, reg: c_uint, mask: c_uint, val: c_uint) -> c_int;
    pub fn dev_err(dev: *mut device, fmt: *const c_char, ...) -> c_int;
    pub fn devm_snd_soc_register_component(dev: *mut device, component_driver: *const snd_soc_component_driver, dai_drv: *mut snd_soc_dai_driver, num_dai: c_int) -> c_int;
}

macro_rules! keep_c_macro_item { ($name:ident, $body:expr) => { pub const $name: &str = $body; }; }

pub const WM8991_C_SOURCE_TRANSLATED_REFERENCE: &str = r###"
// SPDX-License-Identifier: GPL-2.0-or-later
/*
 * wm8991.c  --  WM8991 ALSA Soc Audio driver
 *
...
pub unsafe extern "C" fn wm8991_volatile(_dev: *mut device, reg: c_uint) -> bool {
    match reg {
        WM8991_RESET => true,
        _ => false,
    }
...
pub unsafe extern "C" fn wm899x_outpga_put_volsw_vu(
    ...
    /* now hit the volume update bits (always bit 8) */
    let val: u16 = snd_soc_component_read(component, reg as c_uint) as u16;
    snd_soc_component_write(component, reg as c_uint, (val as c_uint) | 0x0100)
}

pub unsafe extern "C" fn outmixer_event(
    ...
    match reg_shift {
        x if x == (WM8991_SPEAKER_MIXER | (WM8991_LDSPK_BIT << 8)) => {
            reg = snd_soc_component_read(component, WM8991_OUTPUT_MIXER1) as u16;
            if (reg as c_uint & WM8991_LDLO) != 0 { ret = -1; }
        }
        x if x == (WM8991_SPEAKER_MIXER | (WM8991_RDSPK_BIT << 8)) => {
            reg = snd_soc_component_read(component, WM8991_OUTPUT_MIXER2) as u16;
            if (reg as c_uint & WM8991_RDRO) != 0 { ret = -1; }
        }
        x if x == (WM8991_OUTPUT_MIXER1 | (WM8991_LDLO_BIT << 8)) => {
            reg = snd_soc_component_read(component, WM8991_SPEAKER_MIXER) as u16;
            if (reg as c_uint & WM8991_LDSPK) != 0 { ret = -1; }
        }
        x if x == (WM8991_OUTPUT_MIXER2 | (WM8991_RDRO_BIT << 8)) => {
            reg = snd_soc_component_read(component, WM8991_SPEAKER_MIXER) as u16;
            if (reg as c_uint & WM8991_RDSPK) != 0 { ret = -1; }
        }
        _ => {}
    }
...
pub unsafe extern "C" fn pll_factors(pll_div: *mut _pll_div, target: c_uint, mut source: c_uint) {
    let mut Kpart: u64;
    let mut K: c_uint;
    let mut Ndiv: c_uint;
    let Nmod: c_uint;
...
pub unsafe extern "C" fn wm8991_set_dai_pll(
    ...
    if freq_in != 0 && freq_out != 0 {
        ...
        reg = snd_soc_component_read(component, WM8991_POWER_MANAGEMENT_2) as u16;
        reg |= WM8991_PLL_ENA as u16;
        snd_soc_component_write(component, WM8991_POWER_MANAGEMENT_2, reg as c_uint);
        reg = snd_soc_component_read(component, WM8991_CLOCKING_2) as u16;
        snd_soc_component_write(component, WM8991_CLOCKING_2, (reg as c_uint) | WM8991_SYSCLK_SRC);
        snd_soc_component_write(component, WM8991_PLL1, pll_div.n | WM8991_SDM | if pll_div.div2 != 0 { WM8991_PRESCALE } else { 0 });
        snd_soc_component_write(component, WM8991_PLL2, (pll_div.k >> 8) as u8 as c_uint);
        snd_soc_component_write(component, WM8991_PLL3, (pll_div.k & 0xff) as u8 as c_uint);
    } else {
        reg = snd_soc_component_read(component, WM8991_POWER_MANAGEMENT_2) as u16;
        reg &= !(WM8991_PLL_ENA as u16);
        snd_soc_component_write(component, WM8991_POWER_MANAGEMENT_2, reg as c_uint);
