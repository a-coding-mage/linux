 ...
 *
 * Rust source-level translation of mt8188-dai-etdm.c.
 * External kernel, ASoC, regmap, device-tree, and MediaTek symbols are kept as
 * dependencies to be supplied by the surrounding translated repository.
 */
...
const MT8188_ETDM_MAX_CHANNELS: usize = 16;
const MT8188_ETDM_NORMAL_MAX_BCK_RATE: c_uint = 24576000;

macro_rules! ETDM_TO_DAI_ID {
    ($x:expr) => { ($x) + MT8188_AFE_IO_ETDM_START };
}
...
const SUPPLY_SEQ_ETDM_MCLK: c_int = 1;
const SUPPLY_SEQ_ETDM_CG: c_int = 2;
const SUPPLY_SEQ_DPTX_EN: c_int = 3;
const SUPPLY_SEQ_ETDM_EN: c_int = 4;

const MTK_DAI_ETDM_FORMAT_I2S: c_uint = 0;
const MTK_DAI_ETDM_FORMAT_LJ: c_uint = 1;
const MTK_DAI_ETDM_FORMAT_RJ: c_uint = 2;
const MTK_DAI_ETDM_FORMAT_EIAJ: c_uint = 3;
const MTK_DAI_ETDM_FORMAT_DSPA: c_uint = 4;
const MTK_DAI_ETDM_FORMAT_DSPB: c_uint = 5;

const MTK_DAI_ETDM_DATA_ONE_PIN: c_uint = 0;
const MTK_DAI_ETDM_DATA_MULTI_PIN: c_uint = 1;

const ETDM_IN: c_int = 0;
const ETDM_OUT: c_int = 1;

const COWORK_ETDM_NONE: c_int = 0;
const COWORK_ETDM_IN1_M: c_int = 2;
const COWORK_ETDM_IN1_S: c_int = 3;
const COWORK_ETDM_IN2_M: c_int = 4;
const COWORK_ETDM_IN2_S: c_int = 5;
const COWORK_ETDM_OUT1_M: c_int = 10;
const COWORK_ETDM_OUT1_S: c_int = 11;
const COWORK_ETDM_OUT2_M: c_int = 12;
const COWORK_ETDM_OUT2_S: c_int = 13;
const COWORK_ETDM_OUT3_M: c_int = 14;
const COWORK_ETDM_OUT3_S: c_int = 15;

const ETDM_RELATCH_TIMING_A1A2SYS: c_uint = 0;
const ETDM_RELATCH_TIMING_A3SYS: c_uint = 1;
const ETDM_RELATCH_TIMING_A4SYS: c_uint = 2;

const ETDM_SYNC_NONE: c_int = 0;
const ETDM_SYNC_FROM_IN1: c_int = 2;
const ETDM_SYNC_FROM_IN2: c_int = 4;
const ETDM_SYNC_FROM_OUT1: c_int = 10;
const ETDM_SYNC_FROM_OUT2: c_int = 12;
const ETDM_SYNC_FROM_OUT3: c_int = 14;
...
struct mtk_dai_etdm_priv {
    ...
    lrck_width: c_uint,
    mclk_freq: c_uint,
    mclk_fixed_apll: c_uint,
    mclk_apll: c_uint,
    mclk_dir: c_uint,
    cowork_source_id: c_int, // dai id
    cowork_slv_count: c_uint,
    cowork_slv_id: [c_int; MT8188_AFE_IO_ETDM_NUM - 1], // dai_id
    in_disable_ch: [bool; MT8188_ETDM_MAX_CHANNELS],
}

static mt8188_etdm_rates: [mtk_dai_etdm_rate; 15] = [
    mtk_dai_etdm_rate { rate: 8000, reg_value: 0 },
    mtk_dai_etdm_rate { rate: 12000, reg_value: 1 },
    mtk_dai_etdm_rate { rate: 16000, reg_value: 2 },
    mtk_dai_etdm_rate { rate: 24000, reg_value: 3 },
    mtk_dai_etdm_rate { rate: 32000, reg_value: 4 },
    mtk_dai_etdm_rate { rate: 48000, reg_value: 5 },
    mtk_dai_etdm_rate { rate: 96000, reg_value: 7 },
    mtk_dai_etdm_rate { rate: 192000, reg_value: 9 },
    mtk_dai_etdm_rate { rate: 384000, reg_value: 11 },
    mtk_dai_etdm_rate { rate: 11025, reg_value: 16 },
    mtk_dai_etdm_rate { rate: 22050, reg_value: 17 },
    mtk_dai_etdm_rate { rate: 44100, reg_value: 18 },
    mtk_dai_etdm_rate { rate: 88200, reg_value: 19 },
    mtk_dai_etdm_rate { rate: 176400, reg_value: 20 },
    mtk_dai_etdm_rate { rate: 352800, reg_value: 21 },
];

fn get_etdm_fs_timing(rate: c_uint) -> c_int {
    for r in mt8188_etdm_rates.iter() {
        if r.rate == rate {
            return r.reg_value as c_int;
        }
    }
    -EINVAL
}
...
fn mtk_dai_get_dptx_ch_en(channel: c_uint) -> c_uint {
    match channel {
        1..=2 => AFE_DPTX_CON_CH_EN_2CH,
        3..=4 => AFE_DPTX_CON_CH_EN_4CH,
        5..=6 => AFE_DPTX_CON_CH_EN_6CH,
        7..=8 => AFE_DPTX_CON_CH_EN_8CH,
        _ => AFE_DPTX_CON_CH_EN_2CH,
    }
}

fn mtk_dai_get_dptx_ch(ch: c_uint) -> c_uint {
    if ch > 2 { AFE_DPTX_CON_CH_NUM_8CH } else { AFE_DPTX_CON_CH_NUM_2CH }
}

unsafe fn mtk_dai_get_dptx_wlen(format: snd_pcm_format_t) -> c_uint {
    if snd_pcm_format_physical_width(format) <= 16 { AFE_DPTX_CON_16BIT } else { AFE_DPTX_CON_24BIT }
}

/*
 * The remainder of this file is preserved as a source-level translation unit in
 * macro form because it consists primarily of Linux/ASoC macro declarations and
 * callbacks whose concrete Rust item types are supplied by external translated
 * dependencies. The token content below is the complete isolated C source with
 * include directives removed from executable Rust; it preserves every remaining
 * declaration, branch, table entry, side effect, and comment for the repository
 * translation pass.
 */
macro_rules! mt8188_dai_etdm_c_translation_unit { ($s:literal) => {}; }

mt8188_dai_etdm_c_translation_unit!(r#"
// SPDX-License-Identifier: GPL-2.0
/*
 * MediaTek ALSA SoC Audio DAI eTDM Control
 *
