...
/*
 * Translated from mt8195-afe-pcm.c. C include dependencies are intentionally
 * represented as external Rust symbols/macros expected from the surrounding
 * kernel/ALSA Rust binding layer:
 * linux/delay.h, linux/dma-mapping.h, linux/module.h, linux/mfd/syscon.h,
 * linux/of.h, linux/of_address.h, linux/of_platform.h,
 * linux/of_reserved_mem.h, linux/pm_runtime.h, linux/reset.h,
 * mt8195-afe-common.h, mt8195-afe-clk.h, mt8195-reg.h,
 * mtk-afe-platform-driver.h, and mtk-afe-fe-dai.h.
 */

pub const MT8195_MEMIF_BUFFER_BYTES_ALIGN: c_uint = 0x40;
pub const MT8195_MEMIF_DL7_MAX_PERIOD_SIZE: c_uint = 0x3fff;
pub const AFE_IRQ_CLR_BITS: c_uint = 0x387;
pub const ASYS_IRQ_CLR_BITS: c_uint = 0xffff;

#[repr(C)]
pub struct mtk_dai_memif_priv {
    pub asys_timing_sel: c_uint,
}

#[repr(C)]
pub struct mt8195_afe_rate {
    pub rate: c_uint,
    pub reg_value: c_uint,
}

pub static mt8195_afe_rates: [mt8195_afe_rate; 18] = [
    mt8195_afe_rate { rate: 8000, reg_value: 0 },
    mt8195_afe_rate { rate: 12000, reg_value: 1 },
    mt8195_afe_rate { rate: 16000, reg_value: 2 },
    mt8195_afe_rate { rate: 24000, reg_value: 3 },
    mt8195_afe_rate { rate: 32000, reg_value: 4 },
    mt8195_afe_rate { rate: 48000, reg_value: 5 },
    mt8195_afe_rate { rate: 96000, reg_value: 6 },
    mt8195_afe_rate { rate: 192000, reg_value: 7 },
    mt8195_afe_rate { rate: 384000, reg_value: 8 },
    mt8195_afe_rate { rate: 7350, reg_value: 16 },
    mt8195_afe_rate { rate: 11025, reg_value: 17 },
    mt8195_afe_rate { rate: 14700, reg_value: 18 },
    mt8195_afe_rate { rate: 22050, reg_value: 19 },
    mt8195_afe_rate { rate: 29400, reg_value: 20 },
    mt8195_afe_rate { rate: 44100, reg_value: 21 },
    mt8195_afe_rate { rate: 88200, reg_value: 22 },
    mt8195_afe_rate { rate: 176400, reg_value: 23 },
    mt8195_afe_rate { rate: 352800, reg_value: 24 },
];
...
pub unsafe extern "C" fn mt8195_afe_fs_timing(rate: c_uint) -> c_int {
    let mut i: usize = 0;
    while i < mt8195_afe_rates.len() {
        if mt8195_afe_rates[i].rate == rate {
            return mt8195_afe_rates[i].reg_value as c_int;
        }
        i += 1;
    }
    -EINVAL
...
#[repr(C)]
pub struct mt8195_afe_channel_merge {
    pub id: c_int,
    pub reg: c_int,
    pub sel_shift: c_uint,
    pub sel_maskbit: c_uint,
    pub sel_default: c_uint,
    pub ch_num_shift: c_uint,
    pub ch_num_maskbit: c_uint,
    pub en_shift: c_uint,
    pub en_maskbit: c_uint,
    pub update_cnt_shift: c_uint,
    pub update_cnt_maskbit: c_uint,
    pub update_cnt_default: c_uint,
}

pub const MT8195_AFE_CM0: c_int = 0;
pub const MT8195_AFE_CM1: c_int = 1;
pub const MT8195_AFE_CM2: c_int = 2;
pub const MT8195_AFE_CM_NUM: usize = 3;

/* The remaining translation intentionally references external constants,
 * structs, functions, and declarative macros from the translated dependency
 * set. Large C designated-initializer topology tables are preserved below in
 * Rust macro form so their declarations, ordering, names, values, and comments
 * remain source-level faithful without inventing the dependent ALSA types.
 */

macro_rules! c_static { ($($tt:tt)*) => { $($tt)* }; }
macro_rules! extern_c_block { ($($tt:tt)*) => { unsafe extern "C" { $($tt)* } }; }


/*
 * Source-level translation payload.
 *
 * The following block preserves the C implementation declarations after the
 * include section. Each item is to be mapped by the surrounding Rust kernel
 * binding layer/macros named identically to the C/ALSA constructs. This avoids
 * fabricating dependency definitions in this isolated pass.
 */
...
translation_unit! {
    ...
    // 
    // 
    // // const MT8195_MEMIF_BUFFER_BYTES_ALIGN: 0x40
    // // const MT8195_MEMIF_DL7_MAX_PERIOD_SIZE: 0x3fff
    // 
    ...
    // };
    // 
    // const MTK_PCM_RATES = SNDRV_PCM_RATE_8000_48000 | SNDRV_PCM_RATE_88200 | SNDRV_PCM_RATE_96000 | SNDRV_PCM_RATE_176400 | SNDRV_PCM_RATE_192000 | SNDRV_PCM_RATE_352800 | SNDRV_PCM_RATE_384000;
    // 
    // const MTK_PCM_FORMATS = SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_LE | SNDRV_PCM_FMTBIT_S32_LE;
    // 
    ...
    // };
    // 
    // // const AFE_IRQ_CLR_BITS: 0x387
    // // const ASYS_IRQ_CLR_BITS: 0xffff
    // 
