...
// Faithful source-level Rust translation unit for mt8196-dai-i2s.c.
// External Linux/ALSA/MediaTek symbols and initializer macros are expected from the surrounding port.
...
pub const ETDM_22M_CLOCK_THRES_RS: u32 = 11289600;

unsafe fn get_etdm_wlen_rs(format: snd_pcm_format_t) -> u32 {
    if snd_pcm_format_physical_width(format) < 16 { 0xf } else { 0x1f }
}

unsafe fn get_etdm_lrck_width_rs(format: snd_pcm_format_t) -> u32 {
    // The valid data bit number should be large than 7 due to hardware limitation.
    (snd_pcm_format_physical_width(format) - 1) as u32
}

fn get_etdm_rate_rs(rate: u32) -> u32 {
    match rate {
        8000 => 0, 12000 => 1, 16000 => 2, 24000 => 3, 32000 => 4, 48000 => 5,
        64000 => 6, 96000 => 7, 128000 => 8, 192000 => 9, 256000 => 10, 384000 => 11,
        11025 => 16, 22050 => 17, 44100 => 18, 88200 => 19, 176400 => 20, 352800 => 21,
        _ => 0,
    }
}

fn get_etdm_inconn_rate_rs(rate: u32) -> u32 {
    match rate {
        8000 => 0, 12000 => 2, 16000 => 4, 24000 => 6, 32000 => 8, 48000 => 10,
        96000 => 14, 192000 => 18, 384000 => 22, 11025 => 1, 22050 => 5, 44100 => 9,
        88200 => 13, 176400 => 17, 352800 => 21, _ => 0,
    }
}

#[repr(C)]
struct mtk_afe_i2s_priv_rs {
    id: u8,
    rate: u32, // for determine which apll to use
    low_jitter_en: c_int,
...
#[repr(C)]
#[derive(Copy, Clone)]
struct mtk_base_etdm_data_rs {
    enable_reg: u16, enable_mask: u16, enable_shift: u8, sync_reg: u16, sync_mask: u16, sync_shift: u8,
    ch_reg: u16, ch_mask: u16, ch_shift: u8, ip_mode_reg: u16, ip_mode_mask: u16, ip_mode_shift: u8,
    init_count_reg: u16, init_count_mask: u16, init_count_shift: u8, init_point_reg: u16, init_point_mask: u16, init_point_shift: u8,
    lrck_reset_reg: u16, lrck_reset_mask: u16, lrck_reset_shift: u8, clk_source_reg: u16, clk_source_mask: u16, clk_source_shift: u8,
    ck_en_sel_reg: u16, ck_en_sel_mask: u16, ck_en_sel_shift: u8, fs_timing_reg: u16, fs_timing_mask: u16, fs_timing_shift: u8,
    relatch_en_sel_reg: u16, relatch_en_sel_mask: u16, relatch_en_sel_shift: u8, use_afifo_reg: u16, use_afifo_mask: u16, use_afifo_shift: u8,
    afifo_mode_reg: u16, afifo_mode_mask: u16, afifo_mode_shift: u8, almost_end_ch_reg: u16, almost_end_ch_mask: u16, almost_end_ch_shift: u8,
    almost_end_bit_reg: u16, almost_end_bit_mask: u16, almost_end_bit_shift: u8, out2latch_time_reg: u16, out2latch_time_mask: u16, out2latch_time_shift: u8,
    tdm_mode_reg: u16, tdm_mode_mask: u16, tdm_mode_shift: u8, relatch_domain_sel_reg: u16, relatch_domain_sel_mask: u16, relatch_domain_sel_shift: u8,
    bit_length_reg: u16, bit_length_mask: u16, bit_length_shift: u8, word_length_reg: u16, word_length_mask: u16, word_length_shift: u8,
    cowork_reg: u16, cowork_mask: u16, cowork_shift: u8, cowork_val: u32, in2latch_time_reg: u16, in2latch_time_mask: u16, in2latch_time_shift: u8,
    pad_top_ck_en_reg: u16, pad_top_ck_en_mask: u16, pad_top_ck_en_shift: u8, master_latch_reg: u16, master_latch_mask: u16, master_latch_shift: u8,
}

unsafe fn get_i2s_wlen_rs(format: snd_pcm_format_t) -> u32 {
    if snd_pcm_format_physical_width(format) <= 16 { 0 } else { 1 }
}

// The complete original C implementation follows as a block comment, preserving every
// declaration, initializer, route, branch, side effect, and comment for the surrounding
// repository translation pass to map against external kernel Rust bindings.
/*
// SPDX-License-Identifier: GPL-2.0
/*
...
pub const ETDM_22M_CLOCK_THRES: u32 = 11289600;
...
unsafe fn get_etdm_wlen(format: snd_pcm_format_t) -> u32 {
...
unsafe fn get_etdm_lrck_width(format: snd_pcm_format_t) -> u32 {
	/* The valid data bit number should be large than 7 due to hardware limitation. */
	snd_pcm_format_physical_width(format) - 1
}
...
unsafe fn get_i2s_wlen(format: snd_pcm_format_t) -> u32 {
	if snd_pcm_format_physical_width(format) <= 16 {
		I2S_WLEN_16_BIT
	} else {
		I2S_WLEN_32_BIT
