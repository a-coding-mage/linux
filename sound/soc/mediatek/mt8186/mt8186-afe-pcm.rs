...
// Includes from the C source become external dependencies in the repository context:
// linux/delay.h, linux/dma-mapping.h, linux/module.h, linux/of.h,
// linux/of_address.h, linux/of_reserved_mem.h, linux/pm_runtime.h,
// linux/reset.h, sound/soc.h, and the local MediaTek AFE headers.

// The following file is a translation-only artifact. Types, constants,
// functions, and kernel/ASoC helper macros referenced here are supplied by
// the surrounding repository bindings.
...
#[repr(C)] struct snd_pcm_runtime { rate: c_uint, period_size: c_uint, buffer_size: c_uint, stop_threshold: c_uint }
#[repr(C)] struct snd_pcm_hw_params { _private: [u8; 0] }
#[repr(C)] struct snd_pcm_hardware { info: c_uint, formats: c_uint, period_bytes_min: c_uint, period_bytes_max: c_uint, periods_min: c_uint, periods_max: c_uint, buffer_bytes_max: c_uint, fifo_size: c_uint }
...
type c_long = i64;
#[repr(C)] struct mtk_base_afe { _private: [u8; 0] }
#[repr(C)] struct mt8186_afe_private { _private: [u8; 0] }
#[repr(C)] struct mtk_base_afe_memif { _private: [u8; 0] }
#[repr(C)] struct mtk_base_afe_irq { _private: [u8; 0] }
#[repr(C)] struct mtk_base_irq_data { _private: [u8; 0] }
...
#[repr(C)] struct snd_soc_dapm_route { _private: [u8; 0] }
#[repr(C)] struct mtk_base_memif_data { _private: [u8; 0] }
#[repr(C)] struct regmap_config { _private: [u8; 0] }
#[repr(C)] struct snd_soc_component_driver { _private: [u8; 0] }
#[repr(C)] struct mtk_base_afe_dai { _private: [u8; 0] }
...
macro_rules! external_c { ($($tt:tt)*) => {}; }
macro_rules! c_static_translation { ($name:ident, $body:expr) => { const $name: &str = $body; }; }
