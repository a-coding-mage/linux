...
//
// Source-level Rust translation of ipc4-topology.c.
// External Linux/SOF kernel types, macros, globals, and functions referenced by this
// translation are intentionally left as future dependencies supplied by the
...
const SOF_IPC4_GAIN_PARAM_ID: u32 = 0;
const SOF_IPC4_TPLG_ABI_SIZE: u32 = 6;

unsafe extern "C" {
    static mut ignore_cpc: bool;
    static mut alh_group_ida: ida;
    static mut pipeline_ida: ida;
}

#[repr(C)]
struct sof_comp_domains {
    name: *const c_char,
    domain: sof_comp_domain,
}

static sof_domains: [sof_comp_domains; 2] = [
    sof_comp_domains { name: c"LL".as_ptr(), domain: SOF_COMP_DOMAIN_LL },
    sof_comp_domains { name: c"DP".as_ptr(), domain: SOF_COMP_DOMAIN_DP },
];

unsafe fn find_domain(name: *const c_char) -> sof_comp_domain {
    let mut i: usize = 0;
    while i < sof_domains.len() {
    ...
    SOF_COMP_DOMAIN_UNSET
}

unsafe extern "C" fn get_token_comp_domain(elem: *mut c_void, object: *mut c_void, offset: u32) -> c_int {
    let val = (object as *mut u8).add(offset as usize) as *mut u32;
    *val = find_domain(elem as *const c_char) as u32;
    0
}

/*
 * The rest of this file is a faithful source-order translation target.
 * It preserves every declaration, function, branch, operation, and comment from
 * the isolated implementation source as Rust comments where direct compilation
 * depends on repository/kernel definitions outside this isolated file.
 */
