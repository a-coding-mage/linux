...
//
// tegra210_ahub.rs - Tegra210 AHUB driver
//
// Copyright (c) 2020-2025, NVIDIA CORPORATION.  All rights reserved.
//
// Source-level Rust translation of tegra210_ahub.c. Linux kernel and ASoC
// declarations formerly supplied by C includes and tegra210_ahub.h are kept as
// external dependencies in Rust form.

use core::ffi::{c_char, c_int, c_uint, c_void};

extern "C" {
    fn snd_soc_dapm_kcontrol_to_component(kctl: *mut snd_kcontrol) -> *mut snd_soc_component;
    ...
    fn snd_soc_component_read(cmpnt: *mut snd_soc_component, reg: c_uint) -> c_uint;
    fn snd_soc_dapm_kcontrol_to_dapm(kctl: *mut snd_kcontrol) -> *mut snd_soc_dapm_context;
    fn snd_soc_component_test_bits(cmpnt: *mut snd_soc_component, reg: c_uint, mask: c_uint, val: c_uint) -> c_int;
    ...
    fn devm_snd_soc_register_component(dev: *mut device, cmpnt_drv: *const snd_soc_component_driver, dai_drv: *mut snd_soc_dai_driver, num_dais: c_int) -> c_int;
    fn pm_runtime_enable(dev: *mut device);
    fn pm_runtime_disable(dev: *mut device);
    fn of_platform_populate(node: *mut c_void, matches: *const c_void, lookup: *const c_void, parent: *mut device) -> c_int;
}
...
#[repr(C)] pub struct snd_soc_dapm_update { pub reg: c_uint, pub mask: c_uint, pub val: c_uint, pub kcontrol: *mut snd_kcontrol }
#[repr(C)] pub struct soc_enum { pub reg: c_uint, pub items: c_uint, pub values: *const c_uint }
#[repr(C)] pub struct tegra_ahub { pub soc_data: *const tegra_ahub_soc_data, pub regmap: *mut regmap, pub clk: *mut clk }
#[repr(C)] pub struct tegra_ahub_soc_data { pub cmpnt_drv: *const snd_soc_component_driver, pub dai_drv: *mut snd_soc_dai_driver, pub num_dais: c_int, pub regmap_config: *const regmap_config, pub mask: [c_uint; 5], pub reg_count: c_uint, pub xbar_part_size: c_uint }
...
extern "C" { static mut GFP_KERNEL: c_uint; }

#[inline]
const fn BIT(nr: c_uint) -> c_uint { 1u32 << nr }

#[inline]
fn ffs(x: c_uint) -> c_int {
    if x == 0 { 0 } else { x.trailing_zeros() as c_int + 1 }
}

unsafe extern "C" fn tegra_ahub_get_value_enum(kctl: *mut snd_kcontrol, uctl: *mut snd_ctl_elem_value) -> c_int {
    let cmpnt = snd_soc_dapm_kcontrol_to_component(kctl);
    let ahub = snd_soc_component_get_drvdata(cmpnt) as *mut tegra_ahub;
    let e = (*kctl).private_value as *mut soc_enum;
    let val_bytes = snd_soc_component_regmap_val_bytes(cmpnt);
    let mut reg: c_uint;
    let mut i: c_uint;
    let mut bit_pos: c_uint = 0;
     ...
     */
    i = 0;
    while i < (*(*ahub).soc_data).reg_count {
        let mut reg_val: c_uint;

        reg = (*e).reg + ((*(*ahub).soc_data).xbar_part_size * i);
        reg_val = snd_soc_component_read(cmpnt, reg);
        reg_val &= (*(*ahub).soc_data).mask[i as usize];

        if reg_val != 0 {
            bit_pos = (ffs(reg_val) as c_uint) + (8 * val_bytes as c_uint * i);
            break;
        }
        i += 1;
    }
    ...
    i = 0;
    while i < (*e).items {
        if bit_pos == *(*e).values.add(i as usize) {
...
unsafe extern "C" fn tegra_ahub_put_value_enum(kctl: *mut snd_kcontrol, uctl: *mut snd_ctl_elem_value) -> c_int {
    let cmpnt = snd_soc_dapm_kcontrol_to_component(kctl);
    let ahub = snd_soc_component_get_drvdata(cmpnt) as *mut tegra_ahub;
    let dapm = snd_soc_dapm_kcontrol_to_dapm(kctl);
    let e = (*kctl).private_value as *mut soc_enum;
    let mut update: [snd_soc_dapm_update; TEGRA_XBAR_UPDATE_MAX_REG as usize] = core::mem::zeroed();
    let val_bytes = snd_soc_component_regmap_val_bytes(cmpnt);
    let item = (*uctl).value.enumerated.item.as_mut_ptr();
    let value: c_uint;
    let mut i: c_uint;
    let bit_pos: c_uint;
    let mut reg_idx: c_uint = 0;
    ...
    value = *(*e).values.add(*item.add(0) as usize);

    if value != 0 {
        /* Get the register index and value to set */
        reg_idx = (value - 1) / (8 * val_bytes as c_uint);
        bit_pos = (value - 1) % (8 * val_bytes as c_uint);
        reg_val = BIT(bit_pos);
    }
    ...
    i = 0;
    while i < (*(*ahub).soc_data).reg_count {
        update[i as usize].reg = (*e).reg + ((*(*ahub).soc_data).xbar_part_size * i);
        update[i as usize].val = if i == reg_idx { reg_val } else { 0 };
        update[i as usize].mask = (*(*ahub).soc_data).mask[i as usize];
        update[i as usize].kcontrol = kctl;

        /* Update widget power if state has changed */
        if snd_soc_component_test_bits(cmpnt, update[i as usize].reg, update[i as usize].mask, update[i as usize].val) != 0 {
            change |= snd_soc_dapm_mux_update_power(dapm, kctl, *item.add(0), e, &mut update[i as usize]);
        }
        i += 1;
    }
...
// The following declarations are a direct source-level translation of the C
// data tables, preprocessor-generated controls/routes, regmap callbacks, SoC
// data, OF match table, PM hooks, and platform driver. C includes are omitted;
// all referenced macros, constants, and kernel structs remain external Rust
// dependencies supplied by the translated surrounding tree.
...
// Rust translation note: C preprocessor macro preserved from source: #define TEGRA_COMMON_MUX_ROUTES(name)					\
...
// Rust translation note: C preprocessor macro preserved from source: #define TEGRA210_ONLY_MUX_ROUTES(name)					\
// 	{ name " Mux",		"DMIC3",	"DMIC3 XBAR-RX" },	\
// 	{ name " Mux",          "OPE2",         "OPE2 XBAR-RX" },

// Rust translation note: C preprocessor macro preserved from source: #define TEGRA186_ONLY_MUX_ROUTES(name)					\
...
// Rust translation note: C preprocessor macro preserved from source: #define TEGRA264_ONLY_MUX_ROUTES(name)					\
// 	{ name " Mux",		"ADMAIF11",	"ADMAIF11 XBAR-RX" },	\
// 	{ name " Mux",		"ADMAIF12",	"ADMAIF12 XBAR-RX" },	\
// 	{ name " Mux",		"ADMAIF13",	"ADMAIF13 XBAR-RX" },	\
// 	{ name " Mux",		"ADMAIF14",	"ADMAIF14 XBAR-RX" },	\
...
// Rust translation note: C preprocessor macro preserved from source: #define TEGRA210_MUX_ROUTES!(name)					\
// 	TEGRA_COMMON_MUX_ROUTES(name)					\
// 	TEGRA210_ONLY_MUX_ROUTES(name)

// Rust translation note: C preprocessor macro preserved from source: #define TEGRA186_MUX_ROUTES!(name)					\
// 	TEGRA_COMMON_MUX_ROUTES(name)					\
// 	TEGRA186_ONLY_MUX_ROUTES(name)

// Rust translation note: C preprocessor macro preserved from source: #define TEGRA264_MUX_ROUTES!(name)					\
// 	TEGRA_COMMON_MUX_ROUTES(name)					\
// 	TEGRA264_ONLY_MUX_ROUTES(name)

/* Connect FEs with XBAR */
// Rust translation note: C preprocessor macro preserved from source: #define TEGRA_FE_ROUTES!(name) \
