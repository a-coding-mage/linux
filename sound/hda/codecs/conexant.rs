...
// dependency removed from executable Rust: #include <linux/init.h>
// dependency removed from executable Rust: #include <linux/delay.h>
// dependency removed from executable Rust: #include <linux/slab.h>
// dependency removed from executable Rust: #include <linux/module.h>
// dependency removed from executable Rust: #include <sound/core.h>
// dependency removed from executable Rust: #include <sound/jack.h>

// dependency removed from executable Rust: #include <sound/hda_codec.h>
// dependency removed from executable Rust: #include "hda_local.h"
// dependency removed from executable Rust: #include "hda_auto_parser.h"
// dependency removed from executable Rust: #include "hda_beep.h"
// dependency removed from executable Rust: #include "hda_jack.h"
// dependency removed from executable Rust: #include "generic.h"

pub struct conexant_spec {
	struct hda_gen_spec gen;

	/* extra EAPD pins */
	core::ffi::c_uint num_eapds;
	hda_nid_t eapds[4];
	bool dynamic_eapd;
	hda_nid_t mute_led_eapd;

	core::ffi::c_uint parse_flags; /* flag for snd_hda_parse_pin_defcfg() */

	/* OLPC XO specific */
	bool recording;
	bool dc_enable;
	core::ffi::c_uint dc_input_bias; /* offset into olpc_xo_dc_bias */
	struct nid_path *dc_mode_path;

	int mute_led_polarity;
	core::ffi::c_uint gpio_led;
	core::ffi::c_uint gpio_mute_led_mask;
	core::ffi::c_uint gpio_mic_led_mask;
...
static int set_beep_amp(pub struct conexant_spec *spec, hda_nid_t nid,
			int idx, int dir)
{
	struct snd_kcontrol_new *knew;
	core::ffi::c_uint beep_amp = HDA_COMPOSE_AMP_VAL(nid, 1, idx, dir);
	int i;
...
static int cx_auto_parse_beep(hda_codec *codec)
{
	pub struct conexant_spec *spec = (*codec).spec;
	hda_nid_t nid;
...
/* parse EAPDs */
static void cx_auto_parse_eapd(hda_codec *codec)
{
	pub struct conexant_spec *spec = (*codec).spec;
	hda_nid_t nid;
...
static void cx_auto_turn_eapd(hda_codec *codec, int num_pins,
			      const hda_nid_t *pins, on: bool)
{
	int i;
...
static int cx_auto_vmaster_mute_led(led_classdev *led_cdev,
...
static void cxt_init_gpio_led(hda_codec *codec)
{
	pub struct conexant_spec *spec = (*codec).spec;
	core::ffi::c_uint mask = (*spec).gpio_mute_led_mask | (*spec).gpio_mic_led_mask;

	if (mask)
		snd_hda_codec_set_gpio(codec, mask, mask, (*spec).gpio_led, 0);
}

static void cx_fixup_headset_recog(hda_codec *codec)
{
	core::ffi::c_uint mic_present;
...
static int cx_init(hda_codec *codec)
{
	pub struct conexant_spec *spec = (*codec).spec;
...
static void cx_auto_shutdown(hda_codec *codec)
{
	pub struct conexant_spec *spec = (*codec).spec;
...
static void cx_remove(hda_codec *codec)
{
	cx_auto_shutdown(codec);
	snd_hda_gen_remove(codec);
}

static void cx_process_headset_detect_plug_type(hda_codec *codec)
{
	core::ffi::c_uint val;
...
static void cx_update_headset_mic_vref(hda_codec *codec, hda_jack_callback *event)
{
	core::ffi::c_uint mic_present;
...
static int cx_suspend(hda_codec *codec)
{
	cx_auto_shutdown(codec);
	return 0;
}
 ...
 * pin fix-up
 */
pub const CXT_PINCFG_LENOVO_X200: i32 = 0;
pub const CXT_PINCFG_LENOVO_TP410: i32 = CXT_PINCFG_LENOVO_X200 + 1;
pub const CXT_PINCFG_LEMOTE_A1004: i32 = CXT_PINCFG_LENOVO_TP410 + 1;
pub const CXT_PINCFG_LEMOTE_A1205: i32 = CXT_PINCFG_LEMOTE_A1004 + 1;
pub const CXT_PINCFG_COMPAQ_CQ60: i32 = CXT_PINCFG_LEMOTE_A1205 + 1;
pub const CXT_FIXUP_STEREO_DMIC: i32 = CXT_PINCFG_COMPAQ_CQ60 + 1;
pub const CXT_PINCFG_LENOVO_NOTEBOOK: i32 = CXT_FIXUP_STEREO_DMIC + 1;
pub const CXT_FIXUP_INC_MIC_BOOST: i32 = CXT_PINCFG_LENOVO_NOTEBOOK + 1;
pub const CXT_FIXUP_HEADPHONE_MIC_PIN: i32 = CXT_FIXUP_INC_MIC_BOOST + 1;
pub const CXT_FIXUP_HEADPHONE_MIC: i32 = CXT_FIXUP_HEADPHONE_MIC_PIN + 1;
pub const CXT_FIXUP_GPIO1: i32 = CXT_FIXUP_HEADPHONE_MIC + 1;
pub const CXT_FIXUP_ASPIRE_DMIC: i32 = CXT_FIXUP_GPIO1 + 1;
pub const CXT_FIXUP_THINKPAD_ACPI: i32 = CXT_FIXUP_ASPIRE_DMIC + 1;
pub const CXT_FIXUP_LENOVO_XPAD_ACPI: i32 = CXT_FIXUP_THINKPAD_ACPI + 1;
pub const CXT_FIXUP_OLPC_XO: i32 = CXT_FIXUP_LENOVO_XPAD_ACPI + 1;
pub const CXT_FIXUP_CAP_MIX_AMP: i32 = CXT_FIXUP_OLPC_XO + 1;
pub const CXT_FIXUP_TOSHIBA_P105: i32 = CXT_FIXUP_CAP_MIX_AMP + 1;
pub const CXT_FIXUP_HP_530: i32 = CXT_FIXUP_TOSHIBA_P105 + 1;
pub const CXT_FIXUP_CAP_MIX_AMP_5047: i32 = CXT_FIXUP_HP_530 + 1;
pub const CXT_FIXUP_MUTE_LED_EAPD: i32 = CXT_FIXUP_CAP_MIX_AMP_5047 + 1;
pub const CXT_FIXUP_HP_DOCK: i32 = CXT_FIXUP_MUTE_LED_EAPD + 1;
pub const CXT_FIXUP_HP_SPECTRE: i32 = CXT_FIXUP_HP_DOCK + 1;
pub const CXT_FIXUP_HP_GATE_MIC: i32 = CXT_FIXUP_HP_SPECTRE + 1;
pub const CXT_FIXUP_MUTE_LED_GPIO: i32 = CXT_FIXUP_HP_GATE_MIC + 1;
pub const CXT_FIXUP_HP_ELITEONE_OUT_DIS: i32 = CXT_FIXUP_MUTE_LED_GPIO + 1;
pub const CXT_FIXUP_HP_ZBOOK_MUTE_LED: i32 = CXT_FIXUP_HP_ELITEONE_OUT_DIS + 1;
pub const CXT_FIXUP_HEADSET_MIC: i32 = CXT_FIXUP_HP_ZBOOK_MUTE_LED + 1;
pub const CXT_FIXUP_HP_MIC_NO_PRESENCE: i32 = CXT_FIXUP_HEADSET_MIC + 1;
pub const CXT_PINCFG_SWS_JS201D: i32 = CXT_FIXUP_HP_MIC_NO_PRESENCE + 1;
pub const CXT_PINCFG_LENOVO_IDEAPAD_SLIM5_16AKP10: i32 = CXT_PINCFG_SWS_JS201D + 1;
pub const CXT_PINCFG_TOP_SPEAKER: i32 = CXT_PINCFG_LENOVO_IDEAPAD_SLIM5_16AKP10 + 1;
pub const CXT_FIXUP_HP_A_U: i32 = CXT_PINCFG_TOP_SPEAKER + 1;
pub const CXT_FIXUP_ACER_SWIFT_HP: i32 = CXT_FIXUP_HP_A_U + 1;
pub const CXT_FIXUP_HUAWEI_MATEBOOK_HP: i32 = CXT_FIXUP_ACER_SWIFT_HP + 1;

/* for hda_fixup_thinkpad_acpi() */
// dependency removed from executable Rust: #include "helpers/thinkpad.c"

/* for hda_fixup_ideapad_acpi() */
// dependency removed from executable Rust: #include "helpers/ideapad_hotkey_led.c"

static void cxt_fixup_stereo_dmic(hda_codec *codec,
...
static void cxt_fixup_update_pinctl(hda_codec *codec,
				   const struct hda_fixup *fix, int action)
{
...
static void cxt5066_increase_mic_boost(hda_codec *codec,
				   const struct hda_fixup *fix, int action)
{
...
static void cxt_update_headset_mode(hda_codec *codec)
{
...
static void cxt_update_headset_mode_hook(hda_codec *codec,
...
static void cxt_fixup_headphone_mic(hda_codec *codec,
				    const struct hda_fixup *fix, int action)
{
	pub struct conexant_spec *spec = (*codec).spec;
...
static void cxt_fixup_headset_mic(hda_codec *codec,
				    const struct hda_fixup *fix, int action)
{
	pub struct conexant_spec *spec = (*codec).spec;
...
static void olpc_xo_update_mic_boost(hda_codec *codec)
{
	pub struct conexant_spec *spec = (*codec).spec;
	int ch, val;
...
static void olpc_xo_update_mic_pins(hda_codec *codec)
{
	pub struct conexant_spec *spec = (*codec).spec;
	int cur_input, val;
...
/* mic_autoswitch hook */
static void olpc_xo_automic(hda_codec *codec,
...
/* pcm_capture hook */
static void olpc_xo_capture_hook(hda_pcm_stream *hinfo,
				 hda_codec *codec,
...
static int olpc_xo_dc_mode_get(snd_kcontrol *kcontrol,
			       snd_ctl_elem_value *ucontrol)
{
...
static int olpc_xo_dc_mode_put(snd_kcontrol *kcontrol,
			       snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	pub struct conexant_spec *spec = (*codec).spec;
...
static int olpc_xo_dc_bias_enum_get(snd_kcontrol *kcontrol,
				    snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	pub struct conexant_spec *spec = (*codec).spec;
...
static int olpc_xo_dc_bias_enum_info(snd_kcontrol *kcontrol,
				     snd_ctl_elem_info *uinfo)
{
	return snd_hda_input_mux_info(&olpc_xo_dc_bias, uinfo);
}

static int olpc_xo_dc_bias_enum_put(snd_kcontrol *kcontrol,
				    snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	pub struct conexant_spec *spec = (*codec).spec;
 ...
 * DC mode is disabled
 */
static int olpc_xo_mic_boost_put(snd_kcontrol *kcontrol,
				 snd_ctl_elem_value *ucontrol)
{
	struct hda_codec *codec = snd_kcontrol_chip(kcontrol);
	pub struct conexant_spec *spec = (*codec).spec;
...
static void cxt_fixup_olpc_xo(hda_codec *codec,
				    const struct hda_fixup *fix, int action)
{
	pub struct conexant_spec *spec = (*codec).spec;
...
static void cxt_fixup_mute_led_eapd(hda_codec *codec,
				    const struct hda_fixup *fix, int action)
{
	pub struct conexant_spec *spec = (*codec).spec;
 ...
 */
static void cxt_fixup_cap_mix_amp(hda_codec *codec,
				  const struct hda_fixup *fix, int action)
{
 ...
 */
static void cxt_fixup_cap_mix_amp_5047(hda_codec *codec,
				  const struct hda_fixup *fix, int action)
{
...
static void cxt_fixup_hp_gate_mic_jack(hda_codec *codec,
...
static void cxt_update_gpio_led(hda_codec *codec, mask: core::ffi::c_uint,
				led_on: bool)
{
	pub struct conexant_spec *spec = (*codec).spec;
	core::ffi::c_uint oldval = (*spec).gpio_led;
...
static int cxt_gpio_mute_update(led_classdev *led_cdev,
				led_brightness brightness)
{
	struct hda_codec *codec = dev_to_hda_codec((*(*led_cdev).dev).parent);
	pub struct conexant_spec *spec = (*codec).spec;
...
static int cxt_gpio_micmute_update(led_classdev *led_cdev,
				   led_brightness brightness)
{
	struct hda_codec *codec = dev_to_hda_codec((*(*led_cdev).dev).parent);
	pub struct conexant_spec *spec = (*codec).spec;
...
static void cxt_setup_mute_led(hda_codec *codec,
			       mute: core::ffi::c_uint, mic_mute: core::ffi::c_uint)
{
	pub struct conexant_spec *spec = (*codec).spec;
...
static void cxt_setup_gpio_unmute(hda_codec *codec,
				  gpio_mute_mask: core::ffi::c_uint)
{
	if (gpio_mute_mask) {
		// set gpio data to 0.
...
static void cxt_fixup_mute_led_gpio(hda_codec *codec,
				const struct hda_fixup *fix, int action)
{
	if (action == HDA_FIXUP_ACT_PRE_PROBE)
...
static void cxt_fixup_hp_zbook_mute_led(hda_codec *codec,
					const struct hda_fixup *fix, int action)
{
	if (action == HDA_FIXUP_ACT_PRE_PROBE)
...
static void cxt_fixup_hp_a_u(hda_codec *codec,
			     const struct hda_fixup *fix, int action)
{
 ...
 */
static void add_cx5051_fake_mutes(hda_codec *codec)
{
	pub struct conexant_spec *spec = (*codec).spec;
...
static int cx_probe(hda_codec *codec, const struct hda_device_id *id)
{
	'error: {
	pub struct conexant_spec *spec;
