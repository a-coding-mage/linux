...
extern "C" {
    ...
    fn rcu_read_lock(); fn rcu_read_unlock(); fn srcu_read_lock(_: *mut srcu_struct) -> c_int;
    fn srcu_read_unlock(_: *mut srcu_struct, _: c_int); fn srcu_batches_completed(_: *mut srcu_struct)->c_ulong;
    fn call_rcu_hurry(_: *mut rcu_head, _: rcu_callback_t); fn call_srcu(_: *mut srcu_struct,_: *mut rcu_head,_: rcu_callback_t);
    fn rcu_barrier(); fn srcu_barrier(_: *mut srcu_struct); fn synchronize_rcu();
    fn synchronize_rcu_expedited(); fn synchronize_srcu(_: *mut srcu_struct); fn synchronize_srcu_expedited(_: *mut srcu_struct);
    fn rcu_get_gp_seq()->c_ulong; fn rcu_seq_diff(_: c_ulong,_: c_ulong)->c_ulong; fn rcu_exp_batches_completed()->c_ulong;
    fn rcu_barrier_tasks(); fn synchronize_rcu_tasks(); fn call_rcu_tasks(_: *mut rcu_head,_: rcu_callback_t);
    fn rcu_barrier_tasks_trace(); fn synchronize_rcu_tasks_trace(); fn call_rcu_tasks_trace(_: *mut rcu_head,_: rcu_callback_t);
    ...
    fn rcu_read_lock_trace(); fn rcu_read_unlock_trace(); fn rcu_tasks_torture_stats_print(_: *const c_char,_: *const c_char);
    fn srcu_torture_stats_print(_: *mut srcu_struct,_: *const c_char,_: *const c_char);
    fn ktime_get_mono_fast_ns()->u64; fn num_online_cpus()->c_int; fn torture_must_stop()->bool;
    fn schedule_timeout_uninterruptible(_: c_ulong); fn schedule_timeout_interruptible(_: c_ulong); fn schedule_timeout_idle(_: c_ulong);
    fn cond_resched_tasks_rcu_qs(); fn cond_resched(); fn udelay(_: c_ulong); fn set_cpus_allowed_ptr(_: *mut task_struct,_: *const c_void);
    fn set_user_nice(_: *mut task_struct,_: c_int); fn sched_set_fifo_low(_: *mut task_struct); fn sched_set_normal(_: *mut task_struct,_: c_int);
    fn torture_kthread_stopping(_: *const c_char); fn kthread_should_stop()->bool; fn torture_random(_: *mut c_void)->c_ulong;
    fn torture_stop_kthread(_: unsafe extern "C" fn(*mut c_void)->c_int,*mut task_struct); fn torture_create_kthread(_: unsafe extern "C" fn(*mut c_void)->c_int,*mut c_void,*mut *mut task_struct)->c_int;
    fn torture_cleanup_begin()->bool; fn torture_cleanup_end(); fn torture_init_begin(_: *const c_char,_: c_int)->bool; fn torture_init_end();
    fn torture_shutdown_init(_: c_int,_: unsafe extern "C" fn()); fn torture_init_error(_: c_int)->bool; fn kernel_power_off();
    fn rcu_ftrace_dump(_: c_int); fn show_rcu_gp_kthreads(); fn rcu_gp_is_expedited()->bool; fn rcu_gp_is_normal()->bool;
    fn pr_alert(_: *const c_char,...); fn pr_info(_: *const c_char,...); fn pr_warn(_: *const c_char,...); fn pr_cont(_: *const c_char,...);
    fn init_srcu_struct(_: *mut srcu_struct); fn cleanup_srcu_struct(_: *mut srcu_struct);
    fn rcu_get_jiffies_lazy_flush()->c_ulong; fn rcu_set_jiffies_lazy_flush(_: c_ulong); fn si_mem_available()->i64;
