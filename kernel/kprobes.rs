// Faithful source-level Rust translation of kprobes.c; kernel dependencies remain external.
 ...
 */
static hlist_head kprobe_table[KPROBE_TABLE_SIZE];
...
/*
 * Blacklist -- list of 'kprobe_blacklist_entry' to store info where
...
kprobe_insn_page {
	...
	kprobe_insn_cache *cache;
	int nused;
	int ngarbage;
	char slot_used[];
};

static int slots_per_page(kprobe_insn_cache *c)
{
...
kprobe_insn_cache kprobe_insn_slots = {
...
static int collect_garbage_slots(kprobe_insn_cache *c);
 ...
 * Return: Pointer to instruction slot on success, core::ptr::null_mut() on failure.
 */
kprobe_opcode_t *__get_insn_slot(kprobe_insn_cache *c)
{
...
static bool collect_one_slot(kprobe_insn_page *kip, int idx)
{
...
static long __find_insn_page(kprobe_insn_cache *c,
	kprobe_opcode_t *slot, kprobe_insn_page **pkip)
{
	kprobe_insn_page *kip = core::ptr::null_mut();
	long idx;
	...
	WARN_ON(1);
	*pkip = core::ptr::null_mut();
	return -1;
}

void __free_insn_slot(kprobe_insn_cache *c,
 ...
 */
bool __is_insn_slot_addr(kprobe_insn_cache *c, unsigned long addr)
{
	kprobe_insn_page *kip;
	bool ret = false;
...
int kprobe_cache_get_kallsym(kprobe_insn_cache *c, unsigned int *symnum,
...
kprobe_insn_cache kprobe_optinsn_slots = {
...
#[inline]
static void set_kprobe_instance(kprobe *kp)
{
	__this_cpu_write(kprobe_instance, kp);
}

#[inline]
static void reset_kprobe_instance(void)
{
...
static int aggr_pre_handler(kprobe *p, pt_regs *regs);

/* Return true if 'p' is an aggregator */
#[inline]
static bool kprobe_aggrprobe(kprobe *p)
{
...
#[inline]
static bool kprobe_unused(kprobe *p)
{
...
#[inline]
static void copy_kprobe(kprobe *ap, kprobe *p)
{
	memcpy(&p->opcode, &ap->opcode, sizeof(kprobe_opcode_t));
	memcpy(&p->ainsn, &ap->ainsn, sizeof(arch_specific_insn));
}
 ...
 */
void opt_pre_handler(kprobe *p, pt_regs *regs)
{
	kprobe *kp;
...
static void free_aggr_kprobe(kprobe *p)
{
	optimized_kprobe *op;

	op = container_of(p, optimized_kprobe, kp);
...
#[inline]
static int kprobe_optready(kprobe *p)
{
	optimized_kprobe *op;
...
static bool kprobe_queued(kprobe *p)
{
	optimized_kprobe *op;
 ...
 */
static kprobe *get_optimized_kprobe(kprobe_opcode_t *addr)
{
	int i;
	kprobe *p = core::ptr::null_mut();
	optimized_kprobe *op;
...
static void optimize_kprobe(kprobe *p);
static task_struct *kprobe_optimizer_task;
...
static void do_free_cleaned_kprobes(void)
{
	...
	list_for_each_entry_safe(op, tmp, &freeing_list, list) {
		...
		kprobe *_p = get_optimized_kprobe(op->kp.addr);
		if (unlikely(_p))
			optimize_kprobe(_p);
...
bool optprobe_queued_unopt(optimized_kprobe *op)
{
	optimized_kprobe *_op;
...
static void force_unoptimize_kprobe(optimized_kprobe *op)
{
	lockdep_assert_cpus_held();
...
static void unoptimize_kprobe(kprobe *p, bool force)
{
	optimized_kprobe *op;
...
static int reuse_unused_kprobe(kprobe *ap)
{
	optimized_kprobe *op;
	 ...
	 */
	op = container_of(ap, optimized_kprobe, kp);
...
static void kill_optimized_kprobe(kprobe *p)
{
	optimized_kprobe *op;
...
#[inline]
static
void __prepare_optimized_kprobe(optimized_kprobe *op, kprobe *p)
{
	if (!kprobe_ftrace(p))
...
static void prepare_optimized_kprobe(kprobe *p)
{
	optimized_kprobe *op;
...
static kprobe *alloc_aggr_kprobe(kprobe *p)
{
	optimized_kprobe *op;

	op = kzalloc_obj(optimized_kprobe);
	if (!op)
		return core::ptr::null_mut();
...
static void init_aggr_kprobe(kprobe *ap, kprobe *p);

/*
 * Prepare an optimized_kprobe and optimize it.
 * NOTE: 'p' must be a normal registered kprobe.
 */
static void try_to_optimize_kprobe(kprobe *p)
{
	kprobe *ap;
	optimized_kprobe *op;
...
static int proc_kprobes_optimization_handler(const ctl_table *table,
...
static const ctl_table kprobe_sysctls[] = {
	{
...
static void __arm_kprobe(kprobe *p)
{
	kprobe *_p;
...
static void __disarm_kprobe(kprobe *p, bool reopt)
{
	kprobe *_p;
...
// #endif
	...
	atomic_set(&optimizer_state, OPTIMIZER_ST_IDLE);
	kprobe_optimizer_task = kthread_run(kprobe_optimizer_thread, core::ptr::null_mut(),
					    "kprobe-optimizer");
}
...
static kprobe *alloc_aggr_kprobe(kprobe *p)
{
	return kzalloc_obj(kprobe);
}
// #endif /* CONFIG_OPTPROBES */

// #ifdef CONFIG_KPROBES_ON_FTRACE
static ftrace_ops kprobe_ftrace_ops __read_mostly = {
	.func = kprobe_ftrace_handler,
	.flags = FTRACE_OPS_FL_SAVE_REGS,
};

static ftrace_ops kprobe_ipmodify_ops __read_mostly = {
	.func = kprobe_ftrace_handler,
...
static int __arm_kprobe_ftrace(kprobe *p, ftrace_ops *ops,
			       int *cnt)
{
	int ret;
...
static int arm_kprobe_ftrace(kprobe *p)
{
	bool ipmodify = (p->post_handler != core::ptr::null_mut());
...
static int __disarm_kprobe_ftrace(kprobe *p, ftrace_ops *ops,
				  int *cnt)
{
	int ret;
...
static int disarm_kprobe_ftrace(kprobe *p)
{
	bool ipmodify = (p->post_handler != core::ptr::null_mut());
...
static int prepare_kprobe(kprobe *p)
{
...
static int arm_kprobe(kprobe *kp)
{
...
static int disarm_kprobe(kprobe *kp, bool reopt)
{
	if (unlikely(kprobe_ftrace(kp)))
...
void kprobes_inc_nmissed_count(kprobe *p)
{
	kprobe *kp;
...
static int add_new_kprobe(kprobe *ap, kprobe *p)
{
	if (p->post_handler)
 ...
 */
static int register_aggr_kprobe(kprobe *orig_p, kprobe *p)
{
	int ret = 0;
	kprobe *ap = orig_p;
...
static kprobe_opcode_t *kprobe_addr(kprobe *p)
{
	bool on_func_entry;
 ...
 * at the same address.
 */
static kprobe *__get_valid_kprobe(kprobe *p)
{
	kprobe *ap, *list_p;
 ...
 */
#[inline]
static int warn_kprobe_rereg(kprobe *p)
{
	guard(mutex)(&kprobe_mutex);
...
static int check_kprobe_address_safe(kprobe *p,
				     module **probed_mod)
{
	int ret;
...
static int __register_kprobe(kprobe *p)
{
	int ret;
	kprobe *old_p;
...
static bool aggr_kprobe_disabled(kprobe *ap)
{
	kprobe *kp;
...
static kprobe *__disable_kprobe(kprobe *p)
{
	kprobe *orig_p;
	int ret;
 ...
 */
static int __unregister_kprobe_top(kprobe *p)
{
	kprobe *ap, *list_p;
...
static void __unregister_kprobe_bottom(kprobe *p)
{
	kprobe *ap;
	...
	if (list_empty(&p->list))
		...
		ap = list_entry(p->list.next, kprobe, list);
		list_del(&p->list);
		free_aggr_kprobe(ap);
	}
...
int register_kprobes(kprobe **kps, int num)
{
	int i, ret = 0;
...
void unregister_kprobe(kprobe *p)
{
	unregister_kprobes(&p, 1);
}
EXPORT_SYMBOL_GPL(unregister_kprobe);

void unregister_kprobes(kprobe **kps, int num)
{
	int i;
...
static notifier_block kprobe_exceptions_nb = {
...
{
	kretprobe_instance *ri = nod;

	ri->rph = context;
	return 0;
}
static int kretprobe_fini_pool(objpool_head *head, void *context)
{
	kfree(context);
	return 0;
}

static void free_rp_inst_rcu(rcu_head *head)
{
	kretprobe_instance *ri = container_of(head, kretprobe_instance, rcu);
	kretprobe_holder *rph = ri->rph;

	objpool_drop(ri, &rph->pool);
}
NOKPROBE_SYMBOL(free_rp_inst_rcu);

static void recycle_rp_inst(kretprobe_instance *ri)
{
	kretprobe *rp = get_kretprobe(ri);
 ...
 */
void kprobe_flush_task(task_struct *tk)
{
	...
	while (node) {
		ri = container_of(node, kretprobe_instance, llist);
		node = node->next;
...
#[inline]
static void free_rp_inst(kretprobe *rp)
{
	kretprobe_holder *rph = rp->rph;
...
static kprobe_opcode_t *__kretprobe_find_ret_addr(task_struct *tsk,
						  llist_node **cur)
{
 ...
 */
unsigned long kretprobe_find_ret_addr(task_struct *tsk, void *fp,
				      llist_node **cur)
{
	kretprobe_instance *ri;
	kprobe_opcode_t *ret;
...
void __weak arch_kretprobe_fixup_return(pt_regs *regs,
...
unsigned long __kretprobe_trampoline_handler(pt_regs *regs,
					     void *frame_pointer)
{
	kretprobe_instance *ri = core::ptr::null_mut();
	llist_node *first, *node = core::ptr::null_mut();
	kprobe_opcode_t *correct_ret_addr;
	kretprobe *rp;
	...
	while (first) {
		ri = container_of(first, kretprobe_instance, llist);
 ...
 */
static int pre_handler_kretprobe(kprobe *p, pt_regs *regs)
{
	kretprobe *rp = container_of(p, kretprobe, kp);
	kretprobe_holder *rph = rp->rph;
	kretprobe_instance *ri;
...
static int pre_handler_kretprobe(kprobe *p, pt_regs *regs)
{
	...
	ri = container_of(rhn, kretprobe_instance, node);
				      ...
				      pt_regs *regs)
{
	...
	ri = container_of(rh, kretprobe_instance, node);
	rp->handler(ri, regs);
...
int register_kretprobe(kretprobe *rp)
{
	int ret;
	int i;
	void *addr;
...
// #ifdef CONFIG_KRETPROBE_ON_RETHOOK
	...
	if (ret != 0) {
		rethook_free(rp->rh);
		rp->rh = core::ptr::null_mut();
	}
// #else	/* !CONFIG_KRETPROBE_ON_RETHOOK */
	rp->rph = kzalloc_obj(kretprobe_holder);
	if (!rp->rph)
		return -ENOMEM;

	if (objpool_init(&rp->rph->pool, rp->maxactive, rp->data_size +
			sizeof(kretprobe_instance), GFP_KERNEL,
...
int register_kretprobes(kretprobe **rps, int num)
{
	int ret = 0, i;
...
void unregister_kretprobe(kretprobe *rp)
{
	unregister_kretprobes(&rp, 1);
}
EXPORT_SYMBOL_GPL(unregister_kretprobe);

void unregister_kretprobes(kretprobe **rps, int num)
{
	int i;
...
// #else
		rcu_assign_pointer(rps[i]->rph->rp, core::ptr::null_mut());
// #endif
	}
...
static void kill_kprobe(kprobe *p)
{
	kprobe *kp;
	...
	if (kprobe_aggrprobe(p)) {
		...
		p->post_handler = core::ptr::null_mut();
...
static notifier_block kprobe_module_nb = {
...
static void report_probe(seq_file *pi, kprobe *p,
		const char *sym, int offset, char *modname, kprobe *pp)
{
	char *kprobe_type;
	void *addr = p->addr;
...
static void *kprobe_seq_start(seq_file *f, loff_t *pos)
{
	return (*pos < KPROBE_TABLE_SIZE) ? pos : core::ptr::null_mut();
}

static void *kprobe_seq_next(seq_file *f, void *v, loff_t *pos)
{
	(*pos)++;
...
static void kprobe_seq_stop(seq_file *f, void *v)
{
	/* Nothing to do */
}

static int show_kprobe_addr(seq_file *pi, void *v)
{
	hlist_head *head;
	kprobe *p, *kp;
	const char *sym;
	...
	preempt_disable();
	hlist_for_each_entry_rcu(p, head, hlist) {
		sym = kallsyms_lookup((unsigned long)p->addr, core::ptr::null_mut(),
...
static const seq_operations kprobes_sops = {
...
static void *kprobe_blacklist_seq_start(seq_file *m, loff_t *pos)
{
	mutex_lock(&kprobe_mutex);
	return seq_list_start(&kprobe_blacklist, *pos);
}

static void *kprobe_blacklist_seq_next(seq_file *m, void *v, loff_t *pos)
{
	return seq_list_next(v, &kprobe_blacklist, pos);
}

static int kprobe_blacklist_seq_show(seq_file *m, void *v)
{
	kprobe_blacklist_entry *ent =
		list_entry(v, kprobe_blacklist_entry, list);
...
static void kprobe_blacklist_seq_stop(seq_file *f, void *v)
{
	mutex_unlock(&kprobe_mutex);
}

static const seq_operations kprobe_blacklist_sops = {
 ...
 * available
 */
static ssize_t read_enabled_file_bool(file *file,
...
static ssize_t write_enabled_file_bool(file *file,
...
static const file_operations fops_kp = {
...
static int __init debugfs_kprobe_init(void)
{
	...
	debugfs_create_file("blacklist", 0400, dir, core::ptr::null_mut(),
