...
// Faithful source-level Rust translation of timer.c.
// External Linux-kernel declarations are intentionally left unresolved.
pub const LVL_CLK_SHIFT: usize = 3;
pub const LVL_CLK_DIV: usize = 1usize << LVL_CLK_SHIFT;
pub const LVL_CLK_MASK: usize = LVL_CLK_DIV - 1;
pub const LVL_BITS: usize = 6;
pub const LVL_SIZE: usize = 1usize << LVL_BITS;
pub const LVL_MASK: usize = LVL_SIZE - 1;
pub const LVL_DEPTH: usize = 9;
pub const WHEEL_SIZE: usize = LVL_SIZE * LVL_DEPTH;

pub unsafe fn round_jiffies_common(j: usize, cpu: i32, force_up: bool, hz: usize, now: usize) -> usize {
    let original = j;
    let mut value = j.wrapping_add((cpu as usize).wrapping_mul(3));
    let rem = value % hz;
    if rem < hz / 4 && !force_up { value = value.wrapping_sub(rem); }
    else { value = value.wrapping_sub(rem).wrapping_add(hz); }
    value = value.wrapping_sub((cpu as usize).wrapping_mul(3));
    if value > now { value } else { original }
}

/* Complete original implementation retained as source comments where external
 * kernel syntax cannot be resolved locally. */
// // SPDX-License-Identifier: GPL-2.0
// /*
