...
// external Linux kernel dependency: #include <linux/xarray.h>
// external Linux kernel dependency: #include <linux/module.h>

fn u32 tests_run;
fn u32 tests_passed;

fn const u32 order_limit =
		IS_ENABLED(CONFIG_XARRAY_MULTI) ? BITS_PER_LONG : 1;

// preprocessor directive: #ifndef XA_DEBUG
# ifdef __KERNEL__
...
// preprocessor directive: #ifndef __KERNEL__
...
unsafe fn void check_xas_retry(struct xarray *xa)
{
	...
	*mut core::ffi::c_voidentry;
	...
	rcu_read_lock();
	XA_BUG_ON(xa, xas_find(&xas, u64::MAX) != xa_mk_value(0));
	xa_erase_index(xa, 1);
	XA_BUG_ON(xa, !xa_is_retry(xas_reload(&xas)));
	XA_BUG_ON(xa, xas_retry(&xas, core::ptr::null_mut()));
	XA_BUG_ON(xa, xas_retry(&xas, xa_mk_value(0)));
	xas_reset(&xas);
	XA_BUG_ON(xa, xas.xa_node != XAS_RESTART);
	XA_BUG_ON(xa, xas_next_entry(&xas, u64::MAX) != xa_mk_value(0));
	XA_BUG_ON(xa, xas.xa_node != core::ptr::null_mut());
	rcu_read_unlock();

	XA_BUG_ON(xa, xa_store_index(xa, 1, GFP_KERNEL) != core::ptr::null_mut());
...
unsafe fn void check_xa_load(struct xarray *xa)
{
	...
	for (i = 0;  i < 1024;  i++) {
		for (j = 0;  j < 1024;  j++) {
			*mut core::ffi::c_voidentry = xa_load(xa, j);
			if j < i
				XA_BUG_ON(xa, xa_to_value(entry) != j);
			else
				XA_BUG_ON(xa, entry);
		}
		XA_BUG_ON(xa, xa_store_index(xa, i, GFP_KERNEL) != core::ptr::null_mut());
	}
...
unsafe fn void check_xa_mark_1(struct xarray *xa, u64 index)
{
	...
	/* core::ptr::null_mut() elements have no marks set */
	XA_BUG_ON(xa, xa_get_mark(xa, index, XA_MARK_0));
	xa_set_mark(xa, index, XA_MARK_0);
	XA_BUG_ON(xa, xa_get_mark(xa, index, XA_MARK_0));

	/* Storing a pointer will not make a mark appear */
	XA_BUG_ON(xa, xa_store_index(xa, index, GFP_KERNEL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_get_mark(xa, index, XA_MARK_0));
	xa_set_mark(xa, index, XA_MARK_0);
	XA_BUG_ON(xa, !xa_get_mark(xa, index, XA_MARK_0));
	...
	/* Storing core::ptr::null_mut() clears marks, and they can't be set again */
	...
	for (order = 2;  order < max_order;  order++) {
		u64 base = round_down(index, 1UL << order);
		u64 next = base + (1UL << order);
		u64 i;
...
unsafe fn void check_xa_shrink(struct xarray *xa)
{
	...
	struct xa_node *node;
	u32 order;
	u32 max_order = IS_ENABLED(CONFIG_XARRAY_MULTI) ? 15 : 1;

	XA_BUG_ON(xa, !xa_empty(xa));
	XA_BUG_ON(xa, xa_store_index(xa, 0, GFP_KERNEL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_store_index(xa, 1, GFP_KERNEL) != core::ptr::null_mut());
	...
	XA_BUG_ON(xa, xas_store(&xas, core::ptr::null_mut()) != xa_mk_value(1));
	XA_BUG_ON(xa, xa_load(xa, 1) != core::ptr::null_mut());
	XA_BUG_ON(xa, xas.xa_node != XAS_BOUNDS);
	XA_BUG_ON(xa, xa_entry_locked(xa, node, 0) != XA_RETRY_ENTRY);
	XA_BUG_ON(xa, xas_load(&xas) != core::ptr::null_mut());
	xas_unlock(&xas);
	...
	for (order = 0;  order < max_order;  order++) {
		...
		node = xa_head(xa);
		rcu_read_unlock();
		XA_BUG_ON(xa, xa_store_index(xa, u64::MAX, GFP_KERNEL) !=
				core::ptr::null_mut());
		rcu_read_lock();
...
unsafe fn void check_insert(struct xarray *xa)
{
	...
	for (i = 0;  i < 1024;  i++) {
		...
		XA_BUG_ON(xa, xa_load(xa, i - 1) != core::ptr::null_mut());
		XA_BUG_ON(xa, xa_load(xa, i + 1) != core::ptr::null_mut());
		xa_erase_index(xa, i);
	}
	...
	for (i = 10;  i < BITS_PER_LONG;  i++) {
		...
		XA_BUG_ON(xa, xa_load(xa, (1UL << i) - 1) != core::ptr::null_mut());
		XA_BUG_ON(xa, xa_load(xa, (1UL << i) + 1) != core::ptr::null_mut());
		xa_erase_index(xa, 1UL << i);

		xa_insert_index(xa, (1UL << i) - 1);
		XA_BUG_ON(xa, xa_load(xa, (1UL << i) - 2) != core::ptr::null_mut());
		XA_BUG_ON(xa, xa_load(xa, 1UL << i) != core::ptr::null_mut());
		xa_erase_index(xa, (1UL << i) - 1);
	}

	xa_insert_index(xa, ~0UL);
	XA_BUG_ON(xa, xa_load(xa, 0UL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_load(xa, ~1UL) != core::ptr::null_mut());
...
unsafe fn void check_cmpxchg(struct xarray *xa)
{
	...
	*mut core::ffi::c_voidLOTS = xa_mk_value(12345678);

	XA_BUG_ON(xa, !xa_empty(xa));
	XA_BUG_ON(xa, xa_store_index(xa, 12345678, GFP_KERNEL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_insert(xa, 12345678, xa, GFP_KERNEL) != -EBUSY);
	XA_BUG_ON(xa, xa_cmpxchg(xa, 12345678, SIX, FIVE, GFP_KERNEL) != LOTS);
	XA_BUG_ON(xa, xa_cmpxchg(xa, 12345678, LOTS, FIVE, GFP_KERNEL) != LOTS);
	XA_BUG_ON(xa, xa_cmpxchg(xa, 12345678, FIVE, LOTS, GFP_KERNEL) != FIVE);
	XA_BUG_ON(xa, xa_cmpxchg(xa, 5, FIVE, core::ptr::null_mut(), GFP_KERNEL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_cmpxchg(xa, 5, core::ptr::null_mut(), FIVE, GFP_KERNEL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_insert(xa, 5, FIVE, GFP_KERNEL) != -EBUSY);
	XA_BUG_ON(xa, xa_cmpxchg(xa, 5, FIVE, core::ptr::null_mut(), GFP_KERNEL) != FIVE);
	XA_BUG_ON(xa, xa_insert(xa, 5, FIVE, GFP_KERNEL) == -EBUSY);
...
// preprocessor directive: #ifdef CONFIG_XARRAY_MULTI
	*mut core::ffi::c_voidFIVE = xa_mk_value(5);
	...
	XA_BUG_ON(xa, xa_load(xa, 1 << order) != core::ptr::null_mut());
	...
	/* Conditionally replace FIVE entry at index '0' with core::ptr::null_mut() */
	XA_BUG_ON(xa, xa_cmpxchg(xa, 0, FIVE, core::ptr::null_mut(), GFP_KERNEL) != FIVE);
	...
	for (i = 0;  i < (1 << order);  i++) {
		XA_BUG_ON(xa, xa_load(xa, i) != core::ptr::null_mut());
	...
	xa_store_order(xa, 0, BITS_PER_LONG - 1, core::ptr::null_mut(), GFP_KERNEL);
	XA_BUG_ON(xa, !xa_empty(xa));
// preprocessor directive: #endif
}
...
unsafe fn void check_reserve(struct xarray *xa)
{
	*mut core::ffi::c_voidentry;
	...
	XA_BUG_ON(xa, xa_reserve(xa, 12345678, GFP_KERNEL) != 0);
	XA_BUG_ON(xa, xa_store_index(xa, 12345678, GFP_NOWAIT) != core::ptr::null_mut());
	xa_release(xa, 12345678);
	xa_erase_index(xa, 12345678);
	XA_BUG_ON(xa, !xa_empty(xa));
	...
	XA_BUG_ON(xa, xa_cmpxchg(xa, 12345678, XA_ZERO_ENTRY,
				xa_mk_value(12345678), GFP_NOWAIT) != core::ptr::null_mut());
	xa_release(xa, 12345678);
	xa_erase_index(xa, 12345678);
	XA_BUG_ON(xa, !xa_empty(xa));
	...
	XA_BUG_ON(xa, xa_insert(xa, 12345678, xa_mk_value(12345678), 0) !=
			-EBUSY);
	XA_BUG_ON(xa, xa_empty(xa));
	XA_BUG_ON(xa, xa_erase(xa, 12345678) != core::ptr::null_mut());
	XA_BUG_ON(xa, !xa_empty(xa));
		...
		u32 order)
{
	...
	u64 max = min + (1UL << order);

	xa_store_order(xa, index, order, xa_mk_index(index), GFP_KERNEL);
	XA_BUG_ON(xa, xa_load(xa, min) != xa_mk_index(index));
	XA_BUG_ON(xa, xa_load(xa, max - 1) != xa_mk_index(index));
	XA_BUG_ON(xa, xa_load(xa, max) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_load(xa, min - 1) != core::ptr::null_mut());
...
// preprocessor directive: #ifdef CONFIG_XARRAY_MULTI
	...
	xa_store_order(xa, 0, 2, core::ptr::null_mut(), GFP_KERNEL);
	XA_BUG_ON(xa, !xa_empty(xa));
	...
	for (i = 0;  i < max_order;  i++) {
		for (j = 0;  j < max_order;  j++) {
			...
			for (k = 0;  k < max_order;  k++) {
				*mut core::ffi::c_voidentry = xa_load(xa, (1UL << k) - 1);
				if (i < k) && (j < k)
					XA_BUG_ON(xa, entry != core::ptr::null_mut());
				else
					XA_BUG_ON(xa, entry != xa_mk_index(j));
			}
					      ...
					      u32 order)
{
	...
	XA_BUG_ON(xa, test_get_entry(xa, next_index) != core::ptr::null_mut());
	...
	for (i = 0;  i < nrpages;  i++)
		XA_BUG_ON(xa, test_get_entry(xa, base + i) != core::ptr::null_mut());

	for (i = 0;  i < nrpages;  i++)
		XA_BUG_ON(xa, test_get_entry(xa, next_index + i) != &some_val_2);

	for (i = 0;  i < nrpages;  i++)
		XA_BUG_ON(xa, test_get_entry(xa, next_next_index + i) != core::ptr::null_mut());
	...
	for (i = 0;  i < nrpages;  i++)
		XA_BUG_ON(xa, test_get_entry(xa, next_index + i) != core::ptr::null_mut());
...
unsafe fn void check_xa_alloc_1(struct xarray *xa, u32 base)
{
	...
	XA_BUG_ON(xa, xa_alloc(xa, &id, xa_mk_index(u32::MAX - 1),
				XA_LIMIT(u32::MAX - 1, u32::MAX),
				GFP_KERNEL) != 0);
	XA_BUG_ON(xa, id != 0xfffffffeU);
	XA_BUG_ON(xa, xa_alloc(xa, &id, xa_mk_index(u32::MAX),
				XA_LIMIT(u32::MAX - 1, u32::MAX),
				GFP_KERNEL) != 0);
...
unsafe fn void check_xa_alloc_2(struct xarray *xa, u32 base)
{
	...
	*mut core::ffi::c_voidentry;
	...
	/* Allocate and free a core::ptr::null_mut() and check xa_empty() behaves */
	XA_BUG_ON(xa, !xa_empty(xa));
	XA_BUG_ON(xa, xa_alloc(xa, &id, core::ptr::null_mut(), xa_limit_32b, GFP_KERNEL) != 0);
	XA_BUG_ON(xa, id != base);
	XA_BUG_ON(xa, xa_empty(xa));
	XA_BUG_ON(xa, xa_erase(xa, id) != core::ptr::null_mut());
	XA_BUG_ON(xa, !xa_empty(xa));
	...
	for (i = base;  i < base + 10;  i++) {
		XA_BUG_ON(xa, xa_alloc(xa, &id, core::ptr::null_mut(), xa_limit_32b,
					GFP_KERNEL) != 0);
		XA_BUG_ON(xa, id != i);
	}

	XA_BUG_ON(xa, xa_store(xa, 3, xa_mk_index(3), GFP_KERNEL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_store(xa, 4, xa_mk_index(4), GFP_KERNEL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_store(xa, 4, core::ptr::null_mut(), GFP_KERNEL) != xa_mk_index(4));
	XA_BUG_ON(xa, xa_erase(xa, 5) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_alloc(xa, &id, core::ptr::null_mut(), xa_limit_32b, GFP_KERNEL) != 0);
	XA_BUG_ON(xa, id != 5);
	...
	XA_BUG_ON(xa, xa_erase(xa, 8) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_empty(xa));
	XA_BUG_ON(xa, xa_erase(xa, base + 9) != core::ptr::null_mut());
	XA_BUG_ON(xa, !xa_empty(xa));
...
unsafe fn void check_xa_alloc_3(struct xarray *xa, u32 base)
{
	...
	*mut core::ffi::c_voidentry;
	...
	xa_erase_index(xa, base + 1);
	next = u32::MAX;
	XA_BUG_ON(xa, xa_alloc_cyclic(xa, &id, xa_mk_index(u32::MAX),
				xa_limit_32b, &next, GFP_KERNEL) != 0);
	XA_BUG_ON(xa, id != u32::MAX);
...
fn DEFINE_XARRAY_ALLOC(xa0);
fn DEFINE_XARRAY_ALLOC1(xa1);

unsafe fn void check_xa_alloc(void)
{
...
// preprocessor directive: #ifdef CONFIG_XARRAY_MULTI
	...
	XA_BUG_ON(xa, xa_store_index(xa, next, GFP_KERNEL) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_store_index(xa, next + 1, GFP_KERNEL) != core::ptr::null_mut());

	index = 0;
	XA_BUG_ON(xa, xa_find(xa, &index, u64::MAX, XA_PRESENT) !=
			xa_mk_value(multi));
	...
	XA_BUG_ON(xa, xa_find_after(xa, &index, u64::MAX, XA_PRESENT) !=
			xa_mk_value(next));
	XA_BUG_ON(xa, index != next);
	XA_BUG_ON(xa, xa_find_after(xa, &index, next, XA_PRESENT) != core::ptr::null_mut());
	XA_BUG_ON(xa, index != next);
...
unsafe fn void check_find_1(struct xarray *xa)
{
	...
	for (i = 0;  i < 100;  i++) {
		...
		for (j = 0;  j < i;  j++) {
			...
			for (k = 0;  k < 100;  k++) {
				u64 index = k;
				*mut core::ffi::c_voidentry = xa_find(xa, &index, u64::MAX,
								XA_PRESENT);
				if k <= j
				...
				index = k;
				entry = xa_find(xa, &index, u64::MAX,
								XA_MARK_0);
				if k <= j
					XA_BUG_ON(xa, index != j);
				else if k <= i
...
unsafe fn void check_find_4(struct xarray *xa)
{
	...
	*mut core::ffi::c_voidentry;
	...
	xa_store_index(xa, u64::MAX, GFP_KERNEL);

	entry = xa_find_after(xa, &index, u64::MAX, XA_PRESENT);
	XA_BUG_ON(xa, entry != xa_mk_index(u64::MAX));
...
// preprocessor directive: #endif
	...
	xa_store_index(xa, u64::MAX, GFP_KERNEL);
	XA_BUG_ON(xa, xa_find_entry(xa, xa) != -1);
	XA_BUG_ON(xa, xa_find_entry(xa, xa_mk_index(u64::MAX)) != -1);
	xa_erase_index(xa, u64::MAX);
	XA_BUG_ON(xa, !xa_empty(xa));
}
...
unsafe fn void check_move_tiny(struct xarray *xa)
{
	...
	XA_BUG_ON(xa, !xa_empty(xa));
	rcu_read_lock();
	XA_BUG_ON(xa, xas_next(&xas) != core::ptr::null_mut());
	XA_BUG_ON(xa, xas_next(&xas) != core::ptr::null_mut());
	rcu_read_unlock();
	xa_store_index(xa, 0, GFP_KERNEL);
	rcu_read_lock();
	...
	XA_BUG_ON(xa, xas_prev(&xas) != core::ptr::null_mut());
	rcu_read_unlock();
	xa_erase_index(xa, 0);
	XA_BUG_ON(xa, !xa_empty(xa));
}
...
unsafe fn void check_move_max(struct xarray *xa)
{
	...
	xa_store_index(xa, u64::MAX, GFP_KERNEL);
	rcu_read_lock();
	XA_BUG_ON(xa, xas_find(&xas, u64::MAX) != xa_mk_index(u64::MAX));
	XA_BUG_ON(xa, xas_find(&xas, u64::MAX) != core::ptr::null_mut());
	rcu_read_unlock();
...
unsafe fn void check_move_small(struct xarray *xa, u64 idx)
{
	...
	rcu_read_lock();
	for (i = 0;  i < idx * 4;  i++) {
		*mut core::ffi::c_voidentry = xas_next(&xas);
		if i <= idx
			XA_BUG_ON(xa, xas.xa_node == XAS_RESTART);
	...
	loop {
		*mut core::ffi::c_voidentry = xas_prev(&xas);
		i--;
		if i <= idx
			XA_BUG_ON(xa, xas.xa_node == XAS_RESTART);
		XA_BUG_ON(xa, xas.xa_index != i);
	...
	xas_set(&xas, u64::MAX);
	XA_BUG_ON(xa, xas_next(&xas) != core::ptr::null_mut());
	XA_BUG_ON(xa, xas.xa_index != u64::MAX);
...
unsafe fn void check_create_range_3(void)
{
	XA_STATE(xas, core::ptr::null_mut(), 0);
...
// preprocessor directive: #ifdef CONFIG_XARRAY_MULTI
	...
	XA_BUG_ON(xa, xa_load(xa, first - 1) != core::ptr::null_mut());
	XA_BUG_ON(xa, xa_load(xa, last + 1) != core::ptr::null_mut());

	xa_store_range(xa, first, last, core::ptr::null_mut(), GFP_KERNEL);
// preprocessor directive: #endif
...
fn void check_align_2(struct xarray *xa, *mut u8name)
{
	...
	for (i = 0;  i < 8;  i++) {
		XA_BUG_ON(xa, xa_store(xa, 0, name + i, GFP_KERNEL) != core::ptr::null_mut());
		xa_erase(xa, 0);
	}

	for (i = 0;  i < 8;  i++) {
		XA_BUG_ON(xa, xa_reserve(xa, 0, GFP_KERNEL) != 0);
		XA_BUG_ON(xa, xa_store(xa, 0, name + i, 0) != core::ptr::null_mut());
		xa_erase(xa, 0);
	}
...
unsafe fn void check_destroy(struct xarray *xa)
{
	...
	/* Destroying an array with a single entry at u64::MAX */
	xa_store(xa, u64::MAX, xa, GFP_KERNEL);
	XA_BUG_ON(xa, xa_empty(xa));
	xa_destroy(xa);
	XA_BUG_ON(xa, !xa_empty(xa));
...
fn DEFINE_XARRAY(array);

fn i32 xarray_checks(void)
{
	check_xa_err(&array);
