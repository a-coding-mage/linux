// Translated from Linux kernel xarray.c; external kernel dependencies are intentionally unresolved.
#![allow(warnings)]
...
#[inline] fn u32 xa_lock_type(const *mut xarrayxa)
{
...
#[inline] fn bool xa_track_free(const *mut xarrayxa)
{
	return xa.xa_flags & XA_FLAGS_TRACK_FREE;
}

#[inline] fn bool xa_zero_busy(const *mut xarrayxa)
{
	return xa.xa_flags & XA_FLAGS_ZERO_BUSY;
}

#[inline] fn void xa_mark_set(*mut xarrayxa, xa_mark_t mark)
{
	if (!(xa.xa_flags & XA_FLAGS_MARK(mark)))
		xa.xa_flags |= XA_FLAGS_MARK(mark);
}

#[inline] fn void xa_mark_clear(*mut xarrayxa, xa_mark_t mark)
{
	if (xa.xa_flags & XA_FLAGS_MARK(mark))
		xa.xa_flags &= ~(XA_FLAGS_MARK(mark));
}

#[inline] fn usize *node_marks(*mut xa_nodenode, xa_mark_t mark)
{
	return node.marks[(u32)mark];
}
...
#[inline] fn bool node_any_mark(*mut xa_nodenode, xa_mark_t mark)
{
	return !bitmap_empty(node_marks(node, mark), XA_CHUNK_SIZE);
}

#[inline] fn void node_mark_all(*mut xa_nodenode, xa_mark_t mark)
{
	bitmap_fill(node_marks(node, mark), XA_CHUNK_SIZE);
}

#define mark_inc(mark) loop { \
	mark = (xa_mark_t)((u32)(mark) + 1); \
} while (0)
 ...
 * all sibling entries.
 */
fn xas_squash_marks(const *mut xa_statexas)
{
	xa_mark_t mark = 0;
	u32 limit = xas.xa_offset + xas.xa_sibs + 1;

	loop {
		usize *marks = node_marks(xas.xa_node, mark);
...
fn xas_set_offset(*mut xa_statexas)
{
...
fn xas_next_offset(*mut xa_statexas)
{
	xas.xa_offset++;
	xas_move_index(xas, xas.xa_offset);
}

fn *set_bounds(*mut xa_statexas)
{
 ...
 * error state, return core::ptr::null_mut().  If the index is outside the current scope
 * of the xarray, return core::ptr::null_mut() without changing @xas.xa_node.  Otherwise
 * set @xas.xa_node to core::ptr::null_mut() and return the current head of the array.
 */
fn *xas_start(*mut xa_statexas)
{
...
#[inline] fn *mut core::ffi::c_voidxas_descend(*mut xa_statexas,
					*mut xa_nodenode)
{
	u32 offset = get_offset(xas.xa_index, node);
	*mut core::ffi::c_voidentry = xa_entry(xas.xa, node, offset);
 ...
 * stored at xa_index.  However, it will do nothing and return %core::ptr::null_mut() if
 * @xas is in an error state.  xas_load() will never expand the tree.
 *
 * If the xa_state is set up to operate on a multi-index entry, xas_load()
 * may return %core::ptr::null_mut() or an internal entry, even if there are entries
 * present within the range specified by @xas.
 *
 * Context: Any context.  The caller should hold the xa_lock or the RCU lock.
 * Return: Usually an entry in the XArray, but see description for exceptions.
 */
*mut core::ffi::c_voidxas_load(*mut xa_statexas)
{
	*mut core::ffi::c_voidentry = xas_start(xas);

	while (xa_is_node(entry)) {
		*mut xa_nodenode = xa_to_node(entry);
...
#define XA_RCU_FREE	((*mut xarray)1)

fn xa_node_free(*mut xa_nodenode)
{
 ...
 * by xas_nomem().
 */
void xas_destroy(*mut xa_statexas)
{
	*mut xa_nodenext, *node = xas.xa_alloc;
 ...
 */
bool xas_nomem(*mut xa_statexas, gfp_t gfp)
{
	...
	if (!xas.xa_alloc)
		return false;
	xas.xa_alloc.parent = core::ptr::null_mut();
 ...
 * Return: true if memory was needed, and was successfully allocated.
 */
fn __xas_nomem(*mut xa_statexas, gfp_t gfp)
	__must_hold(xas.xa.xa_lock)
{
	u32 lock_type = xa_lock_type(xas.xa);
...
fn xas_update(*mut xa_statexas, *mut xa_nodenode)
{
	if (xas.xa_update)
		xas.xa_update(node);
	else
...
fn *xas_alloc(*mut xa_statexas, u32 shift)
{
	...
	*mut xa_nodenode = xas.xa_alloc;
	...
	if (node) {
		xas.xa_alloc = core::ptr::null_mut();
	} else {
...
fn long xas_size(const *mut xa_statexas)
{
 ...
 */
fn long xas_max(*mut xa_statexas)
{
	usize max = xas.xa_index;

// cfg: CONFIG_XARRAY_MULTI
	if (xas.xa_shift || xas.xa_sibs) {
		usize mask = xas_size(xas) - 1;
		max |= mask;
		if (mask == max)
			max++;
	}
...
fn xas_shrink(*mut xa_statexas)
{
	*mut xarrayxa = xas.xa;
	*mut xa_nodenode = xas.xa_node;
 ...
 */
fn xas_delete_node(*mut xa_statexas)
{
	*mut xa_nodenode = xas.xa_node;
	...
	loop {
		*mut xa_nodeparent;
		...
		if (!parent) {
			xas.xa.xa_head = core::ptr::null_mut();
			xas.xa_node = XAS_BOUNDS;
			return;
		}

		parent.slots[xas.xa_offset] = core::ptr::null_mut();
		parent.count--;
 ...
 */
fn xas_free_nodes(*mut xa_statexas, *mut xa_nodetop)
{
	u32 offset = 0;
	*mut xa_nodenode = top;

	loop {
		*mut core::ffi::c_voidentry = xa_entry_locked(xas.xa, node, offset);
 ...
 * may be %core::ptr::null_mut(), even if there are non-core::ptr::null_mut() entries at some of the indices
...
*mut core::ffi::c_voidxas_store(*mut xa_statexas, *mut core::ffi::c_voidentry)
{
	...
	*mut core::ffi::c_voidfirst, *next;
	...
	loop {
		/*
		 * Must clear the marks before setting the entry to core::ptr::null_mut(),
		 * otherwise xas_for_each_marked may find a core::ptr::null_mut() entry and
		 * stop early.  rcu_assign_pointer contains a release barrier
		 * so the mark clearing will appear to happen before the
		 * entry is set to core::ptr::null_mut().
		 */
 ...
 */
bool xas_get_mark(const *mut xa_statexas, xa_mark_t mark)
{
	if (xas_invalid(xas))
		return false;
	if (!xas.xa_node)
 ...
 */
void xas_set_mark(const *mut xa_statexas, xa_mark_t mark)
{
	*mut xa_nodenode = xas.xa_node;
	u32 offset = xas.xa_offset;
 ...
 */
void xas_clear_mark(const *mut xa_statexas, xa_mark_t mark)
{
	*mut xa_nodenode = xas.xa_node;
	u32 offset = xas.xa_offset;
 ...
 */
void xas_init_marks(const *mut xa_statexas)
{
	xa_mark_t mark = 0;
	...
	loop {
		if (xas.xa_shift < node.shift) {
			*mut xa_nodechild = xas.xa_alloc;
			...
			u32 canon = offset - xas.xa_sibs;

			node_set_marks(node, canon, core::ptr::null_mut(), 0, marks);
 ...
 * core::ptr::null_mut(). If more new xa_node are needed, the function gives EINVAL error.
 *
 ...
 */
void xas_pause(*mut xa_statexas)
{
	*mut xa_nodenode = xas.xa_node;
 ...
 * out of line.
 */
*mut core::ffi::c_void__xas_prev(*mut xa_statexas)
{
	*mut core::ffi::c_voidentry;
 ...
 * out of line.
 */
*mut core::ffi::c_void__xas_next(*mut xa_statexas)
{
	*mut core::ffi::c_voidentry;
 ...
 */
*mut core::ffi::c_voidxas_find(*mut xa_statexas, usize max)
{
	*mut core::ffi::c_voidentry;
 ...
 * Return: The next entry in the range covered by @xas or %core::ptr::null_mut().
 */
*mut core::ffi::c_voidxas_find_conflict(*mut xa_statexas)
{
	*mut core::ffi::c_voidcurr;
	...
	if (xas_top(xas.xa_node)) {
		...
		while (xa_is_node(curr)) {
			*mut xa_nodenode = xa_to_node(curr);
 ...
 */
*mut core::ffi::c_voidxa_load(*mut xarrayxa, usize index)
{
 ...
 * @index: Index into array.
 *
 * After this function returns, loading from @index will return %core::ptr::null_mut().
 ...
 */
*mut core::ffi::c_void__xa_erase(*mut xarrayxa, usize index)
{
	XA_STATE(xas, xa, index);
	return xas_result(&xas, xa_zero_to_null(xas_store(&xas, core::ptr::null_mut())));
}
 ...
 * Return: The entry which used to be at this index.
 */
*mut core::ffi::c_voidxa_erase(*mut xarrayxa, usize index)
{
	*mut core::ffi::c_voidentry;
 ...
 * The marks associated with @index are unaffected unless @entry is %core::ptr::null_mut().
 *
 * Context: Any context.  Takes and releases the xa_lock.
 ...
 * @gfp: Memory allocation flags.
 *
 * Inserting a core::ptr::null_mut() entry will store a reserved entry (like xa_reserve())
 * if no entry is present.  Inserting will fail if a reserved entry is
 * present, even though loading from this index will return core::ptr::null_mut().
 *
 * Context: Any context.  Expects xa_lock to be held on entry.  May
 * release and reacquire xa_lock if @gfp flags permit.
...
i32 __xa_insert(*mut xarrayxa, usize index, *mut core::ffi::c_voidentry, gfp_t gfp)
{
	*mut core::ffi::c_voidcurr;
	...
	if (!entry)
		entry = XA_ZERO_ENTRY;
	curr = __xa_cmpxchg_raw(xa, index, core::ptr::null_mut(), entry, gfp);
	errno = xa_err(curr);
	if (errno)
		return errno;
	return (curr != core::ptr::null_mut()) ? -EBUSY : 0;
}
 ...
 * Return: %core::ptr::null_mut() on success, xa_err(-EINVAL) if @entry cannot be stored in
...
unlock:
	...
	return xas_result(&xas, core::ptr::null_mut());
}
 ...
 */
i32 xas_get_order(*mut xa_statexas)
{
	i32 order = 0;
	...
	loop {
		u32 slot = xas.xa_offset + (1 << order);
 ...
 * @mark: Mark number.
 *
 * Attempting to set a mark on a %core::ptr::null_mut() entry does not succeed.
 *
 * Context: Any context.  Expects xa_lock to be held on entry.
 */
...
fn xas_sibling(*mut xa_statexas)
{
	*mut xa_nodenode = xas.xa_node;
	usize mask;
	...
	return (xas.xa_index & mask) >
		((usize)xas.xa_offset << node.shift);
}
 ...
 * which case all entries which are not %core::ptr::null_mut() will be copied.
 *
...
void xa_destroy(*mut xarrayxa)
{
	...
	*mut core::ffi::c_voidentry;
	...
	entry = xa_head_locked(xa);
	RCU_INIT_POINTER(xa.xa_head, core::ptr::null_mut());
	xas_init_marks(&xas);
	if (xa_zero_busy(xa))
		xa_mark_clear(xa, XA_FREE_MARK);
...
void xa_dump(const *mut xarrayxa)
{
	*mut core::ffi::c_voidentry = xa.xa_head;
	u32 shift = 0;
