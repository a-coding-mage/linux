...
const TEST_BUFLEN: usize = MAX_LEN + MAX_ALIGN;

const IPv4_MIN_WORDS: usize = 5;
const IPv4_MAX_WORDS: usize = 15;
const NUM_IPv6_TESTS: usize = 200;
const NUM_IP_FAST_CSUM_TESTS: usize = 181;

/* Values for a little endian CPU. Byte swap each half on big endian CPU. */
static random_init_sum: u32 = 0x2847aab;
static random_buf: [u8; 512] = [
...
static expected_results: [u16; 512] = [
...
/* Values for a little endian CPU. Byte swap each half on big endian CPU. */
static init_sums_no_overflow: [u32; 512] = [
...
static expected_csum_ipv6_magic: [u16; 200] = [
...
static expected_fast_csum: [u16; 1810] = [
...
static mut tmp_buf: [u8; TEST_BUFLEN] = [0; TEST_BUFLEN];

extern "C" {
    fn csum_partial(buff: *const u8, len: usize, sum: u32) -> u32;
    fn csum_fold(sum: u32) -> u16;
    fn ip_fast_csum(buff: *const u8, len: usize) -> u16;
    fn csum_ipv6_magic(saddr: *const u8, daddr: *const u8, len: u32, proto: u8, csum: u32) -> u16;
}

unsafe fn to_sum16(x: u16) -> u16 { u16::from_le(x) }
unsafe fn to_wsum(x: u32) -> u32 { ((u16::from_le((x >> 16) as u16) as u32) << 16) | u16::from_le(x as u16) as u32 }

/* Test with randomized input (pre determined random with known results). */
unsafe fn test_csum_fixed_random_inputs() {
    for align in 0..TEST_BUFLEN {
        let count = core::cmp::min(MAX_LEN, TEST_BUFLEN - align);
        tmp_buf[align..align + count].copy_from_slice(&random_buf[..count]);
        for len in 0..MAX_LEN {
            if align + len >= TEST_BUFLEN { break; }
            let result = csum_fold(csum_partial(tmp_buf.as_ptr().add(align), len, to_wsum(random_init_sum)));
            assert_eq!(result, to_sum16(expected_results[len]));
}

unsafe fn test_csum_all_carry_inputs() {
    tmp_buf.fill(0xff);
    for align in 0..TEST_BUFLEN { for len in 0..MAX_LEN {
        if align + len >= TEST_BUFLEN { break; }
        let _ = csum_fold(csum_partial(tmp_buf.as_ptr().add(align), len, to_wsum(0xffff_ffff)));
        let _ = csum_fold(csum_partial(tmp_buf.as_ptr().add(align), len, 0));
    }}
}

unsafe fn test_csum_no_carry_inputs() {
    tmp_buf.fill(0x04);
    for align in 0..TEST_BUFLEN { for len in 0..MAX_LEN {
        if align + len >= TEST_BUFLEN { break; }
        let _ = csum_fold(csum_partial(tmp_buf.as_ptr().add(align), len, to_wsum(init_sums_no_overflow[len])));
        let _ = csum_fold(csum_partial(tmp_buf.as_ptr().add(align), len, to_wsum(init_sums_no_overflow[len].wrapping_add(1))));
    }}
}

unsafe fn test_ip_fast_csum() {
    for len in IPv4_MIN_WORDS..IPv4_MAX_WORDS { for index in 0..NUM_IP_FAST_CSUM_TESTS {
        assert_eq!(to_sum16(expected_fast_csum[(len - IPv4_MIN_WORDS) * NUM_IP_FAST_CSUM_TESTS + index]), ip_fast_csum(random_buf.as_ptr().add(index), len));
    }}
}

unsafe fn test_csum_ipv6_magic() {
    for i in 0..NUM_IPv6_TESTS {
        let len_offset = 32usize;
        let proto_offset = 36usize;
        let csum_offset = 37usize;
        let len = u32::from_le_unaligned(random_buf.as_ptr().add(i + len_offset) as *const u32);
        let proto = *random_buf.as_ptr().add(i + proto_offset);
        let csum = u32::from_ne_unaligned(random_buf.as_ptr().add(i + csum_offset) as *const u32);
        assert_eq!(to_sum16(expected_csum_ipv6_magic[i]), csum_ipv6_magic(random_buf.as_ptr().add(i), random_buf.as_ptr().add(i + 16), len, proto, csum));
    }
}
