/* Faithful source-level Rust translation of locking-selftest.c.
 * External Linux kernel types, functions, macros, and generated test fragments
 * remain unresolved dependencies supplied by the surrounding kernel translation.
 */
#![allow(non_snake_case, non_camel_case_types, dead_code, unused_variables, unused_mut)]

/* Original C includes are external dependencies:
 ...
 */
// external include: <linux/rwsem.h>
// external include: <linux/mutex.h>
// external include: <linux/ww_mutex.h>
// external include: <linux/sched.h>
// external include: <linux/sched/mm.h>
// external include: <linux/delay.h>
// external include: <linux/lockdep.h>
// external include: <linux/spinlock.h>
// external include: <linux/kallsyms.h>
// external include: <linux/interrupt.h>
// external include: <linux/debug_locks.h>
// external include: <linux/irqflags.h>
// external include: <linux/rtmutex.h>
// external include: <linux/local_lock.h>
...
macro_rules! FAILURE		0
macro_rules! SUCCESS		1

macro_rules! LOCKTYPE_SPIN	0x1
macro_rules! LOCKTYPE_RWLOCK	0x2
macro_rules! LOCKTYPE_MUTEX	0x4
macro_rules! LOCKTYPE_RWSEM	0x8
macro_rules! LOCKTYPE_WW	0x10
macro_rules! LOCKTYPE_RTMUTEX 0x20
macro_rules! LOCKTYPE_LL	0x40
macro_rules! LOCKTYPE_SPECIAL 0x80
...
macro_rules! HARDIRQ_DISABLE		local_irq_disable
macro_rules! HARDIRQ_ENABLE		local_irq_enable

macro_rules! HARDIRQ_ENTER()				\
	local_irq_disable();			\
	__irq_enter();				\
	lockdep_hardirq_threaded();		\
	WARN_ON(!in_hardirq());

macro_rules! HARDIRQ_EXIT()				\
	__irq_exit();				\
	local_irq_enable();

macro_rules! SOFTIRQ_DISABLE		local_bh_disable
macro_rules! SOFTIRQ_ENABLE		local_bh_enable

macro_rules! SOFTIRQ_ENTER()				\
		local_bh_disable();		\
		local_irq_disable();		\
		lockdep_softirq_enter();	\
		WARN_ON(!in_softirq());

macro_rules! SOFTIRQ_EXIT()				\
 ...
 */
macro_rules! L(x)			spin_lock(&lock_##x)
macro_rules! U(x)			spin_unlock(&lock_##x)
macro_rules! LU(x)			L(x); U(x)
macro_rules! SI(x)			spin_lock_init(&lock_##x)

macro_rules! WL(x)			write_lock(&rwlock_##x)
macro_rules! WU(x)			write_unlock(&rwlock_##x)
macro_rules! WLU(x)			WL(x); WU(x)

macro_rules! RL(x)			read_lock(&rwlock_##x)
macro_rules! RU(x)			read_unlock(&rwlock_##x)
macro_rules! RLU(x)			RL(x); RU(x)
macro_rules! RWI(x)			rwlock_init(&rwlock_##x)

macro_rules! ML(x)			mutex_lock(&mutex_##x)
macro_rules! MU(x)			mutex_unlock(&mutex_##x)
macro_rules! MI(x)			mutex_init(&mutex_##x)

macro_rules! RTL(x)			rt_mutex_lock(&rtmutex_##x)
macro_rules! RTU(x)			rt_mutex_unlock(&rtmutex_##x)
macro_rules! RTI(x)			rt_mutex_init(&rtmutex_##x)

macro_rules! WSL(x)			down_write(&rwsem_##x)
macro_rules! WSU(x)			up_write(&rwsem_##x)

macro_rules! RSL(x)			down_read(&rwsem_##x)
macro_rules! RSU(x)			up_read(&rwsem_##x)
macro_rules! RWSI(x)			init_rwsem(&rwsem_##x)
...
macro_rules! WWL(x, c)		ww_mutex_lock(x, c)
macro_rules! WWT(x)			ww_mutex_trylock(x, NULL)
macro_rules! WWL1(x)			ww_mutex_lock(x, NULL)
macro_rules! WWU(x)			ww_mutex_unlock(x)


macro_rules! LOCK_UNLOCK_2(x,y)	LOCK(x); LOCK(y); UNLOCK(y); UNLOCK(x)
...
macro_rules! GENERATE_TESTCASE(name)			\
						\
static void name(void) { E(); }

macro_rules! GENERATE_PERMUTATIONS_2_EVENTS(name)	\
						\
static void name##_12(void) { E1(); E2(); }	\
static void name##_21(void) { E2(); E1(); }

macro_rules! GENERATE_PERMUTATIONS_3_EVENTS(name)		\
							\
 ...
 * 6 testcases:
 */
// external include: "locking-selftest-spin.h"
GENERATE_TESTCASE(AA_spin)
// external include: "locking-selftest-wlock.h"
GENERATE_TESTCASE(AA_wlock)
// external include: "locking-selftest-rlock.h"
GENERATE_TESTCASE(AA_rlock)
// external include: "locking-selftest-mutex.h"
GENERATE_TESTCASE(AA_mutex)
// external include: "locking-selftest-wsem.h"
GENERATE_TESTCASE(AA_wsem)
// external include: "locking-selftest-rsem.h"
GENERATE_TESTCASE(AA_rsem)

// conditional: CONFIG_RT_MUTEXES
// external include: "locking-selftest-rtmutex.h"
 ...
 */
// external include: "locking-selftest-spin-hardirq.h"
GENERATE_PERMUTATIONS_2_EVENTS(irqsafe1_hard_spin)

// external include: "locking-selftest-rlock-hardirq.h"
GENERATE_PERMUTATIONS_2_EVENTS(irqsafe1_hard_rlock)

// external include: "locking-selftest-wlock-hardirq.h"
GENERATE_PERMUTATIONS_2_EVENTS(irqsafe1_hard_wlock)

// conditional: CONFIG_PREEMPT_RT
// external include: "locking-selftest-spin-softirq.h"
GENERATE_PERMUTATIONS_2_EVENTS(irqsafe1_soft_spin)

// external include: "locking-selftest-rlock-softirq.h"
GENERATE_PERMUTATIONS_2_EVENTS(irqsafe1_soft_rlock)

// external include: "locking-selftest-wlock-softirq.h"
 ...
 * Generate 24 testcases:
 */
// external include: "locking-selftest-hardirq.h"
// external include: "locking-selftest-rlock.h"
...
// conditional: CONFIG_PREEMPT_RT
// external include: "locking-selftest-softirq.h"
// external include: "locking-selftest-rlock.h"
...
// conditional: I_RTMUTEX
# define I_RTMUTEX(x)
// conditional end

// conditional: CONFIG_RT_MUTEXES
macro_rules! I2_RTMUTEX(x)	rt_mutex_init(&rtmutex_##x)
// conditional else
macro_rules! I2_RTMUTEX(x)
// conditional end
...
// conditional end
	 ...
	 */
// conditional: CONFIG_PROVE_LOCKING
...
// conditional: CONFIG_RT_MUTEXES
macro_rules! dotest_rt(fn, e, m)	dotest((fn), (e), (m))
// conditional else
...
macro_rules! DO_TESTCASE_1(desc, name, nr)				\
	print_testname(desc"/"#nr);				\
	dotest(name##_##nr, SUCCESS, LOCKTYPE_RWLOCK);		\
	pr_cont("\n");

macro_rules! DO_TESTCASE_1B(desc, name, nr)				\
	print_testname(desc"/"#nr);				\
	dotest(name##_##nr, FAILURE, LOCKTYPE_RWLOCK);		\
	pr_cont("\n");

macro_rules! DO_TESTCASE_1RR(desc, name, nr)				\
	print_testname(desc"/"#nr);				\
	pr_cont("             |");				\
	dotest(name##_##nr, SUCCESS, LOCKTYPE_RWLOCK);		\
	pr_cont("\n");

macro_rules! DO_TESTCASE_1RRB(desc, name, nr)			\
	print_testname(desc"/"#nr);				\
	pr_cont("             |");				\
	dotest(name##_##nr, FAILURE, LOCKTYPE_RWLOCK);		\
	pr_cont("\n");
...
macro_rules! DO_TESTCASE_3(desc, name, nr)				\
	print_testname(desc"/"#nr);				\
...
macro_rules! DO_TESTCASE_3RW(desc, name, nr)				\
	print_testname(desc"/"#nr);				\
...
macro_rules! DO_TESTCASE_2RW(desc, name, nr)				\
	print_testname(desc"/"#nr);				\
	pr_cont("      |");					\
	dotest(name##_wlock_##nr, FAILURE, LOCKTYPE_RWLOCK);	\
	dotest(name##_rlock_##nr, SUCCESS, LOCKTYPE_RWLOCK);	\
...
macro_rules! DO_TESTCASE_2x2RW(desc, name, nr)			\
	DO_TESTCASE_2RW("hard-"desc, name##_hard, nr)		\
	NON_RT(DO_TESTCASE_2RW("soft-"desc, name##_soft, nr))	\

macro_rules! DO_TESTCASE_6x2x2RW(desc, name)				\
...
macro_rules! DO_TESTCASE_6(desc, name)				\
	print_testname(desc);					\
...
macro_rules! DO_TESTCASE_6_SUCCESS(desc, name)			\
	print_testname(desc);					\
 ...
 */
macro_rules! DO_TESTCASE_6R(desc, name)				\
	print_testname(desc);					\
	dotest(name##_spin, FAILURE, LOCKTYPE_SPIN);		\
	dotest(name##_wlock, FAILURE, LOCKTYPE_RWLOCK);		\
	dotest(name##_rlock, SUCCESS, LOCKTYPE_RWLOCK);		\
...
macro_rules! DO_TESTCASE_6x2B(desc, name)				\
...
macro_rules! DO_TESTCASE_6x1RR(desc, name)				\
...
macro_rules! DO_TESTCASE_6x1RRB(desc, name)				\
...
macro_rules! DO_TESTCASE_6x6(desc, name)				\
...
macro_rules! DO_TESTCASE_6x6RW(desc, name)				\
...
// conditional: CONFIG_PREEMPT_RT
macro_rules! ww_mutex_base_lock(b)			rt_mutex_lock(b)
macro_rules! ww_mutex_base_trylock(b)		rt_mutex_trylock(b)
macro_rules! ww_mutex_base_lock_nest_lock(b, b2)	rt_mutex_lock_nest_lock(b, b2)
macro_rules! ww_mutex_base_lock_interruptible(b)	rt_mutex_lock_interruptible(b)
macro_rules! ww_mutex_base_lock_killable(b)		rt_mutex_lock_killable(b)
macro_rules! ww_mutex_base_unlock(b)			rt_mutex_unlock(b)
// conditional else
macro_rules! ww_mutex_base_lock(b)			mutex_lock(b)
macro_rules! ww_mutex_base_trylock(b)		mutex_trylock(b)
macro_rules! ww_mutex_base_lock_nest_lock(b, b2)	mutex_lock_nest_lock(b, b2)
macro_rules! ww_mutex_base_lock_interruptible(b)	mutex_lock_interruptible(b)
macro_rules! ww_mutex_base_lock_killable(b)		mutex_lock_killable(b)
macro_rules! ww_mutex_base_unlock(b)			mutex_unlock(b)
// conditional end
...
macro_rules! GENERATE_2_CONTEXT_TESTCASE(outer, outer_lock, inner, inner_lock)	\
										\
...
macro_rules! GENERATE_2_CONTEXT_TESTCASE_FOR_ALL_OUTER(inner, inner_lock)		\
...
macro_rules! DO_CONTEXT_TESTCASE_OUTER_PREEMPTIBLE(outer)			\
 ...
 */
macro_rules! DO_CONTEXT_TESTCASE_OUTER_LIMITED_PREEMPTIBLE(outer)		\
	dotest(RCU_in_##outer, SUCCESS, LOCKTYPE_RWLOCK);		\
	dotest(raw_spinlock_in_##outer, SUCCESS, LOCKTYPE_SPIN);	\
	dotest(spinlock_in_##outer, SUCCESS, LOCKTYPE_SPIN);		\
...
macro_rules! DO_CONTEXT_TESTCASE_OUTER_NOT_PREEMPTIBLE(outer)			\
	dotest(RCU_in_##outer, SUCCESS, LOCKTYPE_RWLOCK);		\
	dotest(raw_spinlock_in_##outer, SUCCESS, LOCKTYPE_SPIN);	\
