...
unsafe fn add_rule(fd:c_int,ty:u32,attr:*const c_void,flags:u32)->c_int { extern "C"{fn landlock_add_rule(_: c_int,_: u32,_: *const c_void,_: u32)->c_int} landlock_add_rule(fd,ty,attr,flags) }
unsafe fn create(attr:*const landlock_ruleset_attr,size:usize,flags:u32)->c_int { extern "C"{fn landlock_create_ruleset(_: *const landlock_ruleset_attr,_: usize,_: u32)->c_int} landlock_create_ruleset(attr,size,flags) }
unsafe fn restrict(fd:c_int,flags:i32)->c_int { extern "C"{fn landlock_restrict_self(_: c_int,_: i32)->c_int} landlock_restrict_self(fd,flags) }
