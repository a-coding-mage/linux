...
#[repr(C)]
pub struct audit_buffer { _opaque: [u8; 0] }
#[repr(C)]
pub struct audit_context { _opaque: [u8; 0] }
#[repr(C)]
pub struct super_block { pub s_id: [u8; 32] }
#[repr(C)]
pub struct inode { pub i_sb: *mut super_block, pub i_ino: u64 }

const TASK_COMM_LEN: usize = 16;
const GFP_KERNEL: u32 = 0x10;

extern "C" {
    fn audit_log_start(ctx: *mut audit_context, flags: u32, msg_type: c_int) -> *mut audit_buffer;
...
unsafe extern "C" fn integrity_audit_setup(str: *mut c_char) -> c_int {
    let mut audit: u64 = 0;
    if kstrtoul(str, 0, &mut audit) == 0 {
...
pub unsafe extern "C" fn integrity_audit_msg(audit_msgno: c_int, inode: *mut inode,
    fname: *const u8, op: *const c_char, cause: *const c_char, result: c_int,
    audit_info: c_int) {
    integrity_audit_message(audit_msgno, inode, fname, op, cause, result, audit_info, 0);
}

pub unsafe extern "C" fn integrity_audit_message(audit_msgno: c_int, inode: *mut inode,
    fname: *const u8, op: *const c_char, cause: *const c_char, result: c_int,
    audit_info: c_int, errno: c_int) {
    let mut name: [u8; TASK_COMM_LEN] = [0; TASK_COMM_LEN];
    if integrity_audit_info == 0 && audit_info == 1 { /* Skip info messages */ return; }
    let ab = audit_log_start(audit_context(), GFP_KERNEL, audit_msgno);
    if ab.is_null() { return; }
    audit_log_format(ab, b"pid=%d uid=%u auid=%u ses=%u\0".as_ptr() as *const c_char,
        task_pid_nr(current),
    ...
    audit_log_format(ab, b" op=%s cause=%s comm=\0".as_ptr() as *const c_char, op, cause);
    ...
    if !inode.is_null() {
        ...
        audit_log_format(ab, b" ino=%llu\0".as_ptr() as *const c_char, (*inode).i_ino);
    }
    audit_log_format(ab, b" res=%d errno=%d\0".as_ptr() as *const c_char,
        (result == 0) as c_int, errno);
    audit_log_end(ab);
}
