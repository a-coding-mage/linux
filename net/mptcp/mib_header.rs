...
// Dependency supplied by the surrounding networking implementation:
// #include <net/inet_common.h>

#[repr(i32)]
pub enum linux_mptcp_mib_field {
	MPTCP_MIB_NUM = 0,
	...
	MPTCP_MIB_ADDADDRTXDROP, // ADD_ADDR with echo-flag=0 not send due to resource exhaustion
	MPTCP_MIB_ECHOADD, // Received ADD_ADDR with echo-flag=1
	MPTCP_MIB_ECHOADDTX, // Send ADD_ADDR with echo-flag=1
	MPTCP_MIB_ECHOADDTXDROP, // ADD_ADDR with echo-flag=1 not send due to resource exhaustion
	...
	MPTCP_MIB_RCVWNDCONFLICTUPDATE, // subflow rcv wnd is overridden by msk's one due to conflict with another subflow while updating msk rcv wnd
	MPTCP_MIB_RCVWNDCONFLICT, // Conflict with while updating msk rcv wnd
	MPTCP_MIB_CURRESTAB, // Current established MPTCP connections
	MPTCP_MIB_BLACKHOLE, // A blackhole has been detected
	MPTCP_MIB_MPCAPABLEDATAFALLBACK, // Missing DSS/MPC+data on first established packet
...
pub const LINUX_MIB_MPTCP_MAX: usize = __MPTCP_MIB_MAX as usize;
...
pub unsafe fn MPTCP_ADD_STATS(net: *mut net, field: linux_mptcp_mib_field, val: i32) {
	if likely((*net).mib.mptcp_statistics) {
		SNMP_ADD_STATS((*net).mib.mptcp_statistics, field, val);
	}
}

pub unsafe fn MPTCP_INC_STATS(net: *mut net, field: linux_mptcp_mib_field) {
	if likely((*net).mib.mptcp_statistics) {
		SNMP_INC_STATS((*net).mib.mptcp_statistics, field);
	}
}

pub unsafe fn __MPTCP_INC_STATS(net: *mut net, field: linux_mptcp_mib_field) {
	if likely((*net).mib.mptcp_statistics) {
		__SNMP_INC_STATS((*net).mib.mptcp_statistics, field);
	}
}

pub unsafe fn MPTCP_DEC_STATS(net: *mut net, field: linux_mptcp_mib_field) {
	if likely((*net).mib.mptcp_statistics) {
...
extern "C" {
	pub fn mptcp_mib_alloc(net: *mut net) -> bool;
}
