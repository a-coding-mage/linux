...
fn arp_hash(pkey: *const core::ffi::c_void, dev: *const net_device, hash_rnd: *mut u32) -> u32 {
    unsafe { arp_hashfn(pkey, dev, hash_rnd) }
}

fn arp_key_eq(neigh: *const neighbour, pkey: *const core::ffi::c_void) -> bool {
    unsafe { neigh_key_eq32(neigh, pkey) }
}

fn arp_constructor(neigh: *mut neighbour) -> i32 {
    unsafe {
        let dev = (*neigh).dev;
...
fn arp_error_report(neigh: *mut neighbour, skb: *mut sk_buff) { unsafe { dst_link_failure(skb); kfree_skb_reason(skb, SKB_DROP_REASON_NEIGH_FAILED); } }

fn arp_send_dst(type_: i32, ptype: i32, dest_ip: u32, dev: *mut net_device, src_ip: u32, dest_hw: *const u8, src_hw: *const u8, target_hw: *const u8, dst: *mut dst_entry) {
    unsafe {
...
fn arp_solicit(neigh: *mut neighbour, skb: *mut sk_buff) {
    unsafe {
...
fn arp_ignore(in_dev: *mut in_device, mut sip: u32, tip: u32) -> i32 { unsafe { let net = dev_net((*in_dev).dev); let scope; match IN_DEV_ARP_IGNORE(in_dev) { 0 | 4..=7 => return 0, 1 => { sip = 0; scope = RT_SCOPE_HOST; }, 2 => scope = RT_SCOPE_HOST, 3 => { sip = 0; scope = RT_SCOPE_LINK; in_dev = core::ptr::null_mut(); }, 8 => return 1, _ => return 0 }; if inet_confirm_addr(net, in_dev, sip, tip, scope) != 0 { 0 } else { 1 } } }
fn arp_accept(in_dev: *mut in_device, sip: u32) -> i32 { unsafe { match IN_DEV_ARP_ACCEPT(in_dev) { 0 => 0, 1 => 1, 2 => (inet_confirm_addr(dev_net((*in_dev).dev), in_dev, sip, 0, RT_SCOPE_LINK) != 0) as i32, _ => 0 } } }
fn arp_filter(sip: u32, tip: u32, dev: *mut net_device) -> i32 { unsafe { let net = dev_net(dev); let rt = ip_route_output(net, sip, tip, 0, l3mdev_master_ifindex_rcu(dev), RT_SCOPE_UNIVERSE); if IS_ERR(rt) { return 1; } let flag = ((*rt).dst.dev != dev) as i32; if flag != 0 { __NET_INC_STATS(net, LINUX_MIB_ARPFILTER); } ip_rt_put(rt); flag } }
fn arp_fwd_proxy(in_dev: *mut in_device, dev: *mut net_device, rt: *mut rtable) -> i32 { unsafe { if (*rt).dst.dev == dev || IN_DEV_PROXY_ARP(in_dev) == 0 { return 0; } let imi = IN_DEV_MEDIUM_ID(in_dev); if imi == 0 { return 1; } if imi == -1 { return 0; } let out = __in_dev_get_rcu((*rt).dst.dev); let omi = if out.is_null() { -1 } else { IN_DEV_MEDIUM_ID(out) }; (omi != imi && omi != -1) as i32 } }
fn arp_fwd_pvlan(in_dev: *mut in_device, dev: *mut net_device, rt: *mut rtable, sip: u32, tip: u32) -> i32 { unsafe { if (*rt).dst.dev != dev || sip == tip { return 0; } (IN_DEV_PROXY_ARP_PVLAN(in_dev) != 0) as i32 } }
...
fn arp_xmit_finish(_net: *mut net, _sk: *mut sock, skb: *mut sk_buff) -> i32 { unsafe { dev_queue_xmit(skb) } }
...
fn arp_is_garp(net: *mut net, dev: *mut net_device, addr_type: *mut i32, ar_op: u16, sip: u32, tip: u32, sha: *mut u8, tha: *mut u8) -> bool { unsafe { let mut g = tip == sip; if g && ar_op == htons(ARPOP_REPLY) { g = !tha.is_null() && memcmp(tha, sha, (*dev).addr_len as usize) == 0; } if g { *addr_type = inet_addr_type_dev_table(net, dev, sip); if *addr_type != RTN_UNICAST { g = false; } } g } }
