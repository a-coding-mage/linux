...
static mut taprio_list: ListHead = ListHead::new();
...
#[repr(C)] struct __tc_taprio_qopt_offload {
	refcount_t users;
...
unsafe fn taprio_get_time(const struct taprio_sched *q)
{
...
unsafe fn should_change_schedules(const struct sched_gate_list *admin,
...
const struct nla_policy entry_policy[TCA_TAPRIO_SCHED_ENTRY_MAX + 1] = {
...
const struct nla_policy taprio_tc_policy[TCA_TAPRIO_TC_ENTRY_MAX + 1] = {
...
unsafe fn taprio_mqprio_cmp(const struct net_device *dev,
...
unsafe fn __exit taprio_module_exit(void)
{
