/* Automatically translated from sm_sideeffect.c; external kernel symbols remain unresolved by design. */
...
unsafe fn sctp_cmd_interpreter(sctp_event_type event_type,
				...
				*mut commands,
				GfpT gfp);
unsafe fn sctp_side_effects(sctp_event_type event_type,
			     sctp_subtype subtype,
			     sctp_state state,
			     *mut ep,
...
unsafe fn sctp_generate_t1_cookie_event(*mut t)
{
	*mut asoc =
...
unsafe fn sctp_generate_t1_init_event(*mut t)
{
	*mut asoc =
		timer_container_of(asoc, t,
...
unsafe fn sctp_generate_t2_shutdown_event(*mut t)
{
	*mut asoc =
		timer_container_of(asoc, t,
...
unsafe fn sctp_generate_t4_rto_event(*mut t)
{
	*mut asoc =
...
unsafe fn sctp_generate_t5_shutdown_guard_event(*mut t)
{
	*mut asoc =
		timer_container_of(asoc, t,
...
unsafe fn sctp_generate_autoclose_event(*mut t)
{
	*mut asoc =
		timer_container_of(asoc, t,
...
unsafe fn sctp_generate_sack_event(*mut t)
{
	*mut asoc =
...
sctp_timer_event_t *sctp_timer_events[SCTP_NUM_TIMEOUT_TYPES] = {
	[SCTP_EVENT_TIMEOUT_NONE] =		core::ptr::null_mut(),
	...
	[SCTP_EVENT_TIMEOUT_T3_RTX] =		core::ptr::null_mut(),
	[SCTP_EVENT_TIMEOUT_T4_RTO] =		sctp_generate_t4_rto_event,
	[SCTP_EVENT_TIMEOUT_T5_SHUTDOWN_GUARD] =
					sctp_generate_t5_shutdown_guard_event,
	[SCTP_EVENT_TIMEOUT_HEARTBEAT] =	core::ptr::null_mut(),
	[SCTP_EVENT_TIMEOUT_RECONF] =		core::ptr::null_mut(),
 ...
 *
 */
unsafe fn sctp_do_8_2_transport_strike(*mut commands,
					 *mut asoc,
					 *mut transport,
					 int is_hb)
{
				 ...
				 unsigned int error)
{
	*mut event;
	...
	event = sctp_ulpevent_make_assoc_change(asoc, 0, SCTP_CANT_STR_ASSOC,
						(u16)error, 0, 0, core::ptr::null_mut(),
						GFP_ATOMIC);
...
unsafe fn sctp_cmd_assoc_failed(*mut commands,
				  *mut asoc,
 ...
 */
unsafe fn sctp_cmd_process_init(*mut commands,
				 *mut asoc,
				 *mut chunk,
				 *mut peer_init,
				 GfpT gfp)
...
unsafe fn sctp_cmd_hb_timers_start(*mut cmds,
				     *mut asoc)
{
	*mut t;
...
unsafe fn sctp_cmd_hb_timers_stop(*mut cmds,
				    *mut asoc)
{
	*mut t;
...
unsafe fn sctp_cmd_t3_rtx_timers_stop(*mut cmds,
					*mut asoc)
{
	*mut t;
...
unsafe fn sctp_cmd_transport_on(*mut cmds,
				  *mut asoc,
				  *mut t,
				  *mut chunk)
{
...
unsafe fn sctp_cmd_process_sack(*mut cmds,
				 *mut asoc,
				 *mut chunk)
{
	int err = 0;
	...
	if (sctp_outq_sack(&asoc.outqueue, chunk)) {
		...
		err = sctp_do_sm(asoc.base.net, SCTP_EVENT_T_OTHER,
				 ...
				 asoc.state, asoc.ep, asoc, core::ptr::null_mut(),
				 GFP_ATOMIC);
	}
 ...
 */
unsafe fn sctp_cmd_setup_t2(*mut cmds,
			      *mut asoc,
			      *mut chunk)
{
	*mut t;
...
unsafe fn sctp_cmd_new_state(*mut cmds,
			       *mut asoc,
			       sctp_state state)
{
	*mut sk = asoc.base.sk;
	...
	if (sctp_state(asoc, ESTABLISHED)) {
		...
		asoc.peer.cookie = core::ptr::null_mut();
	}
...
unsafe fn sctp_cmd_delete_tcb(*mut cmds,
				*mut asoc)
{
	*mut sk = asoc.base.sk;
 ...
 */
unsafe fn sctp_cmd_setup_t4(*mut cmds,
			      *mut asoc,
			      *mut chunk)
{
	*mut t;
...
unsafe fn sctp_cmd_process_operr(*mut cmds,
				   *mut asoc,
				   *mut chunk)
{
	*mut err_hdr;
 ...
 * transports.
 */
unsafe fn sctp_cmd_del_non_primary(*mut asoc)
{
	*mut t;
	*mut temp;
	*mut pos;
...
unsafe fn sctp_cmd_set_sk_err(*mut asoc, int error)
{
	*mut sk = asoc.base.sk;
				  ...
				  u8 state)
{
	*mut ev;
	...
	ev = sctp_ulpevent_make_assoc_change(asoc, 0, state, 0,
					    ...
					    core::ptr::null_mut(), GFP_ATOMIC);
	if (ev)
		asoc.stream.si.enqueue_event(&asoc.ulpq, ev);
}

unsafe fn sctp_cmd_peer_no_auth(*mut commands,
				  *mut asoc)
{
	*mut ev;
...
unsafe fn sctp_cmd_adaptation_ind(*mut commands,
				    *mut asoc)
{
	*mut ev;
...
unsafe fn sctp_cmd_t1_timer_update(*mut asoc,
				     sctp_event_timeout timer,
				     char *name)
{
	*mut t;
 ...
 */
unsafe fn sctp_cmd_send_msg(*mut asoc,
			      *mut msg, GfpT gfp)
{
	*mut chunk;
 ...
 * good place to start.
 */
unsafe fn sctp_do_sm(*mut net, sctp_event_type event_type,
	       sctp_subtype subtype, sctp_state state,
	       *mut ep, *mut asoc,
	       void *event_arg, GfpT gfp)
{
	typedef const char *(printfn_t)(sctp_subtype);
	static printfn_t *table[] = {
		core::ptr::null_mut(), sctp_cname, sctp_tname, sctp_oname, sctp_pname,
	};
	printfn_t *debug_fn   = table[event_type];
	const *mut state_fn;
...
unsafe fn sctp_cmd_interpreter(sctp_event_type event_type,
				...
				*mut commands,
				GfpT gfp)
{
	*mut sp = sctp_sk(ep.base.sk);
	*mut chunk = core::ptr::null_mut(), *new_obj;
	*mut packet;
	...
	*mut cmd;
	 ...
	 * --jgrimm
	 */
	while (core::ptr::null_mut() != (cmd = sctp_next_cmd(commands))) {
		switch (cmd.verb) {
		case SCTP_CMD_NOP:
			/* Do nothing. */
			break;
		...
		case SCTP_CMD_NEW_ASOC:
			...
			asoc = cmd.obj.asoc;
			BUG_ON(asoc.peer.primary_path == core::ptr::null_mut());
