...
unsafe fn netlbl_mgmt_add_common(info: *mut genl_info, audit_info: *mut netlbl_audit) -> c_int {
    ...
    match (*entry).def.r#type {
        case if case == NETLBL_NLTYPE_UNLABELED => (*entry).family = nla_get_u16_default((*info).attrs[NLBL_MGMT_A_FAMILY], AF_UNSPEC),
        case if case == NETLBL_NLTYPE_CIPSOV4 => {
        ...
        case if case == #[cfg(CONFIG_IPV6)] NETLBL_NLTYPE_CALIPSO => {
...
unsafe fn netlbl_mgmt_add(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
...
unsafe fn netlbl_mgmt_remove(_skb: *mut sk_buff, info: *mut genl_info) -> c_int {
...
unsafe fn netlbl_mgmt_listentry(skb: *mut sk_buff, entry: *mut netlbl_dom_map) -> c_int {
    let mut ret = 0;
...
unsafe fn netlbl_mgmt_listall_cb(entry: *mut netlbl_dom_map, arg: *mut c_void) -> c_int {
...
unsafe fn netlbl_mgmt_listall(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
...
unsafe fn netlbl_mgmt_listdef(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
...
unsafe fn netlbl_mgmt_adddef(skb: *mut sk_buff, info: *mut genl_info) -> c_int { netlbl_mgmt_add(skb, info) }
unsafe fn netlbl_mgmt_removedef(_skb: *mut sk_buff, _info: *mut genl_info) -> c_int { let mut a: netlbl_audit = core::mem::zeroed(); netlbl_netlink_auditinfo(&mut a); netlbl_domhsh_remove_default(AF_UNSPEC, &mut a) }

unsafe fn netlbl_mgmt_protocols_cb(skb: *mut sk_buff, cb: *mut netlink_callback, protocol: u32) -> c_int {
...
unsafe fn netlbl_mgmt_protocols(skb: *mut sk_buff, cb: *mut netlink_callback) -> c_int {
    let mut n = (*cb).args[0];
...
unsafe fn netlbl_mgmt_version(skb: *mut sk_buff, info: *mut genl_info) -> c_int {
