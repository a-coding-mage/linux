...
pub unsafe fn xp_assign_dev(pool: *mut xsk_buff_pool, netdev: *mut net_device, queue_id: u16, flags: u16) -> i32 {
    ...
    if (*pool).dma_pages.is_null() { WARN(1, "Driver did not DMA map zero-copy buffers"); err = -EINVAL; break 'err_unreg_xsk; } (*pool).umem.zc = true; (*pool).xdp_zc_max_segs = (*netdev).xdp_zc_max_segs; return 0;
    }
    xp_disable_drv_zc(pool); err_unreg_pool: if !force_zc { err = 0; } if err != 0 { xsk_clear_pool_at_qid(netdev, queue_id); dev_put(netdev); } err
}
