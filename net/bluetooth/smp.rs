...
// Literal source-level Rust translation of bluetooth/smp.c.
// External kernel, crypto, and Bluetooth symbols are intentionally unresolved here.
...
// #define SMP_DEV(hdev) \
// 	((*(smp_dev *)((l2cap_chan *)((*(hdev)).smp_data))).data)
 ...
 */
// #ifdef DEBUG
// #define SMP_DBG(fmt, ...) printk(c"\x017%s: ".as_ptr()fmt, __func__, \
// 				 ##__VA_ARGS__)
// #else
// #define SMP_DBG(fmt, ...) no_printk(c"\x017%s: ".as_ptr()fmt, __func__, \
// 				    ##__VA_ARGS__)
// #endif
...
pub const SMP_FLAG_TK_VALID: i32 = 0;
pub const SMP_FLAG_CFM_PENDING: i32 = SMP_FLAG_TK_VALID + 1;
pub const SMP_FLAG_MITM_AUTH: i32 = SMP_FLAG_CFM_PENDING + 1;
pub const SMP_FLAG_COMPLETE: i32 = SMP_FLAG_MITM_AUTH + 1;
pub const SMP_FLAG_INITIATOR: i32 = SMP_FLAG_COMPLETE + 1;
pub const SMP_FLAG_SC: i32 = SMP_FLAG_INITIATOR + 1;
pub const SMP_FLAG_REMOTE_PK: i32 = SMP_FLAG_SC + 1;
pub const SMP_FLAG_DEBUG_KEY: i32 = SMP_FLAG_REMOTE_PK + 1;
pub const SMP_FLAG_WAIT_USER: i32 = SMP_FLAG_DEBUG_KEY + 1;
pub const SMP_FLAG_DHKEY_PENDING: i32 = SMP_FLAG_WAIT_USER + 1;
pub const SMP_FLAG_REMOTE_OOB: i32 = SMP_FLAG_DHKEY_PENDING + 1;
pub const SMP_FLAG_LOCAL_OOB: i32 = SMP_FLAG_REMOTE_OOB + 1;
pub const SMP_FLAG_CT2: i32 = SMP_FLAG_LOCAL_OOB + 1;
...
unsafe fn swap_buf(const u8 *src, u8 *dst, len: usize)
{
	usize i;
...
unsafe fn smp_aes_cmac(const u8 k[16], const u8 *m, len: usize, u8 mac[16])
{
...
unsafe fn smp_f4(const u8 u[32], const u8 v[32], const u8 x[16], z: u8,
		  u8 res[16])
{
	u8 m[65];
	int err;
...
unsafe fn smp_f5(const u8 w[32], const u8 n1[16], const u8 n2[16],
...
unsafe fn smp_f6(const u8 w[16], const u8 n1[16], const u8 n2[16],
...
unsafe fn smp_g2(const u8 u[32], const u8 v[32], const u8 x[16],
...
unsafe fn smp_h6(const u8 w[16], const u8 key_id[4], u8 res[16])
{
	int err;
...
unsafe fn smp_h7(const u8 w[16], const u8 salt[16], u8 res[16])
{
	int err;
...
unsafe fn smp_e(const u8 *k, u8 *r)
{
	aes_enckey aes;
	u8 tmp[16], data[16];
	int err;
...
unsafe fn smp_c1(const u8 k[16],
		  const u8 r[16], const u8 preq[7], const u8 pres[7], _iat: u8,
		  const bdaddr_t *ia, _rat: u8, const bdaddr_t *ra, u8 res[16])
{
	u8 p1[16], p2[16];
	int err;
...
unsafe fn smp_s1(const u8 k[16],
...
unsafe fn smp_ah(const u8 irk[16], const u8 r[3], u8 res[3])
{
	u8 _res[16];
	int err;
...
bool smp_irk_matches(hci_dev *hdev, const u8 irk[16],
		     const bdaddr_t *bdaddr)
{
	l2cap_chan *chan = (*hdev).smp_data;
	u8 hash[3];
	int err;
...
int smp_generate_rpa(hci_dev *hdev, const u8 irk[16], bdaddr_t *rpa)
{
	l2cap_chan *chan = (*hdev).smp_data;
	int err;
...
int smp_generate_oob(hci_dev *hdev, u8 hash[16], u8 rand[16])
{
	l2cap_chan *chan = (*hdev).smp_data;
	smp_dev *smp;
	int err;
...
static void smp_send_cmd(l2cap_conn *conn, code: u8, len: u16, void *data)
{
	l2cap_chan *chan = (*conn).smp;
	smp_chan *smp;
	kvec iv[2];
	msghdr msg;
	...
	l2cap_chan_send(chan, &msg, 1 + len, core::ptr::null_mut());
...
static u8 authreq_to_seclevel(authreq: u8)
{
...
static void build_pairing_cmd(l2cap_conn *conn,
			      smp_cmd_pairing *req,
			      smp_cmd_pairing *rsp, __u8 authreq)
{
	l2cap_chan *chan = (*conn).smp;
	smp_chan *smp = (*chan).data;
...
static u8 check_enc_key_size(l2cap_conn *conn, __u8 max_key_size)
{
	l2cap_chan *chan = (*conn).smp;
	hci_dev *hdev = (*(*conn).hcon).hdev;
	smp_chan *smp = (*chan).data;
...
static void smp_chan_destroy(l2cap_conn *conn)
{
	l2cap_chan *chan = (*conn).smp;
	smp_chan *smp = (*chan).data;
	hci_conn *hcon = (*conn).hcon;
	...
	(*chan).data = core::ptr::null_mut();
	kfree_sensitive(smp);
	hci_conn_drop(hcon);
}

static void smp_failure(l2cap_conn *conn, reason: u8)
{
	hci_conn *hcon = (*conn).hcon;
	l2cap_chan *chan = (*conn).smp;
...
static u8 get_auth_method(smp_chan *smp, local_io: u8, remote_io: u8)
{
...
unsafe fn tk_request(l2cap_conn *conn, remote_oob: u8, auth: u8,
						local_io: u8, remote_io: u8)
{
	hci_conn *hcon = (*conn).hcon;
	l2cap_chan *chan = (*conn).smp;
	smp_chan *smp = (*chan).data;
...
static u8 smp_confirm(smp_chan *smp)
{
	l2cap_conn *conn = (*smp).conn;
	smp_cmd_pairing_confirm cp;
	int ret;
...
static u8 smp_random(smp_chan *smp)
{
	l2cap_conn *conn = (*smp).conn;
	hci_conn *hcon = (*conn).hcon;
	u8 confirm[16];
...
static void smp_notify_keys(l2cap_conn *conn)
{
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
	hci_conn *hcon = conn->hcon;
	hci_dev *hdev = hcon->hdev;
	smp_cmd_pairing *req = (void *) &smp->preq[1];
	smp_cmd_pairing *rsp = (void *) &smp->prsp[1];
	bool persistent;
...
static void sc_add_ltk(smp_chan *smp)
{
	hci_conn *hcon = smp->conn->hcon;
	key_type: u8, auth;
...
static void sc_generate_link_key(smp_chan *smp)
{
	...
	if (test_bit(SMP_FLAG_CT2, &smp->flags)) {
		...
		if (smp_h7(smp->tk, salt, smp->link_key)) {
			kfree_sensitive(smp->link_key);
			smp->link_key = core::ptr::null_mut();
			return;
		}
	} else {
...
static void smp_allow_key_dist(smp_chan *smp)
{
...
static void sc_generate_ltk(smp_chan *smp)
{
...
static void smp_distribute_keys(smp_chan *smp)
{
...
static void smp_timeout(work_*work)
{
	smp_chan *smp = container_of(work, smp_chan,
					    security_timer.work);
	l2cap_conn *conn = smp->conn;
...
static smp_chan *smp_chan_create(l2cap_conn *conn)
{
	'zfree_smp: {
	hci_conn *hcon = conn->hcon;
	l2cap_chan *chan = conn->smp;
...
unsafe fn sc_mackey_and_ltk(smp_chan *smp, u8 mackey[16], u8 ltk[16])
{
	hci_conn *hcon = smp->conn->hcon;
	u8 *na, *nb, a[7], b[7];
...
static void sc_dhkey_check(smp_chan *smp)
{
	hci_conn *hcon = smp->conn->hcon;
	smp_cmd_dhkey_check check;
...
static u8 sc_passkey_send_confirm(smp_chan *smp)
{
	l2cap_conn *conn = smp->conn;
	hci_conn *hcon = conn->hcon;
	smp_cmd_pairing_confirm cfm;
	u8 r;
...
static u8 sc_passkey_round(smp_chan *smp, smp_op: u8)
{
	l2cap_conn *conn = smp->conn;
	hci_conn *hcon = conn->hcon;
	hci_dev *hdev = hcon->hdev;
...
unsafe fn sc_user_reply(smp_chan *smp, mgmt_op: u16, __le32 passkey)
{
	l2cap_conn *conn = smp->conn;
	hci_conn *hcon = conn->hcon;
	u8 smp_op;
...
int smp_user_confirm_reply(hci_conn *hcon, mgmt_op: u16, __le32 passkey)
{
	'unlock: {
...
static void build_bredr_pairing_cmd(smp_chan *smp,
				    smp_cmd_pairing *req,
				    smp_cmd_pairing *rsp)
{
	l2cap_conn *conn = smp->conn;
...
static u8 smp_cmd_pairing_req(l2cap_conn *conn, sk_buff *skb)
{
	smp_cmd_pairing rsp, *req = (void *) skb->data;
	l2cap_chan *chan = conn->smp;
	hci_dev *hdev = conn->hcon->hdev;
	smp_chan *smp = chan->data;
	key_size: u8, auth, sec_level;
	int ret;
...
static u8 sc_send_public_key(smp_chan *smp)
{
	'done: {
	hci_dev *hdev = smp->conn->hcon->hdev;
...
static u8 smp_cmd_pairing_rsp(l2cap_conn *conn, sk_buff *skb)
{
	smp_cmd_pairing *req, *rsp = (void *) skb->data;
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
	hci_dev *hdev = conn->hcon->hdev;
	key_size: u8, auth;
...
static u8 sc_check_confirm(smp_chan *smp)
{
	l2cap_conn *conn = smp->conn;
 ...
 */
unsafe fn fixup_sc_false_positive(smp_chan *smp)
{
	l2cap_conn *conn = smp->conn;
	hci_conn *hcon = conn->hcon;
	hci_dev *hdev = hcon->hdev;
...
static u8 smp_cmd_pairing_confirm(l2cap_conn *conn, sk_buff *skb)
{
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
	hci_conn *hcon = conn->hcon;
...
static u8 smp_cmd_pairing_random(l2cap_conn *conn, sk_buff *skb)
{
	'mackey_and_ltk: {
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
...
unsafe fn smp_ltk_encrypt(l2cap_conn *conn, sec_level: u8)
{
	smp_ltk *key;
	hci_conn *hcon = conn->hcon;
...
bool smp_sufficient_security(hci_conn *hcon, sec_level: u8,
			     smp_key_pref key_pref)
{
...
static void smp_send_pairing_req(smp_chan *smp, __u8 auth)
{
	smp_cmd_pairing cp;

	if (smp->conn->hcon->type == ACL_LINK)
		build_bredr_pairing_cmd(smp, &cp, core::ptr::null_mut());
	else
		build_pairing_cmd(smp->conn, &cp, core::ptr::null_mut(), auth);
...
static u8 smp_cmd_security_req(l2cap_conn *conn, sk_buff *skb)
{
	smp_cmd_security_req *rp = (void *) skb->data;
	hci_conn *hcon = conn->hcon;
	hci_dev *hdev = hcon->hdev;
	smp_chan *smp;
	sec_level: u8, auth;
...
static void smp_send_security_req(smp_chan *smp, __u8 auth)
{
	smp_cmd_security_req cp;
...
int smp_conn_security(hci_conn *hcon, __u8 sec_level)
{
	'unlock: {
	l2cap_conn *conn;
	l2cap_chan *chan;
	...
	/* This may be core::ptr::null_mut() if there's an unexpected disconnection */
	if (!conn)
		return 1;
...
int smp_cancel_and_remove_pairing(hci_dev *hdev, bdaddr_t *bdaddr,
				  addr_type: u8)
{
	'done: {
	hci_conn *hcon;
	...
	smp = chan->data;
	if (smp) {
		/* Set keys to core::ptr::null_mut() to make sure smp_failure() does not try to
		 * remove and free already invalidated rcu list entries. */
		smp->ltk = core::ptr::null_mut();
		smp->responder_ltk = core::ptr::null_mut();
		smp->remote_irk = core::ptr::null_mut();
...
unsafe fn smp_cmd_encrypt_info(l2cap_conn *conn, sk_buff *skb)
{
...
unsafe fn smp_cmd_initiator_ident(l2cap_conn *conn, sk_buff *skb)
{
	smp_cmd_initiator_ident *rp = (void *)skb->data;
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
	hci_dev *hdev = conn->hcon->hdev;
	hci_conn *hcon = conn->hcon;
...
unsafe fn smp_cmd_ident_info(l2cap_conn *conn, sk_buff *skb)
{
	smp_cmd_ident_info *info = (void *) skb->data;
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
...
unsafe fn smp_cmd_ident_addr_info(l2cap_conn *conn,
				   sk_buff *skb)
{
	'distribute: {
	smp_cmd_ident_addr_info *info = (void *) skb->data;
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
	hci_conn *hcon = conn->hcon;
	bdaddr_t rpa;
...
unsafe fn smp_cmd_sign_info(l2cap_conn *conn, sk_buff *skb)
{
	smp_cmd_sign_info *rp = (void *) skb->data;
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
	smp_csrk *csrk;
...
static u8 sc_select_method(smp_chan *smp)
{
	smp_cmd_pairing *local, *remote;
	local_mitm: u8, remote_mitm, local_io, remote_io, method;
	 ...
	 * the "smp_cmd_pairing" from them we need to skip the
...
unsafe fn smp_cmd_public_key(l2cap_conn *conn, sk_buff *skb)
{
	smp_cmd_public_key *key = (void *) skb->data;
	hci_conn *hcon = conn->hcon;
	l2cap_chan *chan = conn->smp;
	smp_chan *smp = chan->data;
	hci_dev *hdev = hcon->hdev;
	 ...
	 */
	if (test_bit(SMP_FLAG_LOCAL_OOB, &smp->flags)) {
		l2cap_chan *hchan = hdev->smp_data;
		smp_dev *smp_dev;
...
unsafe fn smp_cmd_dhkey_check(l2cap_conn *conn, sk_buff *skb)
{
	smp_cmd_dhkey_check *check = (void *) skb->data;
	l2cap_chan *chan = conn->smp;
	hci_conn *hcon = conn->hcon;
	smp_chan *smp = chan->data;
	u8 a[7], b[7], *local_addr, *remote_addr;
...
unsafe fn smp_cmd_keypress_notify(l2cap_conn *conn,
				   sk_buff *skb)
{
	smp_cmd_keypress_notify *kp = (void *) skb->data;
...
unsafe fn smp_sig_channel(l2cap_chan *chan, sk_buff *skb)
{
	'drop: {
	'done: {
...
static void smp_teardown_cb(l2cap_chan *chan, int err)
{
	l2cap_conn *conn = chan->conn;
	...
	conn->smp = core::ptr::null_mut();
	l2cap_chan_put(chan);
}

static void bredr_pairing(l2cap_chan *chan)
{
	l2cap_conn *conn = chan->conn;
	hci_conn *hcon = conn->hcon;
	hci_dev *hdev = hcon->hdev;
...
static void smp_resume_cb(l2cap_chan *chan)
{
	smp_chan *smp = chan->data;
	l2cap_conn *conn = chan->conn;
	hci_conn *hcon = conn->hcon;
...
static void smp_ready_cb(l2cap_chan *chan)
{
	l2cap_conn *conn = chan->conn;
	hci_conn *hcon = conn->hcon;
...
unsafe fn smp_recv_cb(l2cap_chan *chan, sk_buff *skb)
{
	int err;
...
static const l2cap_ops smp_chan_ops = {
...
unsafe fn int smp_new_conn_cb(l2cap_chan *chan,
				  l2cap_chan *new_chan)
{
...
static const l2cap_ops smp_root_chan_ops = {
...
static l2cap_chan *smp_add_cid(hci_dev *hdev, cid: u16)
{
	'create_chan: {
	l2cap_chan *chan;
	smp_dev *smp;
	...
	l2cap_chan_set_defaults(chan, core::ptr::null_mut());
...
static void smp_del_chan(l2cap_chan *chan)
{
	smp_dev *smp;
...
int smp_force_bredr(hci_dev *hdev, enable: bool)
{
	...
	if (enable) {
		...
		hdev->smp_bredr_data = core::ptr::null_mut();
		smp_del_chan(chan);
	}
...
int smp_register(hci_dev *hdev)
{
	...
	if (WARN_ON(hdev->smp_data)) {
		chan = hdev->smp_data;
		hdev->smp_data = core::ptr::null_mut();
		smp_del_chan(chan);
	}
...
void smp_unregister(hci_dev *hdev)
{
	l2cap_chan *chan;
...
unsafe fn __init test_debug_key(crypto_kpp *tfm_ecdh)
{
	u8 pk[64];
	int err;
...
static susize test_smp_read(file *file, char __user *user_buf,
			     count: usize, loff_t *ppos)
{
	return simple_read_from_buffer(user_buf, count, ppos, test_smp_buffer,
				       strlen(test_smp_buffer));
}

static const file_operations test_smp_fops = {
	open: simple_open,
	read: test_smp_read,
	llseek: default_llseek,
};

unsafe fn __init run_selftests(crypto_kpp *tfm_ecdh)
{
	'done: {
	...
	debugfs_create_file("selftest_smp", 0444, bt_debugfs, core::ptr::null_mut(),
			    &test_smp_fops);
