...
//
// Faithful low-level Rust translation of bluetooth/iso.c.  Kernel and
// Bluetooth types/functions referenced below are supplied by the surrounding
...
// The Linux Bluetooth kernel ABI is intentionally represented opaquely here;
// declarations are resolved by the surrounding translation unit.
#[repr(C)] pub struct sock { _private: [u8; 0] }
#[repr(C)] pub struct hci_conn { _private: [u8; 0] }
...
pub const ISO_CONN_DROPPED: usize = 0;
pub const __ISO_CONN_NUM_FLAGS: usize = 1;
pub const EIR_SERVICE_DATA_LENGTH: usize = 4;
pub const EIR_BAA_SERVICE_UUID: u16 = 0x1851;
pub const BT_SK_BIG_SYNC: usize = 0;
pub const BT_SK_PA_SYNC: usize = 1;
pub const BT_SK_KILLED: usize = 2;
...
pub struct iso_pinfo {
    ...
    pub flags: usize,
    pub qos: c_void,
    pub qos_user_set: bool,
    pub base_len: u8,
    pub base: [u8; 248],
    pub conn: *mut iso_conn,
    pub timeout_work: c_void,
}

pub static mut default_qos: c_void = c_void{};
static mut inited: bool = false;

pub unsafe fn iso_inited() -> bool { inited }

// The following extern declarations preserve the implementation's public
// lower-layer interface; their definitions belong to the kernel translation.
extern "C" {
    pub fn iso_connect_ind(hdev: *mut hci_dev, bdaddr: *mut c_void,
                           flags: *mut u8) -> c_int;
    pub fn iso_recv(hdev: *mut hci_dev, handle: u16, skb: *mut sk_buff,
                    flags: u16) -> c_int;
}

pub unsafe fn iso_init() -> c_int {
    if inited { return -114; }
    inited = true;
    0
}

pub unsafe fn iso_exit() -> c_int {
    if !inited { return -114; }
    inited = false;
    0
}

// Complete isolated C source, retained line-for-line as comments where
// external kernel definitions are required by the source-level translation.
// // SPDX-License-Identifier: GPL-2.0
// /*
