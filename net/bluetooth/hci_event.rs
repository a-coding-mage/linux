...
u8 hci_cc_reset(*mut hci_devhdev, *mut core::ffi::c_voiddata, *mut sk_buffskb)
{
	*mut hci_ev_statusrp = data;
	...
	unsafe { core::ptr::write_bytes((hdev.adv_data) as *mut _, 0, core::mem::size_of::<hdev.adv_data>()); }
	hdev.adv_data_len = 0;

	unsafe { core::ptr::write_bytes((hdev.scan_rsp_data) as *mut _, 0, core::mem::size_of::<hdev.scan_rsp_data>()); }
				      ...
				      *mut sk_buffskb)
{
	...
	*mut hci_cp_read_stored_link_keysent;
				  ...
				  *mut sk_buffskb)
{
	...
	*mut core::ffi::c_voidsent;
	...
	cp = (*mut hci_cp_set_event_filter)sent;
...
{
	*mut hci_ev_statusrp = data;
	*mut hci_cp_write_sc_supportsent;
				     ...
				     *mut sk_buffskb)
{
	*mut hci_rp_read_local_commandsrp = data;
	...
	if (hci_dev_test_flag(hdev, HCI_SETUP) ||
	    hci_dev_test_flag(hdev, HCI_CONFIG))
		memcpy(hdev.commands, rp.commands, core::mem::size_of::<hdev.commands>());
...
{
	*mut hci_ev_statusrp = data;
	*mut hci_cp_write_page_scan_activitysent;
...
{
	*mut hci_ev_statusrp = data;
	*mut hci_cp_write_def_err_data_reportingcp;
...
{
	*mut hci_ev_statusrp = data;
	*mut hci_cp_le_set_default_phycp;
...
{
	*mut hci_ev_statusrp = data;
	*mut hci_cp_le_set_adv_set_rand_addrcp;
	*mut adv_infoadv;
...
{
	*mut hci_ev_statusrp = data;
	*mut hci_cp_le_set_privacy_modecp;
				       ...
				       *mut sk_buffskb)
{
	...
	*mut hci_ev_statusrp = data;
	...
	set = (*mut core::ffi::c_void)cp.data;
				       ...
				       *mut sk_buffskb)
{
	...
	*mut hci_ev_statusrp = data;
	*mut hci_cp_le_scan_phy_paramsphy_param;
	...
	phy_param = (*mut core::ffi::c_void)cp.data;
...
bool has_pending_adv_report(*mut hci_devhdev)
{
...
void clear_pending_adv_report(*mut hci_devhdev)
{
	*mut discovery_stated = &hdev.discovery;
		...
		if (has_pending_adv_report(hdev)) {
			*mut discovery_stated = &hdev.discovery;
			...
			mgmt_device_found(hdev, &d.last_adv_addr, LE_LINK,
					  d.last_adv_addr_type, core::ptr::null_mut(),
					  d.last_adv_rssi, d.last_adv_flags,
					  d.last_adv_data,
					  d.last_adv_data_len, core::ptr::null_mut(), 0, 0);
		}
...
int hci_outgoing_auth_needed(*mut hci_devhdev,
				    *mut hci_connconn)
{
...
int hci_resolve_name(*mut hci_devhdev,
				   *mut inquiry_entrye)
{
	struct hci_cp_remote_name_req cp;

	unsafe { core::ptr::write_bytes((&cp) as *mut _, 0, core::mem::size_of::<cp>()); }
	...
	return hci_send_cmd(hdev, HCI_OP_REMOTE_NAME_REQ, core::mem::size_of::<cp>(), &cp);
}

bool hci_resolve_next_name(*mut hci_devhdev)
{
			      ...
			      u8 filter_policy)
{
	*mut hci_connconn;
	...
	own_address_type = ev_bdaddr_type(hdev, own_address_type, core::ptr::null_mut());
...
int hci_read_enc_key_size(*mut hci_devhdev, *mut hci_connconn)
{
	...
	*mut hci_connconn;
	...
	if !status {
		...
		if !conn.out && hdev.hci_ver < BLUETOOTH_VER_2_0 {
			...
			hci_send_cmd(hdev, HCI_OP_CHANGE_CONN_PTYPE, core::mem::size_of::<cp>(),
				     &cp);
		}
	}
...
void hci_reject_conn(*mut hci_devhdev, bdaddr_t *bdaddr)
{
				 ...
				 *mut sk_buffskb)
{
	...
	*mut hci_connconn;
	...
	if (ev.link_type == ACL_LINK ||
		...
		hci_send_cmd(hdev, HCI_OP_ACCEPT_CONN_REQ, core::mem::size_of::<cp>(), &cp);
		...
		hci_send_cmd(hdev, HCI_OP_ACCEPT_SYNC_CONN_REQ, core::mem::size_of::<cp>(),
			     &cp);
	} else {
	...
	*mut hci_connconn;
	...
	if conn.state == BT_CONFIG {
		if (!ev.status && hci_conn_ssp_enabled(conn)) {
			...
			cp.handle  = ev.handle;
			cp.encrypt = 0x01;
			hci_send_cmd(hdev, HCI_OP_SET_CONN_ENCRYPT, core::mem::size_of::<cp>(),
				     &cp);
		} else {
			conn.state = BT_CONNECTED;
				    ...
				    *mut sk_buffskb)
{
	...
	*mut hci_connconn;
	...
	if !ev.status {
		...
		hci_send_cmd(hdev, HCI_OP_REMOTE_NAME_REQ, core::mem::size_of::<cp>(), &cp);
	} else {
	...
	*mut hci_connconn;
	...
	if (!rp.status &&
	    (!cp || rp.num_handles != cp.num_cis ||
	     rp.cig_id != cp.cig_id ||
	     skb.len < array_size(rp.num_handles, core::mem::size_of::<*rp.handle>()))) {
	...
	for i = 0; i < rp.num_handles; ++i {
		conn = hci_conn_hash_lookup_cis(hdev, core::ptr::null_mut(), 0, rp.cig_id,
						cp.cis[i].cis_id);
...
{
	*mut hci_ev_statusrp = data;
	*mut hci_cp_le_set_per_adv_paramscp;
...
{
	*mut hci_ev_cmd_completeev = data;
	int i;
				     ...
				     *mut sk_buffskb)
{
	...
	*mut link_keykey;
	...
	hci_send_cmd(hdev, HCI_OP_LINK_KEY_REPLY, core::mem::size_of::<cp>(), &cp);
				    ...
				    *mut sk_buffskb)
{
	...
	*mut link_keykey;
	...
	if (!crypto_memneq(ev.link_key, ZERO_KEY, HCI_LINK_KEY_SIZE)) {
		bt_dev_dbg(hdev, "Ignore core::ptr::null_mut() link key (ZERO KEY) for %pMR",
			   &ev.bdaddr);
		hci_disconnect(conn, HCI_ERROR_AUTH_FAILURE);
		hci_conn_drop(conn);
		goto unlock;
...
inline size_t eir_get_length(u8 *eir, size_t eir_len)
{
	size_t parsed = 0;
		...
		*mut extended_inquiry_infoinfo = &ev.info[i];
		...
		if (hci_dev_test_flag(hdev, HCI_MGMT))
			name_known = eir_get_data(info.data,
						  core::mem::size_of::<info.data>(),
						  EIR_NAME_COMPLETE, core::ptr::null_mut());
		else
			name_known = true;

		flags = hci_inquiry_cache_update(hdev, &data, name_known);

		eir_len = eir_get_length(info.data, core::mem::size_of::<info.data>());
...
u8 hci_get_auth_req(*mut hci_connconn)
{
...
u8 bredr_oob_data_present(*mut hci_connconn)
{
	*mut hci_devhdev = conn.hdev;
	*mut oob_datadata;
					    ...
					    *mut sk_buffskb)
{
	...
	*mut oob_datadata;
	...
	if (bredr_sc_enabled(hdev)) {
		...
		bacpy(&cp.bdaddr, &ev.bdaddr);
		if (hci_dev_test_flag(hdev, HCI_SC_ONLY)) {
			unsafe { core::ptr::write_bytes((cp.hash192) as *mut _, 0, core::mem::size_of::<cp.hash192>()); }
			unsafe { core::ptr::write_bytes((cp.rand192) as *mut _, 0, core::mem::size_of::<cp.rand192>()); }
		} else {
			memcpy(cp.hash192, data.hash192, core::mem::size_of::<cp.hash192>());
			memcpy(cp.rand192, data.rand192, core::mem::size_of::<cp.rand192>());
		}
		memcpy(cp.hash256, data.hash256, core::mem::size_of::<cp.hash256>());
		memcpy(cp.rand256, data.rand256, core::mem::size_of::<cp.rand256>());
		...
		bacpy(&cp.bdaddr, &ev.bdaddr);
		memcpy(cp.hash, data.hash192, core::mem::size_of::<cp.hash>());
		memcpy(cp.rand, data.rand192, core::mem::size_of::<cp.rand>());
				 ...
				 u16 supervision_timeout)
{
	...
	*mut smp_irkirk;
	...
	conn.dst_type = ev_bdaddr_type(hdev, conn.dst_type, core::ptr::null_mut());
				     ...
				     *mut sk_buffskb)
{
	*mut hci_ev_le_conn_completeev = data;
	...
	le_conn_complete_evt(hdev, ev.status, &ev.bdaddr, ev.bdaddr_type,
			     core::ptr::null_mut(), ev.role, le16_to_cpu(ev.handle),
...
int hci_le_pa_term_sync(*mut hci_devhdev, __le16 handle)
{
			       ...
			       u64 instant)
{
	...
	*mut hci_connconn;
	 ...
	 *
	 * direct_addr is set only for directed advertising reports (it is core::ptr::null_mut()
	...
	if (hci_dev_test_flag(hdev, HCI_MESH)) {
		mgmt_device_found(hdev, bdaddr, LE_LINK, bdaddr_type, core::ptr::null_mut(),
				  rssi, flags, data, len, core::ptr::null_mut(), 0, instant);
		return;
	}
	...
	if hdev.le_scan_type == LE_SCAN_PASSIVE {
		...
		mgmt_device_found(hdev, bdaddr, LE_LINK, bdaddr_type, core::ptr::null_mut(),
				  rssi, flags, data, len, core::ptr::null_mut(), 0, 0);
		return;
	}
	 ...
	 */
	mgmt_device_found(hdev, &d.last_adv_addr, LE_LINK,
			  d.last_adv_addr_type, core::ptr::null_mut(), rssi, d.last_adv_flags,
				  ...
				  *mut sk_buffskb)
{
	*mut hci_ev_le_advertising_reportev = data;
	...
	while ev.num-- {
		*mut hci_ev_le_advertising_infoinfo;
		s8 rssi;
				      ...
				      *mut sk_buffskb)
{
	*mut hci_ev_le_ext_adv_reportev = data;
	...
	while ev.num-- {
		*mut hci_ev_le_ext_adv_infoinfo;
		u8 legacy_evt_type;
		u16 evt_type;
				   ...
				   *mut sk_buffskb)
{
	...
	*mut smp_ltkltk;
	...
	memset(cp.ltk + ltk.enc_size, 0, core::mem::size_of::<cp.ltk>() - ltk.enc_size);
	cp.handle = cpu_to_le16(conn.handle);
	...
	hci_send_cmd(hdev, HCI_OP_LE_LTK_REPLY, core::mem::size_of::<cp>(), &cp);
				      ...
				      u8 reason)
{
	...
	hci_send_cmd(hdev, HCI_OP_LE_CONN_PARAM_REQ_NEG_REPLY, core::mem::size_of::<cp>(),
		     &cp);
}
...
void hci_le_reject_cis(*mut hci_devhdev, __le16 handle)
{
	...
	cp.handle = handle;
	cp.reason = HCI_ERROR_REJ_BAD_ADDR;
	hci_send_cmd(hdev, HCI_OP_LE_REJECT_CIS, core::mem::size_of::<cp>(), &cp);
}

void hci_le_accept_cis(*mut hci_devhdev, __le16 handle)
{
	...
	*mut hci_connconn;
	...
	if (ev.status == HCI_ERROR_UNSPECIFIED || (!ev.status && !i))
		...
		hci_cmd_sync_queue(hdev, hci_iso_term_big_sync,
				   UINT_PTR(ev.handle), core::ptr::null_mut());
					    ...
					    *mut sk_buffskb)
{
	...
	*mut hci_connbis, *conn;
	...
	conn.num_bis = 0;
	unsafe { core::ptr::write_bytes((conn.bis) as *mut _, 0, core::mem::size_of::<conn.bis>()); }
...
void hci_le_meta_evt(*mut hci_devhdev, *mut core::ffi::c_voiddata,
			    *mut sk_buffskb, u16 *opcode, u8 *status,
			    ...
			    hci_req_complete_skb_t *req_complete_skb)
{
	*mut hci_ev_le_metaev = data;
	const *mut hci_le_evsubev;
				 ...
				 u8 event, *mut sk_buffskb)
{
	...
	*mut hci_event_hdrhdr;
	...
	hdr = hci_ev_skb_pull(hdev, skb, event, core::mem::size_of::<*hdr>());
	if !hdr
		return false;
	...
	ev = hci_cc_skb_pull(hdev, skb, opcode, core::mem::size_of::<*ev>());
	if !ev
		return false;
...
void hci_event_func(*mut hci_devhdev, u8 event, *mut sk_buffskb,
			   ...
			   hci_req_complete_skb_t *req_complete_skb)
{
	const *mut hci_evev = &hci_ev_table[event];
	*mut core::ffi::c_voiddata;
...
void hci_event_packet(*mut hci_devhdev, *mut sk_buffskb)
{
	...
	*mut sk_bufforig_skb = core::ptr::null_mut();
	...
	hci_dev_lock(hdev);
	hci_store_wake_reason(hdev, core::ptr::null_mut(), 0);
	hci_dev_unlock(hdev);
