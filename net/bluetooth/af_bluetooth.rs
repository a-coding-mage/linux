...
extern "C" {
    ...
    fn sock_register(_: *const net_proto_family) -> c_int; fn sock_unregister(_: c_int);
}
...
extern "C" { fn write_lock(_: *mut rwlock_t); fn write_unlock(_: *mut rwlock_t); fn read_lock(_: *mut rwlock_t); fn read_unlock(_: *mut rwlock_t); fn sock_lock_init_class_and_name(_: *mut sock,_: *const u8,_: *mut lock_class_key,_: *const u8,_: *mut lock_class_key); fn sock_allow_reclassification(_: *mut sock)->bool; fn sk_alloc(_: *mut net,_: c_int, _: c_ulong,_: *mut proto,_: c_int)->*mut sock; fn sock_init_data(_: *mut socket,_: *mut sock); fn sock_reset_flag(_: *mut sock,_: c_int); fn sk_add_node(_: *mut sock,_: *mut hlist_head); fn sk_del_node_init(_: *mut sock); fn sock_hold(_: *mut sock); fn sock_put(_: *mut sock); fn lock_sock(_: *mut sock); fn release_sock(_: *mut sock); fn spin_lock_bh(_: *mut spinlock_t); fn spin_unlock_bh(_: *mut spinlock_t); fn sock_error(_: *mut sock)->c_int; fn sock_rcvlowat(_: *mut sock,_: bool,_: usize)->usize; fn sock_rcvtimeo(_: *mut sock,_: bool)->c_long; fn signal_pending(_: *mut c_void)->bool; }
...
#[cfg(CONFIG_PROC_FS)] pub unsafe extern "C" fn bt_procfs_init(_net:*mut net,_name:*const c_char,sk_list:*mut bt_sock_list,_show:*mut c_void)->c_int { (*sk_list).custom_seq_show=None; 0 }
#[cfg(not(CONFIG_PROC_FS))] pub unsafe extern "C" fn bt_procfs_init(_net:*mut net,_name:*const c_char,_sk_list:*mut bt_sock_list,_show:*mut c_void)->c_int { 0 }
