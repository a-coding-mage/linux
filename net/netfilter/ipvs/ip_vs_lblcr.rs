...
unsafe fn ip_vs_dest_set_min(set: *mut ip_vs_dest_set) -> *mut ip_vs_dest {
    ...
    list_for_each_entry_rcu!(e, (*set).list, list, { least = (*e).dest;
        ...
        if atomic_read(&(*least).weight) > 0 && (*least).cflags & IP_VS_DEST_CF_AVAILABLE != 0 { loh = ip_vs_dest_conn_overhead(least); break 'nextstage; }
    }); return ptr::null_mut();
    }
    list_for_each_entry_continue_rcu!(e, (*set).list, list, { dest = (*e).dest;
...
unsafe fn ip_vs_dest_set_max(set: *mut ip_vs_dest_set) -> *mut ip_vs_dest {
    ...
    list_for_each_entry!(e, (*set).list, list, { most = (*e).dest; if atomic_read(&(*most).weight) > 0 { moh = ip_vs_dest_conn_overhead(most); break 'nextstage; } });
    return ptr::null_mut();
    }
    list_for_each_entry_continue!(e, (*set).list, list, { dest = (*e).dest; doh = ip_vs_dest_conn_overhead(dest);
