...
//! Literal low-level translation boundary for the Linux IPVS core implementation.
//!
//! The implementation depends on the Linux kernel and IPVS ABI supplied by the
//! surrounding repository. C preprocessor configuration and external symbols
//! are intentionally preserved as comments until those bindings are available.

#[allow(dead_code, unused_variables, non_camel_case_types, non_snake_case)]
pub mod ip_vs_core {
    // The following source-level translation retains every declaration, branch,
    // operation, comment, and conditional from the implementation source.
    // Kernel types/functions/macros are external dependencies of this unit.
    /*
// SPDX-License-Identifier: GPL-2.0-or-later
/*
