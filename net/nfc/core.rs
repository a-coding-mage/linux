...
pub unsafe fn nfc_fw_download(dev: *mut nfc_dev, firmware_name: *const i8) -> i32 {
    ...
    if (*dev).shutting_down { rc = -ENODEV; break 'error; }
    if (*dev).dev_up { rc = -EBUSY; break 'error; }
    if (*(*dev).ops).fw_download.is_none() { rc = -EOPNOTSUPP; break 'error; }
    (*dev).fw_download_in_progress = true;
    rc = ((*(*dev).ops).fw_download.unwrap())(dev, firmware_name);
    if rc != 0 { (*dev).fw_download_in_progress = false; }
    }
    device_unlock(&mut (*dev).dev); rc
}
...
pub unsafe fn nfc_dev_up(dev: *mut nfc_dev) -> i32 {
    ...
    device_lock(&mut (*dev).dev);
    if (*dev).shutting_down { rc = -ENODEV; break 'error; }
    if !(*dev).rfkill.is_null() && rfkill_blocked((*dev).rfkill) { rc = -ERFKILL; break 'error; }
    if (*dev).fw_download_in_progress { rc = -EBUSY; break 'error; }
    if (*dev).dev_up { rc = -EALREADY; break 'error; }
...
pub unsafe fn nfc_dev_down(dev: *mut nfc_dev) -> i32 {
    ...
    if (*dev).shutting_down { rc = -ENODEV; break 'error; }
    if !(*dev).dev_up { rc = -EALREADY; break 'error; }
    if (*dev).polling || !(*dev).active_target.is_null() { rc = -EBUSY; break 'error; }
...
pub unsafe fn nfc_start_poll(dev: *mut nfc_dev, im_protocols: u32, tm_protocols: u32) -> i32 {
    ...
    device_lock(&mut (*dev).dev);
    if (*dev).shutting_down { rc = -ENODEV; break 'error; }
    if !(*dev).dev_up { rc = -ENODEV; break 'error; }
    if (*dev).polling { rc = -EBUSY; break 'error; }
...
pub unsafe fn nfc_stop_poll(dev: *mut nfc_dev) -> i32 {
    ...
    device_lock(&mut (*dev).dev);
    if (*dev).shutting_down { rc = -ENODEV; break 'error; }
    if !(*dev).polling { rc = -EINVAL; break 'error; }
...
pub unsafe fn nfc_dep_link_up(dev: *mut nfc_dev, target_index: i32, comm_mode: u8) -> i32 {
    ...
    device_lock(&mut (*dev).dev);
    if (*dev).shutting_down { rc = -ENODEV; break 'error; }
    if (*dev).dep_link_up { rc = -EALREADY; break 'error; }
    gb = nfc_llcp_general_bytes(dev, &mut gb_len);
    if gb_len > NFC_MAX_GT_LEN as usize { rc = -EINVAL; break 'error; }
    let target = nfc_find_target(dev, target_index as u32); if target.is_null() { rc = -ENOTCONN; break 'error; }
...
pub unsafe fn nfc_activate_target(dev: *mut nfc_dev, target_idx: u32, protocol: u32) -> i32 {
    let mut rc;
    'error: { pr_debug!("dev_name={} target_idx={} protocol={}\\n", dev_name(&(*dev).dev), target_idx, protocol); device_lock(&mut (*dev).dev);
    if (*dev).shutting_down { rc=-ENODEV; break 'error; } if !(*dev).active_target.is_null() { rc=-EBUSY; break 'error; }
    let target=nfc_find_target(dev,target_idx); if target.is_null(){rc=-ENOTCONN;break 'error;}
...
pub unsafe fn nfc_deactivate_target(dev: *mut nfc_dev, target_idx: u32, mode: u8) -> i32 {
    let mut rc=0;
    'error: { pr_debug!("dev_name={} target_idx={}\\n",dev_name(&(*dev).dev),target_idx); device_lock(&mut (*dev).dev);
    if (*dev).shutting_down {rc=-ENODEV;break 'error;} if (*dev).active_target.is_null(){rc=-ENOTCONN;break 'error;} if (*(*dev).active_target).idx!=target_idx{rc=-ENOTCONN;break 'error;}
...
pub unsafe fn nfc_data_exchange(dev:*mut nfc_dev,target_idx:u32,skb:*mut sk_buff,cb:data_exchange_cb_t,cb_context:*mut core::ffi::c_void)->i32{
 let mut rc;
 'error: { device_lock(&mut (*dev).dev); if (*dev).shutting_down{rc=-ENODEV;kfree_skb(skb);break 'error;}
 if (*dev).rf_mode==NFC_RF_INITIATOR&&!(*dev).active_target.is_null(){if (*(*dev).active_target).idx!=target_idx{rc=-EADDRNOTAVAIL;kfree_skb(skb);break 'error;} if (*dev).ops.check_presence.is_some(){timer_delete_sync(&mut (*dev).check_pres_timer);} rc=((*(*dev).ops).im_transceive.unwrap())(dev,(*dev).active_target,skb,cb,cb_context);
 }else if (*dev).rf_mode==NFC_RF_TARGET&&(*dev).ops.tm_send.is_some(){rc=((*(*dev).ops).tm_send.unwrap())(dev,skb);}else{rc=-ENOTCONN;kfree_skb(skb);break 'error;}
 }
 device_unlock(&mut (*dev).dev);rc
}
...
pub unsafe fn nfc_enable_se(dev:*mut nfc_dev,se_idx:u32)->i32{let mut rc;device_lock(&mut (*dev).dev);if (*dev).shutting_down{rc=-ENODEV;goto error;}if !(*dev).dev_up{rc=-ENODEV;goto error;}if (*dev).polling{rc=-EBUSY;goto error;}if (*dev).ops.enable_se.is_none()||(*dev).ops.disable_se.is_none(){rc=-EOPNOTSUPP;goto error;}let se=nfc_find_se(dev,se_idx);if se.is_null(){rc=-EINVAL;goto error;}if (*se).state==NFC_SE_ENABLED{rc=-EALREADY;goto error;}rc=((*dev).ops.enable_se.unwrap())(dev,se_idx);if rc>=0{(*se).state=NFC_SE_ENABLED;}error:device_unlock(&mut (*dev).dev);rc}

pub unsafe fn nfc_disable_se(dev:*mut nfc_dev,se_idx:u32)->i32{let mut rc;device_lock(&mut (*dev).dev);if (*dev).shutting_down{rc=-ENODEV;goto error;}if !(*dev).dev_up{rc=-ENODEV;goto error;}if (*dev).ops.enable_se.is_none()||(*dev).ops.disable_se.is_none(){rc=-EOPNOTSUPP;goto error;}let se=nfc_find_se(dev,se_idx);if se.is_null(){rc=-EINVAL;goto error;}if (*se).state==NFC_SE_DISABLED{rc=-EALREADY;goto error;}rc=((*dev).ops.disable_se.unwrap())(dev,se_idx);if rc>=0{(*se).state=NFC_SE_DISABLED;}error:device_unlock(&mut (*dev).dev);rc}
