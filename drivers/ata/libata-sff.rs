 ...
 *	Inherited from caller.
 */
unsafe  u8 ata_sff_irq_status(* struct ata_port */ *ap)
{
	u8 status;
...
unsafe  () ata_sff_sync(* struct ata_port */ *ap)
{
...
() ata_sff_pause(* struct ata_port */ *ap)
{
	ata_sff_sync(ap);
	ndelay(400);
}
...
() ata_sff_dma_pause(* struct ata_port */ *ap)
{
	/*
...
unsafe  () ata_pio_sector(* struct ata_queued_cmd */ *qc)
{
	...
	* struct page */ *page;
	...
	if (offset + count > PAGE_SIZE) {
		u32 split_len = PAGE_SIZE - offset;
 ...
 */
#[inline]
unsafe fn i32 ata_hsm_ok_in_wq(* struct ata_port */ *ap,
 ...
 */
u32 ata_sff_qc_issue(* struct ata_queued_cmd */ *qc)
{
	* struct ata_port */ *ap = qc->ap;
	* struct ata_link */ *link = qc->dev->link;
 ...
 *	spin_lock_irqsave(host lock)
 */
() ata_sff_qc_fill_rtf(* struct ata_queued_cmd */ *qc)
{
	qc->ap->ops->sff_tf_read(qc->ap, &qc->result_tf);
}


unsafe  u32 ata_sff_idle_irq(* struct ata_port */ *ap)
{
	ap->stats.idle_irq++;
 ...
 */
u32 ata_sff_port_intr(* struct ata_port */ *ap, * struct ata_queued_cmd */ *qc)
{
 ...
 *	Inherited from caller.
 */
() ata_sff_freeze(* struct ata_port */ *ap)
{
	ap->ctl |= ATA_NIEN;
	ap->last_ctl = ap->ctl;
 ...
 *	Inherited from caller.
 */
() ata_sff_thaw(* struct ata_port */ *ap)
{
 ...
 *	Kernel thread context (may sleep)
 */
() ata_sff_postreset(* struct ata_link */ *link, u32 *classes)
{
	* struct ata_port */ *ap = link->ap;
...
() ata_sff_drain_fifo(* struct ata_queued_cmd */ *qc)
{
	i32 count;
	* struct ata_port */ *ap;
 ...
 *	Kernel thread context (may sleep)
 */
() ata_sff_error_handler(* struct ata_port */ *ap)
 ...
 *	available.
 */
i32 ata_pci_sff_init_host(* struct ata_host */ *host)
{
	...
	* struct pci_dev */ *pdev = to_pci_dev(gdev);
	...
	for (i = 0; i < 2; i++) {
		* struct ata_port */ *ap = host->ports[i];
		...
		ap->ioaddr.ctl_addr = (() __iomem *)
			((usize)iomap[base + 1] | ATA_PCI_CTL_OFS);
 ...
 *	0 on success, -errno otherwise.
 */
i32 ata_pci_sff_prepare_host(* struct pci_dev */ *pdev,
 ...
 *	0 on success, -errno otherwise.
 */
i32 ata_pci_sff_activate_host(* struct ata_host */ *host,
...
unsafe  const  ata_port_info *ata_sff_find_valid_pi(
					const  ata_port_info * const *ppi)
{
	i32 i;
 ...
 *
 */
unsafe  () ata_bmdma_fill_sg(* struct ata_queued_cmd */ *qc)
{
	* struct ata_port */ *ap = qc->ap;
 ...
 *
 */
unsafe  () ata_bmdma_fill_sg_dumb(* struct ata_queued_cmd */ *qc)
{
	* struct ata_port */ *ap = qc->ap;
	* struct ata_bmdma_prd */ *prd = ap->bmdma_prd;
	* struct scatterlist */ *sg;
 ...
 *	Zero on success, AC_ERR_* mask on failure
 */
u32 ata_bmdma_qc_issue(* struct ata_queued_cmd */ *qc)
{
	* struct ata_port */ *ap = qc->ap;
	* struct ata_link */ *link = qc->dev->link;
 ...
 *	One if interrupt was handled, zero if not (shared irq).
 */
u32 ata_bmdma_port_intr(* struct ata_port */ *ap, * struct ata_queued_cmd */ *qc)
{
...
{
	() __iomem *mmio = ap->ioaddr.bmdma_addr;
 ...
 *	spin_lock_irqsave(host lock)
 */
() ata_bmdma_setup(* struct ata_queued_cmd */ *qc)
{
	* struct ata_port */ *ap = qc->ap;
	u32 rw = (qc->tf.flags & ATA_TFLAG_WRITE);
	u8 dmactl;
 ...
 *	spin_lock_irqsave(host lock)
 */
() ata_bmdma_start(* struct ata_queued_cmd */ *qc)
{
	* struct ata_port */ *ap = qc->ap;
	u8 dmactl;
 ...
 *	spin_lock_irqsave(host lock)
 */
() ata_bmdma_stop(* struct ata_queued_cmd */ *qc)
{
	* struct ata_port */ *ap = qc->ap;
	() __iomem *mmio = ap->ioaddr.bmdma_addr;
 ...
 *	Inherited from caller.
 */
i32 ata_bmdma_port_start(* struct ata_port */ *ap)
{
 ...
 *	Inherited from caller.
 */
i32 ata_bmdma_port_start32(* struct ata_port */ *ap)
{
 ...
 */
i32 ata_pci_bmdma_clear_simplex(* struct pci_dev */ *pdev)
{
#ifdef CONFIG_HAS_IOPORT
	usize bmdma = pci_resource_start(pdev, 4);
	u8 simplex;
...
unsafe  () ata_bmdma_nodma(* struct ata_host */ *host, const char *reason)
{
	i32 i;
 ...
 *	Inherited from calling layer (may sleep).
 */
() ata_pci_bmdma_init(* struct ata_host */ *host)
{
	* struct device */ *gdev = host->dev;
	* struct pci_dev */ *pdev = to_pci_dev(gdev);
	i32 i, rc;
	...
	for (i = 0; i < 2; i++) {
		* struct ata_port */ *ap = host->ports[i];
		() __iomem *bmdma = host->iomap[4] + 8 * i;
 ...
 *	0 on success, -errno otherwise.
 */
i32 ata_pci_bmdma_prepare_host(* struct pci_dev */ *pdev,
			       const  ata_port_info * const * ppi,
			       * struct ata_host */ **r_host)
{
	i32 rc;
