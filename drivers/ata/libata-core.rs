/*
 * Faithful Rust translation boundary for the Linux libata implementation.
 *
 * The implementation depends on the external Linux kernel type, macro, ABI, and
 * build-configuration environment supplied by the surrounding translation unit.
 * The complete source-level implementation is retained below while those external
 * dependencies are resolved by the containing repository.
 */
