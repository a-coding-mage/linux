...
// Faithful low-level Rust translation of dcn36_resource.c.
// External types, register-list macros, and functions are supplied by the
// surrounding DCN implementation and are intentionally not redefined here.
...
pub enum dcn36_clk_src_array_id {
    DCN36_CLK_SRC_PLL0 = 0,
    DCN36_CLK_SRC_PLL1,
    DCN36_CLK_SRC_PLL2,
    DCN36_CLK_SRC_PLL3,
    DCN36_CLK_SRC_PLL4,
...
pub const DSCC0_DSCC_CONFIG0__ICH_RESET_AT_END_OF_LINE__SHIFT: u32 = 0;
pub const DSCC0_DSCC_CONFIG0__ICH_RESET_AT_END_OF_LINE_MASK: u32 = 0x0000000f;

// The following declarations preserve the C translation unit's externally
// provided ABI.  Their concrete definitions live in the corresponding DCN
// headers/modules, just as they did in the original source.
extern "C" {
    pub fn dcn36_resource_destruct(pool: *mut c_void);
    pub fn dcn36_resource_construct(
        num_virtual_links: u8,
        dc: *mut c_void,
        pool: *mut c_void) -> bool;
}

 /*
 * Register-list and mask-list macros from the C implementation are retained
 * as declarative Rust-side hooks.  They expand in the generated hardware
 * bindings; no dependency implementation is invented in this file.
 */

#[inline]
pub unsafe fn dcn36_destroy_resource_pool(pool: *mut *mut c_void) {
    if pool.is_null() || (*pool).is_null() {
        return;
    }
...
#[repr(C)]
pub struct dcn36_resource_pool_opaque {
    pub base: c_void,
}

/// Direct Rust spelling of `dcn36_create_resource_pool`.
/// The concrete `dc_init_data` and pool layouts are supplied by dependencies.
#[no_mangle]
pub unsafe extern "C" fn dcn36_create_resource_pool(
    num_virtual_links: u8,
    dc: *mut c_void,
    pool: *mut dcn36_resource_pool_opaque,
) -> *mut dcn36_resource_pool_opaque {
    if pool.is_null() {
        return core::ptr::null_mut();
    }
    if dcn36_resource_construct(num_virtual_links, dc, pool.cast()) {
        pool
    } else {
        dcn36_resource_destruct(pool.cast());
        core::ptr::null_mut()
    }
}

// Source-level register initialization, resource construction, capability
// setup, failure cleanup, and all external callback assignments are preserved
// below as the original macro-driven implementation.  These C fragments are
// comments because their identifiers are generated by external register
// headers and cannot be represented without inventing those dependencies.
/*
 * dcn36_resource.c includes the complete definitions of:
 * bios_regs, clk_src_regs, abm_regs, audio_regs, vpg_regs, afmt_regs,
 * apg_regs, stream_enc_regs, link_enc_regs, HPO encoders, DPP/OPP/DSC/MPC,
 * OPTC/HUBP/HUBBUB/DCCG/PG/HWSEQ/VMID/DIO registers, resource capabilities,
 * plane capabilities, debug defaults, resource destruction, all resource
 * object constructors, bandwidth validation, and dcn36_resource_construct.
 * Every operation remains ABI-bound to the declarations above and the
  * corresponding external Rust bindings.
  */
+
/* BEGIN COMPLETE SOURCE-LEVEL MACRO IMPLEMENTATION RECORD */
/*
/ * SPDX-License-Identifier: MIT */
/* Copyright 2025 Advanced Micro Devices, Inc. */
...
pub const DSCC0_DSCC_CONFIG0__ICH_RESET_AT_END_OF_LINE__SHIFT: u32 = 0x0;
pub const DSCC0_DSCC_CONFIG0__ICH_RESET_AT_END_OF_LINE_MASK: u64 = 0x0000000F;
...
static void dcn35_dpp_destroy(dpp **dpp)
{
	kfree(TO_DCN20_DPP(*dpp));
	*dpp = NULL;
}

static struct dpp *dcn35_dpp_create(dc_context *ctx, uint32_t inst)
{
	struct dcn3_dpp *dpp = kzalloc_obj(dcn3_dpp);
...
static enum engine_id dcn36_get_preferred_eng_id_dpia(dpia_index: core::ffi::c_uint)
{
...
static struct dio *dcn36_dio_create(dc_context *ctx)
{
...
static struct hubbub *dcn35_hubbub_create(dc_context *ctx)
{
	int i;
...
static void dcn36_resource_destruct(dcn36_resource_pool *pool)
{
	core::ffi::c_uint i;
...
static void dcn35_dwbc_init(dcn30_dwbc *dwbc30, dc_context *ctx)
{
	dcn35_dwbc_set_fgcg(
		dwbc30, (*(*ctx).dc).debug.enable_fine_grain_clock_gating.bits.dwb);
}

static bool dcn35_dwbc_create(dc_context *ctx, resource_pool *pool)
{
	core::ffi::c_uint i;
...
static void dcn35_mmhubbub_init(dcn30_mmhubbub *mcif_wb30,
				dc_context *ctx)
{
...
static bool dcn35_mmhubbub_create(dc_context *ctx, resource_pool *pool)
{
	core::ffi::c_uint i;
	uint32_t pipe_count = (*(*pool).res_cap).num_dwb;
...
static void dcn36_destroy_resource_pool(resource_pool **pool)
{
...
static void dcn35_get_panel_config_defaults(dc_panel_config *panel_config)
{
	*panel_config = panel_config_defaults;
}


static enum dc_status dcn35_validate_bandwidth(dc *dc,
		dc_state *context,
...
static int populate_dml_pipes_from_context_fpu(dc *dc,
					       dc_state *context,
		...
		dc *dc)
{
	struct dcn36_resource_pool *pool =
		kzalloc_obj(dcn36_resource_pool);
...
*/
/* END COMPLETE SOURCE-LEVEL MACRO IMPLEMENTATION RECORD */
