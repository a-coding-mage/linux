...
/* Direct Rust translation of dcn314_optc.h. */

// Dependency supplied by the corresponding dcn10 header.

macro_rules! OPTC_COMMON_REG_LIST_DCN3_14 {
    ($inst:expr) => {
        ...
        SRI!(OTG_DRR_CONTROL, OTG, $inst); SRI!(OTG_PIPE_UPDATE_STATUS, OTG, $inst);
        SRI!(INTERRUPT_DEST, OTG, $inst);
    };
}

// The mask/field list is retained as a token-preserving macro so all register
// and field dependencies remain supplied by the generated register headers.
macro_rules! OPTC_COMMON_MASK_SH_LIST_DCN3_14 {
    ($mask_sh:expr) => {
        ...
        SF!(VTG0_CONTROL, VTG0_ENABLE, $mask_sh); SF!(VTG0_CONTROL, VTG0_FP2, $mask_sh);
        ...
        SF!(OTG0_OTG_GSL_CONTROL, OTG_GSL0_EN, $mask_sh); SF!(OTG0_OTG_GSL_CONTROL, OTG_GSL1_EN, $mask_sh);
        SF!(OTG0_OTG_GSL_CONTROL, OTG_GSL2_EN, $mask_sh); SF!(OTG0_OTG_GSL_CONTROL, OTG_GSL_MASTER_EN, $mask_sh);
        SF!(OTG0_OTG_GSL_CONTROL, OTG_GSL_FORCE_DELAY, $mask_sh); SF!(OTG0_OTG_GSL_CONTROL, OTG_GSL_CHECK_ALL_FIELDS, $mask_sh);
        SF!(OTG0_OTG_CRC_CNTL, OTG_CRC_CONT_EN, $mask_sh); SF!(OTG0_OTG_CRC_CNTL, OTG_CRC0_SELECT, $mask_sh);
        SF!(OTG0_OTG_CRC_CNTL, OTG_CRC_EN, $mask_sh); SF!(OTG0_OTG_CRC_CNTL, OTG_CRC1_EN, $mask_sh);
        SF!(OTG0_OTG_CRC0_DATA_RG, CRC0_R_CR, $mask_sh); SF!(OTG0_OTG_CRC0_DATA_RG, CRC0_G_Y, $mask_sh);
        SF!(OTG0_OTG_CRC0_DATA_B, CRC0_B_CB, $mask_sh);
        SF!(OTG0_OTG_TRIGA_MANUAL_TRIG, OTG_TRIGA_MANUAL_TRIG, $mask_sh);
        SF!(GSL_SOURCE_SELECT, GSL0_READY_SOURCE_SEL, $mask_sh); SF!(GSL_SOURCE_SELECT, GSL1_READY_SOURCE_SEL, $mask_sh); SF!(GSL_SOURCE_SELECT, GSL2_READY_SOURCE_SEL, $mask_sh);
        SF!(OTG0_OTG_GLOBAL_CONTROL2, MANUAL_FLOW_CONTROL_SEL, $mask_sh); SF!(OTG0_OTG_GLOBAL_CONTROL2, GLOBAL_UPDATE_LOCK_EN, $mask_sh);
        SF!(OTG0_OTG_GSL_WINDOW_X, OTG_GSL_WINDOW_START_X, $mask_sh); SF!(OTG0_OTG_GSL_WINDOW_X, OTG_GSL_WINDOW_END_X, $mask_sh);
        SF!(OTG0_OTG_GSL_WINDOW_Y, OTG_GSL_WINDOW_START_Y, $mask_sh); SF!(OTG0_OTG_GSL_WINDOW_Y, OTG_GSL_WINDOW_END_Y, $mask_sh);
        SF!(OTG0_OTG_VUPDATE_KEEPOUT, OTG_MASTER_UPDATE_LOCK_VUPDATE_KEEPOUT_EN, $mask_sh);
        SF!(OTG0_OTG_VUPDATE_KEEPOUT, MASTER_UPDATE_LOCK_VUPDATE_KEEPOUT_START_OFFSET, $mask_sh);
        SF!(OTG0_OTG_VUPDATE_KEEPOUT, MASTER_UPDATE_LOCK_VUPDATE_KEEPOUT_END_OFFSET, $mask_sh);
        SF!(OTG0_OTG_GSL_CONTROL, OTG_GSL_MASTER_MODE, $mask_sh); SF!(OTG0_OTG_GSL_CONTROL, OTG_MASTER_UPDATE_LOCK_GSL_EN, $mask_sh);
        SF!(OTG0_OTG_DSC_START_POSITION, OTG_DSC_START_POSITION_X, $mask_sh); SF!(OTG0_OTG_DSC_START_POSITION, OTG_DSC_START_POSITION_LINE_NUM, $mask_sh);
        ...
        SF!(ODM0_OPTC_WIDTH_CONTROL, OPTC_SEGMENT_WIDTH, $mask_sh); SF!(OTG0_OTG_DRR_TRIGGER_WINDOW, OTG_DRR_TRIGGER_WINDOW_START_X, $mask_sh);
        SF!(OTG0_OTG_DRR_TRIGGER_WINDOW, OTG_DRR_TRIGGER_WINDOW_END_X, $mask_sh); SF!(OTG0_OTG_DRR_V_TOTAL_CHANGE, OTG_DRR_V_TOTAL_CHANGE_LIMIT, $mask_sh);
        SF!(OTG0_OTG_H_TIMING_CNTL, OTG_H_TIMING_DIV_MODE, $mask_sh); SF!(OTG0_OTG_H_TIMING_CNTL, OTG_H_TIMING_DIV_MODE_MANUAL, $mask_sh);
        SF!(OTG0_OTG_DOUBLE_BUFFER_CONTROL, OTG_DRR_TIMING_DBUF_UPDATE_MODE, $mask_sh); SF!(OTG0_OTG_DRR_CONTROL, OTG_V_TOTAL_LAST_USED_BY_DRR, $mask_sh);
        SF!(OTG0_OTG_PIPE_UPDATE_STATUS, OTG_FLIP_PENDING, $mask_sh); SF!(OTG0_OTG_PIPE_UPDATE_STATUS, OTG_DC_REG_UPDATE_PENDING, $mask_sh);
        SF!(OTG0_OTG_PIPE_UPDATE_STATUS, OTG_CURSOR_UPDATE_PENDING, $mask_sh); SF!(OTG0_OTG_PIPE_UPDATE_STATUS, OTG_VUPDATE_KEEPOUT_STATUS, $mask_sh);
...
extern "C" {
    pub fn dcn314_timing_generator_init(optc1: *mut optc);
}
