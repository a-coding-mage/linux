 ...
 *
 * Rust translation of dcn42_optc.h.  The register-field identifiers and the
 * `SF` helper are supplied by the surrounding display-controller bindings.
 */

// C dependency: #include "dcn10/dcn10_optc.h"

/* Translate the C preprocessor field-list macros into Rust macro_rules.
 * `SF!` is intentionally external: it has the same field-building meaning
 * as the C helper used by the original header.
 */
macro_rules! OPTC_COMMON_MASK_SH_LIST_DCN42 {
    ($mask_sh:expr) => {
...
/* DCN42B uses the same register-field set, with the B-specific ordering and
 * final status fields retained by this forwarding macro. */
macro_rules! OPTC_COMMON_MASK_SH_LIST_DCN42B {
    ($mask_sh:expr) => {
        OPTC_COMMON_MASK_SH_LIST_DCN42!($mask_sh);
        SF!(OTG0_OTG_CONTROL, OTG_CURRENT_MASTER_EN_STATE, $mask_sh);
        SF!(OTG0_OTG_DOUBLE_BUFFER_CONTROL, OTG_DRR_TIMING_DBUF_UPDATE_PENDING, $mask_sh);
    };
}
...
extern "C" {
    pub fn dcn42_timing_generator_init(optc1: *mut optc);
    pub fn optc42_enable_pwa(optc: *mut timing_generator, pwa_sync_param: *mut otc_pwa_frame_sync);
    pub fn optc42_disable_pwa(optc: *mut timing_generator);
    pub fn optc42_tg_init(optc: *mut timing_generator);
    pub fn optc42_clear_optc_underflow(optc: *mut timing_generator);
    pub fn optc42_is_optc_underflow_occurred(optc: *mut timing_generator) -> bool;
    pub fn optc42_disable_crtc(optc: *mut timing_generator) -> bool;
    pub fn optc42_lock_doublebuffer_enable(optc: *mut timing_generator);
    pub fn optc42_get_crc(
...
// External C types supplied by dcn10/dcn10_optc.h.
#[allow(non_camel_case_types)]
pub enum optc {}
#[allow(non_camel_case_types)]
pub enum timing_generator {}
#[allow(non_camel_case_types)]
pub enum otc_pwa_frame_sync {}
