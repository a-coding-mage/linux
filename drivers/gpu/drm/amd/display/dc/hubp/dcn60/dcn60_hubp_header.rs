...
//
// Translated from dcn60_hubp.h. Included C headers provide the referenced
// register, structure, and field definitions.

macro_rules! HUBP_MASK_SH_LIST_DCN60 {
    ($mask_sh:ident) => {
        (
...
extern "C" {
    ...
    pub fn hubp60_setup_interdependent(
        hubp: *mut hubp,
        pipe_regs: *mut dml2_dchub_per_pipe_register_set,
    );

    pub fn hubp60_cursor_set_attributes(
        hubp: *mut hubp,
        attr: *const dc_cursor_attributes,
    );

    pub fn hubp60_read_state(hubp: *mut hubp);

    pub fn hubp60_construct(
