...
// Faithful low-level Rust translation of the isolated C implementation.

// SPDX-License-Identifier: MIT
//
// Copyright 2024-2025 Advanced Micro Devices, Inc.

// dependency: "dml2_core_dcn5_funcs_mode_programming.h"
// dependency: "dml2_core_dcn5_calcs_dchub.h"
// dependency: "dml2_core_dcn5_calcs_display_pipe.h"
// dependency: "dml2_core_utils.h"
// dependency: "dml2_debug.h"

unsafe fn dcn5_mode_programming(*mut dml2_core_calcs_mode_programming_exin_out_params)
{
	*const dml2_display_cfgdisplay_cfg = in_out_params->in_display_cfg;
	*const dml2_utm_soc_bbutm_soc_bb = in_out_params->utm_soc_bb;
	*const dml2_sop_tablesop_table = &in_out_params->utm_soc_bb->sop_table;
	*const core_display_cfg_support_infocfg_support_info = in_out_params->cfg_support_info;
	*mut dml2_core_internal_display_mode_libmode_lib = in_out_params->mode_lib;
	*const dml2_display_cfg_programmingprogramming = in_out_params->programming;
	*mut dml2_core_calcs_mode_programming_localss = &mode_lib->scratch.dml_core_mode_programming_locals;
	*mut dml2_core_calcs_CalculateWatermarksMALLUseAndDRAMSpeedChangeSupport_paramsCalculateWatermarks_params = &mode_lib->scratch.CalculateWatermarksMALLUseAndDRAMSpeedChangeSupport_params;
	*mut dml2_core_calcs_CalculateVMRowAndSwath_paramsCalculateVMRowAndSwath_params = &mode_lib->scratch.CalculateVMRowAndSwath_params;
	*mut dml2_core_calcs_CalculateSwathAndDETConfiguration_paramsCalculateSwathAndDETConfiguration_params = &mode_lib->scratch.CalculateSwathAndDETConfiguration_params;
	*mut dml2_core_calcs_CalculateStutterEfficiency_paramsCalculateStutterEfficiency_params = &mode_lib->scratch.CalculateStutterEfficiency_params;
	*mut dml2_core_calcs_CalculatePrefetchSchedule_paramsCalculatePrefetchSchedule_params = &mode_lib->scratch.CalculatePrefetchSchedule_params;
	*mut dml2_core_calcs_calculate_mcache_setting_paramscalculate_mcache_setting_params = &mode_lib->scratch.calculate_mcache_setting_params;
	*mut dml2_core_calcs_calculate_tdlut_setting_paramscalculate_tdlut_setting_params = &mode_lib->scratch.calculate_tdlut_setting_params;
	*mut dml2_core_shared_CalculateMetaAndPTETimes_paramsCalculateMetaAndPTETimes_params = &mode_lib->scratch.CalculateMetaAndPTETimes_params;
	*mut dml2_core_calcs_calculate_peak_bandwidth_required_paramscalculate_peak_bandwidth_params = &mode_lib->scratch.calculate_peak_bandwidth_params;
	*mut dml2_core_calcs_calculate_bytes_to_fetch_required_to_hide_latency_paramscalculate_bytes_to_fetch_required_to_hide_latency_params = &mode_lib->scratch.calculate_bytes_to_fetch_required_to_hide_latency_params;

	u32 k, j;
	bool must_support_iflip;
	const min_return_uclk_cycles = 83;
	const min_return_fclk_cycles = 75;
	dml2_soc_operating_point max_sop;
	dml2_soc_operating_point min_sop;
	f64 min_return_latency_in_DCFCLK_cycles = 0;
	...
	for (k = 0; k < s->num_active_planes; ++k) {
		u32 stream_index = display_cfg->plane_descriptors[k].stream_index;
	...
	core::ptr::copy_nonoverlapping(mode_lib->mp.uclk_pstate_switch_modes,
	...
	for (k = 0; k < s->num_active_planes; ++k) {
		dcn5_calculate_single_pipe_dppclk_and_scl_throughput(
			...
			((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000),
	...
	dcn5_calculate_swath_width(
		...
		s->dummy_integer_array[0], // u32 MaximumSwathHeightY[]
		s->dummy_integer_array[1], // u32 MaximumSwathHeightC[]
	...
	for (k = 0; k < s->num_active_planes; ++k) {
		mode_lib->mp.cursor_bw[k] = display_cfg->plane_descriptors[k].cursor.num_cursors * display_cfg->plane_descriptors[k].cursor.cursor_width * display_cfg->plane_descriptors[k].cursor.cursor_bpp / 8.0 /
			((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total / ((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000));
	...
	for (k = 0; k < s->num_active_planes; ++k) {
		s->SurfaceParameters[k].PixelClock = ((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000);
	...
	core::ptr::write_bytes(calculate_mcache_setting_params, 0, sizeof(dml2_core_calcs_calculate_mcache_setting_params));
	for (k = 0; k < s->num_active_planes; k++) {
	...
	for (k = 0; k < s->num_active_planes; ++k) {
		...
		line_time_us = display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total / ((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000);
...

	{
		...
		for (k = 0; k < s->num_active_planes; ++k) {
			*mut dml2_core_internal_DmlPipemyPipe = &s->myPipe;
			...
			myPipe->PixelClock = ((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000);
			...
			CalculatePrefetchSchedule_params->DPP_RECOUT_WIDTH = (u32)(mode_lib->mp.SwathWidthY[k] / display_cfg->plane_descriptors[k].composition.scaler_info.plane0.h_ratio);
		...
		if (mode_lib->mp.PrefetchModeSupported == true) {
			for (k = 0; k < s->num_active_planes; ++k) {
				f64 line_time_us = display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total /
					((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000);
				dcn5_calculate_urgent_burst_factor(
					&display_cfg->plane_descriptors[k],
					mode_lib->mp.swath_width_luma_ub[k],
			...
			dcn5_check_urgent_bandwidth_support(
				&mode_lib->mp.FractionOfUrgentBandwidth, // f64* frac_urg_bandwidth
		...
		if (mode_lib->mp.PrefetchModeSupported == true) { // prefetch schedule and prefetch bw ok, now check flip bw
			...
			for (k = 0; k < s->num_active_planes; ++k) {
				dcn5_calculate_flip_schedule(
					...
					display_cfg->plane_descriptors[k].pixel_format,
					display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total / ((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000),
					display_cfg->plane_descriptors[k].composition.scaler_info.plane0.v_ratio,
					display_cfg->plane_descriptors[k].composition.scaler_info.plane1.v_ratio,
			...
			dcn5_check_immediate_flip_bandwidth_support(
				&mode_lib->mp.FractionOfUrgentBandwidthImmediateFlip, // f64* frac_urg_bandwidth_flip
	...
	if (!mode_lib->mp.PrefetchAndImmediateFlipSupported) {
		...
		for (k = 0; k < s->num_active_planes; ++k) {
			if (display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].writeback.active_writebacks_per_stream > 0) {
				mode_lib->mp.WritebackAllowDRAMClockChangeEndPosition[k] = math_max2(0, mode_lib->mp.VStartupMin[k] * display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total /
					((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000) - mode_lib->mp.Watermark.WritebackDRAMClockChangeWatermark);
				mode_lib->mp.WritebackAllowFCLKChangeEndPosition[k] = math_max2(0, mode_lib->mp.VStartupMin[k] * display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total /
					((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000) - mode_lib->mp.Watermark.WritebackFCLKChangeWatermark);
			} else {
		...
		for (k = 0; k < s->num_active_planes; ++k) {
			...
			s->Tvstartup_margin = (s->MaxVStartupLines[k] - mode_lib->mp.VStartupMin[k]) * display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total / ((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000);
			...
			s->LSetup = math_floor2(4.0 * mode_lib->mp.TSetup[k] / ((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total / ((f64)display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.pixel_clock_khz / 1000)), 1.0) / 4.0;
			...
			// debug only
			if (((mode_lib->mp.VUpdateOffsetPix[k] + mode_lib->mp.VUpdateWidthPix[k] + mode_lib->mp.VReadyOffsetPix[k]) / (f64) display_cfg->stream_descriptors[display_cfg->plane_descriptors[k].stream_index].timing.h_total) <=
				(isInterlaceTiming ?
			...
			for (j = 0; j < display_cfg->stream_descriptors[k].writeback.active_writebacks_per_stream; ++j) {
				...
				s->WRBandwidth = display_cfg->stream_descriptors[k].writeback.writeback_stream[j].output_height
					...
					* display_cfg->stream_descriptors[k].writeback.writeback_stream[j].input_height
					/ ((f64)display_cfg->stream_descriptors[k].timing.pixel_clock_khz / 1000)) * writeback_bytes_per_pixel;
...
// build condition: __DML_VBA_ALLOW_DELTA__
...
// end build condition
	...
	mode_lib->mp.min_return_latency_in_dcfclk = (u32)min_return_latency_in_DCFCLK_cycles;
	mode_lib->mp.dcfclk_deep_sleep_hysteresis = (u32)math_max2(32, (f64)mode_lib->ip.pixel_chunk_size_kbytes * 1024 * 3 / 4 / 64 - min_return_latency_in_DCFCLK_cycles);
...
unsafe fn dcn5_get_global_sync_programming(*const dml2_core_internal_display_mode_libmode_lib, *mut dml2_global_sync_programmingout, int pipe_index)
{
...
unsafe fn dcn5_get_stream_programming(*const dml2_core_internal_display_mode_libmode_lib, *mut dml2_per_stream_programmingout, int pipe_index)
{
	dcn5_get_global_sync_programming(mode_lib, &out->global_sync, pipe_index);
}

unsafe fn dcn5_populate_fams2_programming(*const dml2_core_internal_display_mode_libmode_lib,
		*const dml2_core_calcs_mode_programming_exmode_prog,
		*mut dmub_cmd_fams2_configfams2_base_programming,
		*mut dmub_cmd_fams2_configfams2_sub_programming,
		dml2_pstate_method pstate_method,
		int plane_index)
{
	*const dml2_display_cfgdisp_cfg = mode_prog->in_display_cfg;
	*const dml2_plane_parametersplane_descriptor = &disp_cfg->plane_descriptors[plane_index];
	*const dml2_stream_parametersstream_descriptor = &disp_cfg->stream_descriptors[plane_descriptor->stream_index];
	*const dml2_pstate_metastream_pstate_meta = &mode_prog->uclk_params->stream_pstate_meta[plane_descriptor->stream_index];

	*mut dmub_fams2_cmd_stream_static_base_statebase_programming = &fams2_base_programming->stream_v1.base;
	*mut dmub_fams2_cmd_stream_static_sub_statesub_programming = &fams2_sub_programming->stream_v1.sub_state;
	...
	base_programming->htotal = (u16)stream_descriptor->timing.h_total;
	base_programming->vtotal = (u16)stream_descriptor->timing.v_total;
	base_programming->vblank_start = (u16)(stream_pstate_meta->nom_vtotal -
		stream_descriptor->timing.v_front_porch);
	base_programming->vblank_end = (u16)(stream_pstate_meta->nom_vtotal -
		stream_descriptor->timing.v_front_porch -
	...
	base_programming->otg_vline_time_ns =
		(u32)(stream_pstate_meta->otg_vline_time_us * 1000.0);
	base_programming->scheduling_delay_otg_vlines = (u8)stream_pstate_meta->scheduling_delay_otg_vlines;
	base_programming->contention_delay_otg_vlines = (u8)stream_pstate_meta->contention_delay_otg_vlines;
	base_programming->vline_int_ack_delay_otg_vlines = (u8)stream_pstate_meta->vertical_interrupt_ack_delay_otg_vlines;
	base_programming->drr_keepout_otg_vline = (u16)(stream_pstate_meta->nom_vtotal -
		stream_descriptor->timing.v_front_porch -
		stream_pstate_meta->method_drr.programming_delay_otg_vlines);
	base_programming->allow_to_target_delay_otg_vlines = (u8)stream_pstate_meta->allow_to_target_delay_otg_vlines;
	base_programming->max_vtotal = (u16)stream_pstate_meta->max_vtotal;
	...
	case dml2_pstate_method_fw_vactive_drr:
		...
		sub_programming->legacy.vactive_det_fill_delay_otg_vlines =
			(u8)stream_pstate_meta->method_vactive.max_vactive_det_fill_delay_otg_vlines;
		base_programming->allow_start_otg_vline =
			(u16)stream_pstate_meta->method_vactive.common.allow_start_otg_vline;
		base_programming->allow_end_otg_vline =
			(u16)stream_pstate_meta->method_vactive.common.allow_end_otg_vline;
	...
	case dml2_pstate_method_fw_vblank_drr:
		...
		base_programming->allow_start_otg_vline =
			(u16)stream_pstate_meta->method_vblank.common.allow_start_otg_vline;
		base_programming->allow_end_otg_vline =
			(u16)stream_pstate_meta->method_vblank.common.allow_end_otg_vline;
		base_programming->config.bits.clamp_vtotal_min = true;
		break;
	case dml2_pstate_method_fw_drr:
		...
		sub_programming->drr.programming_delay_otg_vlines =
			(u8)stream_pstate_meta->method_drr.programming_delay_otg_vlines;
		sub_programming->drr.nom_stretched_vtotal =
			(u16)stream_pstate_meta->method_drr.stretched_vtotal;
		base_programming->allow_start_otg_vline =
			(u16)stream_pstate_meta->method_drr.common.allow_start_otg_vline;
		base_programming->allow_end_otg_vline =
			(u16)stream_pstate_meta->method_drr.common.allow_end_otg_vline;
...
unsafe fn dcn5_populate_global_fams2_programming(*mut dml2_core_internal_display_mode_libmode_lib,
		*const dml2_core_calcs_mode_programming_exmode_prog,
		*mut dmub_cmd_fams2_global_configfams2_global_config)
{
...
unsafe fn dcn5_populate_mcache_allocation(*mut dml2_display_cfg_programmingprogramming,
...
unsafe fn dcn5_pack_mode_programming_params(
		*mut dml2_core_instancecore,
		*mut dml2_core_calcs_mode_programming_exmode_prog,
		*mut dml2_display_cfg_programmingprogramming)
{
	*mut dml2_core_internal_display_mode_libmode_lib = mode_prog->mode_lib;
	*const dml2_utm_soc_bbutm_soc_bb = core->utm_soc_bb;
	u32 pipe_offset;
	...
	core::ptr::copy_nonoverlapping(&programming->display_config, mode_prog->in_display_cfg, sizeof(dml2_display_cfg));
	...
	for (plane_index = 0; plane_index < programming->display_config.num_planes; plane_index++) {
		...
		for (pipe_offset = 0; pipe_offset < programming->plane_programming[plane_index].num_dpps_required; pipe_offset++) {
			...
			core::ptr::write_bytes(programming->plane_programming[plane_index].pipe_regs[pipe_offset], 0, sizeof(dml2_dchub_per_pipe_register_set));
			...
			if (!(stream_already_populated_mask & (0x1 << main_stream_index))) {
				...
				for (dwb_index = 0; dwb_index < programming->display_config.stream_descriptors[main_stream_index].writeback.active_writebacks_per_stream; dwb_index++) {
					...
					core::ptr::write_bytes(programming->stream_programming[main_stream_index].mcif_regs[dwb_index], 0, sizeof(dml2_mcif_per_pipe_register_set));
	...
	for (u32 plane_idx = 0; plane_idx < DML2_MAX_PLANES; plane_idx++) {
	...
	f64 dram_clk_change_latency_us = utm_soc_bb->power_management_parameters.dram_clk_change_blackout_us;

	if (programming->display_config.num_planes == 1) {
		// Calculate for all planes (inactive planes should naturally yield 0)
		for (u32 plane_idx = 0; plane_idx < programming->display_config.num_planes; plane_idx++) {
			...
			f64 pixel_clock_mhz = (f64)programming->display_config.stream_descriptors[stream_index].timing.pixel_clock_khz / 1000.0;
			f64 htotal = (f64)programming->display_config.stream_descriptors[stream_index].timing.h_total;
			f64 line_time_us = htotal / pixel_clock_mhz;

			// Use actual calculated values from mode library (not hardcoded defaults)
			f64 tmclk_us = dram_clk_change_latency_us;
			f64 tvstartup_us = mode_lib->mp.VStartup[plane_idx] * line_time_us;
			f64 tsetup_us = mode_lib->mp.TSetup[plane_idx];
			f64 tcalc_us = mode_lib->mp.TCalc;
			f64 tpre_us = mode_lib->mp.dst_y_prefetch[plane_idx] * line_time_us; // As specified: dst_y_prefetch * line_time
			f64 tout_us = (mode_lib->mp.DSTYAfterScaler[plane_idx] +
				((f64)mode_lib->mp.DSTXAfterScaler[plane_idx] / htotal)) * line_time_us; // delay_after_scaler per spec
			...
			programming->informative.misc.pstate_recout_reduction_lines[plane_idx] =
				(u32) math_max2(0.0, (tmclk_us - (tvstartup_us - (tsetup_us + tcalc_us + tpre_us + tout_us))) / line_time_us);
		}
	}
}

unsafe fn dcn5_mp_assign_qos_bound(*mut dml2_core_instancecore,
		*mut dml2_display_cfg_programmingprogramming)
{
...
unsafe fn dml2_core_dcn5_funcs_populate_programming(*mut dml2_core_instancecore,
		*const dml2_display_solutionsolution,
		*mut dml2_display_cfg_programmingprogramming)
{
	*mut dml2_core_calcs_mode_programming_exparams = &core->scratch.mode_programming_locals.mode_programming_ex_params;
	*mut dml2_core_internal_display_mode_libmode_lib = &core->clean_me_up.mode_lib;
	bool result;

	core::ptr::write_bytes(&mode_lib->scratch, 0, sizeof(dml2_core_internal_scratch));
	core::ptr::write_bytes(&mode_lib->mp, 0, sizeof(dml2_core_internal_mode_program));
