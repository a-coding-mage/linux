...
// Dependencies supplied by the corresponding C headers are referenced here.

pub static mut core_dcn6_ip_caps_base: dml2_core_ip_params = dml2_core_ip_params {
    ...
    // Overestimate for now, can reduce later: sum of worst case throttle, programming, scheduling and contention delays
    alt_chan_fw_delay_us: 955,
};
...
unsafe fn patch_ip_params_with_ip_caps(
    ...
    (*ip_params).alt_chan_fw_delay_us = (*ip_caps).fams2.scheduling_delay_us
        ...
        + (((*ip_caps).fams2.vertical_interrupt_ack_delay_us + (*ip_caps).fams2.drr_programming_delay_us
            > (*ip_caps).fams2.allow_programming_delay_us)
            .then_some((*ip_caps).fams2.drr_programming_delay_us)
            .unwrap_or((*ip_caps).fams2.allow_programming_delay_us)) * ((*ip_caps).otg_count - 1);
...
pub unsafe fn dml2_core_dcn6_funcs_initialize(in_out: *mut dml2_core_initialize_in_out) -> bool {
    ...
    if !(*in_out).explicit_ip_bb.is_null() && (*in_out).explicit_ip_bb_size > 0 {
        ...
        core::ptr::copy_nonoverlapping(
            &core_dcn6_ip_caps_base as *const dml2_core_ip_params as *const u8,
            &mut (*core).clean_me_up.mode_lib.ip as *mut _ as *mut u8,
            core::mem::size_of::<dml2_core_ip_params>(),
        );
        patch_ip_params_with_ip_caps(&mut (*core).clean_me_up.mode_lib.ip, (*in_out).ip_caps);
        (*core).clean_me_up.mode_lib.ip.imall_supported = false;
    }

    core::ptr::copy_nonoverlapping(
        (*in_out).ip_caps as *const dml2_ip_capabilities as *const u8,
    ...
    DML_LOG_DEBUG!("%s exit with true\n", c"dml2_core_dcn6_funcs_initialize");
