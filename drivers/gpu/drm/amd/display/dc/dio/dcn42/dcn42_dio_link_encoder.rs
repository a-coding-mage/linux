...
// dcn35/dcn35_dio_link_encoder.h, and dcn42_dio_link_encoder.h.

#[allow(unused_macros)]
macro_rules! min { ($x:expr, $y:expr) => { if $x < $y { $x } else { $y } }; }

pub unsafe extern "C" fn dcn42_get_hpd_state(enc: *mut link_encoder) -> bool {
    let enc10 = TO_DCN10_LINK_ENC(enc);
    let mut state: u32 = 0;
    generic_reg_get(
        (*enc10).base.ctx,
        (*enc10).hpd_regs.DC_HPD_INT_STATUS,
        (*enc10).link_shift.DC_HPD_SENSE,
        (*enc10).link_mask.DC_HPD_SENSE,
        &mut state,
    );
    state != 0
}

pub unsafe extern "C" fn dcn42_program_hpd_filter(
    enc: *mut link_encoder,
    delay_on_connect_in_ms: i32,
    delay_on_disconnect_in_ms: i32,
) -> bool {
    let enc10 = TO_DCN10_LINK_ENC(enc);
    generic_reg_set_ex(
        (*enc10).base.ctx,
        (*enc10).hpd_regs.DC_HPD_TOGGLE_FILT_CNTL,
        0,
        2,
        (*enc10).link_shift.DC_HPD_CONNECT_INT_DELAY,
        (*enc10).link_mask.DC_HPD_CONNECT_INT_DELAY,
        delay_on_connect_in_ms,
        (*enc10).link_shift.DC_HPD_DISCONNECT_INT_DELAY,
        (*enc10).link_mask.DC_HPD_DISCONNECT_INT_DELAY,
        delay_on_disconnect_in_ms,
    );
    true
}

static dcn42_link_enc_funcs: link_encoder_funcs = link_encoder_funcs {
    read_state: Some(link_enc2_read_state),
    validate_output_with_stream: Some(dcn30_link_encoder_validate_output_with_stream),
    hw_init: Some(dcn35_link_encoder_init),
    setup: Some(dcn35_link_encoder_setup),
    ...
    enable_hpd: Some(dcn10_link_encoder_enable_hpd),
    disable_hpd: Some(dcn10_link_encoder_disable_hpd),
    is_dig_enabled: Some(dcn35_is_dig_enabled),
    destroy: Some(dcn10_link_encoder_destroy),
    fec_set_enable: Some(enc2_fec_set_enable),
    fec_set_ready: Some(enc2_fec_set_ready),
    fec_is_active: Some(enc2_fec_is_active),
    get_dig_frontend: Some(dcn10_get_dig_frontend),
    get_dig_mode: Some(dcn401_get_dig_mode),
    is_in_alt_mode: Some(dcn32_link_encoder_is_in_alt_mode),
    get_max_link_cap: Some(dcn32_link_encoder_get_max_link_cap),
    ...
    prog_eq_setting: Some(dpcs401_program_eq_setting),
    get_txffe: Some(dpcs401_get_txffe),
    set_txffe: Some(dpcs401_set_txffe),
    set_dio_phy_mux: Some(dcn31_link_encoder_set_dio_phy_mux),
    enable_dpia_output: Some(dcn35_link_encoder_enable_dpia_output),
    disable_dpia_output: Some(dcn35_link_encoder_disable_dpia_output),
    get_hpd_state: Some(dcn42_get_hpd_state),
    program_hpd_filter: Some(dcn42_program_hpd_filter),
};

pub unsafe extern "C" fn dcn42_link_encoder_construct(
    ...
    if let Some(get_cap) = (*bp_funcs).get_connector_speed_cap_info {
        result = get_cap(enc10.base.ctx.dc_bios, enc10.base.connector, &mut bp_cap_info);
    }
    if result == BP_RESULT_OK {
        ...
        enc10.base.features.flags.bits.IS_HDMI_FRL_CAPABLE = bp_cap_info.FRL_8G_EN || bp_cap_info.FRL_10G_EN || bp_cap_info.FRL_12G_EN;
