/* Translated from gfxhub_v3_0.c. External symbols and register macros are
 * provided by the surrounding AMDGPU Rust bindings. */

static GFXHUB_CLIENT_IDS: [&str; 18] = [
    "CB/DB", "Reserved", "GE1", "GE2", "CPF", "CPC", "CPG", "RLC",
    "TCP", "SQC (inst)", "SQC (data)", "SQG", "Reserved", "SDMA0",
    "SDMA1", "GCR", "SDMA2", "SDMA3",
];

unsafe fn gfxhub_v3_0_get_invalidate_req(vmid: u32, flush_type: u32) -> u32 {
    let mut req: u32 = 0;
...
unsafe fn gfxhub_v3_0_print_l2_protection_fault_status(adev: *mut amdgpu_device, status: u32) {
...
unsafe fn gfxhub_v3_0_get_fb_location(adev: *mut amdgpu_device) -> u64 {
    let mut base = RREG32_SOC15!(GC, 0, regGCMC_VM_FB_LOCATION_BASE) as u64;
    base &= GCMC_VM_FB_LOCATION_BASE__FB_BASE_MASK as u64;
    base << 24
}
unsafe fn gfxhub_v3_0_get_mc_fb_offset(adev: *mut amdgpu_device) -> u64 {
    (RREG32_SOC15!(GC, 0, regGCMC_VM_FB_OFFSET) as u64) << 24
}
unsafe fn gfxhub_v3_0_setup_vm_pt_regs(adev: *mut amdgpu_device, vmid: u32, page_table_base: u64) {
...
unsafe fn gfxhub_v3_0_init_gart_aperture_regs(adev: *mut amdgpu_device) {
...
unsafe fn gfxhub_v3_0_init_system_aperture_regs(adev: *mut amdgpu_device) {
    let value: u64;
...
unsafe fn gfxhub_v3_0_init_tlb_regs(adev: *mut amdgpu_device) {
...
unsafe fn gfxhub_v3_0_init_cache_regs(adev: *mut amdgpu_device) {
...
unsafe fn gfxhub_v3_0_enable_system_domain(adev: *mut amdgpu_device) {
...
unsafe fn gfxhub_v3_0_disable_identity_aperture(adev: *mut amdgpu_device) {
    if amdgpu_sriov_vf!(adev) { return; }
...
unsafe fn gfxhub_v3_0_setup_vmid_config(adev: *mut amdgpu_device) {
    let hub = &mut (*adev).vmhub[AMDGPU_GFXHUB!(0)];
    let mut tmp = 0u32;
    for i in 0..=14 {
...
unsafe fn gfxhub_v3_0_program_invalidation(adev: *mut amdgpu_device) {
    let hub = &mut (*adev).vmhub[AMDGPU_GFXHUB!(0)];
...
unsafe fn gfxhub_v3_0_gart_enable(adev: *mut amdgpu_device) -> i32 {
    ...
    gfxhub_v3_0_init_gart_aperture_regs(adev); gfxhub_v3_0_init_system_aperture_regs(adev); gfxhub_v3_0_init_tlb_regs(adev); gfxhub_v3_0_init_cache_regs(adev); gfxhub_v3_0_enable_system_domain(adev); gfxhub_v3_0_disable_identity_aperture(adev); gfxhub_v3_0_setup_vmid_config(adev); gfxhub_v3_0_program_invalidation(adev); 0
}
unsafe fn gfxhub_v3_0_gart_disable(adev: *mut amdgpu_device) {
    let hub = &mut (*adev).vmhub[AMDGPU_GFXHUB!(0)];
    for i in 0..16 { WREG32_SOC15_OFFSET!(GC, 0, regGCVM_CONTEXT0_CNTL, i * hub.ctx_distance, 0); }
    let mut tmp = RREG32_SOC15!(GC, 0, regGCMC_VM_MX_L1_TLB_CNTL);
    tmp = REG_SET_FIELD!(tmp, GCMC_VM_MX_L1_TLB_CNTL, ENABLE_L1_TLB, 0); tmp = REG_SET_FIELD!(tmp, GCMC_VM_MX_L1_TLB_CNTL, ENABLE_ADVANCED_DRIVER_MODEL, 0); WREG32_SOC15!(GC, 0, regGCMC_VM_MX_L1_TLB_CNTL, tmp);
    WREG32_FIELD15_PREREG!(GC, 0, GCVM_L2_CNTL, ENABLE_L2_CACHE, 0); WREG32_SOC15!(GC, 0, regGCVM_L2_CNTL3, 0);
}
unsafe fn gfxhub_v3_0_set_fault_enable_default(adev: *mut amdgpu_device, value: bool) {
    let mut tmp = RREG32_SOC15!(GC, 0, regCP_DEBUG); tmp = REG_SET_FIELD!(tmp, CP_DEBUG, CPG_UTCL1_ERROR_HALT_DISABLE, 1); WREG32_SOC15!(GC, 0, regCP_DEBUG, tmp);
    if amdgpu_sriov_vf!(adev) { return; }
    tmp = RREG32_SOC15!(GC, 0, regGCVM_L2_PROTECTION_FAULT_CNTL);
    for field in [RANGE_PROTECTION_FAULT_ENABLE_DEFAULT, PDE0_PROTECTION_FAULT_ENABLE_DEFAULT, PDE1_PROTECTION_FAULT_ENABLE_DEFAULT, PDE2_PROTECTION_FAULT_ENABLE_DEFAULT, TRANSLATE_FURTHER_PROTECTION_FAULT_ENABLE_DEFAULT, NACK_PROTECTION_FAULT_ENABLE_DEFAULT, DUMMY_PAGE_PROTECTION_FAULT_ENABLE_DEFAULT, VALID_PROTECTION_FAULT_ENABLE_DEFAULT, READ_PROTECTION_FAULT_ENABLE_DEFAULT, WRITE_PROTECTION_FAULT_ENABLE_DEFAULT, EXECUTE_PROTECTION_FAULT_ENABLE_DEFAULT] { tmp = REG_SET_FIELD!(tmp, GCVM_L2_PROTECTION_FAULT_CNTL, field, value); }
    tmp = REG_SET_FIELD!(tmp, GCVM_L2_PROTECTION_FAULT_CNTL, CRASH_ON_NO_RETRY_FAULT, !value); tmp = REG_SET_FIELD!(tmp, GCVM_L2_PROTECTION_FAULT_CNTL, CRASH_ON_RETRY_FAULT, !value); WREG32_SOC15!(GC, 0, regGCVM_L2_PROTECTION_FAULT_CNTL, tmp);
}

// Function-table definitions preserve the C externally visible interfaces.
static GFXHUB_V3_0_VMHub_FUNCS: amdgpu_vmhub_funcs = amdgpu_vmhub_funcs { print_l2_protection_fault_status: Some(gfxhub_v3_0_print_l2_protection_fault_status), get_invalidate_req: Some(gfxhub_v3_0_get_invalidate_req) };
pub static gfxhub_v3_0_funcs: amdgpu_gfxhub_funcs = amdgpu_gfxhub_funcs { get_fb_location: Some(gfxhub_v3_0_get_fb_location), get_mc_fb_offset: Some(gfxhub_v3_0_get_mc_fb_offset), setup_vm_pt_regs: Some(gfxhub_v3_0_setup_vm_pt_regs), gart_enable: Some(gfxhub_v3_0_gart_enable), gart_disable: Some(gfxhub_v3_0_gart_disable), set_fault_enable_default: Some(gfxhub_v3_0_set_fault_enable_default), init: Some(gfxhub_v3_0_init) };

unsafe fn gfxhub_v3_0_init(adev: *mut amdgpu_device) {
    let hub = &mut (*adev).vmhub[AMDGPU_GFXHUB!(0)];
    hub.ctx0_ptb_addr_lo32 = SOC15_REG_OFFSET!(GC, 0, regGCVM_CONTEXT0_PAGE_TABLE_BASE_ADDR_LO32); hub.ctx0_ptb_addr_hi32 = SOC15_REG_OFFSET!(GC, 0, regGCVM_CONTEXT0_PAGE_TABLE_BASE_ADDR_HI32); hub.vm_inv_eng0_sem = SOC15_REG_OFFSET!(GC, 0, regGCVM_INVALIDATE_ENG0_SEM); hub.vm_inv_eng0_req = SOC15_REG_OFFSET!(GC, 0, regGCVM_INVALIDATE_ENG0_REQ); hub.vm_inv_eng0_ack = SOC15_REG_OFFSET!(GC, 0, regGCVM_INVALIDATE_ENG0_ACK); hub.vm_context0_cntl = SOC15_REG_OFFSET!(GC, 0, regGCVM_CONTEXT0_CNTL); hub.vm_l2_pro_fault_status = SOC15_REG_OFFSET!(GC, 0, regGCVM_L2_PROTECTION_FAULT_STATUS); hub.vm_l2_pro_fault_cntl = SOC15_REG_OFFSET!(GC, 0, regGCVM_L2_PROTECTION_FAULT_CNTL);
