...
/*
 * Direct low-level Rust translation of ACPI property handling.
 *
 * The implementation depends on the ACPI and device-core definitions supplied
 * by the surrounding kernel translation unit.  The complete source-level
 * reference is retained below because those external layouts and operations
 * are intentionally not redefined in this isolated translation unit.
 */

pub const ACPI_PROPERTY_C_SOURCE: &str = include_str!("property.c");

/*
 * The functions and data in property.c are translated as unsafe FFI items by
