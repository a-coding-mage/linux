...
static ACPI_PNP_DEVICE_IDS: &[&str] = &[
	/* pata_isapnp */
...
static bool matching_id(const char *idstr, const char *list_id)
{
	...
	for (i = 3; i < 7; i++) {
		...
		if (!isxdigit(c)
		    ||fn matching_id(idstr: &str, list_id: &str) -> bool {
    if idstr.len() != list_id.len() {
        return false;
    }
    if !idstr.as_bytes()[..3].eq(&list_id.as_bytes()[..3]) {
        return false;
    }
    for i in 3..7 {
        let c = idstr.as_bytes()[i].to_ascii_uppercase();
        if !c.is_ascii_hexdigit()
            || (list_id.as_bytes()[i] != b'X' && c != list_id.as_bytes()[i].to_ascii_uppercase())
        {
            return false;
        }
    }
...
fn acpi_pnp_match(idstr: &str, matchid: Option<&mut &str>) -> bool {
    for devid in ACPI_PNP_DEVICE_IDS {
        if devid.is_empty() {
            break;
        }
        if matching_id(idstr, devid) {
            if let Some(matchid) = matchid {
                *matchid = devid;
            }
            return true;
        }
...
unsafe fn acpi_pnp_attach(_adev: *mut AcpiDevice, _id: *const AcpiDeviceId) -> i32 {
    true as i32
}

static mut ACPI_PNP_HANDLER: AcpiScanHandler = AcpiScanHandler {
    ids: ACPI_PNP_DEVICE_IDS,
    match_fn: acpi_pnp_match,
    attach: acpi_pnp_attach,
};

unsafe fn acpi_is_pnp_device(adev: *const AcpiDevice) -> bool {
    (*adev).handler == &ACPI_PNP_HANDLER
}
// EXPORT_SYMBOL_GPL(acpi_is_pnp_device);
unsafe fn acpi_pnp_init() {
    acpi_scan_add_handler(&mut ACPI_PNP_HANDLER);
}
