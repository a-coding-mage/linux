...
pub unsafe fn acpi_ns_root_initialize() -> acpi_status {
    ...
    while !(*init_val).name.is_null() {
        ...
        (*new_node).r#type = (*init_val).r#type;
        ...
        if !(*init_val).val.is_null() {
            ...
            obj_desc = acpi_ut_create_internal_object((*init_val).r#type);
            if obj_desc.is_null() { status = AE_NO_MEMORY; goto_unlock_and_exit!(); }
            match (*init_val).r#type {
                ACPI_TYPE_METHOD => {
                ...
                _ => { ACPI_ERROR!((AE_INFO, "Unsupported initial type value 0x%X", (*init_val).r#type)); acpi_ut_remove_reference(obj_desc); obj_desc = core::ptr::null_mut(); init_val = init_val.add(1); continue; }
            }
            status = acpi_ns_attach_object(new_node, obj_desc, (*obj_desc).common.r#type);
