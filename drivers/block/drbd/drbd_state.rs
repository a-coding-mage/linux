...
// dependency: #include <linux/drbd_limits.h>
// dependency: #include "drbd_int.h"
// dependency: #include "drbd_protocol.h"
// dependency: #include "drbd_req.h"
// dependency: #include "drbd_state_change.h"
...
static () count_objects( drbd_resource *resource,
			  u32 *n_devices,
			  u32 *n_connections)
{
	 drbd_device *device;
...
static  drbd_state_change *alloc_state_change(u32 n_devices, u32 n_connections, gfp_t gfp)
{
	...
	size = sizeof( drbd_state_change) +
	       ...
	       n_connections * sizeof( drbd_connection_state_change) +
	       n_devices * n_connections * sizeof( drbd_peer_device_state_change);
	state_change = kmalloc(size, gfp);
	if (!state_change)
		return core::ptr::null_mut();
	state_change.n_devices = n_devices;
	state_change.n_connections = n_connections;
	state_change.devices = (() *)(state_change + 1);
	state_change.connections = (() *)&state_change.devices[n_devices];
	state_change.peer_devices = (() *)&state_change.connections[n_connections];
	state_change.resource.resource = core::ptr::null_mut();
	for (n = 0; n < n_devices; n++)
		state_change.devices[n].device = core::ptr::null_mut();
	for (n = 0; n < n_connections; n++)
		state_change.connections[n].connection = core::ptr::null_mut();
	return state_change;
}

 drbd_state_change *remember_old_state( drbd_resource *resource, gfp_t gfp)
{
	 drbd_state_change *state_change;
	 drbd_device *device;
	u32 n_devices;
	...
	i32 vnr;
	 ...
	 drbd_device_state_change *device_state_change;
	 drbd_peer_device_state_change *peer_device_state_change;
...
static () remember_new_state( drbd_state_change *state_change)
{
	 drbd_resource_state_change *resource_state_change;
...
#define OLD_TO_NEW(x) \
	...
	for (n_peer_device = 0; n_peer_device < n_peer_devices; n_peer_device++) {
		 drbd_peer_device_state_change *p =
...
() forget_state_change( drbd_state_change *state_change)
{
	u32 n;
	...
	for (n = 0; n < state_change.n_devices; n++) {
		 drbd_device *device = state_change.devices[n].device;
...
static () after_state_ch( drbd_device *device,  drbd_state os,
			    ...
			    drbd_state_change *);
static  drbd_state_rv is_valid_state( drbd_device *,  drbd_state);
static  drbd_state_rv is_valid_soft_transition( drbd_state,  drbd_state,  drbd_connection *);
static  drbd_state_rv is_valid_transition( drbd_state os,  drbd_state ns);
static  drbd_state sanitize_state( drbd_device *device,  drbd_state os,
				        drbd_state ns,  sanitize_state_warnings *warn);

#[inline]
static bool is_susp( drbd_state s)
{
        return s.susp || s.susp_nod || s.susp_fen;
}

bool conn_all_vols_unconf( drbd_connection *connection)
{
	 drbd_peer_device *peer_device;
	bool rv = true;
	i32 vnr;
...
static  drbd_role max_role( drbd_role role1,  drbd_role role2)
{
...
static  drbd_role min_role( drbd_role role1,  drbd_role role2)
{
 ...
 drbd_role conn_highest_role( drbd_connection *connection)
{
	 drbd_role role = R_SECONDARY;
	 drbd_peer_device *peer_device;
	i32 vnr;
 ...
 drbd_role conn_highest_peer( drbd_connection *connection)
{
	 drbd_role peer = R_UNKNOWN;
	 drbd_peer_device *peer_device;
	i32 vnr;
 ...
 drbd_disk_state conn_highest_disk( drbd_connection *connection)
{
	 drbd_disk_state disk_state = D_DISKLESS;
	 drbd_peer_device *peer_device;
	i32 vnr;
 ...
 drbd_disk_state conn_lowest_disk( drbd_connection *connection)
{
	 drbd_disk_state disk_state = D_MASK;
	 drbd_peer_device *peer_device;
	i32 vnr;
 ...
 drbd_disk_state conn_highest_pdsk( drbd_connection *connection)
{
	 drbd_disk_state disk_state = D_DISKLESS;
	 drbd_peer_device *peer_device;
	i32 vnr;
 ...
 drbd_conns conn_lowest_conn( drbd_connection *connection)
{
	 drbd_conns conn = C_MASK;
	 drbd_peer_device *peer_device;
	i32 vnr;
...
static bool no_peer_wf_report_params( drbd_connection *connection)
{
	 drbd_peer_device *peer_device;
	i32 vnr;
	bool rv = true;
...
static () wake_up_all_devices( drbd_connection *connection)
{
	 drbd_peer_device *peer_device;
	i32 vnr;
 ...
 * @ns:		new (wanted) state.
 */
static i32 cl_wide_st_chg( drbd_device *device,
			   drbd_state os,  drbd_state ns)
{
...
static  drbd_state
apply_mask_val( drbd_state os,  drbd_state mask,  drbd_state val)
{
	 drbd_state ns;
 ...
 drbd_state_rv
drbd_change_state( drbd_device *device,  chg_state_flags f,
		   drbd_state mask,  drbd_state val)
{
	u64 flags;
	 drbd_state ns;
	 drbd_state_rv rv;
	...
	rv = _drbd_set_state(device, ns, f, core::ptr::null_mut());
 ...
 */
() drbd_force_state( drbd_device *device,
	 drbd_state mask,  drbd_state val)
{
...
_req_st_cond( drbd_device *device,  drbd_state mask,
	      drbd_state val)
{
	 drbd_state os, ns;
	u64 flags;
 ...
 */
 drbd_state_rv
_drbd_request_state( drbd_device *device,  drbd_state mask,
		     drbd_state val,  chg_state_flags f)
{
	 drbd_state_rv rv;
 ...
 */
static  drbd_state_rv
request_detach( drbd_device *device)
{
...
i32 drbd_request_detach_interruptible( drbd_device *device)
{
 ...
 drbd_state_rv
_drbd_request_state_holding_state_mutex( drbd_device *device,  drbd_state mask,
		     drbd_state val,  chg_state_flags f)
{
	 drbd_state_rv rv;
...
static () print_st( drbd_device *device, const  *mut i8name,  drbd_state ns)
{
...
() print_st_err( drbd_device *device,  drbd_state os,
	           drbd_state ns,  drbd_state_rv err)
{
...
static () drbd_pr_state_change( drbd_device *device,  drbd_state os,  drbd_state ns,
				  chg_state_flags flags)
{
	char pb[300];
	 *mut i8pbp = pb;
...
static () conn_pr_state_change( drbd_connection *connection,  drbd_state os,  drbd_state ns,
				  chg_state_flags flags)
{
	char pb[300];
	 *mut i8pbp = pb;
 ...
 */
static  drbd_state_rv
is_valid_soft_transition( drbd_state os,  drbd_state ns,  drbd_connection *connection)
{
	 drbd_state_rv rv = SS_SUCCESS;
...
static  drbd_state_rv
is_valid_conn_transition( drbd_conns oc,  drbd_conns nc)
{
 ...
 */
static  drbd_state_rv
is_valid_transition( drbd_state os,  drbd_state ns)
{
	 drbd_state_rv rv;
...
static () print_sanitize_warnings( drbd_device *device,  sanitize_state_warnings warn)
{
	static const  *mut i8msg_table[] = {
		[NO_WARNING] = "",
...
{
	 drbd_fencing_p fp;
	 drbd_disk_state disk_min, disk_max, pdsk_min, pdsk_max;
...
() drbd_resume_al( drbd_device *device)
{
...
static () set_ov_position( drbd_peer_device *peer_device,  drbd_conns cs)
{
	 drbd_device *device = peer_device.device;
	...
	if (cs == C_VERIFY_T) {
		...
		u64 bit = BM_SECT_TO_BIT(device.ov_start_sector);
 ...
 */
 drbd_state_rv
_drbd_set_state( drbd_device *device,  drbd_state ns,
	         chg_state_flags flags,  completion *done)
{
	 drbd_peer_device *peer_device = first_peer_device(device);
	 drbd_connection *connection = peer_device ? peer_device.connection : core::ptr::null_mut();
	 drbd_state os;
	 drbd_state_rv rv = SS_SUCCESS;
	 sanitize_state_warnings ssw;
	...
	if (drbd_should_do_remote(( drbd_dev_state)os.i) !=
	    drbd_should_do_remote(( drbd_dev_state)ns.i))
...
static i32 w_after_state_ch( drbd_work *w, i32 unused)
{
	 ...
	 drbd_device *device = ascw.device;
...
static () broadcast_state_change( drbd_state_change *state_change)
{
	 drbd_resource_state_change *resource_state_change = &state_change.resource[0];
...
#define FINAL_STATE_CHANGE(type) \
	({ if (last_func) \
		last_func(core::ptr::null_mut(), 0, last_arg, type); \
	})
...
static bool lost_contact_to_peer_data( drbd_disk_state os,  drbd_disk_state ns)
{
...
static () after_state_ch( drbd_device *device,  drbd_state os,
			    ...
			    drbd_state_change *state_change)
{
	 drbd_resource *resource = device.resource;
	 drbd_peer_device *peer_device = first_peer_device(device);
	 drbd_connection *connection = peer_device ? peer_device.connection : core::ptr::null_mut();
	 sib_info sib;
	...
	if (ns.susp_nod) {
		...
		if (resource.susp_nod && what != NOTHING) {
			...
			_conn_request_state(connection,
					    ( drbd_state) { { .susp_nod = 1 } },
					    ( drbd_state) { { .susp_nod = 0 } },
					    CS_VERBOSE);
		}
	...
	if (ns.susp_fen) {
		...
		if (resource.susp_fen && conn_lowest_conn(connection) >= C_CONNECTED) {
			...
			_conn_request_state(connection,
					    ( drbd_state) { { .susp_fen = 1 } },
					    ( drbd_state) { { .susp_fen = 0 } },
					    CS_VERBOSE);
		}
		spin_unlock_irq(&device.resource.req_lock);
	}
	...
	if (os.conn != C_WF_BITMAP_S && ns.conn == C_WF_BITMAP_S &&
		...
		drbd_queue_bitmap_io(device, &drbd_send_bitmap, core::ptr::null_mut(),
	...
	if ((os.conn > C_CONNECTED && os.conn < C_AHEAD) &&
		...
		drbd_queue_bitmap_io(device, &drbd_bm_write_copy_pages, core::ptr::null_mut(),
 ...
 after_conn_state_chg_work {
	 ...
	 drbd_conns oc;
	 drbd_state ns_min;
	 drbd_state ns_max; /* new, max state, over all devices */
	 chg_state_flags flags;
	 drbd_connection *connection;
	 drbd_state_change *state_change;
};
...
static i32 w_after_conn_state_ch( drbd_work *w, i32 unused)
{
	 ...
	 drbd_connection *connection = acscw.connection;
	 drbd_conns oc = acscw.oc;
	 drbd_state ns_max = acscw.ns_max;
	 drbd_peer_device *peer_device;
	i32 vnr;
	...
	if (oc == C_DISCONNECTING && ns_max.conn == C_STANDALONE) {
		...
		notify_connection_state(core::ptr::null_mut(), 0, connection, core::ptr::null_mut(), NOTIFY_DESTROY);
		mutex_unlock(&notification_mutex);
		...
		RCU_INIT_POINTER(connection.net_conf, core::ptr::null_mut());
...
static () conn_old_common_state( drbd_connection *connection,  drbd_state *pcs,  chg_state_flags *pf)
{
...
static  drbd_state_rv
conn_is_valid_transition( drbd_connection *connection,  drbd_state mask,  drbd_state val,
			  chg_state_flags flags)
{
	 drbd_state_rv rv = SS_SUCCESS;
	 drbd_state ns, os;
...
static ()
conn_set_state( drbd_connection *connection,  drbd_state mask,  drbd_state val,
	        drbd_state *pns_min,  drbd_state *pns_max,  chg_state_flags flags)
{
	...
	idr_for_each_entry(&connection.peer_devices, peer_device, vnr) {
		...
		ns = sanitize_state(device, os, ns, core::ptr::null_mut());

		if (flags & CS_IGN_OUTD_FAIL && ns.disk == D_OUTDATED && os.disk < D_OUTDATED)
			ns.disk = os.disk;

		rv = _drbd_set_state(device, ns, flags, core::ptr::null_mut());
		BUG_ON(rv < SS_SUCCESS);
		ns.i = device.state.i;
		ns_max.role = max_role(ns.role, ns_max.role);
		ns_max.peer = max_role(ns.peer, ns_max.peer);
		ns_max.conn = max_t( drbd_conns, ns.conn, ns_max.conn);
		ns_max.disk = max_t( drbd_disk_state, ns.disk, ns_max.disk);
		ns_max.pdsk = max_t( drbd_disk_state, ns.pdsk, ns_max.pdsk);
...
static  drbd_state_rv
_conn_rq_cond( drbd_connection *connection,  drbd_state mask,  drbd_state val)
{
	 drbd_state_rv err, rv = SS_UNKNOWN_ERROR; /* continue waiting */;
		     ...
		     chg_state_flags flags)
{
 ...
 drbd_state_rv
conn_request_state( drbd_connection *connection,  drbd_state mask,  drbd_state val,
		    chg_state_flags flags)
{
	 drbd_state_rv rv;
