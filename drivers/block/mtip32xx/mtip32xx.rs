/* Rust translation of the complete mtip32xx implementation. External kernel symbols and types are intentionally unresolved and supplied by surrounding kernel bindings. */
 ...
 */
unsafe fn mtip_major;
static struct dentry *dfs_parent;

unsafe fn cpu_use[NR_CPUS];

static DEFINE_IDA(rssd_index_ida);

unsafe fn mtip_block_initialize(struct driver_data *dd);
 ...
 */
unsafe fn mtip_check_surprise_removal(struct driver_data *dd)
{
	u16 vendor_id = 0;
 ...
 */
unsafe fn mtip_hba_reset(struct driver_data *dd)
{
 ...
 *      None
 */
unsafe fn void mtip_issue_ncq_command(struct mtip_port *port, int tag)
{
	int group = tag >> 5;
 ...
 */
unsafe fn mtip_enable_fis(struct mtip_port *port, int enable)
{
	u32 tmp;
 ...
 *	Previous state: 1 enabled, 0 disabled.
 */
unsafe fn mtip_enable_engine(struct mtip_port *port, int enable)
{
	u32 tmp;
 ...
 *	None
 */
unsafe fn void mtip_start_port(struct mtip_port *port)
{
	/* Enable FIS reception */
 ...
 *	None
 */
unsafe fn void mtip_deinit_port(struct mtip_port *port)
{
 ...
 *	None
 */
unsafe fn mtip_init_port(struct mtip_port *port)
{
	int i;
 ...
 *	None
 */
unsafe fn mtip_restart_port(struct mtip_port *port)
{
	unsigned long timeout;
...
unsafe fn mtip_device_reset(struct driver_data *dd)
{
	int rv = 0;
 ...
 */
unsafe fn print_tags(struct driver_data *dd,
			char *msg,
...
unsafe fn mtip_read_log_page(struct mtip_port *port, u8 page, u16 *buffer,
				dma_addr_t buffer_dma, unsigned int sectors);
unsafe fn mtip_get_smart_attr(struct mtip_port *port, unsigned int id,
						struct smart_attr *attrib);

unsafe fn mtip_complete_command(struct mtip_cmd *cmd, blk_status_t status)
{
 ...
 *	None
 */
unsafe fn mtip_handle_tfe(struct driver_data *dd)
{
 ...
 */
unsafe fn void mtip_workq_sdbfx(struct mtip_port *port, int group,
							u32 completed)
{
 ...
 */
unsafe fn void mtip_process_legacy(struct driver_data *dd, u32 port_stat)
{
 ...
 */
unsafe fn void mtip_process_errors(struct driver_data *dd, u32 port_stat)
{
...
unsafe fn irqreturn_t mtip_handle_irq(struct driver_data *data)
{
 ...
 */
unsafe fn mtip_irq_handler(int irq, void *instance)
{
...
unsafe fn mtip_issue_non_ncq_command(struct mtip_port *port, int tag)
{
	writel(1 << MTIP_TAG_BIT(tag), port->cmd_issue[MTIP_TAG_INDEX(tag)]);
}

unsafe fn mtip_pause_ncq(struct mtip_port *port,
				struct host_to_dev_fis *fis)
{
...
unsafe fn mtip_commands_active(struct mtip_port *port)
{
	unsigned int active;
	unsigned int n;
 ...
 */
unsafe fn mtip_quiesce_io(struct mtip_port *port, unsigned long timeout)
{
	unsigned long to;
	bool active = true;
 ...
 */
unsafe fn mtip_exec_internal_command(struct mtip_port *port,
					struct host_to_dev_fis *fis,
					int fis_len,
					dma_addr_t buffer,
					int buf_len,
 ...
 *	None
 */
unsafe fn void ata_swap_string(u16 *buf, unsigned int len)
{
	int i;
	for (i = 0; i < (len/2); i++)
		be16_to_cpus(&buf[i]);
...
unsafe fn mtip_set_timeout(struct driver_data *dd,
					struct host_to_dev_fis *fis,
 ...
 *	-1	Command failed.
 */
unsafe fn mtip_get_identify(struct mtip_port *port, void __user *user_buffer)
{
	int rv = 0;
	struct host_to_dev_fis fis;
 ...
 */
unsafe fn mtip_standby_immediate(struct mtip_port *port)
{
	int rv;
	struct host_to_dev_fis	fis;
 ...
 *	@rv	return value from mtip_exec_internal_command()
 */
unsafe fn mtip_get_smart_data(struct mtip_port *port, u8 *buffer,
 ...
 */
unsafe fn mtip_hw_get_capacity(struct driver_data *dd, sector_t *sectors)
{
	struct mtip_port *port = dd->port;
 ...
 *	None
 */
unsafe fn mtip_dump_identify(struct mtip_port *port)
{
	sector_t sectors;
 ...
 *	None
 */
unsafe fn void fill_command_sg(struct driver_data *dd,
				struct mtip_cmd *command,
				int nents)
{
	int n;
 ...
 */
unsafe fn exec_drive_task(struct mtip_port *port, u8 *command)
{
	struct host_to_dev_fis	fis;
 ...
 * return value -1 An error occurred while executing the command.
 */
unsafe fn exec_drive_command(struct mtip_port *port, u8 *command,
 ...
 */
unsafe fn exec_drive_taskfile(struct driver_data *dd,
			       void __user *buf,
 ...
 */
unsafe fn mtip_hw_ioctl(struct driver_data *dd, unsigned int cmd,
			 unsigned long arg)
{
	switch (cmd) {
 ...
 */
unsafe fn mtip_hw_submit_io(struct driver_data *dd, struct request *rq,
			     struct mtip_cmd *command,
 ...
 */
unsafe fn mtip_hw_show_status(struct device *dev,
...
unsafe fn mtip_hw_read_registers(struct file *f, char __user *ubuf,
...
unsafe fn mtip_hw_read_flags(struct file *f, char __user *ubuf,
				  size_t len, loff_t *offset)
{
	struct driver_data *dd =  (struct driver_data *)f->private_data;
	char *buf;
...
unsafe fn mtip_hw_debugfs_init(struct driver_data *dd)
{
...
unsafe fn mtip_hw_debugfs_exit(struct driver_data *dd)
{
 ...
 *	None
 */
unsafe fn void hba_setup(struct driver_data *dd)
{
	u32 hwdata;
...
unsafe fn mtip_device_unaligned_constrained(struct driver_data *dd)
{
 ...
 *	None
 */
unsafe fn mtip_detect_product(struct driver_data *dd)
{
	u32 hwdata;
 ...
 */
unsafe fn mtip_ftl_rebuild_poll(struct driver_data *dd)
{
...
unsafe fn mtip_softirq_done_fn(struct request *rq)
{
...
unsafe fn mtip_abort_cmd(struct request *req, void *data)
{
...
unsafe fn mtip_queue_cmd(struct request *req, void *data)
{
	struct driver_data *dd = data;
...
unsafe fn mtip_service_thread(void *data)
{
	struct driver_data *dd = (struct driver_data *)data;
 ...
 *      None
 */
unsafe fn mtip_dma_free(struct driver_data *dd)
{
	struct mtip_port *port = dd->port;
 ...
 */
unsafe fn mtip_dma_alloc(struct driver_data *dd)
{
	struct mtip_port *port = dd->port;
...
unsafe fn mtip_hw_get_identify(struct driver_data *dd)
{
 ...
 */
unsafe fn mtip_hw_init(struct driver_data *dd)
{
	int i;
	int rv;
...
unsafe fn mtip_standby_drive(struct driver_data *dd)
{
	int rv = 0;
 ...
 *	0
 */
unsafe fn mtip_hw_exit(struct driver_data *dd)
{
	if (!dd->sr) {
 ...
 *	0
 */
unsafe fn mtip_hw_shutdown(struct driver_data *dd)
{
	/*
	 * Send standby immediate (E0h) to the drive so that it
	 * saves its state.
 ...
 */
unsafe fn mtip_hw_suspend(struct driver_data *dd)
{
	/*
 ...
 */
unsafe fn mtip_hw_resume(struct driver_data *dd)
{
 ...
 * upon hot insertion.
 */
unsafe fn rssd_disk_name_format(char *prefix,
				 int index,
				 char *buf,
				 int buflen)
{
 ...
 *                 structure pointer.
 */
unsafe fn mtip_block_ioctl(struct block_device *dev,
			    blk_mode_t mode,
			    unsigned cmd,
			    unsigned long arg)
{
 ...
 *                 structure pointer.
 */
unsafe fn mtip_block_compat_ioctl(struct block_device *dev,
			    blk_mode_t mode,
			    unsigned cmd,
			    unsigned long arg)
{
 ...
 */
unsafe fn mtip_block_getgeo(struct gendisk *disk,
...
unsafe fn mtip_block_free_disk(struct gendisk *disk)
{
	struct driver_data *dd = disk->private_data;
...
unsafe fn bool is_se_active(struct driver_data *dd)
{
...
unsafe fn bool is_stopped(struct driver_data *dd, struct request *rq)
{
...
unsafe fn mtip_check_unal_depth(struct blk_mq_hw_ctx *hctx,
				  struct request *rq)
{
...
unsafe fn mtip_issue_reserved_cmd(struct blk_mq_hw_ctx *hctx,
		struct request *rq)
{
	struct driver_data *dd = hctx->queue->queuedata;
	struct mtip_cmd *cmd = blk_mq_rq_to_pdu(rq);
...
unsafe fn mtip_queue_rq(struct blk_mq_hw_ctx *hctx,
...
unsafe fn mtip_free_cmd(struct blk_mq_tag_set *set, struct request *rq,
...
unsafe fn mtip_init_cmd(struct blk_mq_tag_set *set, struct request *rq,
...
unsafe fn blk_eh_timer_return mtip_cmd_timeout(struct request *req)
{
 ...
 *	0
 */
unsafe fn mtip_block_shutdown(struct driver_data *dd)
{
	mtip_hw_shutdown(dd);
...
unsafe fn mtip_block_suspend(struct driver_data *dd)
{
	dev_info(&dd->pdev->dev,
		"Suspending %s ...\n", dd->disk->disk_name);
	mtip_hw_suspend(dd);
...
unsafe fn mtip_block_resume(struct driver_data *dd)
{
...
unsafe fn drop_cpu(int cpu)
{
	cpu_use[cpu]--;
}

unsafe fn get_least_used_cpu_on_node(int node)
{
...
unsafe fn int mtip_get_next_rr_node(void)
{
...
unsafe fn mtip_disable_link_opts(struct driver_data *dd, struct pci_dev *pdev)
{
...
unsafe fn mtip_fix_ero_nosnoop(struct driver_data *dd, struct pci_dev *pdev)
{
	/*
 ...
 *	0 on success else an error code.
 */
unsafe fn mtip_pci_probe(struct pci_dev *pdev,
 ...
 *	None
 */
unsafe fn mtip_pci_remove(struct pci_dev *pdev)
{
 ...
 *	<0 Error
 */
unsafe fn __maybe_unused mtip_pci_suspend(struct device *dev)
{
	int rv = 0;
 ...
 *      <0 Error
 */
unsafe fn __maybe_unused mtip_pci_resume(struct device *dev)
{
	int rv = 0;
	struct driver_data *dd = dev_get_drvdata(dev);
 ...
 *      None
 */
unsafe fn mtip_pci_shutdown(struct pci_dev *pdev)
{
	struct driver_data *dd = pci_get_drvdata(pdev);
	if (dd)
 ...
 */
unsafe fn __init mtip_init(void)
{
	int error;
 ...
 *      none
 */
unsafe fn __exit mtip_exit(void)
{
