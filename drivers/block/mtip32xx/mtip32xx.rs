...
// #define AHCI_CMD_SLOT_SZ        sizeof(mtip_cmd_hdr)
...
// #define AHCI_CMD_TBL_SGL_SZ     (MTIP_MAX_SG * sizeof(mtip_cmd_sg))
...
unsafe fn mtip_block_initialize(driver_data *dd);
 ...
 */
unsafe fn mtip_check_surprise_removal(driver_data *dd)
{
	u16 vendor_id = 0;
 ...
 *      None
 */
unsafe fn void mtip_issue_ncq_command(mtip_port *port, int tag)
{
	int group = tag >> 5;
 ...
 */
unsafe fn mtip_enable_fis(mtip_port *port, int enable)
{
	u32 tmp;
 ...
 *	Previous state: 1 enabled, 0 disabled.
 */
unsafe fn mtip_enable_engine(mtip_port *port, int enable)
{
	u32 tmp;
 ...
 *	None
 */
unsafe fn void mtip_start_port(mtip_port *port)
{
	/* Enable FIS reception */
 ...
 *	None
 */
unsafe fn void mtip_deinit_port(mtip_port *port)
{
 ...
 *	None
 */
unsafe fn mtip_init_port(mtip_port *port)
{
	int i;
...
unsafe fn mtip_device_reset(driver_data *dd)
{
	int rv = 0;
 ...
 *	None
 */
unsafe fn mtip_handle_tfe(driver_data *dd)
{
	...
	for (group = 0; group < (*dd).slot_groups; group++) {
		for (bit = 0; bit < 32; bit++) {
			...
			fis = (*(host_to_dev_fis *)cmd).command;
 ...
 */
unsafe fn void mtip_workq_sdbfx(mtip_port *port, int group,
							completed: u32)
{
 ...
 */
unsafe fn void mtip_process_legacy(driver_data *dd, port_stat: u32)
{
 ...
 */
unsafe fn void mtip_process_errors(driver_data *dd, port_stat: u32)
{
...
unsafe fn irqreturn_t mtip_handle_irq(driver_data *data)
{
	struct driver_data *dd = (driver_data *) data;
	struct mtip_port *port = (*dd).port;
	hba_stat: u32, port_stat;
	int rv = IRQ_NONE;
...
unsafe fn mtip_issue_non_ncq_command(mtip_port *port, int tag)
{
 ...
 */
unsafe fn mtip_exec_internal_command(mtip_port *port,
					host_to_dev_fis *fis,
					int fis_len,
					dma_addr_t buffer,
					int buf_len,
 ...
 *	-1	Command failed.
 */
unsafe fn mtip_get_identify(mtip_port *port, void __user *user_buffer)
{
	'out: {
	int rv = 0;
	struct host_to_dev_fis fis;
 ...
 */
unsafe fn mtip_standby_immediate(mtip_port *port)
{
	int rv;
	struct host_to_dev_fis	fis;
 ...
 *	@rv	return value from mtip_exec_internal_command()
 */
unsafe fn mtip_get_smart_data(mtip_port *port, u8 *buffer,
						...
						smart_attr *attrib)
{
	...
	pattr = (smart_attr *)(port->smart_buf + 2);
	for (i = 0; i < 29; i++, pattr++)
		if (pattr->attr_id == id) {
			memcpy(attrib, pattr, sizeof(smart_attr));
			break;
		}
 ...
 */
unsafe fn mtip_hw_get_capacity(driver_data *dd, sector_t *sectors)
{
	struct mtip_port *port = dd->port;
	total: u64, raw0, raw1, raw2, raw3;
 ...
 *	None
 */
unsafe fn void fill_command_sg(driver_data *dd,
				mtip_cmd *command,
				int nents)
{
	int n;
 ...
 */
unsafe fn exec_drive_task(mtip_port *port, u8 *command)
{
	struct host_to_dev_fis	fis;
 ...
 * return value -1 An error occurred while executing the command.
 */
unsafe fn exec_drive_command(mtip_port *port, u8 *command,
 ...
 */
unsafe fn exec_drive_taskfile(driver_data *dd,
			       void __user *buf,
 ...
 */
unsafe fn mtip_hw_submit_io(driver_data *dd, request *rq,
			     mtip_cmd *command,
			     blk_mq_hw_ctx *hctx)
{
	struct mtip_cmd_hdr *hdr =
		dd->port->command_list + sizeof(mtip_cmd_hdr) * rq->tag;
	struct host_to_dev_fis	*fis;
	struct mtip_port *port = dd->port;
 ...
 */
unsafe fn mtip_hw_show_status(device *dev,
...
unsafe fn mtip_hw_read_registers(file *f, char __user *ubuf,
				  size_t len, loff_t *offset)
{
	struct driver_data *dd =  (driver_data *)f->private_data;
	char *buf;
	u32 group_allocated;
	int size = *offset;
	int n, rv = 0;
...
unsafe fn mtip_hw_read_flags(file *f, char __user *ubuf,
				  size_t len, loff_t *offset)
{
	struct driver_data *dd =  (driver_data *)f->private_data;
	char *buf;
...
unsafe fn mtip_hw_debugfs_init(driver_data *dd)
{
...
unsafe fn mtip_hw_debugfs_exit(driver_data *dd)
{
 ...
 *	None
 */
unsafe fn void hba_setup(driver_data *dd)
{
	u32 hwdata;
...
unsafe fn mtip_device_unaligned_constrained(driver_data *dd)
{
...
unsafe fn mtip_softirq_done_fn(request *rq)
{
...
unsafe fn mtip_abort_cmd(request *req, void *data)
{
...
unsafe fn mtip_queue_cmd(request *req, void *data)
{
	struct driver_data *dd = data;
 ...
 *      None
 */
unsafe fn mtip_dma_free(driver_data *dd)
{
	struct mtip_port *port = dd->port;
 ...
 */
unsafe fn mtip_dma_alloc(driver_data *dd)
{
	struct mtip_port *port = dd->port;
...
unsafe fn mtip_hw_get_identify(driver_data *dd)
{
	...
	memset(&attr242, 0, sizeof(smart_attr));
 ...
 */
unsafe fn mtip_hw_init(driver_data *dd)
{
	'out1: {
	'out2: {
	'out3: {
	...
	dd->port = kzalloc_node(sizeof(mtip_port), GFP_KERNEL,
				dd->numa_node);
	if (!dd->port)
		return -ENOMEM;
...
unsafe fn mtip_standby_drive(driver_data *dd)
{
	int rv = 0;
 ...
 *	0
 */
unsafe fn mtip_hw_exit(driver_data *dd)
{
	if (!dd->sr) {
 ...
 *	0
 */
unsafe fn mtip_hw_shutdown(driver_data *dd)
{
	/*
	 * Send standby immediate (E0h) to the drive so that it
	 * saves its state.
 ...
 */
unsafe fn mtip_hw_suspend(driver_data *dd)
{
	/*
 ...
 */
unsafe fn mtip_hw_resume(driver_data *dd)
{
			    ...
			    arg: core::ffi::c_ulong)
{
	...
	case HDIO_DRIVE_TASKFILE: {
		...
		compat_tasksize =
			sizeof(mtip_compat_ide_task_request_s);

		compat_req_task =
			(mtip_compat_ide_task_request_s __user *) arg;
		...
		outtotal = sizeof(mtip_compat_ide_task_request_s);
 ...
 */
unsafe fn mtip_block_getgeo(gendisk *disk,
				hd_geometry *geo)
{
...
unsafe fn mtip_block_free_disk(gendisk *disk)
{
	struct driver_data *dd = disk->private_data;
...
unsafe fn bool is_se_active(driver_data *dd)
{
...
unsafe fn bool is_stopped(driver_data *dd, request *rq)
{
...
unsafe fn mtip_check_unal_depth(blk_mq_hw_ctx *hctx,
				  request *rq)
{
...
unsafe fn mtip_issue_reserved_cmd(blk_mq_hw_ctx *hctx,
		request *rq)
{
	struct driver_data *dd = hctx->queue->queuedata;
	struct mtip_cmd *cmd = blk_mq_rq_to_pdu(rq);
...
unsafe fn mtip_queue_rq(blk_mq_hw_ctx *hctx,
...
unsafe fn blk_eh_timer_return mtip_cmd_timeout(request *req)
{
	'exit_handler: {
...
unsafe fn mtip_block_initialize(driver_data *dd)
{
	...
	dd->tags.cmd_size = sizeof(mtip_cmd);
	...
	if (rv) {
		...
		break 'block_queue_alloc_tag_error;
	}
	...
	if (IS_ERR(dd->disk)) {
		...
		rv = -ENOMEM;
		dd->disk = NULL;
		break 'block_queue_alloc_init_error;
	}
 ...
 *	0
 */
unsafe fn mtip_block_shutdown(driver_data *dd)
{
	mtip_hw_shutdown(dd);
...
unsafe fn mtip_block_suspend(driver_data *dd)
{
	dev_info(&dd->pdev->dev,
		"Suspending %s ...\n", dd->disk->disk_name);
	mtip_hw_suspend(dd);
...
unsafe fn mtip_block_resume(driver_data *dd)
{
...
unsafe fn mtip_fix_ero_nosnoop(driver_data *dd, pci_dev *pdev)
{
	/*
 ...
 *	0 on success else an error code.
 */
unsafe fn mtip_pci_probe(pci_dev *pdev,
			const struct pci_device_id *ent)
{
	...
	dd = kzalloc_node(sizeof(driver_data), GFP_KERNEL, my_node);
	if (!dd)
		return -ENOMEM;
 ...
 *	<0 Error
 */
unsafe fn __maybe_unused mtip_pci_suspend(device *dev)
{
	int rv = 0;
 ...
 *      <0 Error
 */
unsafe fn __maybe_unused mtip_pci_resume(device *dev)
{
	int rv = 0;
	struct driver_data *dd = dev_get_drvdata(dev);
 ...
 *      None
 */
unsafe fn mtip_pci_shutdown(pci_dev *pdev)
{
	struct driver_data *dd = pci_get_drvdata(pdev);
	if (dd)
