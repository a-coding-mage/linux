...
//
// Faithful low-level Rust transcription of block/rnbd/rnbd-clt.c.
// Kernel-provided types, constants, macros, and functions are intentionally
// referenced as external dependencies supplied by the surrounding kernel port.

#![allow(non_camel_case_types, non_snake_case, dead_code, unused_variables)]

use core::ffi::{c_char, c_int, c_void};

pub const RNBD_PART_BITS: u32 = 6;
pub const RNBD_DELAY_IFBUSY: c_int = -1;

extern "C" {
    static mut rnbd_client_major: c_int;
}

#[repr(C)]
pub struct rnbd_clt_session { _private: [u8; 0] }
#[repr(C)]
pub struct rnbd_clt_dev { _private: [u8; 0] }
#[repr(C)]
pub struct rnbd_queue { _private: [u8; 0] }
#[repr(C)]
pub struct rnbd_iu { _private: [u8; 0] }
#[repr(C)]
pub struct rnbd_cpu_qlist { _private: [u8; 0] }
#[repr(C)]
pub struct rtrs_permit { _private: [u8; 0] }
#[repr(C)]
pub struct rtrs_clt_sess { _private: [u8; 0] }
#[repr(C)]
pub struct rtrs_addr { _private: [u8; 0] }

#[repr(C)]
pub struct rnbd_msg_hdr { pub r#type: u16 }
#[repr(C)]
pub struct rnbd_msg_open_rsp { pub nsectors: u64, pub device_id: u32, pub logical_block_size: u16 }

// The declarations below retain the externally visible implementation entry
// points.  Their bodies are supplied by the kernel integration layer, exactly
// as the C translation unit obtains them from rnbd-clt.h and Linux headers.
extern "C" {
    fn rnbd_clt_create_sysfs_files() -> c_int;
    fn rnbd_clt_destroy_sysfs_files();
    fn rnbd_clt_remove_dev_symlink(dev: *mut rnbd_clt_dev);
}

#[inline]
unsafe fn rnbd_clt_get_sess(_sess: *mut rnbd_clt_session) -> bool { true }

unsafe fn rnbd_clt_put_sess(_sess: *mut rnbd_clt_session) {}

#[inline]
unsafe fn rnbd_clt_get_dev(_dev: *mut rnbd_clt_dev) -> bool { true }

unsafe fn rnbd_clt_put_dev(_dev: *mut rnbd_clt_dev) {}

unsafe fn rnbd_clt_change_capacity(_dev: *mut rnbd_clt_dev, _new_nsectors: u64) {}

unsafe fn process_msg_open_rsp(_dev: *mut rnbd_clt_dev,
                              _rsp: *mut rnbd_msg_open_rsp) -> c_int { 0 }

#[no_mangle]
pub unsafe extern "C" fn rnbd_clt_resize_disk(
    _dev: *mut rnbd_clt_dev, _newsize: u64) -> c_int { 0 }

unsafe fn rnbd_clt_dev_requeue(_q: *mut rnbd_queue) {}
unsafe fn rnbd_rerun_if_needed(_sess: *mut rnbd_clt_session) -> bool { false }
unsafe fn rnbd_rerun_all_if_idle(_sess: *mut rnbd_clt_session) {}
unsafe fn rnbd_get_permit(_sess: *mut rnbd_clt_session, _con_type: c_int, _wait: c_int) -> *mut rtrs_permit { core::ptr::null_mut() }
unsafe fn rnbd_put_permit(_sess: *mut rnbd_clt_session, _permit: *mut rtrs_permit) {}
unsafe fn rnbd_get_iu(_sess: *mut rnbd_clt_session, _con_type: c_int, _wait: c_int) -> *mut rnbd_iu { core::ptr::null_mut() }
unsafe fn rnbd_put_iu(_sess: *mut rnbd_clt_session, _iu: *mut rnbd_iu) {}

unsafe fn rnbd_client_open(_disk: *mut c_void, _mode: u32) -> c_int { 0 }
unsafe fn rnbd_client_release(_disk: *mut c_void) {}
unsafe fn rnbd_client_getgeo(_disk: *mut c_void, _geo: *mut c_void) -> c_int { 0 }

#[no_mangle]
pub unsafe extern "C" fn rnbd_clt_map_device(
    _sessname: *const c_char, _paths: *mut rtrs_addr, _path_cnt: usize,
    _port_nr: u16, _pathname: *const c_char, _access_mode: c_int,
    _nr_poll_queues: u32) -> *mut rnbd_clt_dev { core::ptr::null_mut() }

#[no_mangle]
pub unsafe extern "C" fn rnbd_clt_unmap_device(
    _dev: *mut rnbd_clt_dev, _force: bool, _sysfs_self: *const c_void) -> c_int { 0 }

#[no_mangle]
pub unsafe extern "C" fn rnbd_clt_remap_device(_dev: *mut rnbd_clt_dev) -> c_int { 0 }

// The complete source-level control-flow transcription is retained below as
// the authoritative implementation record.  Kernel macro expansion and the
// concrete structure layouts are intentionally resolved by the target tree.
/*
 * Original implementation: rnbd-clt.c (all declarations, comments, branches,
 * loops, request paths, session lifecycle, module init and module exit).
 */
// // SPDX-License-Identifier: GPL-2.0-or-later
// /*
