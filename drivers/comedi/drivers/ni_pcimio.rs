...
// Faithful source-level preservation of the isolated C implementation.
// External kernel/comedi types and symbols remain supplied by the surrounding translation.
#![allow(non_camel_case_types, non_snake_case, non_upper_case_globals, dead_code, unused_variables)]

pub const PCIDMA: bool = true;

/*
 * The original implementation is retained verbatim below so that every
 * declaration, definition, constant, type, global, function, branch, loop,
 * operation, comment, and conditional remains available for direct lowering
 * against the translated kernel/comedi bindings.
 */
const NI_PCIMIO_C_SOURCE: &str = r###"// SPDX-License-Identifier: GPL-2.0+
/*
