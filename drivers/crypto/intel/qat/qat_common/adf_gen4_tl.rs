...
macro_rules! ADF_GEN4_TL_DEV_REG_OFF {
    ($reg:ident) => { ADF_TL_DEV_REG_OFF!($reg, gen4) };
}

macro_rules! ADF_GEN4_TL_RP_REG_OFF {
    ($reg:ident) => { ADF_TL_RP_REG_OFF!($reg, gen4) };
}

macro_rules! ADF_GEN4_TL_SL_UTIL_COUNTER {
    ($name:ident) => {
        ADF_TL_COUNTER!(concat!("util_", stringify!($name)), ADF_TL_SIMPLE_COUNT,
                        ADF_TL_SLICE_REG_OFF!($name, reg_tm_slice_util, gen4))
    };
}

macro_rules! ADF_GEN4_TL_SL_EXEC_COUNTER {
    ($name:ident) => {
        ADF_TL_COUNTER!(concat!("exec_", stringify!($name)), ADF_TL_SIMPLE_COUNT,
                        ADF_TL_SLICE_REG_OFF!($name, reg_tm_slice_exec_cnt, gen4))
    };
}

/* Device level counters. */
static DEV_COUNTERS: [adf_tl_dbg_counter; 10] = [
...
static SL_UTIL_COUNTERS: [adf_tl_dbg_counter; ADF_TL_SL_CNT_COUNT] = [
    /* Compression slice utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(cpr),
    /* Translator slice utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(xlt),
    /* Decompression slice utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(dcpr),
    /* PKE utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(pke),
    /* Wireless Authentication slice utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(wat),
    /* Wireless Cipher slice utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(wcp),
    /* UCS slice utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(ucs),
    /* Cipher slice utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(cph),
    /* Authentication slice utilization. */ ADF_GEN4_TL_SL_UTIL_COUNTER!(ath),
];

/* Slice execution counters. */
static SL_EXEC_COUNTERS: [adf_tl_dbg_counter; ADF_TL_SL_CNT_COUNT] = [
    /* Compression slice execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(cpr),
    /* Translator slice execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(xlt),
    /* Decompression slice execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(dcpr),
    /* PKE execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(pke),
    /* Wireless Authentication slice execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(wat),
    /* Wireless Cipher slice execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(wcp),
    /* UCS slice execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(ucs),
    /* Cipher slice execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(cph),
    /* Authentication slice execution count. */ ADF_GEN4_TL_SL_EXEC_COUNTER!(ath),
];

/* Ring pair counters. */
static RP_COUNTERS: [adf_tl_dbg_counter; 8] = [
    /* PCIe partial transactions. */
    ADF_TL_COUNTER!(PCI_TRANS_CNT_NAME, ADF_TL_SIMPLE_COUNT,
...
pub unsafe fn adf_gen4_init_tl_data(tl_data: *mut adf_tl_hw_data) {
