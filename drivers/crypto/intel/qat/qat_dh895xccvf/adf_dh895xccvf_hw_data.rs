...
// External declarations and constants are supplied by the corresponding driver headers.

static mut dh895xcciov_class: adf_hw_device_class = adf_hw_device_class {
    name: ADF_DH895XCCVF_DEVICE_NAME,
    type_: DEV_DH895XCCVF,
};

unsafe fn get_accel_mask(_self: *mut adf_hw_device_data) -> u32 {
    ADF_DH895XCCIOV_ACCELERATORS_MASK
}

unsafe fn get_ae_mask(_self: *mut adf_hw_device_data) -> u32 {
    ADF_DH895XCCIOV_ACCELENGINES_MASK
}

unsafe fn get_num_accels(_self: *mut adf_hw_device_data) -> u32 {
    ADF_DH895XCCIOV_MAX_ACCELERATORS
}

unsafe fn get_num_aes(_self: *mut adf_hw_device_data) -> u32 {
    ADF_DH895XCCIOV_MAX_ACCELENGINES
}

unsafe fn get_misc_bar_id(_self: *mut adf_hw_device_data) -> u32 {
    ADF_DH895XCCIOV_PMISC_BAR
}

unsafe fn get_etr_bar_id(_self: *mut adf_hw_device_data) -> u32 {
    ADF_DH895XCCIOV_ETR_BAR
}

unsafe fn get_sku(_self: *mut adf_hw_device_data) -> dev_sku_info {
    DEV_SKU_VF
}
...
pub unsafe fn adf_init_hw_data_dh895xcciov(hw_data: *mut adf_hw_device_data) {
    (*hw_data).dev_class = &raw mut dh895xcciov_class;
...
pub unsafe fn adf_clean_hw_data_dh895xcciov(hw_data: *mut adf_hw_device_data) {
    (*(*hw_data).dev_class).instances -= 1;
    adf_devmgr_update_class_index(hw_data);
}
