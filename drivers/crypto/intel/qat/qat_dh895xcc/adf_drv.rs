...
static adf_pci_tbl: [pci_device_id; 2] = [
    pci_device_id { vendor: PCI_VENDOR_ID_INTEL, device: PCI_DEVICE_ID_INTEL_QAT_DH895XCC },
    pci_device_id::default(),
];

static mut adf_driver: pci_driver = pci_driver {
    id_table: adf_pci_tbl.as_ptr(), name: ADF_DH895XCC_DEVICE_NAME,
...
unsafe extern "C" fn adfdrv_init() -> i32 {
    request_module!("intel_qat");
    if pci_register_driver(&mut adf_driver) != 0 { pr_err!("QAT: Driver initialization failed\n"); return -EFAULT; }
    0
}

unsafe extern "C" fn adfdrv_release() { pci_unregister_driver(&mut adf_driver); }

// module_init(adfdrv_init); module_exit(adfdrv_release);
// MODULE_LICENSE("Dual BSD/GPL"); MODULE_AUTHOR("Intel");
// MODULE_FIRMWARE(ADF_DH895XCC_FW); MODULE_FIRMWARE(ADF_DH895XCC_MMP);
