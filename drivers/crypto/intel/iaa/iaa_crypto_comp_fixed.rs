...
/* Dependencies: idxd.h and iaa_crypto.h */
...
static FIXED_LL_SYM: [u32_; 286] = [
    ...
    0x400C0, 0x400C1, 0x400C2, 0x400C3, 0x400C4,
];

static FIXED_D_SYM: [u32_; 30] = [
...
extern "C" {
    fn add_iaa_compression_mode(
        name: *const u8,
        fixed_ll_sym: *const u32,
        fixed_ll_size: usize,
        fixed_d_sym: *const u32,
        fixed_d_size: usize,
        init_fixed_mode: unsafe extern "C" fn(*mut iaa_device_compression_mode) -> i32,
        arg: *mut c_void,
    ) -> i32;
    fn remove_iaa_compression_mode(name: *const u8);
}

const FIXED_HDR: u32 = 0;
const FIXED_HDR_SIZE: u32 = 3;

unsafe extern "C" fn init_fixed_mode(mode: *mut iaa_device_compression_mode) -> i32 {
    let comp_table = (*mode).aecs_comp_table;
    let bfinal: u32 = 1;
    let offset: usize = ((*comp_table).num_output_accum_bits / 8) as usize;

    /* Configure aecs table using fixed Huffman table */
    (*comp_table).crc = 0;
    (*comp_table).xor_checksum = 0;
    (*comp_table).output_accum.as_mut_ptr().add(offset).write((FIXED_HDR | bfinal) as u8);
    (*comp_table).num_output_accum_bits = FIXED_HDR_SIZE;

    0
}

pub unsafe extern "C" fn iaa_aecs_init_fixed() -> i32 {
    let ret = add_iaa_compression_mode(
        b"fixed\0".as_ptr(),
        FIXED_LL_SYM.as_ptr(),
        core::mem::size_of_val(&FIXED_LL_SYM),
        FIXED_D_SYM.as_ptr(),
        core::mem::size_of_val(&FIXED_D_SYM),
        init_fixed_mode,
        core::ptr::null_mut(),
    );
    if ret == 0 {
...
pub unsafe extern "C" fn iaa_aecs_cleanup_fixed() {
    remove_iaa_compression_mode(b"fixed\0".as_ptr());
}
