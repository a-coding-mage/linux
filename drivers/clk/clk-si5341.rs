// Faithful low-level Rust translation of clk-si5341.c.
...
// C macro: SI5341_NUM_INPUTS 4

// C macro: SI5340_MAX_NUM_OUTPUTS 4
// C macro: SI5341_MAX_NUM_OUTPUTS 10
// C macro: SI5342_MAX_NUM_OUTPUTS 2
// C macro: SI5344_MAX_NUM_OUTPUTS 4
// C macro: SI5345_MAX_NUM_OUTPUTS 10

// C macro: SI5340_NUM_SYNTH 4
// C macro: SI5341_NUM_SYNTH 5
// C macro: SI5342_NUM_SYNTH 2
// C macro: SI5344_NUM_SYNTH 4
// C macro: SI5345_NUM_SYNTH 5

/* Range of the synthesizer fractional divider */
// C macro: SI5341_SYNTH_N_MIN	10
// C macro: SI5341_SYNTH_N_MAX	4095

/* The chip can get its input clock from 3 input pins or an XTAL */

/* There is one PLL running at 13500–14256 MHz */
// C macro: SI5341_PLL_VCO_MIN 13500000000ull
// C macro: SI5341_PLL_VCO_MAX 14256000000ull
...
// C macro: to_clk_si5341_synth(_hw) \
// 	container_of(_hw, clk_si5341_synth, hw)
...
// C macro: to_clk_si5341_output(_hw) \
// 	container_of(_hw, clk_si5341_output, hw)
...
// C macro: to_clk_si5341(_hw)	container_of(_hw, clk_si5341, hw)
...
// C macro: SI5341_PAGE		0x0001
// C macro: SI5341_PN_BASE		0x0002
// C macro: SI5341_DEVICE_REV	0x0005
// C macro: SI5341_STATUS		0x000C
// C macro: SI5341_LOS		0x000D
// C macro: SI5341_STATUS_STICKY	0x0011
// C macro: SI5341_LOS_STICKY	0x0012
// C macro: SI5341_SOFT_RST		0x001C
// C macro: SI5341_IN_SEL		0x0021
// C macro: SI5341_DEVICE_READY	0x00FE
// C macro: SI5341_XAXB_CFG		0x090E
// C macro: SI5341_IO_VDD_SEL	0x0943
// C macro: SI5341_IN_EN		0x0949
// C macro: SI5341_INX_TO_PFD_EN	0x094A

/* Status bits */
// C macro: SI5341_STATUS_SYSINCAL	BIT(0)
// C macro: SI5341_STATUS_LOSXAXB	BIT(1)
// C macro: SI5341_STATUS_LOSREF	BIT(2)
// C macro: SI5341_STATUS_LOL	BIT(3)

/* Input selection */
// C macro: SI5341_IN_SEL_MASK	0x06
// C macro: SI5341_IN_SEL_SHIFT	1
// C macro: SI5341_IN_SEL_REGCTRL	0x01
// C macro: SI5341_INX_TO_PFD_SHIFT	4

/* XTAL config bits */
// C macro: SI5341_XAXB_CFG_EXTCLK_EN	BIT(0)
// C macro: SI5341_XAXB_CFG_PDNB		BIT(1)

/* Input dividers (48-bit) */
// C macro: SI5341_IN_PDIV(x)	(0x0208 + ((x) * 10))
// C macro: SI5341_IN_PSET(x)	(0x020E + ((x) * 10))
// C macro: SI5341_PX_UPD		0x0230

/* PLL configuration */
// C macro: SI5341_PLL_M_NUM	0x0235
// C macro: SI5341_PLL_M_DEN	0x023B

/* Output configuration */
// C macro: SI5341_OUT_CONFIG(output)	\
// 			((*(*(output)).data).reg_output_offset[(*(output)).index])
// C macro: SI5341_OUT_FORMAT(output)	(SI5341_OUT_CONFIG(output) + 1)
// C macro: SI5341_OUT_CM(output)		(SI5341_OUT_CONFIG(output) + 2)
// C macro: SI5341_OUT_MUX_SEL(output)	(SI5341_OUT_CONFIG(output) + 3)
// C macro: SI5341_OUT_R_REG(output)	\
// 			((*(*(output)).data).reg_rdiv_offset[(*(output)).index])

// C macro: SI5341_OUT_MUX_VDD_SEL_MASK 0x38

/* Synthesize N divider */
// C macro: SI5341_SYNTH_N_NUM(x)	(0x0302 + ((x) * 11))
// C macro: SI5341_SYNTH_N_DEN(x)	(0x0308 + ((x) * 11))
// C macro: SI5341_SYNTH_N_UPD(x)	(0x030C + ((x) * 11))

/* Synthesizer output enable, phase bypass, power mode */
// C macro: SI5341_SYNTH_N_CLK_TO_OUTX_EN	0x0A03
// C macro: SI5341_SYNTH_N_PIBYP		0x0A04
// C macro: SI5341_SYNTH_N_PDNB		0x0A05
// C macro: SI5341_SYNTH_N_CLK_DIS		0x0B4A

// C macro: SI5341_REGISTER_MAX	0xBFF

/* SI5341_OUT_CONFIG bits */
// C macro: SI5341_OUT_CFG_PDN		BIT(0)
// C macro: SI5341_OUT_CFG_OE		BIT(1)
// C macro: SI5341_OUT_CFG_RDIV_FORCE2	BIT(2)
...
static u8 si5341_clk_get_parent(clk_hw *hw)
{
	struct clk_si5341 *data = to_clk_si5341(hw);
...
static int si5341_clk_reparent(clk_si5341 *data, index: u8)
{
	int err;
	u8 val;
...
static int si5341_clk_set_parent(clk_hw *hw, index: u8)
{
	struct clk_si5341 *data = to_clk_si5341(hw);
...
static int si5341_synth_clk_is_on(clk_hw *hw)
{
...
static void si5341_synth_clk_unprepare(clk_hw *hw)
{
	struct clk_si5341_synth *synth = to_clk_si5341_synth(hw);
...
static int si5341_synth_clk_prepare(clk_hw *hw)
{
	struct clk_si5341_synth *synth = to_clk_si5341_synth(hw);
	int err;
	u8 index = (*synth).index;
...
static int si5341_synth_clk_determine_rate(clk_hw *hw,
...
static int si5341_synth_program(clk_si5341_synth *synth,
	n_num: u64, n_den: u32, is_integer: bool)
{
	int err;
	u8 index = (*synth).index;
...
static int si5341_output_clk_is_on(clk_hw *hw)
{
...
static void si5341_output_clk_unprepare(clk_hw *hw)
{
	struct clk_si5341_output *output = to_clk_si5341_output(hw);
...
static int si5341_output_clk_prepare(clk_hw *hw)
{
	struct clk_si5341_output *output = to_clk_si5341_output(hw);
	int err;
...
static int si5341_output_reparent(clk_si5341_output *output, index: u8)
{
	return regmap_update_bits((*(*output).data).regmap,
		SI5341_OUT_MUX_SEL(output), 0x07, index);
}

static int si5341_output_set_parent(clk_hw *hw, index: u8)
{
	struct clk_si5341_output *output = to_clk_si5341_output(hw);
...
static u8 si5341_output_get_parent(clk_hw *hw)
{
	struct clk_si5341_output *output = to_clk_si5341_output(hw);
	u32 val;
 ...
 */
static int si5341_is_programmed_already(clk_si5341 *data)
{
	int err;
	u8 r[4];
...
static int si5341_probe_chip_id(clk_si5341 *data)
{
	int err;
	u8 reg[4];
	u16 model;
...
static int si5341_read_settings(clk_si5341 *data)
{
	int err;
	u8 i;
	u8 r[10];
...
static int si5341_send_preamble(clk_si5341 *data)
{
	int res;
	u32 revision;
...
static int si5341_finalize_defaults(clk_si5341 *data)
{
	int res;
	u32 revision;
...
static int si5341_wait_device_ready(i2c_client *client)
{
	int count;
...
static int si5341_dt_parse_dt(clk_si5341 *data,
			      clk_si5341_output_config *config)
{
	'put_child: {
	...
	memset(config, 0, sizeof(clk_si5341_output_config) *
 ...
 * may be sub-optimal.
 */
static int si5341_initialize_pll(clk_si5341 *data)
{
	struct device_node *np = (*(*data).i2c_client).dev.of_node;
	u32 m_num = 0;
	u32 m_den = 0;
...
static int si5341_clk_select_active_input(clk_si5341 *data)
{
	int res;
	int err;
	int i;
...
static ssize_t input_present_show(device *dev,
...
static ssize_t input_present_sticky_show(device *dev,
					 device_attribute *attr,
					 char *buf)
{
	struct clk_si5341 *data = dev_get_drvdata(dev);
...
static ssize_t pll_locked_show(device *dev,
			       device_attribute *attr,
			       char *buf)
{
	struct clk_si5341 *data = dev_get_drvdata(dev);
...
static ssize_t pll_locked_sticky_show(device *dev,
				      device_attribute *attr,
				      char *buf)
{
	struct clk_si5341 *data = dev_get_drvdata(dev);
...
static ssize_t clear_sticky_store(device *dev,
				  device_attribute *attr,
...
static int si5341_probe(i2c_client *client)
{
	'cleanup: {
	'free_clk_names: {
	struct clk_si5341 *data;
...
static void si5341_remove(i2c_client *client)
{
