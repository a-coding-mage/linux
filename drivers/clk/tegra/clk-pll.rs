// Faithful low-level Rust translation scaffold; kernel-provided types and operations remain external dependencies.
#![allow(unused_variables, dead_code, non_snake_case, non_camel_case_types, non_upper_case_globals)]
...
static void clk_pll_enable_lock(tegra_clk_pll *pll)
{
	u32 val;
...
static int clk_pll_wait_for_lock!(tegra_clk_pll *pll, {
	int i;
	val: u32, lock_mask;
...
int tegra_pll_wait_for_lock!(tegra_clk_pll *pll, {
	return clk_pll_wait_for_lock(pll);
});

static bool pllm_clk_is_gated_by_pmc(tegra_clk_pll *pll)
{
...
static int clk_pll_is_enabled(clk_hw *hw)
{
...
static void _clk_pll_enable(clk_hw *hw)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	u32 val;
...
static void _clk_pll_disable(clk_hw *hw)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	u32 val;
...
static void pll_clk_start_ss(tegra_clk_pll *pll)
{
...
static void pll_clk_stop_ss(tegra_clk_pll *pll)
{
	if ((*(*pll).params).defaults_set && (*(*pll).params).ssc_ctrl_reg) {
		u32 val = pll_readl((*(*pll).params).ssc_ctrl_reg, pll);
...
static int _p_div_to_hw(clk_hw *hw, p_div: u8)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
...
int tegra_pll_p_div_to_hw(tegra_clk_pll *pll, p_div: u8)
{
	return _p_div_to_hw((*&pll).hw, p_div);
}

static int _hw_to_p_div(clk_hw *hw, p_div_hw: u8)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	const struct pdiv_map *p_tohw = (*(*pll).params).pdiv_tohw;
 ...
 */
static void clk_pll_set_sdm_data(clk_hw *hw,
				 tegra_clk_pll_freq_table *cfg)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	u32 val;
...
static void _update_pll_mnp(tegra_clk_pll *pll,
			    tegra_clk_pll_freq_table *cfg)
{
	u32 val;
...
static void _get_pll_mnp(tegra_clk_pll *pll,
			 tegra_clk_pll_freq_table *cfg)
{
	u32 val;
	struct tegra_clk_pll_params *params = (*pll).params;
...
static int clk_pll_determine_rate(clk_hw *hw,
...
static int clk_plle_enable(clk_hw *hw)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	struct tegra_clk_pll_freq_table sel;
...
static void tegra_clk_pll_restore_context(clk_hw *hw)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
...
static int clk_pllu_enable(clk_hw *hw)
{
	'out: {
	struct tegra_clk_pll *pll = to_clk_pll(hw);
...
static int clk_pll_ramp_determine_rate(clk_hw *hw,
				       clk_rate_request *req)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	struct tegra_clk_pll_freq_table cfg;
...
static void _pllcx_strobe(tegra_clk_pll *pll)
{
	u32 val;
...
static int clk_pllc_enable(clk_hw *hw)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	u32 val;
	int ret;
...
static void _clk_pllc_disable(clk_hw *hw)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	u32 val;
...
static int clk_pllre_determine_rate(clk_hw *hw,
				    clk_rate_request *req)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
...
static int clk_plle_tegra114_enable(clk_hw *hw)
{
	'out: {
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	struct tegra_clk_pll_freq_table sel;
...
static int clk_pllu_tegra114_enable(clk_hw *hw)
{
	'out: {
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	const struct utmi_clk_param *params = NULL;
...
static void _clk_plle_tegra_init_parent(tegra_clk_pll *pll)
{
	val: u32, val_aux;
...
static struct tegra_clk_pll *_tegra_init_pll(void __iomem *clk_base,
		void __iomem *pmc, tegra_clk_pll_params *pll_params,
		spinlock_t *lock)
{
...
static int clk_plle_tegra210_is_enabled(clk_hw *hw)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	u32 val;
...
static int clk_plle_tegra210_enable(clk_hw *hw)
{
	'out: {
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	struct tegra_clk_pll_freq_table sel;
...
static void clk_plle_tegra210_disable(clk_hw *hw)
{
	'out: {
	struct tegra_clk_pll *pll = to_clk_pll(hw);
	core::ffi::c_ulong flags = 0;
...
static void tegra_clk_plle_t210_restore_context(clk_hw *hw)
{
	struct tegra_clk_pll *pll = to_clk_pll(hw);
