...
// Kernel clock types, macros, and external symbols are supplied by the surrounding crate.
//
// The original declarative initializer topology is retained verbatim below because these
// kernel structures are external ABI types and must be represented by their Rust bindings.
// C-only preprocessing directives are preserved as comments.
#[repr(usize)]
