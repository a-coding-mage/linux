...
static R8A779F0_CORE_CLKS: [CpgCoreClk; 57] = [
...
static R8A779F0_MOD_CLKS: &[MssrModClk] = &[
    DEF_MOD!("hscif0", 514, R8A779F0_CLK_SASYNCPERD1), DEF_MOD!("hscif1", 515, R8A779F0_CLK_SASYNCPERD1),
    DEF_MOD!("hscif2", 516, R8A779F0_CLK_SASYNCPERD1), DEF_MOD!("hscif3", 517, R8A779F0_CLK_SASYNCPERD1),
    DEF_MOD!("i2c0", 518, R8A779F0_CLK_S0D6_PER), DEF_MOD!("i2c1", 519, R8A779F0_CLK_S0D6_PER),
    DEF_MOD!("i2c2", 520, R8A779F0_CLK_S0D6_PER), DEF_MOD!("i2c3", 521, R8A779F0_CLK_S0D6_PER),
    DEF_MOD!("i2c4", 522, R8A779F0_CLK_S0D6_PER), DEF_MOD!("i2c5", 523, R8A779F0_CLK_S0D6_PER),
    DEF_MOD!("msiof0", 618, R8A779F0_CLK_MSO), DEF_MOD!("msiof1", 619, R8A779F0_CLK_MSO),
    DEF_MOD!("msiof2", 620, R8A779F0_CLK_MSO), DEF_MOD!("msiof3", 621, R8A779F0_CLK_MSO),
    DEF_MOD!("pcie0", 624, R8A779F0_CLK_S0D2), DEF_MOD!("pcie1", 625, R8A779F0_CLK_S0D2),
    DEF_MOD!("scif0", 702, R8A779F0_CLK_SASYNCPERD4), DEF_MOD!("scif1", 703, R8A779F0_CLK_SASYNCPERD4),
    DEF_MOD!("scif3", 704, R8A779F0_CLK_SASYNCPERD4), DEF_MOD!("scif4", 705, R8A779F0_CLK_SASYNCPERD4),
    DEF_MOD!("sdhi0", 706, R8A779F0_CLK_SD0), DEF_MOD!("sys-dmac0", 709, R8A779F0_CLK_S0D3_PER),
    DEF_MOD!("sys-dmac1", 710, R8A779F0_CLK_S0D3_PER), DEF_MOD!("tmu0", 713, R8A779F0_CLK_SASYNCRT),
    DEF_MOD!("tmu1", 714, R8A779F0_CLK_SASYNCPERD2), DEF_MOD!("tmu2", 715, R8A779F0_CLK_SASYNCPERD2),
    DEF_MOD!("tmu3", 716, R8A779F0_CLK_SASYNCPERD2), DEF_MOD!("tmu4", 717, R8A779F0_CLK_SASYNCPERD2),
    DEF_MOD!("wdt", 907, R8A779F0_CLK_R), DEF_MOD!("cmt0", 910, R8A779F0_CLK_R),
    DEF_MOD!("cmt1", 911, R8A779F0_CLK_R), DEF_MOD!("cmt2", 912, R8A779F0_CLK_R),
    DEF_MOD!("cmt3", 913, R8A779F0_CLK_R), DEF_MOD!("pfc0", 915, R8A779F0_CLK_CPEX),
    DEF_MOD!("tsc", 919, R8A779F0_CLK_CL16M), DEF_MOD!("rswitch2", 1505, R8A779F0_CLK_RSW2),
    DEF_MOD!("ether-serdes", 1506, R8A779F0_CLK_S0D2_HSC), DEF_MOD!("ufs", 1514, R8A779F0_CLK_S0D4_HSC),
];

static R8A779F0_CRIT_MOD_CLKS: &[u32] = &[MOD_CLK_ID!(907)]; // WDT

/* CPG Clock Data */
/* MD 14 13 select EXTAL/PLL divisors; prohibited setting is index 2. */
macro_rules! CPG_PLL_CONFIG_INDEX { ($md:expr) => { ((($md & BIT!(14)) >> 13) | (($md & BIT!(13)) >> 13)) }; }

static CPG_PLL_CONFIGS: [RcarGen4CpgPllConfig; 4] = [
    RcarGen4CpgPllConfig { extal_div: 1, pll1_mult: 200, pll1_div: 1, pll5_mult: 200, pll5_div: 1, osc_prediv: 15 },
    RcarGen4CpgPllConfig { extal_div: 1, pll1_mult: 160, pll1_div: 1, pll5_mult: 160, pll5_div: 1, osc_prediv: 19 },
    RcarGen4CpgPllConfig { extal_div: 0, pll1_mult: 0, pll1_div: 0, pll5_mult: 0, pll5_div: 0, osc_prediv: 0 },
    RcarGen4CpgPllConfig { extal_div: 2, pll1_mult: 160, pll1_div: 1, pll5_mult: 160, pll5_div: 1, osc_prediv: 38 },
];

unsafe fn r8a779f0_cpg_mssr_init(dev: *mut Device) -> i32 {
...
static R8A779F0_CPG_MSSR_INFO: CpgMssrInfo = CpgMssrInfo {
    core_clks: R8A779F0_CORE_CLKS.as_ptr(),
    num_core_clks: ARRAY_SIZE!(R8A779F0_CORE_CLKS),
    last_dt_core_clk: LAST_DT_CORE_CLK,
    num_total_core_clks: MOD_CLK_BASE,
    mod_clks: R8A779F0_MOD_CLKS.as_ptr(),
    num_mod_clks: ARRAY_SIZE!(R8A779F0_MOD_CLKS),
    num_hw_mod_clks: 28 * 32,
    crit_mod_clks: R8A779F0_CRIT_MOD_CLKS.as_ptr(),
    num_crit_mod_clks: ARRAY_SIZE!(R8A779F0_CRIT_MOD_CLKS),
    init: Some(r8a779f0_cpg_mssr_init),
