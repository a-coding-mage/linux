...
// C dependencies: linux/device.h, linux/init.h, linux/kernel.h,
// linux/soc/renesas/rcar-rst.h, dt-bindings/clock/r8a774c0-cpg-mssr.h,
...
enum clk_ids {
    ...
    CLK_MAIN, CLK_PLL0, CLK_PLL1, CLK_PLL3, CLK_PLL0D4, CLK_PLL0D6,
    CLK_PLL0D8, CLK_PLL0D20, CLK_PLL0D24, CLK_PLL1D2, CLK_PE, CLK_S0,
    CLK_S1, CLK_S2, CLK_S3, CLK_SDSRC, CLK_RPCSRC, CLK_RINT, CLK_OCO,
    /* Module Clocks */
    MOD_CLK_BASE,
}

static r8a774c0_core_clks: [struct_cpg_core_clk; _] = [
...
static r8a774c0_mod_clks: [struct_mssr_mod_clk; _] = [
    DEF_MOD!("tmu4", 121, R8A774C0_CLK_S0D6C), DEF_MOD!("tmu3", 122, R8A774C0_CLK_S3D2C),
    DEF_MOD!("tmu2", 123, R8A774C0_CLK_S3D2C), DEF_MOD!("tmu1", 124, R8A774C0_CLK_S3D2C),
    ...
    DEF_MOD!("scif1", 206, R8A774C0_CLK_S3D4C), DEF_MOD!("scif0", 207, R8A774C0_CLK_S3D4C),
    DEF_MOD!("msiof3", 208, R8A774C0_CLK_MSO), DEF_MOD!("msiof2", 209, R8A774C0_CLK_MSO),
    DEF_MOD!("msiof1", 210, R8A774C0_CLK_MSO), DEF_MOD!("msiof0", 211, R8A774C0_CLK_MSO),
    DEF_MOD!("sys-dmac2", 217, R8A774C0_CLK_S3D1), DEF_MOD!("sys-dmac1", 218, R8A774C0_CLK_S3D1),
    DEF_MOD!("sys-dmac0", 219, R8A774C0_CLK_S3D1), DEF_MOD!("cmt3", 300, R8A774C0_CLK_R),
    DEF_MOD!("cmt2", 301, R8A774C0_CLK_R), DEF_MOD!("cmt1", 302, R8A774C0_CLK_R),
    DEF_MOD!("cmt0", 303, R8A774C0_CLK_R), DEF_MOD!("scif2", 310, R8A774C0_CLK_S3D4C),
    DEF_MOD!("sdif3", 311, R8A774C0_CLK_SD3), DEF_MOD!("sdif1", 313, R8A774C0_CLK_SD1),
    DEF_MOD!("sdif0", 314, R8A774C0_CLK_SD0), DEF_MOD!("pcie0", 319, R8A774C0_CLK_S3D1),
    DEF_MOD!("usb3-if0", 328, R8A774C0_CLK_S3D1), DEF_MOD!("usb-dmac0", 330, R8A774C0_CLK_S3D1),
    DEF_MOD!("usb-dmac1", 331, R8A774C0_CLK_S3D1), DEF_MOD!("rwdt", 402, R8A774C0_CLK_R),
    DEF_MOD!("intc-ex", 407, R8A774C0_CLK_CP), DEF_MOD!("intc-ap", 408, R8A774C0_CLK_S0D3),
    DEF_MOD!("audmac0", 502, R8A774C0_CLK_S1D2), DEF_MOD!("hscif4", 516, R8A774C0_CLK_S3D1C),
    DEF_MOD!("hscif3", 517, R8A774C0_CLK_S3D1C), DEF_MOD!("hscif2", 518, R8A774C0_CLK_S3D1C),
    DEF_MOD!("hscif1", 519, R8A774C0_CLK_S3D1C), DEF_MOD!("hscif0", 520, R8A774C0_CLK_S3D1C),
    DEF_MOD!("thermal", 522, R8A774C0_CLK_CP), DEF_MOD!("pwm", 523, R8A774C0_CLK_S3D4C),
    DEF_MOD!("fcpvd1", 602, R8A774C0_CLK_S1D2), DEF_MOD!("fcpvd0", 603, R8A774C0_CLK_S1D2),
    DEF_MOD!("fcpvb0", 607, R8A774C0_CLK_S0D1), DEF_MOD!("fcpvi0", 611, R8A774C0_CLK_S0D1),
    DEF_MOD!("fcpf0", 615, R8A774C0_CLK_S0D1), DEF_MOD!("fcpcs", 619, R8A774C0_CLK_S0D1),
    DEF_MOD!("vspd1", 622, R8A774C0_CLK_S1D2), DEF_MOD!("vspd0", 623, R8A774C0_CLK_S1D2),
    DEF_MOD!("vspb", 626, R8A774C0_CLK_S0D1), DEF_MOD!("vspi0", 631, R8A774C0_CLK_S0D1),
    DEF_MOD!("ehci0", 703, R8A774C0_CLK_S3D2), DEF_MOD!("hsusb", 704, R8A774C0_CLK_S3D2),
    DEF_MOD!("csi40", 716, R8A774C0_CLK_CSI0), DEF_MOD!("du1", 723, R8A774C0_CLK_S1D1),
    DEF_MOD!("du0", 724, R8A774C0_CLK_S1D1), DEF_MOD!("lvds", 727, R8A774C0_CLK_S2D1),
    DEF_MOD!("vin5", 806, R8A774C0_CLK_S1D2), DEF_MOD!("vin4", 807, R8A774C0_CLK_S1D2),
    DEF_MOD!("etheravb", 812, R8A774C0_CLK_S3D2),
    DEF_MOD!("gpio6", 906, R8A774C0_CLK_S3D4), DEF_MOD!("gpio5", 907, R8A774C0_CLK_S3D4),
    DEF_MOD!("gpio4", 908, R8A774C0_CLK_S3D4), DEF_MOD!("gpio3", 909, R8A774C0_CLK_S3D4),
    DEF_MOD!("gpio2", 910, R8A774C0_CLK_S3D4), DEF_MOD!("gpio1", 911, R8A774C0_CLK_S3D4),
    DEF_MOD!("gpio0", 912, R8A774C0_CLK_S3D4), DEF_MOD!("can-fd", 914, R8A774C0_CLK_S3D2),
    DEF_MOD!("can-if1", 915, R8A774C0_CLK_S3D4), DEF_MOD!("can-if0", 916, R8A774C0_CLK_S3D4),
    DEF_MOD!("rpc-if", 917, R8A774C0_CLK_RPCD2), DEF_MOD!("i2c6", 918, R8A774C0_CLK_S3D2),
    DEF_MOD!("i2c5", 919, R8A774C0_CLK_S3D2), DEF_MOD!("adg", 922, R8A774C0_CLK_ZA2),
    DEF_MOD!("iic-pmic", 926, R8A774C0_CLK_CP), DEF_MOD!("i2c4", 927, R8A774C0_CLK_S3D2),
    DEF_MOD!("i2c3", 928, R8A774C0_CLK_S3D2), DEF_MOD!("i2c2", 929, R8A774C0_CLK_S3D2),
    DEF_MOD!("i2c1", 930, R8A774C0_CLK_S3D2), DEF_MOD!("i2c0", 931, R8A774C0_CLK_S3D2),
    DEF_MOD!("i2c7", 1003, R8A774C0_CLK_S3D2), DEF_MOD!("ssi-all", 1005, R8A774C0_CLK_S3D4),
    ...
    DEF_MOD!("scu-all", 1017, R8A774C0_CLK_S3D4), DEF_MOD!("scu-dvc1", 1018, MOD_CLK_ID!(1017)),
...
static r8a774c0_crit_mod_clks: [u32; 2] = [MOD_CLK_ID!(402), MOD_CLK_ID!(408)];

// MD19 EXTAL/PLL configuration table.
macro_rules! CPG_PLL_CONFIG_INDEX { ($md:expr) => { (($md & BIT!(19)) >> 19) }; }

static cpg_pll_configs: [struct_rcar_gen3_cpg_pll_config; 2] = [
    struct_rcar_gen3_cpg_pll_config { extal_div: 1, pll1_mult: 100, pll1_div: 3, pll3_mult: 100, pll3_div: 3 },
    struct_rcar_gen3_cpg_pll_config { extal_div: 1, pll1_mult: 100, pll1_div: 3, pll3_mult: 58, pll3_div: 3 },
];

unsafe fn r8a774c0_cpg_mssr_init(dev: *mut device) -> i32 {
...
static r8a774c0_cpg_mssr_info: struct_cpg_mssr_info = struct_cpg_mssr_info {
    ...
    init: Some(r8a774c0_cpg_mssr_init),
