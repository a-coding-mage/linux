...
// dt-bindings/reset/mediatek,mt6735-pericfg.h

const PERI_GLOBALCON_RST0: u16 = 0x00;
const PERI_GLOBALCON_RST1: u16 = 0x04;
const PERI_GLOBALCON_PDN0_SET: u16 = 0x08;
const PERI_GLOBALCON_PDN0_CLR: u16 = 0x10;
const PERI_GLOBALCON_PDN0_STA: u16 = 0x18;

const RST_NR_PER_BANK: usize = 32;

static mut peri_cg_regs: mtk_gate_regs = mtk_gate_regs {
    set_ofs: PERI_GLOBALCON_PDN0_SET,
    clr_ofs: PERI_GLOBALCON_PDN0_CLR,
    sta_ofs: PERI_GLOBALCON_PDN0_STA,
};

static pericfg_gates: [mtk_gate; 30] = [
...
static pericfg_rst_bank_ofs: [u16; 2] = [PERI_GLOBALCON_RST0, PERI_GLOBALCON_RST1];

static pericfg_rst_idx_map: [u16; 29] = [
...
static pericfg_resets: mtk_clk_rst_desc = mtk_clk_rst_desc {
    version: MTK_RST_SIMPLE,
    rst_bank_ofs: &pericfg_rst_bank_ofs,
    rst_bank_nr: pericfg_rst_bank_ofs.len(),
    rst_idx_map: &pericfg_rst_idx_map,
    rst_idx_map_nr: pericfg_rst_idx_map.len(),
};

static pericfg_clks: mtk_clk_desc = mtk_clk_desc {
    clks: &pericfg_gates,
    num_clks: pericfg_gates.len(),
    rst_desc: &pericfg_resets,
};

static of_match_mt6735_pericfg: [of_device_id; 2] = [
    of_device_id { compatible: "mediatek,mt6735-pericfg", data: &pericfg_clks },
    of_device_id { /* sentinel */ },
];

// MODULE_DEVICE_TABLE(of, of_match_mt6735_pericfg);

static mut clk_mt6735_pericfg: platform_driver = platform_driver {
