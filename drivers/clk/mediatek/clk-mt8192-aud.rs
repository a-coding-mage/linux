...
static aud0_cg_regs: MtkGateRegs = MtkGateRegs {
    set_ofs: 0x0,
    clr_ofs: 0x0,
    sta_ofs: 0x0,
};

static aud1_cg_regs: MtkGateRegs = MtkGateRegs {
    set_ofs: 0x4,
    clr_ofs: 0x4,
    sta_ofs: 0x4,
};

static aud2_cg_regs: MtkGateRegs = MtkGateRegs {
    set_ofs: 0x8,
    clr_ofs: 0x8,
    sta_ofs: 0x8,
};
...
macro_rules! GATE_AUD0 {
    ($id:ident, $name:literal, $parent:literal, $shift:expr) => {
        GATE_MTK!($id, $name, $parent, &aud0_cg_regs, $shift, &mtk_clk_gate_ops_no_setclr)
    };
}

macro_rules! GATE_AUD1 {
    ($id:ident, $name:literal, $parent:literal, $shift:expr) => {
        GATE_MTK!($id, $name, $parent, &aud1_cg_regs, $shift, &mtk_clk_gate_ops_no_setclr)
    };
}

macro_rules! GATE_AUD2 {
    ($id:ident, $name:literal, $parent:literal, $shift:expr) => {
        GATE_MTK!($id, $name, $parent, &aud2_cg_regs, $shift, &mtk_clk_gate_ops_no_setclr)
    };
}

static aud_clks: [MtkGate; 32] = [
    // AUD0
...
unsafe fn clk_mt8192_aud_probe(pdev: *mut PlatformDevice) -> i32 {
    let mut r: i32;
    ...
    r = devm_of_platform_populate(unsafe { &mut (*pdev).dev });
    if r != 0 {
...
unsafe fn clk_mt8192_aud_remove(pdev: *mut PlatformDevice) {
    of_platform_depopulate(unsafe { &mut (*pdev).dev });
    mtk_clk_simple_remove(pdev);
}

static of_match_clk_mt8192_aud: [OfDeviceId; 2] = [
    OfDeviceId {
...
static mut clk_mt8192_aud_drv: PlatformDriver = PlatformDriver {
    probe: Some(clk_mt8192_aud_probe),
    remove: Some(clk_mt8192_aud_remove),
    driver: Driver {
        name: "clk-mt8192-aud",
        of_match_table: of_match_clk_mt8192_aud.as_ptr(),
    },
};
