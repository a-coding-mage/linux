...
// External Linux clock-provider, platform-device, clock/reset binding, and
// MediaTek clock-gate definitions are supplied by the surrounding kernel.

static infra_ao0_cg_regs: MtkGateRegs = MtkGateRegs {
    set_ofs: 0x80,
    clr_ofs: 0x84,
    sta_ofs: 0x90,
};

static infra_ao1_cg_regs: MtkGateRegs = MtkGateRegs {
    set_ofs: 0x88,
    clr_ofs: 0x8c,
    sta_ofs: 0x94,
};

static infra_ao2_cg_regs: MtkGateRegs = MtkGateRegs {
    set_ofs: 0xa4,
    clr_ofs: 0xa8,
    sta_ofs: 0xac,
};

static infra_ao3_cg_regs: MtkGateRegs = MtkGateRegs {
    set_ofs: 0xc0,
    clr_ofs: 0xc4,
    sta_ofs: 0xc8,
};

macro_rules! gate_infra_ao0_flags { ($id:expr, $name:expr, $parent:expr, $shift:expr, $flag:expr) => { gate_mtk_flags!($id, $name, $parent, &infra_ao0_cg_regs, $shift, &mtk_clk_gate_ops_setclr, $flag) }; }
macro_rules! gate_infra_ao0 { ($id:expr, $name:expr, $parent:expr, $shift:expr) => { gate_infra_ao0_flags!($id, $name, $parent, $shift, 0) }; }
macro_rules! gate_infra_ao1_flags { ($id:expr, $name:expr, $parent:expr, $shift:expr, $flag:expr) => { gate_mtk_flags!($id, $name, $parent, &infra_ao1_cg_regs, $shift, &mtk_clk_gate_ops_setclr, $flag) }; }
macro_rules! gate_infra_ao1 { ($id:expr, $name:expr, $parent:expr, $shift:expr) => { gate_infra_ao1_flags!($id, $name, $parent, $shift, 0) }; }
macro_rules! gate_infra_ao2_flags { ($id:expr, $name:expr, $parent:expr, $shift:expr, $flag:expr) => { gate_mtk_flags!($id, $name, $parent, &infra_ao2_cg_regs, $shift, &mtk_clk_gate_ops_setclr, $flag) }; }
macro_rules! gate_infra_ao2 { ($id:expr, $name:expr, $parent:expr, $shift:expr) => { gate_infra_ao2_flags!($id, $name, $parent, $shift, 0) }; }
macro_rules! gate_infra_ao3_flags { ($id:expr, $name:expr, $parent:expr, $shift:expr, $flag:expr) => { gate_mtk_flags!($id, $name, $parent, &infra_ao3_cg_regs, $shift, &mtk_clk_gate_ops_setclr, $flag) }; }
macro_rules! gate_infra_ao3 { ($id:expr, $name:expr, $parent:expr, $shift:expr) => { gate_infra_ao3_flags!($id, $name, $parent, $shift, 0) }; }

static infra_ao_clks: &[MtkGate] = &[
    // INFRA_AO0
    ...
    gate_infra_ao0_flags!(CLK_INFRA_AO_SCP_CORE, "infra_ao_scp_core", "top_scp", 4, CLK_IS_CRITICAL),
    ...
    gate_infra_ao1_flags!(CLK_INFRA_AO_DVFSRC, "infra_ao_dvfsrc", "top_dvfsrc", 7, CLK_IS_CRITICAL),
    ...
    gate_infra_ao2!(CLK_INFRA_AO_SSUSB_TOP_P1_XHCI, "infra_ao_ssusb_p1_xhci", "top_ssusb_xhci_1p", 14),
    ...
    gate_infra_ao2!(CLK_INFRA_AO_SSUSB_TOP_P1_SYS, "infra_ao_ssusb_p1_sys", "top_ssusb_1p", 16),
    ...
    gate_infra_ao3_flags!(CLK_INFRA_AO_SSPM_26M_SELF, "infra_ao_sspm_26m", "clk26m", 3, CLK_IS_CRITICAL),
    gate_infra_ao3_flags!(CLK_INFRA_AO_SSPM_32K_SELF, "infra_ao_sspm_32k", "clk32k", 4, CLK_IS_CRITICAL),
    ...
    gate_infra_ao3_flags!(CLK_INFRA_AO_SEJ_F13M, "infra_ao_sej_f13m", "clk26m", 15, CLK_IS_CRITICAL),
    // infra_ao_aes_top0_bclk is for secure encryption
    gate_infra_ao3_flags!(CLK_INFRA_AO_AES_TOP0_BCLK, "infra_ao_aes_top0_bclk", "top_axi", 16, CLK_IS_CRITICAL),
...
static mut infra_ao_idx_map: [u16; 2] = [
    /* [MT8186_INFRA_THERMAL_CTRL_RST] = */ 0 * RST_NR_PER_BANK + 0,
    /* [MT8186_INFRA_PTP_CTRL_RST] = */ 1 * RST_NR_PER_BANK + 0,
];

static mut infra_ao_rst_desc: MtkClkRstDesc = MtkClkRstDesc {
    version: MTK_RST_SET_CLR,
    rst_bank_ofs: unsafe { &infra_ao_rst_ofs },
    rst_bank_nr: infra_ao_rst_ofs.len(),
    rst_idx_map: unsafe { &infra_ao_idx_map },
    rst_idx_map_nr: infra_ao_idx_map.len(),
};

static infra_ao_desc: MtkClkDesc = MtkClkDesc {
    clks: infra_ao_clks,
    num_clks: infra_ao_clks.len(),
    rst_desc: unsafe { &infra_ao_rst_desc },
};

static of_match_clk_mt8186_infra_ao: &[OfDeviceId] = &[
    OfDeviceId {
...
static mut clk_mt8186_infra_ao_drv: PlatformDriver = PlatformDriver {
...
platform_driver!(clk_mt8186_infra_ao_drv);
