...
// C includes and build-time dependencies are supplied by the surrounding kernel bindings.
...
macro_rules! GATE_AUDIO2 {
    ($id:expr, $name:expr, $parent:expr, $shift:expr) => {
        GATE_MTK!($id, $name, $parent, &audio2_cg_regs, $shift, &mtk_clk_gate_ops_no_setclr)
    };
}

macro_rules! GATE_AUDIO3 {
    ($id:expr, $name:expr, $parent:expr, $shift:expr) => {
        GATE_MTK!($id, $name, $parent, &audio3_cg_regs, $shift, &mtk_clk_gate_ops_no_setclr)
    };
}
...
static of_match_clk_mt2701_aud: [of_device_id; 2] = [
    of_device_id { compatible: "mediatek,mt2701-audsys", data: &audio_desc },
    of_device_id { /* sentinel */ },
];

MODULE_DEVICE_TABLE!(of, of_match_clk_mt2701_aud);

unsafe fn clk_mt2701_aud_probe(pdev: *mut platform_device) -> i32 {
    let mut r: i32;
...
unsafe fn clk_mt2701_aud_remove(pdev: *mut platform_device) {
    of_platform_depopulate(&(*pdev).dev);
    mtk_clk_simple_remove(pdev);
}

static mut clk_mt2701_aud_drv: platform_driver = platform_driver {
    probe: Some(clk_mt2701_aud_probe),
    remove: Some(clk_mt2701_aud_remove),
    driver: driver {
        name: "clk-mt2701-aud",
