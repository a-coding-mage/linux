...
// External Linux clock-provider, platform-device, device-tree, and local
// clock-gate definitions are supplied by other translation units.

static VDO1_0_CG_REGS: mtk_gate_regs = mtk_gate_regs {
    set_ofs: 0x104,
    clr_ofs: 0x108,
    sta_ofs: 0x100,
};

static VDO1_1_CG_REGS: mtk_gate_regs = mtk_gate_regs {
    set_ofs: 0x114,
    clr_ofs: 0x118,
    sta_ofs: 0x110,
};

static VDO1_2_CG_REGS: mtk_gate_regs = mtk_gate_regs {
    set_ofs: 0x124,
    clr_ofs: 0x128,
    sta_ofs: 0x120,
};

static VDO1_3_CG_REGS: mtk_gate_regs = mtk_gate_regs {
    set_ofs: 0x134,
    clr_ofs: 0x138,
    sta_ofs: 0x130,
};

static VDO1_4_CG_REGS: mtk_gate_regs = mtk_gate_regs {
    set_ofs: 0x144,
    clr_ofs: 0x148,
    sta_ofs: 0x140,
};

static VDO1_5_CG_REGS: mtk_gate_regs = mtk_gate_regs {
    set_ofs: 0x400,
    clr_ofs: 0x400,
    sta_ofs: 0x400,
};

macro_rules! GATE_VDO1_0 { ($id:ident, $name:literal, $parent:literal, $shift:expr) => { GATE_MTK!($id, $name, $parent, &VDO1_0_CG_REGS, $shift, &mtk_clk_gate_ops_setclr) }; }
macro_rules! GATE_VDO1_1 { ($id:ident, $name:literal, $parent:literal, $shift:expr) => { GATE_MTK!($id, $name, $parent, &VDO1_1_CG_REGS, $shift, &mtk_clk_gate_ops_setclr) }; }
macro_rules! GATE_VDO1_2 { ($id:ident, $name:literal, $parent:literal, $shift:expr) => { GATE_MTK!($id, $name, $parent, &VDO1_2_CG_REGS, $shift, &mtk_clk_gate_ops_setclr) }; }
macro_rules! GATE_VDO1_3 { ($id:ident, $name:literal, $parent:literal, $shift:expr) => { GATE_MTK!($id, $name, $parent, &VDO1_3_CG_REGS, $shift, &mtk_clk_gate_ops_setclr) }; }
macro_rules! GATE_VDO1_3_FLAGS { ($id:ident, $name:literal, $parent:literal, $shift:expr, $flags:expr) => { GATE_MTK_FLAGS!($id, $name, $parent, &VDO1_3_CG_REGS, $shift, &mtk_clk_gate_ops_setclr, $flags) }; }
macro_rules! GATE_VDO1_4 { ($id:ident, $name:literal, $parent:literal, $shift:expr) => { GATE_MTK!($id, $name, $parent, &VDO1_4_CG_REGS, $shift, &mtk_clk_gate_ops_setclr) }; }
macro_rules! GATE_VDO1_5 { ($id:ident, $name:literal, $parent:literal, $shift:expr) => { GATE_MTK!($id, $name, $parent, &VDO1_5_CG_REGS, $shift, &mtk_clk_gate_ops_setclr) }; }

static VDO1_CLKS: [mtk_gate; 70] = [
    // VDO1_0
    ...
    // Remaining gate declarations preserve the source's external macro calls.
...
static VDO1_DESC: mtk_clk_desc = mtk_clk_desc {
    clks: VDO1_CLKS.as_ptr(),
    num_clks: VDO1_CLKS.len(),
};

static CLK_MT8188_VDO1_ID_TABLE: [platform_device_id; 2] = [
    platform_device_id { name: "clk-mt8188-vdo1", driver_data: &VDO1_DESC as *const _ as kernel_ulong_t },
    platform_device_id { /* sentinel */ ..Default::default() },
];

MODULE_DEVICE_TABLE!(platform, CLK_MT8188_VDO1_ID_TABLE);

static mut CLK_MT8188_VDO1_DRV: platform_driver = platform_driver {
    probe: Some(mtk_clk_pdev_probe),
    remove: Some(mtk_clk_pdev_remove),
    driver: driver { name: "clk-mt8188-vdo1" },
    id_table: CLK_MT8188_VDO1_ID_TABLE.as_ptr(),
};
