...
// MediaTek clock infrastructure are intentionally referenced but not defined here.

static BDP0_CG_REGS: mtk_gate_regs = mtk_gate_regs {
    set_ofs: 0x0104,
    clr_ofs: 0x0108,
    sta_ofs: 0x0100,
};

static BDP1_CG_REGS: mtk_gate_regs = mtk_gate_regs {
    set_ofs: 0x0114,
    clr_ofs: 0x0118,
    sta_ofs: 0x0110,
};

macro_rules! GATE_BDP0 {
    ($id:ident, $name:literal, $parent:literal, $shift:expr) => {
        GATE_MTK!($id, $name, $parent, &BDP0_CG_REGS, $shift,
                  &mtk_clk_gate_ops_setclr_inv)
    };
}

macro_rules! GATE_BDP1 {
    ($id:ident, $name:literal, $parent:literal, $shift:expr) => {
        GATE_MTK!($id, $name, $parent, &BDP1_CG_REGS, $shift,
                  &mtk_clk_gate_ops_setclr_inv)
    };
}

static BDP_CLKS: [mtk_gate; 50] = [
...
static OF_MATCH_CLK_MT2701_BDP: [of_device_id; 2] = [
    of_device_id {
...
static mut CLK_MT2701_BDP_DRV: platform_driver = platform_driver {
    probe: mtk_clk_simple_probe,
    remove: mtk_clk_simple_remove,
    driver: device_driver {
        name: "clk-mt2701-bdp",
        of_match_table: OF_MATCH_CLK_MT2701_BDP.as_ptr(),
    },
};
