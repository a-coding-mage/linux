...
// External Linux/kernel dependencies are supplied by the surrounding build.
// External Linux/kernel dependencies are supplied by the surrounding build.
// External Linux/kernel dependencies are supplied by the surrounding build.
// External Linux/kernel dependencies are supplied by the surrounding build.

/* 0xFF4A0000 + 0x0A00 */
macro_rules! rk3528_cru_reset_offset { ($id:ident, $reg:expr, $bit:expr) => { ($id, 0 + $reg * 16 + $bit) }; }

/* mapping table for reset ID to register offset */
static RK3528_REGISTER_OFFSET: static const int rk3528_register_offset[] = {[(usize, i32)] = &[
	/* CRU_SOFTRST_CON03 */
...
pub unsafe fn rk3528_rst_init(np: *mut device_node, reg_base: *mut core::ffi::c_void) {
	rockchip_register_softrst_lut(
{
	np,
				      RK3528_REGISTER_OFFSET,
				      RK3528_REGISTER_OFFSET.len(),
				      reg_base.add(RK3528_SOFTRST_CON(0) as usize),
