...
/* 0xFD7C0000 + 0x0A00 */
macro_rules! RK3588_CRU_RESET_OFFSET { ($id:ident, $reg:expr, $bit:expr) => { ($id, 0 + $reg * 16 + $bit) }; }

/* 0xFD7C8000 + 0x0A00 */
macro_rules! RK3588_PHPTOPCRU_RESET_OFFSET { ($id:ident, $reg:expr, $bit:expr) => { ($id, 0x8000 * 4 + $reg * 16 + $bit) }; }

/* 0xFD7D0000 + 0x0A00 */
macro_rules! RK3588_SECURECRU_RESET_OFFSET { ($id:ident, $reg:expr, $bit:expr) => { ($id, 0x10000 * 4 + $reg * 16 + $bit) }; }

/* 0xFD7F0000 + 0x0A00 */
macro_rules! RK3588_PMU1CRU_RESET_OFFSET { ($id:ident, $reg:expr, $bit:expr) => { ($id, 0x30000 * 4 + $reg * 16 + $bit) }; }

/* mapping table for reset ID to register offset */
static rk3588_register_offset: &[(usize, usize)] = &[
	/* SOFTRST_CON01 */
...
extern "C" {
    fn rockchip_register_softrst_lut(np: *mut device_node, lut: *const (usize, usize), count: usize, reg_base: *mut core::ffi::c_void, flags: usize);
}

#[allow(non_camel_case_types)]
pub struct device_node { _private: [u8; 0] }

pub unsafe fn rk3588_rst_init(np: *mut device_node, reg_base: *mut core::ffi::c_void) {
{
    rockchip_register_softrst_lut(
        rk3588_register_offset.as_ptr(),
        rk3588_register_offset.len(),
        (reg_base as *mut u8).add(RK3588_SOFTRST_CON!(0)) as *mut core::ffi::c_void,
