/* Direct low-level Rust translation; external kernel symbols are supplied by other modules. */
			     ...
			     struct intel_boot_params *params)
{
	...
	skb = __hci_cmd_sync(hdev, 0xfc0d, 0, core::ptr::null_mut(), HCI_INIT_TIMEOUT);
	if (IS_ERR(skb)) {
...
static void btintel_coredump(struct hci_dev *hdev)
{
	...
	skb = __hci_cmd_sync(hdev, 0xfc4e, 0, core::ptr::null_mut(), HCI_CMD_TIMEOUT);
	if (IS_ERR(skb)) {
...
static int btintel_register_devcoredump_support(struct hci_dev *hdev)
{
	...
	hci_devcd_register(hdev, btintel_coredump, btintel_dmp_hdr, core::ptr::null_mut());
				      ...
				      const u8 **fw_ptr, int *disable_patch)
{
	...
	const u8 *cmd_param;
	struct hci_event_hdr *evt = core::ptr::null_mut();
	const u8 *evt_param = core::ptr::null_mut();
...
static int btintel_configure_offload(struct hci_dev *hdev)
{
	...
	skb = __hci_cmd_sync(hdev, 0xfc86, 0, core::ptr::null_mut(), HCI_INIT_TIMEOUT);
	if (IS_ERR(skb)) {
...
static void btintel_set_ppag(struct hci_dev *hdev, struct intel_version_tlv *ver)
{
	...
	status = acpi_evaluate_object(handle, "PPAG", core::ptr::null_mut(), &buffer);
...
int btintel_acpi_reset_method(struct hci_dev *hdev)
{
	...
	status = acpi_evaluate_object(ACPI_HANDLE(GET_HCIDEV_DEV(hdev)), "_PRR", core::ptr::null_mut(), &buffer);
...
static int btintel_uefi_get_dsbr(u32 *dsbr_var)
{
	...
	status = efi.get_variable(BTINTEL_EFI_DSBR, &guid, core::ptr::null_mut(), &data_size,
				  &data);
...
void btintel_print_fseq_info(struct hci_dev *hdev)
{
	...
	skb = __hci_cmd_sync(hdev, 0xfcb3, 0, core::ptr::null_mut(), HCI_CMD_TIMEOUT);
	if (IS_ERR(skb)) {
...
static int btintel_setup_combined(struct hci_dev *hdev)
{
	...
	if (btintel_test_flag(hdev, INTEL_BROKEN_INITIAL_NCMD) ||
		...
		skb = __hci_cmd_sync(hdev, HCI_OP_RESET, 0, core::ptr::null_mut(),
				     HCI_INIT_TIMEOUT);
		if (IS_ERR(skb)) {
			bt_dev_err(hdev,
...
int btintel_shutdown_combined(struct hci_dev *hdev)
{
	...
	if (btintel_test_flag(hdev, INTEL_BROKEN_SHUTDOWN_LED)) {
		skb = __hci_cmd_sync(hdev, 0xfc3f, 0, core::ptr::null_mut(), HCI_INIT_TIMEOUT);
		if (IS_ERR(skb)) {
			ret = PTR_ERR(skb);
