...
unsafe fn _put_device_wrapper(arg: *mut device *) { put_device(arg); }
unsafe fn _cs_dsp_stop_wrapper(arg: *mut cs_dsp *) { cs_dsp_stop(arg); }
unsafe fn _cs_dsp_remove_wrapper(arg: *mut cs_dsp *) { cs_dsp_remove(arg); }

struct cs_dsp_test_local {
	*mut cs_dsp_mock_xm_header xm_header;
	*mut cs_dsp_mock_wmfw_builder wmfw_builder;
	i32 wmfw_version;
};
...
const struct cs_dsp_mock_alg_def cs_dsp_ctl_cache_test_algs[] = {
	{
		id: 0xfafa,
		ver: 0x100000,
		xm_base_words: 60,
...
const struct cs_dsp_mock_coeff_def mock_coeff_template = {
...
static const i8 * const cs_dsp_ctl_cache_test_fw_names[] = {
	"misc", "mbc/vss", "haps",
};

unsafe fn _find_alg_entry(*mut kunit test, alg_id: u32)
{
	i32 i;

	for (i = 0; i < (cs_dsp_ctl_cache_test_algs.len());  += 1i) {
		if (cs_dsp_ctl_cache_test_algs[i].id == alg_id)
			break;
	}

	KUNIT_ASSERT_LT(test, i, (cs_dsp_ctl_cache_test_algs.len()));

	return i;
}

unsafe fn _get_alg_mem_base_words(*mut kunit test, alg_index: i32, mem_type: i32)
{
	switch (mem_type) {
	case WMFW_ADSP2_XM:
...
*mut cs_dsp_mock_wmfw_builder _create_dummy_wmfw(*mut kunit test)
{
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp_test_local local = (*priv).local;
	*mut cs_dsp_mock_wmfw_builder builder;
 ...
 * wmfw V2 and later.
 */
unsafe fn cs_dsp_ctl_v2_cache_alloc(*mut kunit test)
{
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp_test_local local = (*priv).local;
	*mut cs_dsp dsp = (*priv).dsp;
	struct cs_dsp_mock_coeff_def def = mock_coeff_template;
	reg: u32, alg_base_words, alg_size_bytes;
	*mut cs_dsp_coeff_ctl ctl;
	*mut firmware wmfw;
	i8 [ctl_name; 4];
	u32 *reg_vals;
	i32 num_ctls;
	...
	for (def.length_bytes = 4; def.length_bytes <= 64; def.length_bytes += 4) {
		snprintf(ctl_name, (ctl_name.len()), "%x", def.length_bytes);
		cs_dsp_mock_wmfw_add_coeff_desc((*local).wmfw_builder, &def);
		num_ctls += 1;
		def.offset_dsp_words += def.length_bytes / 4;
	}
	cs_dsp_mock_wmfw_end_alg_info_block((*local).wmfw_builder);

	wmfw = cs_dsp_mock_wmfw_get_firmware((*(*priv).local).wmfw_builder);
	KUNIT_ASSERT_EQ(test, cs_dsp_power_up(dsp, wmfw, "mock_fw", std::ptr::null_mut(), std::ptr::null_mut(), "misc"), 0);
 ...
 */
unsafe fn cs_dsp_ctl_cache_init(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = (*test).param_value;
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp_test_local local = (*priv).local;
	*mut cs_dsp dsp = (*priv).dsp;
	struct cs_dsp_mock_coeff_def def = mock_coeff_template;
	i32 alg_idx = _find_alg_entry(test, (*param).alg_id);
	reg: u32, alg_base_words;
	*mut cs_dsp_coeff_ctl ctl;
	*mut firmware wmfw;
 ...
 */
unsafe fn cs_dsp_ctl_cache_init_write_only(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = (*test).param_value;
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp_test_local local = (*priv).local;
 ...
 */
unsafe fn cs_dsp_ctl_cache_init_multiple_fw_same_controls(*mut kunit test)
{
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp dsp = (*priv).dsp;
	struct cs_dsp_mock_coeff_def def = mock_coeff_template;
	*mut cs_dsp_mock_wmfw_builder [builder; 3];
	reg: u32, alg_base_words;
	*mut cs_dsp_coeff_ctl walkctl, *[ctl; 3];
	*mut firmware wmfw;
	u32 *[reg_vals; 3], *readback;
	i32 i;

	static_assert((ctl.len()) == (builder.len()));
	static_assert((reg_vals.len()) == (builder.len()));
	static_assert((cs_dsp_ctl_cache_test_fw_names.len()) >= (builder.len()));

	/* Create an identical control in each firmware but with different alg id */
	for (i = 0; i < (builder.len()); i += 1) {
	 ...
	 */
	for (i = 0; i < (ctl.len()); i += 1)
		ctl[i] = std::ptr::null_mut();
 ...
 * the firmware containing it.
 */
unsafe fn cs_dsp_ctl_cache_init_multiple_fwalgid_same_controls(*mut kunit test)
{
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp dsp = (*priv).dsp;
	struct cs_dsp_mock_coeff_def def = mock_coeff_template;
 ...
 */
unsafe fn cs_dsp_ctl_cache_init_multiple_mems(*mut kunit test)
{
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp dsp = (*priv).dsp;
	*mut cs_dsp_test_local local = (*priv).local;
	...
	*mut firmware wmfw;
	...
	static_assert((ctl.len()) ==  (reg_vals.len()));
	...
	list_for_each_entry!(walkctl, (*&dsp).ctl_list, list, {
		if ((*walkctl).alg_region.r#type == WMFW_ADSP2_YM)
			[ctl; 0] = walkctl;
		if ((*walkctl).alg_region.r#type == WMFW_ADSP2_XM)
			[ctl; 1] = walkctl;
		if ((*walkctl).alg_region.r#type == WMFW_ADSP2_ZM)
			[ctl; 2] = walkctl;
	});
 ...
 */
unsafe fn cs_dsp_ctl_cache_init_multiple_algs(*mut kunit test)
{
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp dsp = (*priv).dsp;
	*mut cs_dsp_test_local local = (*priv).local;
	...
	*mut firmware wmfw;
	...
	static_assert((ctl.len()) ==  (reg_vals.len()));
	static_assert((reg_vals.len()) <= (cs_dsp_ctl_cache_test_algs.len()));
 ...
 */
unsafe fn cs_dsp_ctl_cache_init_multiple_offsets(*mut kunit test)
{
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp dsp = (*priv).dsp;
	*mut cs_dsp_test_local local = (*priv).local;
	struct cs_dsp_mock_coeff_def def = mock_coeff_template;
	reg: u32, alg_base_words, alg_base_reg;
	*mut cs_dsp_coeff_ctl walkctl, *[ctl; 3];
	*mut firmware wmfw;
	u32 *[reg_vals; 3], *readback;
	i32 i;
 ...
 */
unsafe fn cs_dsp_ctl_cache_read_not_started(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = (*test).param_value;
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp_test_local local = (*priv).local;
 ...
 * Should return the data in the cache.
 */
unsafe fn cs_dsp_ctl_cache_read_stopped(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = (*test).param_value;
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp_test_local local = (*priv).local;
 ...
 * Should return the data in the cache.
 */
unsafe fn cs_dsp_ctl_cache_read_powered_down(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = (*test).param_value;
	*mut cs_dsp_test priv = (*test).priv;
	*mut cs_dsp_test_local local = (*priv).local;
 ...
 * Should return the data in the cache.
 */
unsafe fn cs_dsp_ctl_cache_read_stopped_powered_down(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * Should return the data in the cache.
 */
unsafe fn cs_dsp_ctl_cache_read_not_current_loaded_fw(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
	*mut cs_dsp dsp = priv->dsp;
	struct cs_dsp_mock_coeff_def def = mock_coeff_template;
	*mut cs_dsp_mock_wmfw_builder builder2 = _create_dummy_wmfw(test);
	i32 alg_idx = _find_alg_entry(test, param->alg_id);
	reg: u32, alg_base_words;
	*mut cs_dsp_coeff_ctl ctl;
	*mut firmware wmfw;
	...
	KUNIT_ASSERT_EQ(test, cs_dsp_power_up(dsp, wmfw, "mock_fw2", std::ptr::null_mut(), std::ptr::null_mut(), "mbc.vss"), 0);
 ...
 * Should return the data in the cache.
 */
unsafe fn cs_dsp_ctl_cache_read_not_current_running_fw(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 */
unsafe fn cs_dsp_ctl_cache_read_running(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 */
unsafe fn cs_dsp_ctl_cache_read_running_zero_flags(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * the registers.
 */
unsafe fn cs_dsp_ctl_cache_writethrough(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * didn't change.
 */
unsafe fn cs_dsp_ctl_cache_writethrough_unchanged(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * didn't change.
 */
unsafe fn cs_dsp_ctl_cache_write_unchanged_not_started(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 */
unsafe fn cs_dsp_ctl_cache_write_not_started(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * This should write to the cache only.
 */
unsafe fn cs_dsp_ctl_cache_write_stopped(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * This should write to the cache only.
 */
unsafe fn cs_dsp_ctl_cache_write_powered_down(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * This should write to the cache only.
 */
unsafe fn cs_dsp_ctl_cache_write_stopped_powered_down(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * This should write to the cache only.
 */
unsafe fn cs_dsp_ctl_cache_write_not_current_loaded_fw(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * This should write to the cache only.
 */
unsafe fn cs_dsp_ctl_cache_write_not_current_running_fw(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 */
unsafe fn cs_dsp_ctl_cache_sync_write_before_run(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 */
unsafe fn cs_dsp_ctl_cache_sync_write_while_running(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * backing the control when the firmware is next run.
 */
unsafe fn cs_dsp_ctl_cache_sync_write_after_stop(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * control is run.
 */
unsafe fn cs_dsp_ctl_cache_sync_write_not_current_fw(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * is run.
 */
unsafe fn cs_dsp_ctl_cache_sync_reapply_every_run(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 */
unsafe fn cs_dsp_ctl_cache_sync_reapply_after_fw_reload(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
 ...
 * backing the control.
 */
unsafe fn cs_dsp_ctl_cache_sync_reapply_after_fw_swap(*mut kunit test)
{
	const *mut cs_dsp_ctl_cache_test_param param = test->param_value;
	*mut cs_dsp_test priv = test->priv;
	*mut cs_dsp_test_local local = priv->local;
...
unsafe fn cs_dsp_ctl_cache_test_common_init(*mut kunit test, *mut cs_dsp dsp,
					     wmfw_version: i32)
{
	*mut cs_dsp_test priv;
	*mut cs_dsp_test_local local;
	*mut device test_dev;
	i32 ret;

	priv = kunit_kzalloc(test, std::mem::size_of::<*priv>(), GFP_KERNEL);
	if (!priv)
		return -ENOMEM;

	local = kunit_kzalloc(test, std::mem::size_of::<cs_dsp_test_local>(), GFP_KERNEL);
	if (!local)
		return -ENOMEM;
	...
	local->xm_header = cs_dsp_create_mock_xm_header(priv,
							...
							(cs_dsp_ctl_cache_test_algs.len()));
	...
	/* Init cs_dsp */
	dsp->client_ops = kunit_kzalloc(test, std::mem::size_of::<*dsp->client_ops>(), GFP_KERNEL);
...
unsafe fn cs_dsp_ctl_cache_test_halo_init(*mut kunit test)
{
	*mut cs_dsp dsp;

	/* Fill in cs_dsp and initialize */
	dsp = kunit_kzalloc(test, std::mem::size_of::<*dsp>(), GFP_KERNEL);
	if (!dsp)
		return -ENOMEM;
...
unsafe fn cs_dsp_ctl_cache_test_adsp2_32bit_init(*mut kunit test, wmfw_ver: i32)
{
	*mut cs_dsp dsp;
...
unsafe fn cs_dsp_ctl_cache_test_adsp2_32bit_wmfw1_init(*mut kunit test)
{
	return cs_dsp_ctl_cache_test_adsp2_32bit_init(test, 1);
}

unsafe fn cs_dsp_ctl_cache_test_adsp2_32bit_wmfw2_init(*mut kunit test)
{
	return cs_dsp_ctl_cache_test_adsp2_32bit_init(test, 2);
}

unsafe fn cs_dsp_ctl_cache_test_adsp2_16bit_init(*mut kunit test, wmfw_ver: i32)
{
	*mut cs_dsp dsp;
...
unsafe fn cs_dsp_ctl_cache_test_adsp2_16bit_wmfw1_init(*mut kunit test)
{
	return cs_dsp_ctl_cache_test_adsp2_16bit_init(test, 1);
}

unsafe fn cs_dsp_ctl_cache_test_adsp2_16bit_wmfw2_init(*mut kunit test)
{
	return cs_dsp_ctl_cache_test_adsp2_16bit_init(test, 2);
}

unsafe fn cs_dsp_ctl_all_param_desc(const *mut cs_dsp_ctl_cache_test_param param,
				      i8 *desc)
{
...
const struct cs_dsp_ctl_cache_test_param all_pop_varying_len_cases[] = {
...
const struct cs_dsp_ctl_cache_test_param all_pop_varying_offset_cases[] = {
...
const struct cs_dsp_ctl_cache_test_param all_pop_varying_xy_cases[] = {
...
const struct cs_dsp_ctl_cache_test_param all_pop_z_cases[] = {
...
const struct cs_dsp_ctl_cache_test_param all_pop_varying_alg_cases[] = {
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 4 },
 ...
 */
const struct cs_dsp_ctl_cache_test_param all_pop_nonvol_readable_flags_cases[] = {
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 4,
	  flags: 0
	},
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 4,
 ...
 */
const struct cs_dsp_ctl_cache_test_param all_pop_nonvol_readable_nonzero_flags_cases[] = {
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 4,
	  flags: WMFW_CTL_FLAG_READABLE,
	},
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 4,
 ...
 */
const struct cs_dsp_ctl_cache_test_param all_pop_nonvol_writeable_flags_cases[] = {
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 4,
	  flags: 0
	},
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 4,
 ...
 */
const struct cs_dsp_ctl_cache_test_param all_pop_nonvol_write_only_length_cases[] = {
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 4,
	  flags: WMFW_CTL_FLAG_WRITEABLE,
	},
	{ .alg_id = 0xfafa,   .mem_type = WMFW_ADSP2_YM, .offs_words = 1, .len_bytes = 512,
...
struct kunit_case cs_dsp_ctl_cache_test_cases_v1[] = {
...
struct kunit_case cs_dsp_ctl_cache_test_cases_v2[] = {
...
struct kunit_case cs_dsp_ctl_cache_test_cases_v3[] = {
	KUNIT_CASE(cs_dsp_ctl_v2_cache_alloc),
...
struct kunit_suite cs_dsp_ctl_cache_test_halo = {
...
struct kunit_suite cs_dsp_ctl_cache_test_adsp2_32bit_wmfw1 = {
...
struct kunit_suite cs_dsp_ctl_cache_test_adsp2_32bit_wmfw2 = {
...
struct kunit_suite cs_dsp_ctl_cache_test_adsp2_16bit_wmfw1 = {
...
struct kunit_suite cs_dsp_ctl_cache_test_adsp2_16bit_wmfw2 = {
