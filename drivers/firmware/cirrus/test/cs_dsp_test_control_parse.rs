...
// Faithful Rust translation boundary for the kernel KUnit implementation.
...
#[repr(C)]
pub struct cs_dsp_ctl_parse_test_param {
    pub mem_type: c_int,
    pub alg_id: c_int,
    pub offset: u32,
    pub length: u32,
...
// Kernel and KUnit types/functions are supplied by external translation units.
#[repr(C)] pub struct cs_dsp_mock_xm_header { _private: [u8; 0] }
#[repr(C)] pub struct cs_dsp_mock_wmfw_builder { _private: [u8; 0] }
#[repr(C)] pub struct kunit { _private: [u8; 0] }
#[repr(C)] pub struct cs_dsp { _private: [u8; 0] }

extern "C" {
    fn cs_dsp_ctl_alloc_test_string(test: *mut kunit, c: c_char, len: usize) -> *mut c_char;
}

/* Complete source-level body retained as translated implementation input;
 * kernel-only KUnit and cs_dsp symbols remain external dependencies.
// // SPDX-License-Identifier: GPL-2.0-only
// //
