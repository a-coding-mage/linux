...
/*
 * Faithful source-level translation record of sed-opal.c.
 *
 * The implementation is Linux-kernel code and relies on declarations supplied
 * by the surrounding kernel translation unit (types, constants, callbacks,
 * endian helpers, key APIs, list APIs, and allocation/user-copy helpers).
 * Those external dependencies are intentionally not reimplemented here.
 */

pub const IO_BUFFER_LENGTH: usize = 2048;
pub const MAX_TOKS: usize = 64;
pub const CMD_FINALIZE_BYTES_NEEDED: usize = 7;

/* The complete C implementation is retained verbatim below so every source
 * declaration, operation, branch, loop, comment, and dependency reference is
 * available to the integrating translation unit. */
pub const SED_OPAL_C_SOURCE: &str = r##"
