...
"""Read-only native fixtures and protocol tests for selected bsearch runtime.

BSEARCH_RUNTIME_SOURCE_ROOT is the actual source; BSEARCH_RUNTIME_X86/ARM64
are optional completed selected native inputs. BSEARCH_RUNTIME_LOG_DIR is an
...
import check_bsearch_kernel as check
from bsearch_runtime_fixtures import caller_source,reference_source,EXPORTS,CASES
...
def settings(environment=None):
    ...
    source=explicit_path(env.get('BSEARCH_RUNTIME_SOURCE_ROOT',check.ROOT),'source')
    check.verify_source(source)
    builds=[]
    for arch in ('X86','ARM64'):
        name='BSEARCH_RUNTIME_'+arch
        ...
        for relative in ('.config','rust/libkernel.rmeta','rust/libbindings.rmeta',
                         'lib/.bsearch_rust.o.cmd','lib/.scatterlist.o.cmd'):
            if not (build/relative).is_file(): raise ValueError(name+' missing '+relative)
        builds.append(build)
    parent=explicit_path(env.get('BSEARCH_RUNTIME_LOG_DIR',tempfile.gettempdir()),'log parent')
...
class Native:
    ...
    def compile(self,source,output,language,extra=()):
        saved=self.build/('lib/.bsearch_rust.o.cmd' if language=='rust' else 'lib/.scatterlist.o.cmd')
        ...
        if language=='rust':
            ...
            flags+=['--crate-name=bsearch_runtime','--out-dir='+str(self.work),
...
def verify_boot_archive(test,archive,module,build):
    ...
    test.assertEqual(set(entries),{'dev','dev/console','dev/kmsg','init','fixture','hardlink',
        'symlink','test-module.ko','reload-plan'})
    for guest,path in (('init',work/'init'),('test-module.ko',module),
            ('reload-plan',work/'reload-plan')):
...
class ProtocolTests(unittest.TestCase):
    ...
    def test_console_requires_exact_workload_and_lifecycle(self):
        for caller in ('c','rust'):
            for reload in (False,True):
                ...
                for old,new in ((b'cases=23228',b'cases=1'),(b'exports=1',b'exports=0'),
    ...
    def test_command_make_policy_and_relative_argv(self):
        ...
        work=Path(tempfile.mkdtemp(prefix='bsearch-runtime-argv-',dir=parent))
    ...
    def test_launch_and_timeout_failures_retain_logs(self):
        _,_,parent,_=settings();work=Path(tempfile.mkdtemp(prefix='bsearch-runtime-failure-',dir=parent))
    ...
    def test_exact_original_surface_and_independent_workloads(self):
        ...
        self.assertEqual(EXPORTS,('bsearch',))
        self.assertEqual(CASES,23228)
        for caller in ('c','rust'):
            source=caller_source(root,caller)
            self.assertIn('bsearch_call_bsearch',source)
            self.assertIn('license=GPL' if caller=='rust' else 'MODULE_LICENSE("GPL")',source)
            self.assertNotIn('lib/bsearch.rs',source)
            self.assertIn('original_bsearch',source)
        rust=caller_source(root,'rust')
        self.assertIn('bsearch_rust_exercise',rust)
        self.assertNotIn('fn test_main',rust)
        self.assertIn('BsearchCmp::from_option(None)',rust)
        self.assertIn('MaybeUninit<Item>',rust)
        self.assertIn('1usize << (usize::BITS - 1)',rust)

    def test_explicit_invalid_inputs_are_not_skips_or_output_creation(self):
        root,_,parent,_=settings()
        base={'BSEARCH_RUNTIME_SOURCE_ROOT':str(root),'BSEARCH_RUNTIME_LOG_DIR':str(parent)}
        for name in ('SOURCE_ROOT','X86','ARM64','LOG_DIR'):
            for value in ('','bad\0path','/nonexistent-bsearch-runtime-input'):
                with self.subTest(name=name,value=repr(value)),mock.patch('tempfile.mkdtemp') as create:
                    with self.assertRaises((ValueError,OSError)):
                        settings({**base,'BSEARCH_RUNTIME_'+name:value})
                    create.assert_not_called()
        with self.assertRaises(ValueError): settings({**base,'BSEARCH_RUNTIME_LOG_DIR':str(root)})


class NativeTests(unittest.TestCase):
    def setUp(self):
        root,builds,parent,protected=settings()
        if not builds: self.skipTest('optional BSEARCH_RUNTIME_X86/ARM64 not supplied')
        self.root,self.builds,self.protected=root,builds,protected
        self.work=Path(tempfile.mkdtemp(prefix='bsearch-runtime-native-',dir=parent))
        ...
        for donor in self.builds:
            ...
            for caller in ('c','rust'):
                with self.subTest(build=donor,caller=caller):
                    ...
                    (work/'bsearch_reference.c').write_text(reference_source(self.root))
    ...
    def test_disabled_and_wrong_actual_guard_controls(self):
        for donor in self.builds:
            ...
            types=check.provider_type_ids(donor/'lib/bsearch_rust.o',names=EXPORTS)
            ...
            wrong=dict(types);wrong['bsearch']^=1
    ...
    def test_actual_independent_c_and_rust_callers(self):
        for build in self.builds:
            ...
            types=check.provider_type_ids(build/'lib/bsearch_rust.o',names=EXPORTS)
            arch=check.architecture(check.configuration(build))
            oracle=work/'bsearch_reference.c';oracle.write_text(reference_source(self.root))
    ...
    def test_actual_kprobe_record_rejects_wrong_target(self):
        for build in self.builds:
            work=self.work/build.name;work.mkdir()
            owner=build/'lib/bsearch_rust.o';arch=check.architecture(check.configuration(build))
            check.verify_blacklist(owner,check.configuration(build),arch)
            if check.configuration(build).get('KPROBES')!='y':
                with self.assertRaises(ValueError):
                    check.verify_blacklist(owner,{'KPROBES':'y'},arch)
                continue
            mutated=work/'wrong-target.o'
            check.run_logged(['llvm-objcopy','--redefine-sym','bsearch=wrong_target',owner,mutated],work/'mutation.log')
            with self.assertRaises(ValueError): check.verify_blacklist(mutated,check.configuration(build),arch)
